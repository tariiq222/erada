...
/**
 * CSD-CA23078-SEEDER-001 — role catalog sync obsolete-pivot cleanup.
 *
 * The canonical role catalog lives in
 * `Database\Seeders\RolesAndPermissionsSeeder::roleCatalog()`. The seeder
 * itself runs the sweep on fresh installs and on `db:seed --force`, but
 * databases that upgraded without a re-seed carry obsolete pivots left
 * behind by an earlier catalog version. This migration is the in-place
 * safety net for those upgraded databases.
 *
 * Scope of the sweep:
 *   - ONLY seeded system roles named in
 *     `RolesAndPermissionsSeeder::SWEPT_SYSTEM_ROLES` (admin, viewer,
 *     dept_manager, member).
 *   - super_admin is excluded — its pivots are admin-managed and any
 *     safety-net overwrite would clobber operator overrides.
 *   - Custom roles (any role whose name is NOT in the catalog) are
...
{
    private const MIGRATION_NAME = '2026_07_12_000018_role_catalog_sync_obsolete_pivots';

    private const AUDIT_EVENT = 'role_catalog_sync_obsolete_pivot_removed';

    /**
     * Seeded system roles that participate in the sweep.
     *
     * @var list<string>
     */
    ...
    public function up(): void
    {
        ...
        // Build the desired (resource_id|action) set per swept role by
        // replaying the same mapping the seeder uses. The migration
        // must agree with the seeder byte-for-byte; a CI test diffs the
        // two arrays on every PR to catch drift early.
