...
vi.mock('@shared/contexts/OrganizationContext', () => ({
  ...
  OrganizationProvider: ({ children }: { children?: unknown }) => children,
}));

// Mock the nasaq app shell (Sidebar/Topbar moved to @/nasaq/app)
vi.mock('@shared/nasaq/app', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@shared/nasaq/app')>()),
  NASAQ_NAV: [
  ...
  NASAQ_PALETTES: {
    blue: {
      light: { p: '#000', p2: '#000', soft: '#000', soft2: '#000', ring: '#000' },
      dark: { p: '#000', p2: '#000', soft: '#000', soft2: '#000', ring: '#000' },
    },
  },
  NASAQ_BG_THEMES: {
    cool: {
      light: { bg: '#fff', grad: '#fff', s1: '#fff', s2: '#fff', s3: '#fff', b1: '#fff', b2: '#fff' },
      dark: { bg: '#000', grad: '#000', s1: '#000', s2: '#000', s3: '#000', b1: '#000', b2: '#000' },
    },
  },
  Sidebar: ({ isAdmin }: { isAdmin?: boolean }) => (
    <div data-testid="sidebar" data-open="true" data-is-admin={String(!!isAdmin)}>
  ...
  App: ({ children }: { children?: React.ReactNode }) => (
    <div data-testid="app-shell" dir="rtl">
      <div data-testid="sidebar" data-open="true">
        <button data-testid="sidebar-toggle">Toggle</button>
      </div>
      <div data-testid="header">
        <button data-testid="menu-button">Menu</button>
...
import AppLayout from '@widgets/app-shell/ui/AppLayout';
...
describe('AppLayout Component', () => {
  ...
  it('renders RTL direction', async () => {
    render(<AppLayout />);
...
describe('AppLayout Sidebar Toggle', () => {
  ...
  it('sidebar is open by default on desktop', async () => {
    render(<AppLayout />);
  ...
  it('toggles sidebar when header menu is clicked', async () => {
    render(<AppLayout />);
    ...
    // Sidebar visibility is no longer toggled by AppLayout state; it stays mounted.
    await waitFor(() => {
      const sidebar = screen.getByTestId('sidebar');
      expect(sidebar).toHaveAttribute('data-open', 'true');
    });
...
describe('AppLayout Loading Animation', () => {
  beforeEach(() => {
    mockAuthState = {
      isAuthenticated: false,
      isLoading: true,
  ...
  it('shows loading spinner', async () => {
    ...
    await waitFor(() => {
      const spinner = document.querySelector('.tabler-icon-loader');
      expect(spinner).toBeInTheDocument();
    });
  });

  it('shows sparkles icon during loading', async () => {
    render(<AppLayout />);

    await waitFor(() => {
      const sparklesIcon = document.querySelector('.tabler-icon-sparkles');
