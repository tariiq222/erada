...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'nav.settings': 'الإعدادات', 'nav.logout': 'تسجيل الخروج', 'nav.admin_panel': 'لوحة الإدارة', 'nav.admin_section': 'الإدارة',
    'nav.ovr': 'تقارير الحوادث', 'nav.ovr_dashboard': 'لوحة الحوادث', 'nav.ovr_settings': 'إعدادات الحوادث',
    'nav.risks': 'المخاطر', 'nav.risks_list': 'سجل المخاطر', 'nav.risks_matrix': 'مصفوفة المخاطر', 'nav.help': 'الدعم والمساعدة', 'nav.incidents': 'الحوادث', 'meetings.notification.label': 'الإشعارات',
    ...
    'tasks.owner': 'المالك', 'tasks.private': 'خاصة', 'admin.organizations.title': 'المنظمات', 'admin.departments.title': 'الأقسام',
    'admin.users.title': 'المستخدمين', 'admin.roles.title': 'الأدوار', 'admin.scopeTypes.title': 'أنواع النطاق',
    'admin.incidentTypes.title': 'أنواع الحوادث', 'admin.activityLogs.title': 'سجل النشاط', 'users.title': 'المستخدمون', 'users.create': 'إنشاء مستخدم',
...
vi.mock('react-router-dom', () => ({
  ...
  NavLink: ({ children, to, className }: any) => {
    const isActive = mockLocation.pathname === to || mockLocation.pathname.startsWith(to);
    const resolvedClassName = typeof className === 'function' ? className({ isActive }) : className;
    const renderedChildren = typeof children === 'function' ? children({ isActive }) : children;
    return <a href={to} className={resolvedClassName}>{renderedChildren}</a>;
  },
...
vi.mock('@shared/contexts/SystemSettingsContext', () => ({
  useSystemSettings: () => ({
    settings: { name: 'منصة إرادة', code: 'مكتب إدارة المشاريع' },
    loading: false,
    error: null,
    refresh: vi.fn(),
  }),
...
vi.mock('@shared/contexts/OrganizationContext', () => ({
  useOrganization: () => ({
    organizations: [{ id: 1, name: 'جهة الاختبار', code: 'TEST' }],
    currentOrganization: { id: 1, name: 'جهة الاختبار', code: 'TEST' },
...
describe('Header Component', () => {
  ...
  it('renders user name', async () => {
    ...
    // User name appears in dropdown trigger
  ...
  it('renders search input', async () => {
    render(<Header {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByPlaceholderText('بحث في النظام...')).toBeInTheDocument();
  });
  ...
  it('renders user avatar with first letter', async () => {
    ...
    const avatars = screen.getAllByText('أ');
  ...
  it('opens dropdown when user button clicked', async () => {
    render(<Header {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    // Find and click the dropdown trigger (has chevron)
  ...
  it('renders notification bell', async () => {
    ...
    // The bell is now the meetings NotificationBell component.
    const notificationButton = screen.getByRole('button', { name: 'الإشعارات' });
    expect(notificationButton).toBeInTheDocument();

    // The unread indicator (status-danger dot) is conditional on unread > 0; with no
    // unread notifications it is absent. The bell icon itself is always rendered.
    expect(notificationButton.querySelector('.tabler-icon-bell')).toBeInTheDocument();
  });
  ...
  it('renders profile link', async () => {
    ...
    const profileLink = screen.getByText('الملف الشخصي').closest('a');
...
describe('Header Responsive Behavior', () => {
  it('uses the desktop sidebar offset', async () => {
    ...
    rerender(<Header onMenuClick={vi.fn()} sidebarOpen={false} />);
    await act(async () => { await Promise.resolve(); });
