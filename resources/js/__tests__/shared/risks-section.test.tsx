...
vi.mock('@entities/project', () => ({
  projectsApi: {
    ...
    updateRisk: vi.fn().mockResolvedValue({}),
    removeRisk: vi.fn().mockResolvedValue({}),
  },
}));
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, leftIcon, variant, size }: {
    ...
    <button onClick={onClick} data-variant={variant}>{leftIcon}{children}</button>
  ),
...
vi.mock('@pages/projects/components/modals', () => ({
  AddRiskModal: ({ isOpen, onClose, formData, onFormChange, onAdd, isLoading, mode }: any) => (
    isOpen ? (
      <div data-testid="add-risk-modal">
        <span>نافذة {mode === 'edit' ? 'تعديل' : 'إضافة'} خطر</span>
        <input
  ...
  RiskStatusModal: ({ isOpen, onClose, currentStatus, onSave, isLoading }: any) => (
    isOpen ? (
      <div data-testid="status-modal">
        <span>تغيير الحالة</span>
        <span>الحالة: {currentStatus}</span>
        <button onClick={() => onSave('closed', 'تم الإغلاق')} disabled={isLoading}>إغلاق الخطر</button>
        <button onClick={onClose}>إلغاء</button>
      </div>
    ) : null
  ),
...
import RisksSection from '@pages/projects/components/sections/RisksSection';

const mockRisks = [
  {
    id: 1,
    risk: 'خطر تأخر التسليم',
    probability: 'high',
    impact: 'high',
    response: 'خطة استجابة',
    status: 'open',
  },
  {
    id: 2,
    risk: 'خطر نقص الموارد',
    probability: 'medium',
    impact: 'medium',
    response: '',
    status: 'mitigated',
...
describe('RisksSection Basic', () => {
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('سجل المخاطر')).toBeInTheDocument();
  });

  it('shows risk count', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('3 خطر')).toBeInTheDocument();
  });
...
describe('RisksSection Empty State', () => {
  const emptyProps = {
    risks: [],
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('shows empty state message', () => {
    render(<RisksSection {...emptyProps} />);
    expect(screen.getByText('لا توجد مخاطر')).toBeInTheDocument();
  });

  it('shows empty state description', () => {
    render(<RisksSection {...emptyProps} />);
    expect(screen.getByText('أضف المخاطر المحتملة للمشروع')).toBeInTheDocument();
  });

  it('shows add risk button in empty state', () => {
    render(<RisksSection {...emptyProps} />);
    const addButtons = screen.getAllByText('إضافة خطر');
    expect(addButtons.length).toBeGreaterThanOrEqual(1);
  });
});

describe('RisksSection Table View', () => {
  const defaultProps = {
    risks: mockRisks,
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('shows table headers', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('الخطر')).toBeInTheDocument();
  ...
  it('displays risk titles', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('خطر تأخر التسليم')).toBeInTheDocument();
    expect(screen.getByText('خطر نقص الموارد')).toBeInTheDocument();
    expect(screen.getByText('خطر تقني')).toBeInTheDocument();
  });
  ...
  it('displays status badges', () => {
    ...
    expect(screen.getByText('مخفف')).toBeInTheDocument();
    expect(screen.getByText('مغلق')).toBeInTheDocument();
  });

  it('shows edit buttons', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByTestId('pencil-icon').length).toBe(3);
  });

  it('shows status change buttons', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByTestId('refresh-icon').length).toBe(3);
  });

  it('shows delete buttons', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByTestId('trash-icon').length).toBe(3);
  });
});

describe('RisksSection Cards View', () => {
  const defaultProps = {
    risks: mockRisks,
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('switches to cards view', () => {
    render(<RisksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('grid-icon').parentElement!);

    // Cards view should show probability and impact labels
    expect(screen.getAllByText('الاحتمالية:').length).toBeGreaterThan(0);
    expect(screen.getAllByText('التأثير:').length).toBeGreaterThan(0);
  });

  it('shows response plan in cards', () => {
    render(<RisksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('grid-icon').parentElement!);

    expect(screen.getByText('خطة استجابة')).toBeInTheDocument();
  });
});

describe('RisksSection Modal Interactions', () => {
  const defaultProps = {
    risks: mockRisks,
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('opens add modal when add button clicked', () => {
    render(<RisksSection {...defaultProps} />);
    fireEvent.click(screen.getByText('إضافة خطر'));
    expect(screen.getByTestId('add-risk-modal')).toBeInTheDocument();
    expect(screen.getByText('نافذة إضافة خطر')).toBeInTheDocument();
  });
  ...
  it('closes add modal', () => {
    ...
    expect(screen.queryByTestId('add-risk-modal')).not.toBeInTheDocument();
  });
  ...
  it('opens edit modal when edit button clicked', () => {
    ...
    expect(screen.getByTestId('add-risk-modal')).toBeInTheDocument();
    expect(screen.getByText('نافذة تعديل خطر')).toBeInTheDocument();
  });

  it('opens status modal when status button clicked', () => {
    render(<RisksSection {...defaultProps} />);
    const statusButtons = screen.getAllByTestId('refresh-icon');
    fireEvent.click(statusButtons[0].parentElement!);
    expect(screen.getByTestId('status-modal')).toBeInTheDocument();
    expect(screen.getByText('تغيير الحالة')).toBeInTheDocument();
  });

  it('closes status modal', () => {
    render(<RisksSection {...defaultProps} />);
    const statusButtons = screen.getAllByTestId('refresh-icon');
    fireEvent.click(statusButtons[0].parentElement!);
...
describe('RisksSection API Actions', () => {
  const defaultProps = {
    risks: mockRisks,
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('calls removeRisk when delete confirmed', async () => {
    ...
    await waitFor(() => {
      expect(projectsApi.removeRisk).toHaveBeenCalledWith(1, 1);
    });
  });
  ...
  it('shows success toast on delete', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', 'تم حذف الخطر بنجاح');
    });
  });

  it('calls onRiskRemoved after delete', async () => {
    render(<RisksSection {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(defaultProps.onRiskRemoved).toHaveBeenCalled();
    });
  });
  ...
  it('does not delete when confirm cancelled', async () => {
    ...
    expect(projectsApi.removeRisk).not.toHaveBeenCalled();
  });
});

describe('RisksSection View Toggle', () => {
  const defaultProps = {
    risks: mockRisks,
    projectId: 1,
    onRiskAdded: vi.fn(),
  ...
  it('can toggle to cards view', () => {
    render(<RisksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('grid-icon').parentElement!);
    // Card-specific text should appear
    expect(screen.getAllByText('الاحتمالية:').length).toBeGreaterThan(0);
  });

  it('can toggle back to table view', () => {
    render(<RisksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('grid-icon').parentElement!);
    fireEvent.click(screen.getByTestId('list-icon').parentElement!);
