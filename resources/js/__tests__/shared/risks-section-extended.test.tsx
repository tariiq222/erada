...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, leftIcon, type, variant, size, className }: any) => (
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconAlertTriangle: () => <span data-testid="alert-icon">⚠️</span>,
...
const mockRisks = [
  { description: 'خطر أول', impact: 'high', probability: 'medium', mitigation: 'خطة التخفيف' },
  { description: 'خطر ثاني', impact: 'low', probability: 'low', mitigation: '' },
];

describe('RisksSection Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('RisksSection Risks Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows risk description inputs', () => {
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
    ...
    const inputs = screen.getAllByPlaceholderText('[Enter Risk Description]');
    expect(inputs.length).toBe(2);
  });

  it('shows mitigation inputs', () => {
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
    ...
    const inputs = screen.getAllByPlaceholderText('[Enter Mitigation Plan]');
    expect(inputs.length).toBe(2);
  });

  it('shows impact labels', () => {
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
    ...
    const labels = screen.getAllByText('التأثير');
    expect(labels.length).toBe(2);
  });

  it('shows probability labels', () => {
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
    ...
    const labels = screen.getAllByText('الاحتمالية');
    expect(labels.length).toBe(2);
  });
});

describe('RisksSection Add Risk', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('calls onAddRisk when add button clicked', () => {
    ...
    expect(onAddRisk).toHaveBeenCalledTimes(1);
  });
});

describe('RisksSection Remove Risk', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows delete buttons for multiple risks', () => {
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
  ...
  it('calls onRemoveRisk when delete clicked', () => {
    const onRemoveRisk = vi.fn();
    render(
      <RisksSection
        risks={mockRisks}
        onRiskChange={vi.fn()}
    ...
    expect(onRemoveRisk).toHaveBeenCalledWith(0);
  });
});

describe('RisksSection Risk Change', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('calls onRiskChange when description changes', () => {
    ...
    const descInputs = screen.getAllByPlaceholderText('[Enter Risk Description]');
    fireEvent.change(descInputs[0], { target: { value: 'خطر جديد' } });
    expect(onRiskChange).toHaveBeenCalledWith(0, 'description', 'خطر جديد');
  });
  ...
  it('calls onRiskChange when mitigation changes', () => {
    ...
    const mitInputs = screen.getAllByPlaceholderText('[Enter Mitigation Plan]');
    fireEvent.change(mitInputs[0], { target: { value: 'خطة جديدة' } });
    expect(onRiskChange).toHaveBeenCalledWith(0, 'mitigation', 'خطة جديدة');
  });
  ...
  it('calls onRiskChange when impact changes', () => {
    ...
    // First select is impact
    fireEvent.change(selects[0], { target: { value: 'low' } });
    expect(onRiskChange).toHaveBeenCalledWith(0, 'impact', 'low');
  });
  ...
  it('calls onRiskChange when probability changes', () => {
    ...
    const selects = screen.getAllByTestId('select');
    // Second select is probability
    fireEvent.change(selects[1], { target: { value: 'high' } });
    expect(onRiskChange).toHaveBeenCalledWith(0, 'probability', 'high');
  });
});
