...
import UserStatsCards from '@pages/users/components/StatsCards';
import TasksFilters from '@pages/tasks/list/TasksFilters';
// The standalone TasksHeader component was removed; its title/description/
// create-link/stats are now composed inline in TasksList from the shared
// PageHeader + StatStrip primitives. We reconstruct that exact composition
// here so the header rendering behavior stays under test.
import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import { Button, PageHeader, StatStrip } from '@shared/ui';
import { IconPlus } from '@tabler/icons-react';

type TasksHeaderTask = { id: number; status: string; due_date: string };
function TasksHeader({
  tasks,
  pagination,
  isOverdue,
}: {
  tasks: TasksHeaderTask[];
  pagination: { total: number; per_page: number; current_page: number };
  isOverdue: (task: TasksHeaderTask) => boolean;
}) {
...
describe('User StatsCards', () => {
  const mockUsers = [
    { id: 1, name: 'User 1', is_active: true, roles: ['admin'] },
    { id: 2, name: 'User 2', is_active: true, roles: ['user'] },
    { id: 3, name: 'User 3', is_active: false, roles: ['user'] },
    { id: 4, name: 'User 4', is_active: true, roles: ['super_admin'] },
  ] as any[];

  it('renders all stat labels', () => {
    render(<UserStatsCards users={mockUsers} total={10} />);
  ...
  it('renders total correctly', () => {
    render(<UserStatsCards users={mockUsers} total={50} />);
...
describe('TasksHeader Component', () => {
  const mockTasks = [
    { id: 1, title: 'Task 1', status: 'in_progress', due_date: '2024-01-01' },
    { id: 2, title: 'Task 2', status: 'completed', due_date: '2024-01-02' },
    { id: 3, title: 'Task 3', status: 'todo', due_date: '2024-01-03' },
  ] as any[];

  const defaultProps = {
    tasks: mockTasks,
    pagination: { total: 100, per_page: 10, current_page: 1 },
    isOverdue: (task: any) => new Date(task.due_date) < new Date(),
  };
