...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconAlertCircle: () => <span data-testid="alert-icon">AlertCircle</span>,
  IconAlertTriangle: () => <span data-testid="alert-triangle-icon">AlertTriangle</span>,
  IconMessage: () => <span data-testid="message-icon">MessageSquare</span>,
  IconBulb: () => <span data-testid="lightbulb-icon">Lightbulb</span>,
...
import StatusTransitionModal from '@pages/projects/components/modals/StatusTransitionModal';

describe('StatusTransitionModal Basic', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    onConfirm: vi.fn(),
    taskTitle: 'مهمة اختبار',
    newStatus: 'in_progress',
    confirmationMessage: 'هل تريد تغيير حالة المهمة؟',
    isCompleting: false,
  };
  ...
  it('renders when open', () => {
    render(<StatusTransitionModal {...defaultProps} />);
    expect(screen.getByText('تأكيد تغيير الحالة')).toBeInTheDocument();
  });

  it('does not render when closed', () => {
    render(<StatusTransitionModal {...defaultProps} isOpen={false} />);
    expect(screen.queryByText('تأكيد تغيير الحالة')).not.toBeInTheDocument();
  });
  ...
  it('displays confirmation message', () => {
    render(<StatusTransitionModal {...defaultProps} />);
    expect(screen.getByText('هل تريد تغيير حالة المهمة؟')).toBeInTheDocument();
  });
  ...
  it('shows new status label', () => {
    render(<StatusTransitionModal {...defaultProps} />);
    expect(screen.getByText(/نقل إلى:/)).toBeInTheDocument();
  });
...
describe('StatusTransitionModal Review Status', () => {
  const reviewProps = {
    isOpen: true,
    onClose: vi.fn(),
    onConfirm: vi.fn(),
    taskTitle: 'مهمة للمراجعة',
    newStatus: 'in_review',
    confirmationMessage: 'سيتم إرسال المهمة للمراجعة',
    isCompleting: false,
  ...
  it('shows review button text', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByText('إرسال للمراجعة')).toBeInTheDocument();
  });

  it('shows eye icon for review', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByTestId('eye-icon')).toBeInTheDocument();
  });

  it('shows comment field for review', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByPlaceholderText('اكتب سبب إرسال المهمة للمراجعة...')).toBeInTheDocument();
  });

  it('shows comment required label', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByText(/سبب الإرسال للمراجعة/)).toBeInTheDocument();
  });

  it('shows warning for members', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByText('تنبيه مهم')).toBeInTheDocument();
    expect(screen.getByText(/بعد نقل المهمة لقيد المراجعة/)).toBeInTheDocument();
  });

  it('shows validation message when comment empty', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    expect(screen.getByText('يجب كتابة سبب إرسال المهمة للمراجعة')).toBeInTheDocument();
  });

  it('disables confirm when comment empty', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    const confirmButton = screen.getByText('إرسال للمراجعة');
    expect(confirmButton).toBeDisabled();
  });

  it('enables confirm when comment filled', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    const textarea = screen.getByPlaceholderText('اكتب سبب إرسال المهمة للمراجعة...');
    fireEvent.change(textarea, { target: { value: 'سبب الإرسال' } });
    const confirmButton = screen.getByText('إرسال للمراجعة');
    expect(confirmButton).not.toBeDisabled();
  });

  it('passes comment to onConfirm', () => {
    render(<StatusTransitionModal {...reviewProps} />);
    const textarea = screen.getByPlaceholderText('اكتب سبب إرسال المهمة للمراجعة...');
    fireEvent.change(textarea, { target: { value: 'سبب الإرسال' } });
    fireEvent.click(screen.getByText('إرسال للمراجعة'));
    expect(reviewProps.onConfirm).toHaveBeenCalledWith('سبب الإرسال');
  });
});

describe('StatusTransitionModal Completing', () => {
  const completingProps = {
    isOpen: true,
    onClose: vi.fn(),
    onConfirm: vi.fn(),
    taskTitle: 'مهمة للإكمال',
    newStatus: 'completed',
    confirmationMessage: 'سيتم إكمال المهمة',
    isCompleting: true,
  };
  ...
  it('shows complete button text', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByText('إغلاق المهمة نهائياً')).toBeInTheDocument();
  });

  it('shows check icon for completing', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByTestId('check-icon')).toBeInTheDocument();
  });

  it('shows challenges field', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByText('التحديات وكيف تم حلها')).toBeInTheDocument();
  });

  it('shows lessons learned field', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByText('الدروس المستفادة')).toBeInTheDocument();
  });

  it('shows challenges placeholder', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByPlaceholderText(/اذكر التحديات التي واجهتها/)).toBeInTheDocument();
  });

  it('shows lessons placeholder', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByPlaceholderText(/ما الدروس التي تعلمتها/)).toBeInTheDocument();
  });

  it('passes completion data to onConfirm', () => {
    render(<StatusTransitionModal {...completingProps} />);

    const challengesField = screen.getByPlaceholderText(/اذكر التحديات التي واجهتها/);
    const lessonsField = screen.getByPlaceholderText(/ما الدروس التي تعلمتها/);

    fireEvent.change(challengesField, { target: { value: 'التحديات' } });
    fireEvent.change(lessonsField, { target: { value: 'الدروس' } });

    fireEvent.click(screen.getByText('إغلاق المهمة نهائياً'));

    expect(completingProps.onConfirm).toHaveBeenCalledWith('', {
      comment: '',
      challenges: 'التحديات',
      lessonsLearned: 'الدروس',
    });
  });

  it('shows target icon for challenges', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByTestId('target-icon')).toBeInTheDocument();
  });

  it('shows lightbulb icon for lessons', () => {
    render(<StatusTransitionModal {...completingProps} />);
    expect(screen.getByTestId('lightbulb-icon')).toBeInTheDocument();
  });
});

describe('StatusTransitionModal User Roles', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('does not show member warning for project manager', () => {
    render(
      <StatusTransitionModal
        ...
        taskTitle="مهمة"
        newStatus="in_review"
        confirmationMessage="رسالة"
        isCompleting={false}
        userRole="project_manager"
      />
    );
    expect(screen.queryByText('تنبيه مهم')).not.toBeInTheDocument();
  });

  it('does not show member warning for super admin', () => {
    render(
      <StatusTransitionModal
        isOpen={true}
        onClose={vi.fn()}
  ...
  it('shows member warning for member role', () => {
    render(
      <StatusTransitionModal
        isOpen={true}
        onClose={vi.fn()}
...
describe('StatusTransitionModal Close Behavior', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    onConfirm: vi.fn(),
  ...
  it('clears form on close', () => {
    const { rerender } = render(<StatusTransitionModal {...defaultProps} newStatus="in_review" />);
    ...
    // Re-open modal
    rerender(<StatusTransitionModal {...defaultProps} newStatus="in_review" />);

    const newTextarea = screen.getByPlaceholderText('اكتب سبب إرسال المهمة للمراجعة...');
