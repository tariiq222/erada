...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, loading, leftIcon, variant, type }: any) => (
...
vi.mock('@shared/ui/Modal', () => ({
  Modal: ({ children, open, onClose }: any) =>
    open ? <div data-testid="modal" onClick={(e: any) => e.target === e.currentTarget && onClose()}>{children}</div> : null,
...
describe('MilestoneModal Form Fields', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
  ...
  it('renders duration input placeholder', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('أدخل الرقم')).toBeInTheDocument();
  });

  it('renders duration unit options', () => {
    render(<MilestoneModal {...defaultProps} />);
...
describe('MilestoneModal onChange', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
  ...
  it('calls onChange when name changes', () => {
    render(<MilestoneModal {...defaultProps} />);
    const nameInput = screen.getByPlaceholderText('مثال: المرحلة الأولى');
    fireEvent.change(nameInput, { target: { value: 'مرحلة جديدة' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('name', 'مرحلة جديدة');
  });

  it('calls onChange when description changes', () => {
    render(<MilestoneModal {...defaultProps} />);
    const descInput = screen.getByPlaceholderText('وصف موجز للمرحلة...');
    fireEvent.change(descInput, { target: { value: 'وصف المرحلة' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('description', 'وصف المرحلة');
  });

  it('calls onChange when duration value changes', () => {
    render(<MilestoneModal {...defaultProps} />);
    const durationInput = screen.getByPlaceholderText('أدخل الرقم');
    fireEvent.change(durationInput, { target: { value: '7' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('duration_value', '7');
  });

  it('calls onChange when duration unit changes', () => {
    render(<MilestoneModal {...defaultProps} />);
    const trigger = screen.getByRole('button', { name: /يوم/ });
    fireEvent.click(trigger);
...
describe('MilestoneModal Actions', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
...
describe('MilestoneModal Errors', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
  ...
  it('shows name error', () => {
    render(<MilestoneModal {...defaultProps} errors={{ name: ['اسم المرحلة مطلوب'] }} />);
    expect(screen.getByText('اسم المرحلة مطلوب')).toBeInTheDocument();
  });

  it('shows duration_value error', () => {
    render(<MilestoneModal {...defaultProps} errors={{ duration_value: ['المدة مطلوبة'] }} />);
    expect(screen.getByText('المدة مطلوبة')).toBeInTheDocument();
  });
});

describe('MilestoneModal Values', () => {
  const filledProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
      name: 'مرحلة التطوير',
      description: 'وصف مرحلة التطوير',
      duration_value: '14',
      duration_unit: 'week',
    },
    errors: {},
  ...
  it('displays name value', () => {
    render(<MilestoneModal {...filledProps} />);
    const nameInput = screen.getByPlaceholderText('مثال: المرحلة الأولى');
    expect(nameInput).toHaveValue('مرحلة التطوير');
  });

  it('displays description value', () => {
    render(<MilestoneModal {...filledProps} />);
    const descInput = screen.getByPlaceholderText('وصف موجز للمرحلة...');
    expect(descInput).toHaveValue('وصف مرحلة التطوير');
  });
  ...
  it('displays duration unit value', () => {
    render(<MilestoneModal {...filledProps} />);
    const trigger = screen.getByRole('button', { name: /أسبوع/ });
    expect(trigger).toBeInTheDocument();
    expect(trigger).toHaveTextContent('أسبوع');
  });
});
