...
import { MemoryRouter, BrowserRouter as _BrowserRouter } from 'react-router-dom';
...
vi.mock('@entities/hr', () => ({
  departmentsApi: {
    getAll: vi.fn(() => Promise.resolve({ data: [], current_page: 1, last_page: 1, per_page: 15, total: 0 })),
...
vi.mock('@entities/incident', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@entities/incident')>();
  return {
    ...actual,
    incidentsApi: {
      ...actual.incidentsApi,
      getAll: vi.fn(() => Promise.resolve({ data: [], current_page: 1, last_page: 1, per_page: 15, total: 0 })),
      getStats: vi.fn(() => Promise.resolve({ total: 0, open: 0, closed: 0 })),
    },
    incidentCategoriesApi: {
      ...actual.incidentCategoriesApi,
      getList: vi.fn(() => Promise.resolve([])),
      getAll: vi.fn(() => Promise.resolve({ data: [] })),
    },
  };
...
vi.mock('@shared/api/settings', () => ({
  uploadApi: {
    uploadClusterLogo: vi.fn(),
  },
  dashboardApi: {
    getStats: vi.fn(() => Promise.resolve({ projects: 0, tasks: 0, users: 0 })),
    getAdvancedStats: vi.fn(() => Promise.resolve({})),
    getProjectsByStatus: vi.fn(() => Promise.resolve([])),
    getRecentProjects: vi.fn(() => Promise.resolve([])),
    getOverdueTasks: vi.fn(() => Promise.resolve([])),
    getMyUpcomingTasks: vi.fn(() => Promise.resolve([])),
  },
}));

// Login (and other pages) now consume LocaleContext; provide a stub provider/hook.
vi.mock('@shared/contexts/LocaleContext', () => ({
  LocaleProvider: ({ children }: { children: React.ReactNode }) => <>{children}</>,
  useLocale: () => ({ locale: 'ar', direction: 'rtl', setLocale: vi.fn(), toggleLocale: vi.fn() }),
}));
...
describe('Smoke Tests - No Console Errors', () => {
  ...
  beforeEach(() => {
    consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation((...args) => {
      // تجاهل أخطاء React المتوقعة في الاختبارات
  ...
  describe('Page Components Smoke Tests', () => {
    // The @pages barrel is large; compiling it on the first import inside a 5s test
    // can blow the per-test timeout. Warm it once up front so each test only renders.
    beforeAll(async () => {
      await import('@pages');
    }, 30000);

    it('Login page should render without React errors', async () => {
    ...
    it('Dashboard page should render without React errors', async () => {
    ...
    it('ProjectsList page should render without React errors', async () => {
      const { ProjectsList } = await import('@pages');
    ...
    it('TasksList page should render without React errors', async () => {
      const { TasksList } = await import('@pages');
    ...
    it('UsersList page should render without React errors', async () => {
      const { UsersList } = await import('@pages');
    ...
    it('DepartmentsList page should render without React errors', async () => {
      const { DepartmentsList } = await import('@pages');
    ...
    it('IncidentsList page should render without React errors', async () => {
      const { IncidentsList } = await import('@pages');
  ...
  describe('Critical Error Detection', () => {
    it('should not have any "Element type is invalid" errors (React #130)', () => {
      const error130Pattern = /Element type is invalid|#130/;
      const errorCalls = consoleErrorSpy.mock.calls;

      const error130Found = errorCalls.some((call) =>
        call.some((arg) => error130Pattern.test(String(arg)))
      );

      expect(error130Found).toBe(false);
    });

    it('should not have any "Objects are not valid as React child" errors (React #31)', () => {
      const error31Pattern = /Objects are not valid as a React child|#31/;
      const errorCalls = consoleErrorSpy.mock.calls;

      const error31Found = errorCalls.some((call) =>
        call.some((arg) => error31Pattern.test(String(arg)))
      );

      expect(error31Found).toBe(false);
    });
  });
});

describe('Import Validation Tests', () => {
  it('all page exports should be valid React components', async () => {
    ...
    componentNames.forEach((name) => {
      const component = (pages as Record<string, unknown>)[name];

      // التأكد من أن المكون موجود
      expect(component).toBeDefined();

      // التأكد من أن المكون ليس undefined
      expect(component).not.toBeUndefined();

      // التأكد من أن المكون من نوع صحيح
      expect(
  ...
  it('all UI component exports should be valid', async () => {
