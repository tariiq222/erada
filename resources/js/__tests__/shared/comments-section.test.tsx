...
vi.mock('@shared/lib/utils', () => ({
  formatDate: (date: Date | string) => '01/01/2025',
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  MentionInput: ({
    ...
    <div data-testid="mention-input">
      ...
      />
      <button onClick={onSubmit} disabled={disabled} data-testid="submit-comment">
        إرسال
      </button>
      <label htmlFor="comment-attachment-input">إرفاق ملف</label>
      <input
        id="comment-attachment-input"
        aria-label="إرفاق ملف"
        type="file"
        data-testid="comment-attachment-input"
        onChange={(event) => onAttachmentsChange?.(Array.from(event.currentTarget.files ?? []))}
      />
      <div aria-live="polite" data-testid="attachment-count">
        {attachments.length > 0 ? `${attachments.length} مرفق` : 'لا توجد مرفقات'}
      </div>
    </div>
  ),
}));

import CommentsSection from '@pages/tasks/components/CommentsSection';
...
const mockComments = [
  ...
  {
    id: 2,
    content: 'تعليق ثاني من @محمد',
    user: { id: 2, name: 'محمد' },
    created_at: '2025-01-14T09:00:00Z',
    attachments: [
      { id: 1, name: 'document.pdf', file_type: 'application/pdf', url: '/files/1' },
    ],
  },
];
...
describe('CommentsSection Component', () => {
  ...
  it('renders comments list', async () => {
    ...
    expect(screen.getByText('تعليق أول')).toBeInTheDocument();
  });

  it('renders user names for comments', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    // User names appear multiple times (in avatars and names)
    expect(screen.getAllByText('أحمد').length).toBeGreaterThan(0);
    expect(screen.getAllByText('محمد').length).toBeGreaterThan(0);
  });

  it('renders mention input component', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders input placeholder', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByPlaceholderText('اكتب تعليقاً... استخدم @ للإشارة لشخص')).toBeInTheDocument();
  });
});
...
describe('CommentsSection Adding Comment', () => {
  const defaultProps = {
    taskId: 1,
    initialComments: mockComments,
    onCommentsCountChange: vi.fn(),
  ...
  it('allows typing in comment input', async () => {
    ...
    await userEvent.type(input, 'تعليق جديد');

    expect(input).toHaveValue('تعليق جديد');
  });

  it('submits comment when clicking submit button', async () => {
  ...
  it('calls onCommentsCountChange after adding comment', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    ...
    await waitFor(() => {
      expect(defaultProps.onCommentsCountChange).toHaveBeenCalledWith(3); // 2 + 1
    });
  });
});

describe('CommentsSection Delete Comment', () => {
  const defaultProps = {
    taskId: 1,
    initialComments: mockComments,
    onCommentsCountChange: vi.fn(),
  ...
  it('shows delete button for own comments', async () => {
    ...
    // First comment belongs to user (id: 1); delete button has aria-label "حذف"
    expect(screen.getAllByRole('button', { name: /حذف/i }).length).toBeGreaterThan(0);
  });

  it('calls delete API when deleting comment', async () => {
    const { commentsApi } = await import('@entities/comment');

    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    // Find first delete button (user's own comment)
    const deleteButton = screen.getAllByRole('button', { name: /حذف/i })[0];
    await userEvent.click(deleteButton);

    await waitFor(() => {
      expect(commentsApi.delete).toHaveBeenCalledWith(1);
    });
  });

  it('calls onCommentsCountChange after deleting comment', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    ...
    await waitFor(() => {
      expect(defaultProps.onCommentsCountChange).toHaveBeenCalledWith(1); // 2 - 1
    });
  });
});
...
describe('CommentsSection Mentions', () => {
  ...
  const defaultProps = {
    taskId: 1,
    initialComments: commentsWithMentions,
    onCommentsCountChange: vi.fn(),
  };

  it('renders mention with special styling', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    // Mentions should be styled with @ icon
    const mentionElement = document.querySelector('.bg-\\[var\\(--accent-subtle\\)\\].text-\\[var\\(--accent-default\\)\\]');
    expect(mentionElement).toBeInTheDocument();
  });
});

describe('CommentsSection Attachments', () => {
  const commentsWithAttachments = [
    {
      id: 1,
      content: 'تعليق مع مرفقات',
      user: { id: 1, name: 'أحمد' },
      created_at: '2025-01-15T10:00:00Z',
      attachments: [
        { id: 1, name: 'ملف.pdf', file_type: 'application/pdf', url: '/files/1' },
        { id: 2, name: 'صورة.png', file_type: 'image/png', url: '/files/2' },
      ],
    },
  ];
  ...
  it('renders attachments for comment', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    expect(screen.getByText('ملف.pdf')).toBeInTheDocument();
    expect(screen.getByText('صورة.png')).toBeInTheDocument();
  });

  it('renders attachments as links', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    const attachmentLinks = document.querySelectorAll('a[target="_blank"]');
    expect(attachmentLinks.length).toBe(2);
    expect(attachmentLinks[0]).toHaveAttribute('href', '/files/1');
    expect(attachmentLinks[1]).toHaveAttribute('href', '/files/2');
  });

  it('submits attachments through the typed comment API path with accessible upload controls', async () => {
    const { commentsApi } = await import('@entities/comment');
    ...
    const file = new File(['محتوى'], 'evidence.pdf', { type: 'application/pdf' });
    await user.upload(fileInput, file);

    expect(screen.getByTestId('attachment-count')).toHaveTextContent('1 مرفق');
...
describe('CommentsSection User Avatar', () => {
  const defaultProps = {
    taskId: 1,
    initialComments: mockComments,
    onCommentsCountChange: vi.fn(),
  ...
  it('renders user avatar with first letter', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    // Avatar should show first letter of name
    const avatars = document.querySelectorAll('.h-8.w-8.rounded-full');
    expect(avatars.length).toBe(2);

    // First comment by أحمد - should show أ
    expect(avatars[0].textContent).toContain('أ');

    // Second comment by محمد - should show م
    expect(avatars[1].textContent).toContain('م');
  });
});

describe('CommentsSection Relative Time', () => {
  const defaultProps = {
    taskId: 1,
    initialComments: mockComments,
    onCommentsCountChange: vi.fn(),
  ...
  it('shows relative time for comments', async () => {
    ...
    // The component formats time relatively
    // With mocked formatDate it should show something
    const timeElements = document.querySelectorAll('.text-xs.text-\\[var\\(--text-tertiary\\)\\]');
    expect(timeElements.length).toBeGreaterThan(0);
  });
});

describe('CommentsSection API Error', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('handles create comment error gracefully', async () => {
    const { commentsApi } = await import('@entities/comment');
    (commentsApi.create as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
    ...
    const input = screen.getByTestId('comment-input');
    await userEvent.type(input, 'تعليق فاشل');
  ...
  it('handles delete comment error gracefully', async () => {
    const { commentsApi } = await import('@entities/comment');
    (commentsApi.delete as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
    ...
    const deleteButton = document.querySelector('button.opacity-0');
    if (deleteButton) {
      await userEvent.click(deleteButton);
