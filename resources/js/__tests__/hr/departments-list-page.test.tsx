...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'common.settings': '[Settings]', 'common.view_details': 'عرض التفاصيل', 'common.view_link': '[View Link]', 'hr.add_department': 'إضافة قسم',
    'hr.add_new_department': 'إضافة قسم جديد', 'hr.add_new_employee': '[Add New Employee]', 'hr.chart': '[Chart]', 'hr.chart_view': '[Chart View]',
    'hr.contact_info': 'معلومات الاتصال', 'hr.department': 'القسم', 'hr.department_delete_error': 'فشل في حذف القسم', 'hr.department_delete_success': 'تم حذف القسم بنجاح',
    'hr.departments_load_error': 'فشل في تحميل الأقسام', 'hr.departments_subtitle': 'إدارة الهيكل التنظيمي للأقسام', 'hr.employee': 'موظف', 'hr.job_title': 'المسمى الوظيفي',
    'hr.manager': 'المدير', 'hr.no_departments_desc': 'ابدأ بإضافة أول قسم', 'hr.no_employees_desc': '[No Employees Desc]', 'hr.table': 'جدول',
...
vi.mock('@entities/hr', () => ({
  departmentsApi: {
    getAll: vi.fn().mockResolvedValue({
      data: [
        { id: 1, name: 'قسم تقنية المعلومات', code: 'IT', is_active: true, employees_count: 10, manager: { name: 'محمد' } },
        { id: 2, name: 'قسم الموارد البشرية', code: 'HR', is_active: true, employees_count: 5, manager: null },
        { id: 3, name: 'قسم المالية', code: 'FIN', is_active: false, employees_count: 8, manager: { name: 'أحمد' } },
      ],
      current_page: 1,
      last_page: 1,
      per_page: 15,
...
vi.mock('@shared/contexts/AuthContext', () => ({
  useAuth: () => ({
    user: {
      id: 1,
      name: 'Test User',
      // Phase 9.3: production code reads useCan('departments.*') from
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon, onClick, loading, disabled }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void; loading?: boolean; disabled?: boolean }) => (
...
vi.mock('@pages/hr/components', () => ({
  OrgChart: ({ onDepartmentClick }: { onDepartmentClick?: (dept: any) => void }) => (
    <div data-testid="org-chart" onClick={() => onDepartmentClick?.({ id: 1, name: 'Test' })} />
  ),
  DepartmentsTable: ({
    departments,
    isLoading,
    ...
    <div data-testid="departments-table">
      {isLoading ? (
        ...
        departments.map((dept: any) => (
          <div key={dept.id} data-testid="department-row">
            <span>{dept.name}</span>
            <span>{dept.code}</span>
            <button onClick={() => onEdit(dept)}>تعديل</button>
            <button onClick={() => onDelete(dept)}>حذف</button>
          </div>
        ))
      )}
      <button onClick={() => onPageChange(2)}>الصفحة التالية</button>
      <button onClick={onAddNew}>إضافة جديد</button>
    </div>
  ),
...
describe('DepartmentsList Page', () => {
  ...
  it('renders page description', async () => {
    render(<DepartmentsList />);
    await waitFor(() => {
      expect(screen.getByText('إدارة الهيكل التنظيمي للأقسام')).toBeInTheDocument();
    });
  });
  ...
  it('renders add department button', async () => {
    render(<DepartmentsList />);
    await waitFor(() => {
      expect(screen.getByText('إضافة قسم')).toBeInTheDocument();
    });
  });
  ...
  it('fetches departments on mount', async () => {
    ...
    render(<DepartmentsList />);
    await waitFor(() => {
      expect(departmentsApi.getAll).toHaveBeenCalled();
    });
  });

  it('renders view mode toggle', async () => {
    render(<DepartmentsList />);
    // Stats cards were replaced by a table/chart view-mode toggle in the header.
    await waitFor(() => {
  ...
  it('renders search input', async () => {
    render(<DepartmentsList />);
    await waitFor(() => {
      expect(screen.getByPlaceholderText('البحث في الأقسام...')).toBeInTheDocument();
    });
  });
});

describe('DepartmentsList Search', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('filters departments on search input change', async () => {
    const user = userEvent.setup();
    const { departmentsApi } = await import('@entities/hr');

    render(<DepartmentsList />);

    const searchInput = screen.getByPlaceholderText('البحث في الأقسام...');
    await user.type(searchInput, 'IT');

    // Wait for debounce
    await waitFor(() => {
      expect(departmentsApi.getAll).toHaveBeenCalledWith(
        expect.objectContaining({ search: 'IT' })
      );
    }, { timeout: 500 });
  });
});

describe('DepartmentsList Delete', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('deletes department on confirm', async () => {
    const { departmentsApi } = await import('@entities/hr');
    ...
    await waitFor(() => {
      expect(departmentsApi.delete).toHaveBeenCalled();
    });
  });
});

describe('DepartmentsList Form Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('navigates to create department page', async () => {
    render(<DepartmentsList />);
    ...
    fireEvent.click(screen.getByText('إضافة قسم'));

    expect(mockNavigate).toHaveBeenCalledWith('/hr/departments/new');
  });
  ...
  it('navigates to edit department page', async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith(expect.stringMatching(/^\/hr\/departments\/\d+\/edit$/));
  });
});

describe('DepartmentsList API Error', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows error toast on fetch failure', async () => {
    const { departmentsApi } = await import('@entities/hr');
    (departmentsApi.getAll as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));

    render(<DepartmentsList />);

    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في تحميل الأقسام');
    });
  });

  it('shows error toast on delete failure', async () => {
    const { departmentsApi } = await import('@entities/hr');
    (departmentsApi.delete as ReturnType<typeof vi.fn>).mockRejectedValueOnce({ message: 'Cannot delete' });
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'Cannot delete');
    });
  });
});

describe('DepartmentsList Pagination', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
