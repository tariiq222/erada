...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconUserCircle: () => <span data-testid="user-icon">UserCircle</span>,
...
import StatsCards from '@pages/hr/components/StatsCards';

const mockEmployees = [
  { id: 1, name: 'موظف 1', status: 'active' },
  { id: 2, name: 'موظف 2', status: 'active' },
  { id: 3, name: 'موظف 3', status: 'active' },
  { id: 4, name: 'موظف 4', status: 'on_leave' },
  { id: 5, name: 'موظف 5', status: 'terminated' },
];

describe('StatsCards Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders 4 stat cards', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    expect(screen.getAllByTestId('stat-card').length).toBe(4);
  });

  it('shows total employees label', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
...
describe('StatsCards Values', () => {
  ...
  it('shows correct active count', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const values = screen.getAllByTestId('stat-value');
    expect(values[1]).toHaveTextContent('3'); // 3 active
  });

  it('shows correct on leave count', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const values = screen.getAllByTestId('stat-value');
    expect(values[2]).toHaveTextContent('1'); // 1 on leave
  });

  it('shows correct terminated count', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const values = screen.getAllByTestId('stat-value');
    expect(values[3]).toHaveTextContent('1'); // 1 terminated
  });
});

describe('StatsCards Colors', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('uses accent color for total', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const cards = screen.getAllByTestId('stat-card');
    expect(cards[0]).toHaveAttribute('data-color', 'accent');
  });

  it('uses success color for active', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const cards = screen.getAllByTestId('stat-card');
    expect(cards[1]).toHaveAttribute('data-color', 'success');
  });

  it('uses warning color for on leave', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const cards = screen.getAllByTestId('stat-card');
    expect(cards[2]).toHaveAttribute('data-color', 'warning');
  });

  it('uses danger color for terminated', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    const cards = screen.getAllByTestId('stat-card');
...
describe('StatsCards Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows zero for all counts when no employees', () => {
    render(<StatsCards employees={[]} total={0} />);
    const values = screen.getAllByTestId('stat-value');
...
describe('StatsCards All Active', () => {
  ...
  it('shows all as active', () => {
    render(<StatsCards employees={allActiveEmployees as any} total={3} />);
    const values = screen.getAllByTestId('stat-value');
    expect(values[0]).toHaveTextContent('3'); // Total
    expect(values[1]).toHaveTextContent('3'); // Active
    expect(values[2]).toHaveTextContent('0'); // On leave
    expect(values[3]).toHaveTextContent('0'); // Terminated
  });
});
...
describe('StatsCards Icons', () => {
  ...
  it('renders user icons', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
    // 3 UserCircle icons (total, active, terminated) + 1 Calendar for on_leave
    expect(screen.getAllByTestId('user-icon').length).toBe(3);
  });

  it('renders calendar icon for on leave', () => {
    render(<StatsCards employees={mockEmployees as any} total={5} />);
