...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'hr.manager': 'المدير', 'hr.no_departments_desc': 'ابدأ بإضافة أول قسم', 'hr.no_employees_desc': 'لم يتم إضافة أي موظفين بعد', 'hr.table': 'جدول',
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconNetwork: () => <span data-testid="network-icon">Network</span>,
...
// Mock departmentTypes
vi.mock('@pages/hr/components/departmentTypes', () => ({
...
vi.mock('@shared/ui', () => ({
  ...
  // Lightweight stand-in for the real DataTable (which internally calls
  // useNavigate and would require a Router). Renders headers, rows, actions,
  // empty state, and pagination so the existing assertions still resolve.
  RowAction: ({ icon: Icon, label, onClick }: any) => (
    <button type="button" onClick={onClick} title={label} aria-label={label}>
      <Icon />
    </button>
  ),
  DataTable: ({ data, columns, loading, actions, empty, pagination }: any) => {
    if (loading) {
      return (
    ...
    if (!data || data.length === 0) {
      ...
      return (
        <div data-testid="card">
          {Icon ? <Icon /> : null}
          {empty?.title ? <p>{empty.title}</p> : null}
...
import DepartmentsTable from '@pages/hr/components/DepartmentsTable';
...
describe('DepartmentsTable Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders table component', () => {
    render(<DepartmentsTable {...defaultProps} />);
  ...
  it('shows table headers', () => {
    ...
    expect(screen.getByText('مستوى')).toBeInTheDocument();
    expect(screen.getByText('القسم الأعلى')).toBeInTheDocument();
...
describe('DepartmentsTable Data Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows department code when available', () => {
    render(<DepartmentsTable {...defaultProps} />);
  ...
  it('shows level badges', () => {
    render(<DepartmentsTable {...defaultProps} />);
    expect(screen.getByText('إدارة رئيسية')).toBeInTheDocument();
  });

  it('shows parent department name', () => {
    render(<DepartmentsTable {...defaultProps} />);
    expect(screen.getByText('الإدارة العامة')).toBeInTheDocument();
  });

  it('shows dash for no parent', () => {
    render(<DepartmentsTable {...defaultProps} />);
...
describe('DepartmentsTable Actions', () => {
  ...
  it('calls onEdit when edit clicked', () => {
    ...
    expect(defaultProps.onEdit).toHaveBeenCalledWith(mockDepartments[0]);
  });
  ...
  it('calls onDelete when delete clicked', () => {
    ...
    expect(defaultProps.onDelete).toHaveBeenCalledWith(mockDepartments[0]);
  });
});

describe('DepartmentsTable Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows skeletons when loading', () => {
    render(<DepartmentsTable {...defaultProps} isLoading={true} />);
...
describe('DepartmentsTable Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows empty state message', () => {
    render(<DepartmentsTable {...defaultProps} departments={[]} />);
    expect(screen.getByText('لا توجد أقسام')).toBeInTheDocument();
  });

  it('shows empty state description', () => {
    render(<DepartmentsTable {...defaultProps} departments={[]} />);
    expect(screen.getByText('ابدأ بإضافة أول قسم')).toBeInTheDocument();
  });

  it('shows add button in empty state', () => {
    render(<DepartmentsTable {...defaultProps} departments={[]} />);
    expect(screen.getByRole('button', { name: /إضافة قسم جديد/ })).toBeInTheDocument();
  });

  it('calls onAddNew when add button clicked', () => {
    render(<DepartmentsTable {...defaultProps} departments={[]} />);
    fireEvent.click(screen.getByRole('button', { name: /إضافة قسم جديد/ }));
    expect(defaultProps.onAddNew).toHaveBeenCalled();
  });
});

describe('DepartmentsTable Pagination', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('hides pagination for single page', () => {
    render(<DepartmentsTable {...defaultProps} pagination={{ currentPage: 1, lastPage: 1, perPage: 10, total: 5 }} />);
