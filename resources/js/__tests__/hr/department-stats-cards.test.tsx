...
import DepartmentStatsCards from '@pages/hr/components/DepartmentStatsCards';

const mockDepartments = [
  { id: 1, name: 'قسم التقنية', is_active: true, level: 1, employees_count: 10 },
  { id: 2, name: 'قسم الموارد البشرية', is_active: true, level: 1, employees_count: 5 },
  { id: 3, name: 'قسم المالية', is_active: false, level: 1, employees_count: 3 },
];

describe('DepartmentStatsCards Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders two cards', () => {
    render(<DepartmentStatsCards departments={mockDepartments} total={3} />);
    expect(screen.getAllByTestId('card').length).toBe(2);
  });

  it('shows total departments label', () => {
    render(<DepartmentStatsCards departments={mockDepartments} total={3} />);
    expect(screen.getByText('إجمالي الأقسام')).toBeInTheDocument();
  });
...
describe('DepartmentStatsCards Active Departments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows active departments label', () => {
    render(<DepartmentStatsCards departments={mockDepartments} total={3} />);
    expect(screen.getByText('أقسام نشطة')).toBeInTheDocument();
  });

  it('counts active departments correctly', () => {
    render(<DepartmentStatsCards departments={mockDepartments} total={3} />);
    expect(screen.getByText('2')).toBeInTheDocument();
  });

  it('shows zero for no active departments', () => {
    const inactiveDepartments = [
      { id: 1, name: 'قسم 1', is_active: false, level: 1, employees_count: 0 },
    ];
    render(<DepartmentStatsCards departments={inactiveDepartments} total={1} />);
    expect(screen.getByText('0')).toBeInTheDocument();
  });

  it('counts all when all active', () => {
    const allActive = [
      { id: 1, name: 'قسم 1', is_active: true, level: 1, employees_count: 0 },
      { id: 2, name: 'قسم 2', is_active: true, level: 1, employees_count: 0 },
      { id: 3, name: 'قسم 3', is_active: true, level: 1, employees_count: 0 },
    ];
    render(<DepartmentStatsCards departments={allActive} total={3} />);
    // Both total (3) and active count (3) should appear
    expect(screen.getAllByText('3').length).toBe(2);
  });
});

describe('DepartmentStatsCards Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows zero total for empty departments', () => {
    render(<DepartmentStatsCards departments={[]} total={0} />);
    // Both cards show 0
    expect(screen.getAllByText('0').length).toBe(2);
  });
});

describe('DepartmentStatsCards Large Numbers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays large total number', () => {
    render(<DepartmentStatsCards departments={[]} total={150} />);
