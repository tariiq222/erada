...
// Note: @shared/ui is intentionally NOT mocked. The wrapper delegates to the
// real DeleteConfirmationModal which renders the canonical dialog (createPortal,
// <h2> title, item panel, warning, real Button). Tests assert against real DOM
// roles/text — see assertions below.

import DeleteEmployeeModal from '@pages/hr/components/DeleteEmployeeModal';
...
describe('DeleteEmployeeModal Basic', () => {
  const defaultProps = {
    isOpen: true,
  ...
  it('renders when open', () => {
    render(<DeleteEmployeeModal {...defaultProps} />);
    expect(screen.getByRole('dialog')).toBeInTheDocument();
  });

  it('does not render when closed', () => {
    render(<DeleteEmployeeModal {...defaultProps} isOpen={false} />);
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
  });

  it('renders modal title', () => {
    render(<DeleteEmployeeModal {...defaultProps} />);
    expect(screen.getByRole('heading', { name: 'حذف الموظف' })).toBeInTheDocument();
  });
...
describe('DeleteEmployeeModal Buttons', () => {
  const defaultProps = {
    isOpen: true,
    employee: mockEmployee,
    isDeleting: false,
  ...
  it('shows loading spinner when deleting', () => {
    render(<DeleteEmployeeModal {...defaultProps} isDeleting={true} />);
    expect(document.querySelector('.animate-spin')).toBeInTheDocument();
  });

  it('disables delete button when deleting', () => {
    render(<DeleteEmployeeModal {...defaultProps} isDeleting={true} />);
    const deleteButton = screen.getByText('حذف').closest('button');
    expect(deleteButton).toBeDisabled();
  });
});

describe('DeleteEmployeeModal with null employee', () => {
  const propsWithNullEmployee = {
    isOpen: true,
    employee: null,
    isDeleting: false,
    onClose: vi.fn(),
  ...
  it('does not render when employee is null', () => {
    render(<DeleteEmployeeModal {...propsWithNullEmployee} />);
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
  });

  it('does not show confirmation message when employee is null', () => {
    render(<DeleteEmployeeModal {...propsWithNullEmployee} />);
