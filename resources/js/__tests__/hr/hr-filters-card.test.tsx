...
vi.mock('@shared/ui', () => ({
  ...
  FilterRow: ({ children, onClear, clearLabel }: {
  ...
  Input: ({ value, onChange, placeholder, leftIcon }: {
    value: string;
...
import FiltersCard from '@pages/hr/components/FiltersCard';
...
describe('FiltersCard Basic', () => {
  const defaultProps = {
    filters: { search: '', department_id: '', status: '' },
...
describe('FiltersCard Search Input', () => {
  const defaultProps = {
    filters: { search: '', department_id: '', status: '' },
    departments: mockDepartments,
    onFiltersChange: vi.fn(),
  ...
  it('shows search placeholder', () => {
    render(<FiltersCard {...defaultProps} />);
    expect(screen.getByPlaceholderText('بحث بالاسم أو الرقم الوظيفي...')).toBeInTheDocument();
  });
  ...
  it('displays existing search value', () => {
    render(<FiltersCard {...defaultProps} filters={{ search: 'موظف', department_id: '', status: '' }} />);
    expect(screen.getByTestId('search-input')).toHaveValue('موظف');
  });
});

describe('FiltersCard Department Select', () => {
  const defaultProps = {
    filters: { search: '', department_id: '', status: '' },
    departments: mockDepartments,
    onFiltersChange: vi.fn(),
  ...
  it('shows all departments option', () => {
    render(<FiltersCard {...defaultProps} />);
    expect(screen.getByText('كل الأقسام')).toBeInTheDocument();
  });
  ...
  it('calls onFiltersChange when department changes', () => {
    render(<FiltersCard {...defaultProps} />);
...
describe('FiltersCard Status Select', () => {
  const defaultProps = {
    filters: { search: '', department_id: '', status: '' },
    departments: mockDepartments,
    onFiltersChange: vi.fn(),
...
describe('FiltersCard Empty Departments', () => {
  const propsWithEmptyDepartments = {
    filters: { search: '', department_id: '', status: '' },
    departments: [],
    onFiltersChange: vi.fn(),
  };
  ...
  it('shows only all departments option when empty', () => {
    render(<FiltersCard {...propsWithEmptyDepartments} />);
    expect(screen.getByText('كل الأقسام')).toBeInTheDocument();
    expect(screen.queryByText('قسم التقنية')).not.toBeInTheDocument();
  });
});

describe('FiltersCard With Values', () => {
  const propsWithValues = {
    filters: { search: 'موظف', department_id: '2', status: 'active' },
    departments: mockDepartments,
    onFiltersChange: vi.fn(),
  };
  ...
  it('displays existing filter values', () => {
    render(<FiltersCard {...propsWithValues} />);
    expect(screen.getByTestId('search-input')).toHaveValue('موظف');
  });

  it('maintains filter values on render', () => {
    render(<FiltersCard {...propsWithValues} />);
    const selects = screen.getAllByTestId('select');
