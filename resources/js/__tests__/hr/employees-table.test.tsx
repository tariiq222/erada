...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'hr.add_new_department': 'إضافة قسم جديد', 'hr.add_new_employee': 'إضافة موظف جديد', 'hr.chart': '[Chart]', 'hr.chart_view': '[Chart View]',
    ...
    'hr.departments_load_error': 'فشل في تحميل الأقسام', 'hr.departments_subtitle': 'إدارة الهيكل التنظيمي للأقسام', 'hr.employee': 'الموظف', 'hr.job_title': 'المسمى الوظيفي',
    'hr.manager': 'المدير', 'hr.no_departments_desc': 'ابدأ بإضافة أول قسم', 'hr.no_employees_desc': 'ابدأ بإضافة أول موظف', 'hr.table': 'جدول',
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconUserCircle: () => <span data-testid="user-circle-icon">UserCircle</span>,
...
// Mock constants
vi.mock('@pages/hr/components/constants', () => ({
  statusLabels: {
    active: 'نشط',
    on_leave: 'في إجازة',
    terminated: 'منتهي',
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Avatar: ({ name }: any) => <span data-testid="avatar">{(name ?? '').charAt(0)}</span>,
...
import EmployeesTable from '@pages/hr/components/EmployeesTable';
...
describe('EmployeesTable Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders table component', () => {
    render(<EmployeesTable {...defaultProps} />);
...
describe('EmployeesTable Data Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows dash for no department', () => {
    render(<EmployeesTable {...defaultProps} />);
...
describe('EmployeesTable Icons', () => {
  ...
  it('renders building icons', () => {
    render(<EmployeesTable {...defaultProps} />);
    expect(screen.getByTestId('building-icon')).toBeInTheDocument();
  });

  it('renders briefcase icons', () => {
    render(<EmployeesTable {...defaultProps} />);
...
describe('EmployeesTable Actions', () => {
  ...
  it('calls onDelete when delete clicked', () => {
    ...
    expect(defaultProps.onDelete).toHaveBeenCalledWith(mockEmployees[0]);
  });
});

describe('EmployeesTable Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows skeletons when loading', () => {
    render(<EmployeesTable {...defaultProps} isLoading={true} />);
...
describe('EmployeesTable Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows empty state message', () => {
    render(<EmployeesTable {...defaultProps} employees={[]} />);
    expect(screen.getByText('لا يوجد موظفون')).toBeInTheDocument();
  });

  it('shows empty state description', () => {
    render(<EmployeesTable {...defaultProps} employees={[]} />);
    expect(screen.getByText('ابدأ بإضافة أول موظف')).toBeInTheDocument();
  });

  it('shows add button in empty state', () => {
    render(<EmployeesTable {...defaultProps} employees={[]} />);
    expect(screen.getByRole('button', { name: /إضافة موظف جديد/ })).toBeInTheDocument();
  });

  it('calls onAddNew when add button clicked', () => {
    render(<EmployeesTable {...defaultProps} employees={[]} />);
    fireEvent.click(screen.getByRole('button', { name: /إضافة موظف جديد/ }));
    expect(defaultProps.onAddNew).toHaveBeenCalled();
  });

  it('shows user circle icon in empty state', () => {
    render(<EmployeesTable {...defaultProps} employees={[]} />);
    expect(screen.getByTestId('user-circle-icon')).toBeInTheDocument();
  });
});

describe('EmployeesTable Pagination', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('hides pagination for single page', () => {
    render(<EmployeesTable {...defaultProps} pagination={{ currentPage: 1, lastPage: 1, perPage: 10, total: 5 }} />);
