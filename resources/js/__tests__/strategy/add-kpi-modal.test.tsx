...
import AddKPIModal from '@pages/projects/components/modals/AddKPIModal';

describe('AddKPIModal Basic', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
  ...
  it('renders when open', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('إضافة مؤشر أداء')).toBeInTheDocument();
  });

  it('does not render when closed', () => {
    render(<AddKPIModal {...defaultProps} isOpen={false} />);
    expect(screen.queryByText('إضافة مؤشر أداء')).not.toBeInTheDocument();
  });

  it('shows indicator field label', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('المؤشر *')).toBeInTheDocument();
  });

  it('shows target field label', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('القيمة المستهدفة *')).toBeInTheDocument();
  });

  it('shows current value field label', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('القيمة الحالية')).toBeInTheDocument();
  });

  it('shows unit field label', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('الوحدة')).toBeInTheDocument();
  });

  it('shows indicator placeholder', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('اسم مؤشر الأداء')).toBeInTheDocument();
  });

  it('shows target placeholder', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('مثال: 100')).toBeInTheDocument();
  });

  it('shows current value placeholder', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('مثال: 50')).toBeInTheDocument();
  });

  it('shows unit placeholder', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('مثال: % أو ريال أو عدد')).toBeInTheDocument();
  });
});

describe('AddKPIModal Form Interaction', () => {
  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    formData: {
  ...
  it('calls onFormChange when indicator changes', () => {
    render(<AddKPIModal {...defaultProps} />);
    const input = screen.getByPlaceholderText('اسم مؤشر الأداء');
    fireEvent.change(input, { target: { value: 'مؤشر جديد' } });
  ...
  it('calls onFormChange when target changes', () => {
    render(<AddKPIModal {...defaultProps} />);
    const input = screen.getByPlaceholderText('مثال: 100');
  ...
  it('calls onFormChange when current value changes', () => {
    render(<AddKPIModal {...defaultProps} />);
    const input = screen.getByPlaceholderText('مثال: 50');
  ...
  it('calls onFormChange when unit changes', () => {
    render(<AddKPIModal {...defaultProps} />);
    const input = screen.getByPlaceholderText('مثال: % أو ريال أو عدد');
...
describe('AddKPIModal Buttons', () => {
  ...
  it('renders add button', () => {
    render(<AddKPIModal {...defaultProps} />);
    expect(screen.getByText('إضافة المؤشر')).toBeInTheDocument();
  });
  ...
  it('calls onClose when X icon clicked', () => {
    render(<AddKPIModal {...defaultProps} />);
    const closeButton = screen.getByTestId('x-circle-icon').parentElement;
    if (closeButton) {
...
describe('AddKPIModal Validation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('disables add button when indicator empty', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
    ...
    expect(screen.getByText('إضافة المؤشر')).toBeDisabled();
  });

  it('disables add button when target empty', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
  ...
  it('enables add button when required fields filled', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
    ...
    expect(screen.getByText('إضافة المؤشر')).not.toBeDisabled();
  });

  it('calls onAdd when add button clicked with valid data', () => {
    const onAdd = vi.fn();
    render(
      <AddKPIModal
        isOpen={true}
    ...
    fireEvent.click(screen.getByText('إضافة المؤشر'));
    expect(onAdd).toHaveBeenCalled();
  });
});

describe('AddKPIModal Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('disables add button when loading', () => {
    ...
    expect(screen.getByText('جاري...')).toBeDisabled();
  });
...
describe('AddKPIModal Form Data Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays existing indicator value', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
        formData={{
          indicator: 'نسبة الإنجاز',
          target: '',
          current_value: '',
          unit: '',
        }}
    ...
    expect(screen.getByDisplayValue('نسبة الإنجاز')).toBeInTheDocument();
  });

  it('displays existing target value', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
  ...
  it('displays existing current value', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
  ...
  it('displays existing unit value', () => {
    render(
      <AddKPIModal
        isOpen={true}
        onClose={vi.fn()}
