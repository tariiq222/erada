...
import GoldenChainView from '../../pages/strategy/components/GoldenChainView';

// Mock full chain data - Updated to new structure (portfolio, program, project)
const mockFullChain = {
  portfolio: { id: 1, code: 'PF-2024-001', name: 'الالتزام الأول' },
  program: { id: 2, code: 'PRG-2024-001', name: 'المبادرة الأولى' },
  project: { id: 3, code: 'PRJ-2024-001', name: 'المشروع الأول' },
};

// Mock unlinked project chain
const mockUnlinkedChain = {
  portfolio: null,
  program: null,
  project: { id: 1, code: 'PRJ-2024-001', name: 'مشروع غير مرتبط' },
};
...
vi.mock('@entities/strategy', () => ({
  strategyDashboardApi: {
    getSummary: vi.fn(),
    getGoldenChain: vi.fn(() => Promise.resolve(mockFullChain)),
  },
}));
...
describe('GoldenChainView', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders loading state initially', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockReturnValueOnce(new Promise(() => {}) as never);
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    );
  ...
  it('renders chain title after loading', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('السلسلة الذهبية')).toBeInTheDocument();
    });
  });

  it('renders portfolio in chain', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('الالتزام التنفيذي')).toBeInTheDocument();
      expect(screen.getByText('الالتزام الأول')).toBeInTheDocument();
    });
  });

  it('renders program in chain', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('المبادرة')).toBeInTheDocument();
      expect(screen.getByText('المبادرة الأولى')).toBeInTheDocument();
    });
  });

  it('renders project in chain', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('المشروع الأول')).toBeInTheDocument();
    });
  });

  it('shows warning for unlinked project', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockResolvedValueOnce(mockUnlinkedChain);

    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} showWarning={true} />
      </TestWrapper>
    );

    await waitFor(() => {
      // When project has no program, warning appears
      // The project name from mock data also renders
      expect(screen.getByText('مشروع غير مرتبط')).toBeInTheDocument();
    });
  });

  it('does not show warning when showWarning is false', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockResolvedValueOnce(mockUnlinkedChain);

    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} showWarning={false} />
      </TestWrapper>
    );

    await waitFor(() => {
      expect(screen.queryByText('المشروع غير مرتبط')).not.toBeInTheDocument();
    });
  });

  it('renders navigation links for each chain item', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      const portfolioLink = document.querySelector('a[href="/strategy/portfolios/1"]');
      const programLink = document.querySelector('a[href="/strategy/programs/2"]');
      const projectLink = document.querySelector('a[href="/projects/3"]');
  ...
  it('renders separator arrows between items', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
    ...
    await waitFor(() => {
      // ChevronLeft icons used as separators
      const separators = document.querySelectorAll('.tabler-icon-chevron-left');
  ...
  it('handles API error gracefully', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockRejectedValueOnce(new Error('API Error'));
    ...
    await waitFor(() => {
      // يظهر رسالة الخطأ (err.message من الخطأ المرسل)
      expect(screen.getByText('API Error')).toBeInTheDocument();
    });
  });

  it('renders icons for each entity type', async () => {
    render(
      <TestWrapper>
        <GoldenChainView type="project" id={1} />
      </TestWrapper>
...
describe('GoldenChainView - Different Entity Types', () => {
  it('fetches chain for portfolio type', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');

    render(
      <TestWrapper>
        <GoldenChainView type="portfolio" id={1} />
      </TestWrapper>
    );

    await waitFor(() => {
      expect(strategyDashboardApi.getGoldenChain).toHaveBeenCalledWith('portfolio', 1);
    });
  });

  it('fetches chain for program type', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');

    render(
      <TestWrapper>
        <GoldenChainView type="program" id={2} />
      </TestWrapper>
    );

    await waitFor(() => {
      expect(strategyDashboardApi.getGoldenChain).toHaveBeenCalledWith('program', 2);
    });
  });

  it('fetches chain for project type', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');

    render(
      <TestWrapper>
        <GoldenChainView type="project" id={3} />
      </TestWrapper>
    );

    await waitFor(() => {
      expect(strategyDashboardApi.getGoldenChain).toHaveBeenCalledWith('project', 3);
    });
  });
});

describe('GoldenChainView - Edge Cases', () => {
  it('handles empty chain gracefully', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockResolvedValueOnce({
      portfolio: null,
      program: null,
      project: null,
    });
  ...
  it('handles partial chain (only portfolio)', async () => {
    const { strategyDashboardApi } = await import('@entities/strategy');
    vi.mocked(strategyDashboardApi.getGoldenChain).mockResolvedValueOnce({
      portfolio: { id: 1, code: 'PF-001', name: 'الالتزام' },
      program: null,
      project: null,
    });
    ...
    await waitFor(() => {
      expect(screen.getByText('الالتزام')).toBeInTheDocument();
    });
  });
  ...
  it('refetches when id changes', async () => {
    ...
    await waitFor(() => {
      expect(strategyDashboardApi.getGoldenChain).toHaveBeenCalledWith('project', 1);
    });

    rerender(
      <TestWrapper>
        <GoldenChainView type="project" id={2} />
      </TestWrapper>
    );

    await waitFor(() => {
      expect(strategyDashboardApi.getGoldenChain).toHaveBeenCalledWith('project', 2);
    });
  });
});
