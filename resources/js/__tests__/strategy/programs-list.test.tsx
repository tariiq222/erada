...
import ProgramsList from '../../pages/strategy/programs/ProgramsList';
  ...
  data: [
    {
      ...
      program_manager: { id: 1, name: 'أحمد محمد' },
    },
    {
      id: 2,
      code: 'PRG-2024-002',
      name: 'مبادرة تطوير الموارد البشرية',
      status: 'planning',
      status_label: 'تخطيط',
      priority: 'medium',
      ...
      progress: 10,
      projects_count: 2,
      portfolio: { id: 2, code: 'PF-002', name: 'الالتزام الثاني' },
...
const mockPortfolios = [
  { id: 1, code: 'PF-001', name: 'الالتزام الأول' },
  { id: 2, code: 'PF-002', name: 'الالتزام الثاني' },
];
...
vi.mock('@entities/strategy', () => ({
  programsApi: {
    getAll: vi.fn(() => Promise.resolve(mockPrograms)),
    delete: vi.fn(() => Promise.resolve()),
  },
  portfoliosApi: {
    getList: vi.fn(() => Promise.resolve(mockPortfolios)),
  },
}));
...
describe('ProgramsList', () => {
  ...
  it('renders loading state initially', async () => {
    const { programsApi } = await import('@entities/strategy');
    vi.mocked(programsApi.getAll).mockReturnValueOnce(new Promise(() => {}) as never);
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
  ...
  it('renders page title and description', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/إدارة المبادرات التنفيذية/)).toBeInTheDocument();
    });
  });
  ...
  it('renders programs table with data', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('مبادرة التحول الرقمي')).toBeInTheDocument();
      expect(screen.getByText('مبادرة تطوير الموارد البشرية')).toBeInTheDocument();
    });
  });

  it('displays program codes', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('PRG-2024-001')).toBeInTheDocument();
      expect(screen.getByText('PRG-2024-002')).toBeInTheDocument();
    });
  });
  ...
  it('displays program status badges', async () => {
    ...
    await waitFor(() => {
      // Stats card has "قيد التنفيذ" label, and table might have status badges
      expect(screen.getAllByText('قيد التنفيذ').length).toBeGreaterThan(0);
    });
  });

  it('displays program priority badges', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
  ...
  it('displays portfolio links', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
  ...
  it('displays project count', async () => {
    ...
    await waitFor(() => {
      // Check that programs are displayed with their data
      expect(screen.getByText('مبادرة التحول الرقمي')).toBeInTheDocument();
      // Project count rendered in the table "projects" column
      expect(screen.getByText('المشاريع')).toBeInTheDocument();
      expect(screen.getByText('3')).toBeInTheDocument();
    });
  });

  it('displays program manager name', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
  ...
  it('renders stats cards', async () => {
    ...
    await waitFor(() => {
      // List now uses a DataTable; verify table column headers render
      expect(screen.getByText('المبادرات')).toBeInTheDocument();
      expect(screen.getByText('المشاريع')).toBeInTheDocument();
    });
  });
  ...
  it('renders new program button', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('مبادرة جديدة')).toBeInTheDocument();
    });
  });
  ...
  it('toggles filter panel when filter button clicked', async () => {
    ...
    // The FilterBar is now rendered inline (no toggle button);
    // the search input and status filter are always available.
    await waitFor(() => {
      expect(screen.getAllByText('كل الحالات').length).toBeGreaterThan(0);
    });
  ...
  it('has link to create new program', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      const newProgramLink = document.querySelector('a[href="/strategy/programs/new"]');
      expect(newProgramLink).toBeInTheDocument();
    });
  });

  it('has action buttons for each program', async () => {
    render(
      <TestWrapper>
        <ProgramsList />
      </TestWrapper>
...
describe('ProgramsList - Empty State', () => {
  it('shows empty state when no programs', async () => {
    const { programsApi } = await import('@entities/strategy');
    vi.mocked(programsApi.getAll).mockResolvedValueOnce({
      data: [],
      current_page: 1,
      last_page: 1,
      total: 0,
    ...
    await waitFor(() => {
      expect(screen.getByText('لا توجد مبادرات')).toBeInTheDocument();
      expect(screen.getByText(/ابدأ بإنشاء أول مبادرة/)).toBeInTheDocument();
    });
  });
  ...
  it('shows create button in empty state', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('إنشاء مبادرة جديدة')).toBeInTheDocument();
    });
  });
});

describe('ProgramsList - Delete Modal', () => {
...
describe('ProgramsList - Filters', () => {
  it('fetches programs with search filter', async () => {
    const { programsApi } = await import('@entities/strategy');
