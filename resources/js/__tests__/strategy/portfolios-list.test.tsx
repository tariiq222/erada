...
import DirectionsList from '../../pages/strategy/portfolios/DirectionsList';
  ...
  data: [
    {
      id: 1,
      code: 'PF-2024-001',
      name: 'الهدف التنفيذي الأول',
      status: 'active',
      status_label: 'نشط',
      portfolio_status: 'active',
      portfolio_status_label: 'نشطة',
      portfolio_progress: 75,
      programs_count: 5,
      directive_source: 'moh',
      directive_source_label: 'وزارة الصحة',
    ...
    },
    {
      id: 2,
      code: 'PF-2024-002',
      name: 'الهدف التنفيذي الثاني',
      status: 'draft',
      status_label: 'مسودة',
      portfolio_status: 'frozen',
      portfolio_status_label: 'مجمدة',
      portfolio_progress: 0,
      programs_count: 0,
      directive_source: null,
      directive_source_label: '',
      owner: null,
      start_date: null,
      end_date: null,
    },
...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'nav.strategy_dashboard': 'لوحة التحكم', 'nav.portfolios': 'الأهداف التنفيذية', 'nav.programs': 'المبادرات', 'nav.projects': 'المشاريع',
    ...
    'projects.portfolio': 'الهدف التنفيذي', 'projects.members': 'الأعضاء', 'projects.add_member': 'إضافة عضو', 'projects.impact_low': 'منخفض',
    ...
    'strategy.portfolios': 'الأهداف التنفيذية', 'strategy.portfolio': 'هدف تنفيذي', 'strategy.programs': 'المبادرات', 'strategy.program': 'مبادرة',
    'strategy.create_portfolio': 'إنشاء هدف تنفيذي', 'strategy.create_program': 'إنشاء مبادرة', 'strategy.edit_portfolio': 'تعديل الهدف التنفيذي', 'strategy.edit_program': 'تعديل المبادرة',
    'strategy.view_portfolio': 'عرض الهدف التنفيذي', 'strategy.view_program': 'عرض المبادرة', 'strategy.portfolio_name': 'اسم الهدف', 'strategy.program_name': 'اسم المبادرة',
    'strategy.linked_projects': 'المشاريع المرتبطة', 'strategy.no_portfolios': 'لا توجد أهداف تنفيذية', 'strategy.no_programs': 'لا توجد مبادرات', 'strategy.owner': 'المسؤول',
    ...
    'projects.select_manager': 'اختر مدير المشروع', 'projects.select_department': 'اختر القسم', 'projects.select_initiative': 'اختر المبادرة', 'projects.select_portfolio': 'اختر الهدف التنفيذي',
    ...
    'strategy.start_date': 'تاريخ البداية', 'strategy.end_date': 'تاريخ النهاية', 'strategy.budget': 'الميزانية', 'strategy.search_portfolios': 'البحث في الأهداف...',
    ...
    'strategy.no_blockers': 'لا توجد معوقات', 'strategy.no_decisions': 'لا توجد قرارات', 'strategy.no_reviews': 'لا توجد مراجعات', 'strategy.delete_portfolio': 'حذف الهدف',
    ...
    'strategy.goldenChain.projectUnlinkedDesc': 'هذا المشروع غير مرتبط ببرنامج أو هدف تنفيذي', 'strategy.goldenChain.title': 'السلسلة الذهبية', 'strategy.portfolios.addNewPortfolio': 'إضافة هدف تنفيذي جديد', 'strategy.portfolios.allPortfolios': 'كل الأهداف',
    'strategy.portfolios.deletePortfolio': 'حذف الهدف التنفيذي', 'strategy.portfolios.deletePortfolioWarning': 'سيتم حذف الهدف التنفيذي وجميع البيانات المرتبطة. لا يمكن التراجع عن هذا الإجراء.', 'strategy.portfolios.executiveCommitment': 'الهدف التنفيذي', 'strategy.portfolios.executiveCommitments': 'الأهداف التنفيذية',
    'strategy.portfolios.totalPortfolios': 'إجمالي الأهداف', 'strategy.programs.addNewProgram': 'إضافة مبادرة جديدة', 'strategy.programs.avgProgress': 'متوسط التقدم', 'strategy.programs.budgetUtilization': 'نسبة استخدام الميزانية',
    ...
    'strategy.executive_planning': '[Executive Planning]', 'strategy.executive_sponsor': '[Executive Sponsor]', 'strategy.manager': '[Manager]', 'strategy.new_portfolio': 'هدف جديد',
...
vi.mock('@entities/strategy', () => ({
  portfoliosApi: {
    getAll: vi.fn(() => Promise.resolve(mockPortfolios)),
...
describe('DirectionsList (Portfolios)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders loading state initially', async () => {
    const { portfoliosApi } = await import('@entities/strategy');
    vi.mocked(portfoliosApi.getAll).mockReturnValueOnce(new Promise(() => {}) as never);
    render(
      <TestWrapper>
        <DirectionsList />
      </TestWrapper>
  ...
  it('renders page title', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('الأهداف التنفيذية')).toBeInTheDocument();
    });
  });

  it('renders portfolios table with data', async () => {
    render(
      <TestWrapper>
        <DirectionsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('الهدف التنفيذي الأول')).toBeInTheDocument();
      expect(screen.getByText('الهدف التنفيذي الثاني')).toBeInTheDocument();
    });
  });

  it('displays portfolio codes', async () => {
    render(
      <TestWrapper>
        <DirectionsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      expect(screen.getByText('PF-2024-001')).toBeInTheDocument();
      expect(screen.getByText('PF-2024-002')).toBeInTheDocument();
    });
  });
  ...
  it('displays programs count in stats', async () => {
    render(
      <TestWrapper>
        <DirectionsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      // Programs count is rendered in the table "programs" column
  ...
  it('displays portfolio data', async () => {
    ...
    await waitFor(() => {
      // Check that portfolio names are displayed
      expect(screen.getByText('الهدف التنفيذي الأول')).toBeInTheDocument();
    });
  });
  ...
  it('renders filter button', async () => {
    ...
    await waitFor(() => {
      // Filtering is provided by the status Select in the FilterBar
      expect(screen.getAllByText('كل الحالات').length).toBeGreaterThan(0);
    });
  });
  ...
  it('renders new portfolio button', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('هدف جديد')).toBeInTheDocument();
    });
  });

  it('displays table with portfolios', async () => {
    render(
      <TestWrapper>
        <DirectionsList />
      </TestWrapper>
    ...
    await waitFor(() => {
      // Check table is rendered with portfolio data
      expect(screen.getByText('الهدف التنفيذي الأول')).toBeInTheDocument();
      expect(screen.getByText('PF-2024-001')).toBeInTheDocument();
    });
  });
  ...
  it('renders action buttons', async () => {
    ...
    await waitFor(() => {
      const viewButtons = document.querySelectorAll('[title="عرض"]');
      const editButtons = document.querySelectorAll('[title="تعديل"]');
      const deleteButtons = document.querySelectorAll('[title="حذف"]');
...
describe('DirectionsList - Empty State', () => {
  it('shows empty state when no portfolios', async () => {
    const { portfoliosApi } = await import('@entities/strategy');
    vi.mocked(portfoliosApi.getAll).mockResolvedValueOnce({
      data: [],
      current_page: 1,
      last_page: 1,
      total: 0,
    ...
    await waitFor(() => {
      expect(screen.getByText('لا توجد أهداف تنفيذية')).toBeInTheDocument();
    });
  });
});

describe('DirectionsList - Delete Modal', () => {
  it('opens delete modal when delete button clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/هل أنت متأكد من حذف/)).toBeInTheDocument();
    });
  });
});

describe('DirectionsList - Stats Cards', () => {
  it('renders stats cards', async () => {
    ...
    await waitFor(() => {
      // List now uses a DataTable; verify the table column headers render
      expect(screen.getByText('المبادرات')).toBeInTheDocument();
    });
  });
});
