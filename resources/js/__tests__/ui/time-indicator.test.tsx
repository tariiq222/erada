...
vi.mock('@pages/tasks/list/types', () => ({
...
describe('TimeIndicator Component', () => {
  ...
  it('returns null when indicator is undefined', () => {
    const { container } = render(
      <TimeIndicator indicator={undefined} taskStatus="todo" />
    );
    expect(container.firstChild).toBeNull();
  });

  it('returns null when indicator has no due date', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: false, status: 'normal', time_progress: 0, days_remaining: null, total_days: null }}
        taskStatus="todo"
      />
    );
    expect(container.firstChild).toBeNull();
  ...
  it('shows completed text when task is completed', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'completed', time_progress: 100, days_remaining: 0, total_days: 10 }}
  ...
  it('shows today text when days remaining is 0', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'urgent', time_progress: 95, days_remaining: 0, total_days: 10 }}
  ...
  it('shows tomorrow text when days remaining is 1', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'warning', time_progress: 90, days_remaining: 1, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
  ...
  it('shows days remaining when more than 1 day', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 50, days_remaining: 5, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
  ...
  it('shows overdue text when days remaining is negative', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'overdue', time_progress: 100, days_remaining: -3, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
  ...
  it('shows dash when days remaining is null', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 0, days_remaining: null, total_days: null }}
        taskStatus="in_progress"
      />
    );
  ...
  it('shows header text for remaining time', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 50, days_remaining: 5, total_days: 10 }}
        taskStatus="in_progress"
  ...
  it('renders progress bar when has total days', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 50, days_remaining: 5, total_days: 10 }}
        taskStatus="in_progress"
    ...
    expect(screen.getByTestId('time-indicator-track')).toBeInTheDocument();
  });

  it('does not render progress bar when no total days', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 0, days_remaining: null, total_days: null }}
        taskStatus="in_progress"
    ...
    expect(screen.queryByTestId('time-indicator-track')).toBeNull();
  });

  it('caps progress at 100%', () => {
    render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'overdue', time_progress: 150, days_remaining: -5, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    const progressFill = screen.getByTestId('time-indicator-fill');
...
describe('TimeIndicator Status Colors', () => {
  it('uses normal colors for normal status', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'normal', time_progress: 30, days_remaining: 7, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    expect(container.innerHTML).toContain('bg-[var(--border-strong)]');
  });

  it('uses warning colors for warning status', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'warning', time_progress: 70, days_remaining: 2, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    expect(container.innerHTML).toContain('bg-[var(--status-warning-subtle)]');
  });

  it('uses urgent colors for urgent status', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'urgent', time_progress: 90, days_remaining: 1, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    expect(container.innerHTML).toContain('bg-[var(--status-warning-subtle)]');
  ...
  it('uses overdue colors for overdue status', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'overdue', time_progress: 100, days_remaining: -2, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    expect(container.innerHTML).toContain('bg-[var(--status-danger-subtle)]');
  });
  ...
  it('uses completed colors for completed status', () => {
    ...
    expect(container.innerHTML).toContain('bg-[var(--status-success-subtle)]');
  });

  it('falls back to normal colors for unknown status', () => {
    const { container } = render(
      <TimeIndicator
        indicator={{ has_due_date: true, status: 'unknown' as any, time_progress: 50, days_remaining: 5, total_days: 10 }}
        taskStatus="in_progress"
      />
    );
    expect(container.innerHTML).toContain('bg-[var(--border-strong)]');
