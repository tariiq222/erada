...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'strategy.strategic_plan_link_placeholder': '[Strategic Plan Link Placeholder]',
...
describe('StatusBadge Component', () => {
  ...
  describe('Options', () => {
    it('renders with sm size', () => {
      const { container } = render(<StatusBadge type="task" status="todo" size="sm" />);
      // sm variant uses zero vertical padding (md uses py-1)
      expect(container.firstChild).toHaveClass('py-0');
    });

    it('renders with showDot', () => {
      render(<StatusBadge type="task" status="todo" showDot />);
      const badge = screen.getByText('للتنفيذ');
  ...
  describe('Helper Functions', () => {
    it('getProjectStatusLabel returns correct label', () => {
      // Without t function, returns raw i18n key
      expect(getProjectStatusLabel('draft')).toBe('status.draft');
      expect(getProjectStatusLabel('completed')).toBe('status.completed');
    });

    it('getTaskStatusLabel returns correct label', () => {
      // Without t function, returns raw i18n key
      expect(getTaskStatusLabel('todo')).toBe('status.todo');
      expect(getTaskStatusLabel('completed')).toBe('status.completed_female');
    });

    it('getPriorityLabel returns correct label', () => {
      // Without t function, returns raw i18n key
      expect(getPriorityLabel('low')).toBe('priority.low');
      expect(getPriorityLabel('critical')).toBe('priority.critical');
    });
  });
});
...
describe('Avatar Component', () => {
  ...
  it('shows fallback on image error', async () => {
    render(<Avatar src="invalid-url.jpg" name="Test User" />);
...
describe('Pagination Component', () => {
  ...
  it('calls onPageChange when clicking page', async () => {
    const onPageChange = vi.fn();
    render(
      <Pagination
        currentPage={1}
  ...
  it('disables previous on first page', () => {
    ...
    // Check if first page indicator is present
    expect(screen.getByText('1')).toBeInTheDocument();
  });
});
...
describe('Table Component', () => {
  ...
  it('renders with custom className', () => {
    ...
    expect(cell.closest('table')?.className).toContain('custom-table');
  });
});

describe('Breadcrumb Component', () => {
  it('renders breadcrumb items with items prop', () => {
    render(
      <Breadcrumb
        items={[
...
describe('FilterButton Component', () => {
  it('renders with فلتر label', () => {
    render(<FilterButton isOpen={false} onClick={() => {}} />);
    expect(screen.getByText('تصفية')).toBeInTheDocument();
  });

  it('calls onClick when clicked', async () => {
    const onClick = vi.fn();
    render(<FilterButton isOpen={false} onClick={onClick} />);
    await userEvent.click(screen.getByRole('button'));
    expect(onClick).toHaveBeenCalled();
  });

  it('shows active count badge', () => {
    render(<FilterButton isOpen={false} onClick={() => {}} activeCount={3} />);
    expect(screen.getByText('3')).toBeInTheDocument();
  });

  it('shows active state when open', () => {
    render(<FilterButton isOpen={true} onClick={() => {}} />);
    const button = screen.getByRole('button');
    expect(button).toHaveClass('bg-[var(--accent-subtle)]');
  });
});

describe('Radio Component with RadioGroup', () => {
  it('renders with label inside RadioGroup', () => {
    render(
  ...
  it('can be disabled at group level', () => {
    render(
  ...
  it('renders with description', () => {
    render(
      <RadioGroup name="test" value="" onChange={() => {}}>
        <Radio label="Option" value="1" description="Description text" />
      </RadioGroup>
    );
