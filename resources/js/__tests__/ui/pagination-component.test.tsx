...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconChevronLeft: () => <span data-testid="chevron-left">ChevronLeft</span>,
  IconChevronRight: () => <span data-testid="chevron-right">ChevronRight</span>,
...
describe('Pagination Basic', () => {
  ...
  it('has aria-label for navigation', () => {
    render(<Pagination {...defaultProps} />);
    expect(screen.getByLabelText('common.pagination')).toBeInTheDocument();
  });

  it('renders previous button', () => {
    render(<Pagination {...defaultProps} />);
    expect(screen.getByLabelText('common.previous_page')).toBeInTheDocument();
  });

  it('renders next button', () => {
    render(<Pagination {...defaultProps} />);
    expect(screen.getByLabelText('common.next_page')).toBeInTheDocument();
  });

  it('renders page numbers', () => {
    render(<Pagination {...defaultProps} />);
    expect(screen.getByLabelText('صفحة 1')).toBeInTheDocument();
    expect(screen.getByLabelText('صفحة 2')).toBeInTheDocument();
    expect(screen.getByLabelText('صفحة 3')).toBeInTheDocument();
    expect(screen.getByLabelText('صفحة 4')).toBeInTheDocument();
    expect(screen.getByLabelText('صفحة 5')).toBeInTheDocument();
  });
});
...
describe('Pagination Navigation', () => {
  ...
  it('calls onPageChange when clicking page number', () => {
    render(<Pagination {...defaultProps} />);
    fireEvent.click(screen.getByLabelText('صفحة 2'));
  ...
  it('calls onPageChange when clicking previous', () => {
    render(<Pagination {...defaultProps} />);
    fireEvent.click(screen.getByLabelText('common.previous_page'));
    expect(defaultProps.onPageChange).toHaveBeenCalledWith(2);
  });
});
...
describe('Pagination Disabled States', () => {
  ...
  it('disables previous on first page', () => {
    ...
    expect(screen.getByLabelText('common.previous_page')).toBeDisabled();
  });
  ...
  it('disables next on last page', () => {
    ...
    expect(screen.getByLabelText('common.next_page')).toBeDisabled();
  });
  ...
  it('enables both buttons in middle', () => {
    ...
    expect(screen.getByLabelText('common.previous_page')).not.toBeDisabled();
    expect(screen.getByLabelText('common.next_page')).not.toBeDisabled();
  });
});

describe('Pagination Active State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('marks current page as active', () => {
    ...
    expect(screen.getByLabelText('صفحة 3')).toHaveAttribute('aria-current', 'page');
  });

  it('other pages are not active', () => {
    render(
      <Pagination
        currentPage={3}
        totalPages={5}
    ...
    expect(screen.getByLabelText('صفحة 1')).not.toHaveAttribute('aria-current');
    expect(screen.getByLabelText('صفحة 2')).not.toHaveAttribute('aria-current');
    expect(screen.getByLabelText('صفحة 4')).not.toHaveAttribute('aria-current');
    expect(screen.getByLabelText('صفحة 5')).not.toHaveAttribute('aria-current');
  });
});

describe('Pagination Hidden on Single Page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns null for zero pages', () => {
    const { container } = render(
      <Pagination
        currentPage={1}
        totalPages={0}
...
describe('Pagination with Dots', () => {
  ...
  it('shows dots for many pages', () => {
    ...
    expect(screen.getAllByTestId('dots-icon').length).toBeGreaterThan(0);
  });

  it('shows dots on both sides in middle', () => {
    render(
      <Pagination
        currentPage={5}
        totalPages={10}
    ...
    expect(screen.getAllByTestId('dots-icon').length).toBe(2);
  });

  it('shows right dots only at start', () => {
    render(
      <Pagination
        currentPage={2}
        totalPages={10}
    ...
    expect(screen.getAllByTestId('dots-icon').length).toBe(1);
  });

  it('shows left dots only at end', () => {
    render(
      <Pagination
        currentPage={9}
        totalPages={10}
...
describe('Pagination Custom Props', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders more pages with larger sibling count', () => {
    render(
      <Pagination
        currentPage={5}
        totalPages={10}
    ...
    // With siblingCount=2, should show pages 3,4,5,6,7 around current
    expect(screen.getByLabelText('صفحة 3')).toBeInTheDocument();
    expect(screen.getByLabelText('صفحة 7')).toBeInTheDocument();
  });
});
...
describe('Pagination Icons', () => {
  ...
  it('renders chevron right icon for previous', () => {
    render(
      <Pagination
        currentPage={3}
        totalPages={5}
  ...
  it('renders chevron left icon for next', () => {
    render(
      <Pagination
        currentPage={3}
        totalPages={5}
