...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'invitations.invitation_link_description': 'سيتم إرسال رابط دعوة للمستخدم عبر البريد الإلكتروني', 'invitations.no_matching_users': 'لا يوجد مستخدمون مطابقون', 'invitations.permission': 'الصلاحية', 'invitations.roles.general': 'عام',
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, loading, leftIcon, variant, className, type }: any) => (
    <button onClick={onClick} disabled={loading} data-variant={variant} className={className} type={type}>
      {leftIcon}
      {children}
...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, error, type, leftIcon }: any) => (
    <div>
      {leftIcon}
      <input
        value={value}
...
describe('UserModal Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders when open', () => {
    render(<UserModal {...createDefaultProps()} />);
    expect(screen.getByTestId('modal')).toBeInTheDocument();
  });

  it('does not render when closed', () => {
    render(<UserModal {...createDefaultProps({ isOpen: false })} />);
    expect(screen.queryByTestId('modal')).not.toBeInTheDocument();
  });

  it('renders header with user selection title', () => {
    render(<UserModal {...createDefaultProps()} />);
    expect(screen.getByText('اختيار المكلف')).toBeInTheDocument();
  });
...
describe('UserModal User Selection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('calls onSearchChange when searching', () => {
    ...
    expect(props.onSearchChange).toHaveBeenCalledWith('أحمد');
  });
  ...
  it('calls onSelectUser when clicking a user', () => {
    ...
    expect(props.onSelectUser).toHaveBeenCalledWith(1);
  });

  it('shows check icon for selected user', () => {
    render(<UserModal {...createDefaultProps({ selectedUserId: '1' })} />);
    expect(screen.getByTestId('check-icon')).toBeInTheDocument();
  });
});

describe('UserModal Search Filtering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('filters users by name', () => {
    render(<UserModal {...createDefaultProps({ searchQuery: 'أحمد' })} />);
    expect(screen.getByText('أحمد محمد')).toBeInTheDocument();
    expect(screen.getByText('سارة أحمد')).toBeInTheDocument();
  ...
  it('filters users by email', () => {
    render(<UserModal {...createDefaultProps({ searchQuery: 'sara' })} />);
    expect(screen.getByText('سارة أحمد')).toBeInTheDocument();
    expect(screen.queryByText('أحمد محمد')).not.toBeInTheDocument();
  });

  it('shows no users message when no match', () => {
    render(<UserModal {...createDefaultProps({ searchQuery: 'xyz123' })} />);
    expect(screen.getByText('لا يوجد مستخدمون مطابقون')).toBeInTheDocument();
  });

  it('handles case insensitive search', () => {
    render(<UserModal {...createDefaultProps({ searchQuery: 'AHMED' })} />);
    expect(screen.getByText('أحمد محمد')).toBeInTheDocument();
  });
});

describe('UserModal Footer Actions - Selection Mode', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('does not show remove button when no user selected', () => {
    render(<UserModal {...createDefaultProps()} />);
    expect(screen.queryByText('إزالة المكلف')).not.toBeInTheDocument();
  });

  it('shows remove button when user selected', () => {
    render(<UserModal {...createDefaultProps({ selectedUserId: '1' })} />);
    expect(screen.getByText('إزالة المكلف')).toBeInTheDocument();
  });

  it('calls onRemoveUser when remove clicked', () => {
    const props = createDefaultProps({ selectedUserId: '1' });
    render(<UserModal {...props} />);
    fireEvent.click(screen.getByText('إزالة المكلف'));
    expect(props.onRemoveUser).toHaveBeenCalled();
  });
});
