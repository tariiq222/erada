...
describe('UserForm Component', () => {
  ...
  it('renders in create mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(heading?.textContent).toContain('إنشاء مستخدم جديد');
    });
  });
  ...
  it('renders page description', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('أدخل بيانات المستخدم الجديد')).toBeInTheDocument();
    });
  });
});

describe('UserForm Basic Info Section', () => {
  it('renders name field with required indicator', async () => {
    render(<UserForm />);
  ...
  it('renders email field with required indicator', async () => {
    render(<UserForm />);
  ...
  it('renders password confirmation field', async () => {
    render(<UserForm />);
  ...
  it('renders basic info card title', async () => {
    render(<UserForm />);
  ...
  it('allows typing in name field', async () => {
    ...
    const nameInput = screen.getByPlaceholderText('أدخل اسم المستخدم');
    await userEvent.type(nameInput, 'أحمد محمد');

    expect(nameInput).toHaveValue('أحمد محمد');
  });
  ...
  it('allows typing in email field', async () => {
    ...
    expect(emailInput).toHaveValue('ahmed@example.com');
  });
});

describe('UserForm Contact Info Section', () => {
  it('renders contact info card title', async () => {
    render(<UserForm />);
  ...
  it('renders extension field', async () => {
    render(<UserForm />);

    await waitFor(() => {
      expect(screen.getByText('التحويلة')).toBeInTheDocument();
    });
  });

  it('renders job title field', async () => {
    render(<UserForm />);
  ...
  it('allows typing extension', async () => {
    render(<UserForm />);
    ...
    const extInput = screen.getByPlaceholderText('1234');
    await userEvent.type(extInput, '999');

    expect(extInput).toHaveValue('999');
  });

  it('allows typing job title', async () => {
    render(<UserForm />);

    await waitFor(() => {
      expect(screen.getByPlaceholderText('مثال: مهندس برمجيات')).toBeInTheDocument();
    });

    const jobInput = screen.getByPlaceholderText('مثال: مهندس برمجيات');
    await userEvent.type(jobInput, 'مدير مشروع');

    expect(jobInput).toHaveValue('مدير مشروع');
  });
});

describe('UserForm Roles Section', () => {
  it('renders roles section title', async () => {
    render(<UserForm />);

    await waitFor(() => {
      expect(screen.getByText('القسم والأدوار')).toBeInTheDocument();
    });
  });

  it('renders department dropdown', async () => {
    render(<UserForm />);
  ...
  it('renders status checkbox', async () => {
    render(<UserForm />);

    await waitFor(() => {
      expect(screen.getByText('الحالة')).toBeInTheDocument();
      expect(screen.getByText('المستخدم نشط')).toBeInTheDocument();
    });
  });

  it('renders roles label', async () => {
    render(<UserForm />);
  ...
  it('does not render super admin role option', async () => {
    render(<UserForm />);
  ...
  it('renders admin role option', async () => {
    render(<UserForm />);
  ...
  it('does not render legacy team member role option', async () => {
    render(<UserForm />);

    await waitFor(() => {
      expect(screen.getByText('مدير إدارة')).toBeInTheDocument();
    });
    expect(screen.queryByText('عضو فريق')).not.toBeInTheDocument();
  });

  it('renders viewer role option', async () => {
    render(<UserForm />);
  ...
  it('renders role descriptions', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('إدارة المستخدمين والمشاريع داخل الإدارة')).toBeInTheDocument();
      expect(screen.getByText('عرض البيانات فقط دون تعديل')).toBeInTheDocument();
      expect(screen.getByText('pmo_member')).toBeInTheDocument();
    });
    expect(screen.queryByText('صلاحيات كاملة على جميع أقسام النظام')).not.toBeInTheDocument();
    expect(screen.queryByText('إدارة المشاريع والمهام')).not.toBeInTheDocument();
    expect(screen.queryByText('تنفيذ المهام والمشاركة في المشاريع')).not.toBeInTheDocument();
  });
});
...
describe('UserForm Actions', () => {
  ...
  it('cancel button links to users list', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const cancelLink = screen.getByText('إلغاء').closest('a');
      expect(cancelLink).toHaveAttribute('href', '/users');
    });
  });
});

describe('UserForm Password Visibility', () => {
  it('renders password as hidden by default', async () => {
    render(<UserForm />);
  ...
  it('has toggle button for password visibility', async () => {
    render(<UserForm />);

    await waitFor(() => {
      // There should be eye icons for toggling password visibility
      const buttons = screen.getAllByRole('button');
      // At least 2 toggle buttons (one for each password field)
      expect(buttons.length).toBeGreaterThanOrEqual(2);
    });
  });
});

describe('UserForm Role Toggle', () => {
  it('allows selecting multiple roles', async () => {
    ...
    // Find the role label and click on it to toggle
    const customRole = screen.getByText('عضو مكتب المشاريع').closest('label');
    if (customRole) {
      await userEvent.click(customRole);
    }

    // The role card should now have the selected styles
    // This is verified by the component's internal state
  });
});

describe('UserForm Department Selection', () => {
  it('loads departments from API', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const selects = screen.getAllByText('-- اختر القسم --');
      expect(selects.length).toBeGreaterThan(0);
    });
  });
});

describe('UserForm Validation Messages', () => {
  it('has required indicator on name field', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const nameLabel = screen.getByText('الاسم').parentElement;
      expect(nameLabel?.textContent).toContain('*');
    });
  });

  it('has required indicator on email field', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const emailLabel = screen.getByText('البريد الإلكتروني').parentElement;
      expect(emailLabel?.textContent).toContain('*');
    });
  });

  it('has required indicator on password field in create mode', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const passwordLabel = screen.getByText('كلمة المرور').parentElement;
      expect(passwordLabel?.textContent).toContain('*');
    });
  });

  it('has required indicator on roles field', async () => {
    render(<UserForm />);

    await waitFor(() => {
      const rolesLabel = screen.getByText('الأدوار').parentElement;
      expect(rolesLabel?.textContent).toContain('*');
    });
  });
});
