...
// Import components
import StatsCards from '@pages/users/components/StatsCards';
import FiltersCard from '@pages/users/components/FiltersCard';
import UsersTable from '@pages/users/components/UsersTable';
import DeleteUserModal from '@pages/users/components/DeleteUserModal';
import { roleLabels, roleColors } from '@pages/users/components/constants';
...
describe('Users StatsCards Component', () => {
  ...
  it('calculates admins correctly', () => {
    render(<StatsCards users={mockUsers} total={4} />);
    // 2 users have admin or super_admin role
    const twos = screen.getAllByText('2');
    expect(twos.length).toBeGreaterThan(0);
  });

  it('renders with empty users array', () => {
    render(<StatsCards users={[]} total={0} />);
    expect(screen.getByText('إجمالي المستخدمين')).toBeInTheDocument();
  });
});

describe('Users FiltersCard Component', () => {
  const defaultProps = {
    filters: {
      search: '',
      department_id: '',
  ...
  it('renders filter section title', () => {
    ...
    // FiltersCard now uses FilterRow/FilterField; the search field label is the
    // first visible text that confirms the filter bar rendered.
  ...
  it('calls onFiltersChange when search input changes', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('بحث بالاسم أو البريد...');
    await userEvent.type(searchInput, 'أحمد');

    expect(onFiltersChange).toHaveBeenCalled();
  });

  it('displays initial filter values', () => {
    const filters = { search: 'test', department_id: '', role: '', is_active: '' };
    render(<FiltersCard {...defaultProps} filters={filters} />);

    const searchInput = screen.getByPlaceholderText('بحث بالاسم أو البريد...') as HTMLInputElement;
    expect(searchInput.value).toBe('test');
  });

  it('renders department filter options', () => {
    render(<FiltersCard {...defaultProps} />);
    // Should have default option plus departments
...
describe('Users Table Component', () => {
  const defaultProps = {
    users: mockUsers,
    loading: false,
    pagination: { currentPage: 1, lastPage: 3, perPage: 10, total: 30 },
  ...
  it('shows create button in empty state when has permission', () => {
    render(<UsersTable {...defaultProps} users={[]} hasCreatePermission={true} />);
    expect(screen.getByText('إضافة مستخدم جديد')).toBeInTheDocument();
  });

  it('hides create button in empty state when no permission', () => {
    render(<UsersTable {...defaultProps} users={[]} hasCreatePermission={false} />);
  ...
  it('renders table headers', () => {
    ...
    // The actions column header is intentionally empty (aria-hidden); the
    // column's presence is observable through the per-row action controls
    // (the delete control renders as a real button carrying its aria-label).
    expect(screen.getAllByLabelText('حذف').length).toBeGreaterThan(0);
  });

  it('renders user names', () => {
    render(<UsersTable {...defaultProps} />);

    // User names may appear multiple times (in name column and created_by/manager columns)
  ...
  it('renders user emails', () => {
    ...
    expect(screen.getByText('sara@test.com')).toBeInTheDocument();
  });

  it('renders department names', () => {
    render(<UsersTable {...defaultProps} />);

    // Multiple users may be in same department
    const techDepts = screen.getAllByText('تقنية المعلومات');
    expect(techDepts.length).toBeGreaterThan(0);
  ...
  it('renders role badges', () => {
    ...
    // Multiple users may have 'عضو فريق' role
    const teamMembers = screen.getAllByText('عضو فريق');
  ...
  it('renders active/inactive status', () => {
    ...
    // There should be active and inactive badges
  ...
  it('renders pagination when multiple pages', () => {
    ...
    // DataTable renders the Pagination <nav> only when lastPage > 1.
    expect(screen.getByRole('navigation')).toBeInTheDocument();
  });

  it('hides pagination when single page', () => {
    render(<UsersTable {...defaultProps} pagination={{ currentPage: 1, lastPage: 1, perPage: 10, total: 4 }} />);
...
describe('Delete User Modal', () => {
  ...
  it('shows loading state when deleting', () => {
    render(<DeleteUserModal {...defaultProps} isDeleting={true} />);
  ...
  it('does not render when closed', () => {
    ...
    expect(screen.queryByText('حذف المستخدم')).not.toBeInTheDocument();
  });
});

describe('User Role Constants', () => {
  it('has all role labels', () => {
    expect(roleLabels.super_admin).toBe('role.super_admin');
    expect(roleLabels.admin).toBe('role.admin');
    expect(roleLabels.project_manager).toBe('role.project_manager');
    expect(roleLabels.team_member).toBe('role.team_member');
    expect(roleLabels.viewer).toBe('role.viewer');
  });

  it('has all role colors', () => {
    expect(roleColors.super_admin).toBe('danger');
    expect(roleColors.admin).toBe('accent');
    expect(roleColors.project_manager).toBe('warning');
    expect(roleColors.team_member).toBe('success');
    expect(roleColors.viewer).toBe('default');
  });
});
