...
describe('Profile Page', () => {
  ...
  it('renders user name in card', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    // Name appears multiple times - in card and form
    const names = screen.getAllByText('أحمد محمد');
    expect(names.length).toBeGreaterThan(0);
  });

  it('renders user email in card', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    const emails = screen.getAllByText('ahmed@test.com');
  ...
  it('renders user job title', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    // Job title appears in card and in form
    const jobTitles = screen.getAllByText('مهندس برمجيات');
  ...
  it('renders user roles as badges', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders personal info form section', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders password change section', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    // "تغيير كلمة المرور" appears in section title and button
    const passwordChangeTexts = screen.getAllByText('تغيير كلمة المرور');
    expect(passwordChangeTexts.length).toBeGreaterThan(0);
  });

  it('renders name input with value', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders email input with value', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    const emailInput = screen.getByPlaceholderText('example@domain.com');
    expect(emailInput).toHaveValue('ahmed@test.com');
  });

  it('renders phone input with value', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders extension input', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    const extensionInput = screen.getByPlaceholderText('1234');
  ...
  it('renders change password button', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    // Text appears in both title and button
    const texts = screen.getAllByText('تغيير كلمة المرور');
    expect(texts.length).toBe(2); // Section title and button
  });
  ...
  it('renders new password input', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByPlaceholderText('أدخل كلمة المرور الجديدة')).toBeInTheDocument();
  });

  it('renders confirm password input', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByPlaceholderText('أعد إدخال كلمة المرور')).toBeInTheDocument();
  });

  it('renders password requirements hint', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByText('يجب أن تكون كلمة المرور 8 أحرف على الأقل')).toBeInTheDocument();
  });
});
...
describe('Profile Form Interactions', () => {
  ...
  it('submits password form successfully', async () => {
    ...
    // Fill password form
    await userEvent.type(screen.getByPlaceholderText('أدخل كلمة المرور الحالية'), 'oldpass123');
    await userEvent.type(screen.getByPlaceholderText('أدخل كلمة المرور الجديدة'), 'newpass123');
    await userEvent.type(screen.getByPlaceholderText('أعد إدخال كلمة المرور'), 'newpass123');

    // Get the button (second one with this text - the first is section title)
    const changePasswordButtons = screen.getAllByText('تغيير كلمة المرور');
    const submitButton = changePasswordButtons.find(el => el.closest('button'));
    if (submitButton) {
...
describe('Password Visibility Toggle', () => {
  ...
  it('toggles current password visibility', async () => {
    ...
    const passwordInput = screen.getByPlaceholderText('أدخل كلمة المرور الحالية');
    expect(passwordInput).toHaveAttribute('type', 'password');

    // Find and click toggle button (first eye button in the form)
    const toggleButtons = document.querySelectorAll('button[type="button"]');
    const firstToggle = Array.from(toggleButtons).find(btn =>
      btn.closest('.relative')?.querySelector('input[placeholder="أدخل كلمة المرور الحالية"]')
    );
  ...
  it('toggles new password visibility', async () => {
    ...
    const passwordInput = screen.getByPlaceholderText('أدخل كلمة المرور الجديدة');
    expect(passwordInput).toHaveAttribute('type', 'password');

    const toggleButtons = document.querySelectorAll('button[type="button"]');
    const newPasswordToggle = Array.from(toggleButtons).find(btn =>
      btn.closest('.relative')?.querySelector('input[placeholder="أدخل كلمة المرور الجديدة"]')
    );
  ...
  it('toggles confirm password visibility', async () => {
    ...
    const passwordInput = screen.getByPlaceholderText('أعد إدخال كلمة المرور');
    expect(passwordInput).toHaveAttribute('type', 'password');

    const toggleButtons = document.querySelectorAll('button[type="button"]');
    const confirmToggle = Array.from(toggleButtons).find(btn =>
      btn.closest('.relative')?.querySelector('input[placeholder="أعد إدخال كلمة المرور"]')
    );
...
describe('Profile Form Labels', () => {
  it('renders required field indicators', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });

    // RequiredIndicator exposes data-testid="required-indicator"
    expect(screen.getAllByTestId('required-indicator').length).toBeGreaterThan(0);
  });
  ...
  it('renders extension label', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
  ...
  it('renders current password label', async () => {
    render(<Profile />);
    await act(async () => { await Promise.resolve(); });
