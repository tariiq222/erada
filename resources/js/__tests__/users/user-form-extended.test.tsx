...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, type, loading, leftIcon, variant }: any) => (
    <button onClick={onClick} type={type} data-loading={loading} data-variant={variant} disabled={loading}>
      {leftIcon}
      {children}
    </button>
  ),
...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, type, leftIcon, error }: any) => (
    <div>
      {leftIcon}
      <input
        type={type || 'text'}
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconArrowRight: () => <span data-testid="arrow-icon">→</span>,
  ...
  IconShield: () => <span data-testid="shield-icon">Shield</span>,
  IconLock: () => <span data-testid="lock-icon">Lock</span>,
  IconEye: () => <span data-testid="eye-icon">Eye</span>,
  IconEyeOff: () => <span data-testid="eyeoff-icon">EyeOff</span>,
...
describe('UserForm Create Mode', () => {
  ...
  it('renders create form title', async () => {
    render(<UserForm />);
    await waitFor(() => {
      // Title appears in both breadcrumb and header
      const titles = screen.getAllByText('إنشاء مستخدم جديد');
      expect(titles.length).toBeGreaterThan(0);
    });
  });

  it('renders breadcrumb with users link', async () => {
    render(<UserForm />);
    await act(async () => { await Promise.resolve(); });
    const usersLink = screen.getByText('المستخدمون');
  ...
  it('renders contact info card', async () => {
    render(<UserForm />);
    await waitFor(() => {
  ...
  it('renders department and roles card', async () => {
    render(<UserForm />);
    await waitFor(() => {
  ...
  it('fetches departments on mount', async () => {
    render(<UserForm />);
    await waitFor(() => {
      expect(departmentsApi.getList).toHaveBeenCalled();
    });
  });
...
describe('UserForm Input Fields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('UserForm Roles Selection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows admin role option', async () => {
    render(<UserForm />);
    await waitFor(() => {
  ...
  it('shows viewer role option', async () => {
    render(<UserForm />);
    await waitFor(() => {
  ...
  it('does not show super admin role in the assignable role list', async () => {
    render(<UserForm />);
    await waitFor(() => {
      expect(screen.getByText('مدير إدارة')).toBeInTheDocument();
    });
...
describe('UserForm Form Submission', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('submits form data', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('أدخل اسم المستخدم'), {
      target: { value: 'اسم جديد' },
    });
    ...
    const form = screen.getByText('إنشاء مستخدم').closest('form');
    if (form) {
  ...
  it('shows success toast on create', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', 'تم إنشاء المستخدم بنجاح');
    });
  });

  it('navigates to users list on success', async () => {
    render(<UserForm />);
  ...
  it('shows error toast on API failure', async () => {
    (usersApi.create as any).mockRejectedValueOnce({ message: 'حدث خطأ' });
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'حدث خطأ');
    });
  });
});

describe('UserForm Password Toggle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('UserForm Active Checkbox', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('has active checkbox checked by default', async () => {
    render(<UserForm />);
    await waitFor(() => {
      const checkboxes = screen.getAllByTestId('checkbox');
      // First checkbox should be the active one
...
describe('UserForm Department Selection', () => {
  ...
  it('loads departments in select', async () => {
    render(<UserForm />);
    await waitFor(() => {
  ...
  it('shows empty option in department select', async () => {
    render(<UserForm />);
    await waitFor(() => {
...
describe('UserForm Cancel Button', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
