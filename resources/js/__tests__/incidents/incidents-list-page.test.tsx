...
vi.mock('@entities/hr', () => ({
  employeesApi: {
    getList: vi.fn().mockResolvedValue([
      { id: 1, name: 'أحمد', employee_id: 'EMP-001' },
      { id: 2, name: 'محمد', employee_id: 'EMP-002' },
    ]),
  },
}));
vi.mock('@entities/incident', () => ({
  incidentsApi: {
    getAll: vi.fn().mockResolvedValue({
      data: [
        { id: 1, title: 'حادثة 1', severity: 'high', status: 'open', category: { name: 'طبية' }, occurred_at: '2025-01-01' },
        { id: 2, title: 'حادثة 2', severity: 'medium', status: 'resolved', category: { name: 'إدارية' }, occurred_at: '2025-01-02' },
      ],
      current_page: 1,
      last_page: 1,
      per_page: 15,
    ...
    exportUrl: (format = 'csv', params?: Record<string, string>) =>
      `/api/ovr/incidents/export?${new URLSearchParams({ ...(params ?? {}), format }).toString()}`,
  },
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon, onClick, loading }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void; loading?: boolean }) => (
    <button onClick={onClick} disabled={loading}>{leftIcon}{children}</button>
  ),
  Skeleton: ({ className }: any) => <div data-testid="skeleton" className={className} />,
}));

// Mock child components
vi.mock('@pages/ovr/components', () => ({
  IncidentFormModal: ({
    isOpen,
    incident,
    onClose,
    ...
    isOpen ? (
      <div data-testid="form-modal">
        <span>{incident ? 'تعديل حادثة' : 'تسجيل حادثة جديدة'}</span>
        <button onClick={onClose}>إغلاق</button>
        <button onClick={onSuccess}>حفظ</button>
      </div>
    ) : null
  ),
  IncidentViewModal: ({
    ...
    <div data-testid="incidents-table">
      {isLoading ? (
        ...
        incidents.map((incident: any) => (
          <div key={incident.id} data-testid="incident-row">
            <span>{incident.title}</span>
            <button onClick={() => onView(incident)}>عرض</button>
            <button onClick={() => onEdit(incident)}>تعديل</button>
          </div>
        ))
      )}
    </div>
...
import { IncidentsList } from '@pages/ovr/IncidentsList';

describe('IncidentsList Page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders page title', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('تقارير الحوادث')).toBeInTheDocument();
    });
  });
  ...
  it('renders page description', async () => {
    render(<IncidentsList />);
    await waitFor(() => {
      expect(screen.getByText('تسجيل ومتابعة الحوادث الطبية')).toBeInTheDocument();
    });
  });
  ...
  it('renders add incident button', async () => {
    render(<IncidentsList />);
    await waitFor(() => {
      expect(screen.getByText('تسجيل حادثة')).toBeInTheDocument();
    });
  });
  ...
  it('fetches incidents on mount', async () => {
    ...
    render(<IncidentsList />);
    await waitFor(() => {
      expect(incidentsApi.getAll).toHaveBeenCalled();
    });
  });
  ...
  it('fetches categories on mount', async () => {
    ...
    render(<IncidentsList />);
    await waitFor(() => {
      expect(incidentCategoriesApi.getAll).toHaveBeenCalled();
    });
  });
  ...
  it('renders incidents table', async () => {
    render(<IncidentsList />);
    await waitFor(() => {
...
describe('IncidentsList Modals', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('navigates to new incident form', async () => {
    render(<IncidentsList />);
    ...
    fireEvent.click(screen.getByText('تسجيل حادثة'));

    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith('/ovr/incidents/new');
    });
  });

  it('navigates to edit incident form', async () => {
    render(<IncidentsList />);
    ...
    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith(
        expect.stringMatching(/^\/ovr\/incidents\/.+\/edit$/)
      );
    });
  });
...
describe('IncidentsList API Errors', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows error toast on fetch failure', async () => {
    const { incidentsApi } = await import('@entities/incident');
    (incidentsApi.getAll as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));

    render(<IncidentsList />);

    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في تحميل الحوادث');
    });
  });
});

describe('IncidentsList Filters', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('filters incidents on filter change', async () => {
    const { incidentsApi } = await import('@entities/incident');
    render(<IncidentsList />);
    ...
    fireEvent.change(searchInput, { target: { value: 'حادثة' } });

    await waitFor(() => {
      expect(incidentsApi.getAll).toHaveBeenCalledWith(
        expect.objectContaining({ search: 'حادثة' })
      );
    }, { timeout: 500 });
  });
});
