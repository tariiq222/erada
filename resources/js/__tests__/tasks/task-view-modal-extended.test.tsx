...
// Mock API with full task data (defined inline in factory to avoid hoisting issues)
vi.mock('@entities/task', () => ({
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({ data: {
      ...
      project: { id: 1, name: 'مشروع اختبار', code: 'PRJ-001' },
      assignee: { id: 1, name: 'أحمد' },
      creator: { id: 2, name: 'محمد' },
      milestone: { id: 1, name: 'المرحلة الأولى' },
      parent: null,
      subtasks: [
        { id: 10, title: 'مهمة فرعية 1', status: 'completed', priority: 'medium', assignee: null },
        { id: 11, title: 'مهمة فرعية 2', status: 'todo', priority: 'high', assignee: { id: 1, name: 'أحمد' } },
      ],
      comments: [
        {
          id: 1,
          ...
          attachments: [
            { id: 1, name: 'document.pdf', file_type: 'application/pdf', url: '/files/1', formatted_size: '1.5 MB' },
            { id: 2, name: 'image.png', file_type: 'image/png', url: '/files/2', formatted_size: '500 KB' },
          ],
        },
      ],
    }}),
    create: vi.fn().mockResolvedValue({ task: { id: 100, title: 'New Subtask', status: 'todo' } }),
...
vi.mock('@shared/lib/utils', async (importOriginal) => ({
  ...
  formatDate: (date: any) => typeof date === 'string' ? date : date?.toLocaleDateString(),
}));

// Mock UI components
vi.mock('@shared/ui', async (importOriginal) => ({
  ...(await importOriginal<any>()),
  Button: ({ children, onClick, leftIcon, disabled, variant }: any) => (
  ...
  SkeletonText: ({ lines }: { lines?: number }) => (
    <div data-testid="skeleton-text">Loading {lines} lines</div>
  ),
  ...
  Tabs: ({ children, value, onValueChange, defaultValue }: any) => (
    <div data-testid="tabs" data-value={value || defaultValue}>
      {React.Children.map(children, (child: any) =>
        React.cloneElement(child, { activeTab: value || defaultValue, onTabChange: onValueChange })
      )}
    </div>
  ),
  TabsList: ({ children }: any) => <div data-testid="tabs-list">{children}</div>,
  TabsTrigger: ({ children, value, icon, onClick }: any) => (
...
vi.mock('@widgets/task', () => ({
  TaskTimeIndicator: ({ indicator, taskStatus }: any) => (
    <div data-testid="time-indicator">
      <span>Status: {indicator?.status}</span>
    </div>
  ),
  TaskActivityLog: ({ taskId, maxItems, showHeader }: any) => (
    <div data-testid="activity-log">
      <span>Task: {taskId}, Max: {maxItems}</span>
    </div>
  ),
  TaskDetailsPanel: ({ task, onStatusChange, variant }: any) => (
    <div data-testid="details-panel">
      <span>Task: {task?.title}</span>
      {task?.assignee && <span>المكلف: {task.assignee.name}</span>}
      {task?.creator && <span>المنشئ: {task.creator.name}</span>}
      {task?.milestone && <span>المرحلة: {task.milestone.name}</span>}
      {task?.priority && <span>الأولوية: {task.priority === 'high' ? 'عالية' : task.priority}</span>}
      {task?.actual_hours != null && task?.estimated_hours != null && (
        <span>الساعات: {task.actual_hours} / {task.estimated_hours} ساعة</span>
      )}
      <button onClick={() => onStatusChange?.('completed')}>Mark Complete</button>
    </div>
  ),
}));

// Mock view components
vi.mock('@pages/tasks/components', () => ({
  CommentsSection: ({ taskId, initialComments }: any) => (
    <div data-testid="comments-section">
      <span>Comments: {initialComments?.length || 0}</span>
    </div>
  ),
  SubtaskCard: ({ subtask, onStatusChange, onUpdate, onDelete }: any) => (
    <div data-testid={`subtask-${subtask.id}`}>
      <span>{subtask.title}</span>
      <button onClick={() => onStatusChange(subtask.id, 'completed')}>Complete</button>
      <button onClick={() => onUpdate(subtask.id, { title: 'Updated' })}>Update</button>
      <button onClick={() => onDelete(subtask.id)}>Delete</button>
    </div>
  ),
  DetailsTab: ({ task, isOverdue }: any) => (
    <div data-testid="details-tab">
      <p>{task?.description || 'لا يوجد وصف'}</p>
    </div>
  ),
  SubtasksTab: ({ subtasks, users, showForm, subtaskTitle, onShowForm, onHideForm, onTitleChange, onAdd, onStatusChange, onUpdate, onDelete }: any) => (
    <div data-testid="subtasks-tab">
      <button onClick={onShowForm}>إضافة مهمة فرعية</button>
      {showForm && (
        <div>
          <input
          ...
          <button onClick={onHideForm}>إلغاء</button>
        </div>
      )}
      {subtasks?.map((s: any) => (
        <div key={s.id} data-testid={`subtask-${s.id}`}>
          <span>{s.title}</span>
          <button onClick={() => onStatusChange(s.id, 'completed')}>Complete</button>
          <button onClick={() => onUpdate(s.id, { title: 'Updated' })}>Update</button>
          <button onClick={() => onDelete(s.id)}>Delete</button>
        </div>
      ))}
    </div>
  ),
  AttachmentsTab: ({ comments }: any) => {
    const attachments = comments?.flatMap((c: any) => c.attachments || []) || [];
    return (
      <div data-testid="attachments-tab">
        ...
        {attachments.map((a: any) => (
          <div key={a.id} data-testid={`attachment-${a.id}`}>
            <span>{a.name}</span>
            <a href={a.url} download>Download</a>
          </div>
        ))}
      </div>
    );
  },
  useTaskViewModal: ({ taskId, isOpen, onTaskUpdated }: any) => {
    ...
    const [subtaskAssignee, setSubtaskAssignee] = React.useState<number | null>(null);
    const [isAddingSubtask, setIsAddingSubtask] = React.useState(false);
    const [subtaskUsers, setSubtaskUsers] = React.useState<any[]>([]);

    React.useEffect(() => {
      if (isOpen && taskId) {
        setLoading(true);
        Promise.all([import('@entities/task'), import('@entities/user')]).then(([{ tasksApi }, { usersApi }]) => {
          Promise.all([
            tasksApi.getOne(taskId),
            usersApi.getList()
          ]).then(([taskRes, usersRes]) => {
            setTask(taskRes.data);
            setSubtaskUsers(usersRes.data || []);
            setLoading(false);
          }).catch(() => {
    ...
    const handleAddSubtask = async () => {
      ...
      try {
        ...
        await tasksApi.create({ title: subtaskTitle, parent_id: taskId, assignee_id: subtaskAssignee });
        setSubtaskTitle('');
    ...
    const handleSubtaskStatusChange = async (subtaskId: number, status: string) => {
      const { tasksApi } = await import('@entities/task');
      await tasksApi.updateStatus(subtaskId, status);
      onTaskUpdated?.();
    };

    const handleSubtaskUpdate = async (subtaskId: number, data: any) => {
      const { tasksApi } = await import('@entities/task');
      await tasksApi.update(subtaskId, data);
      onTaskUpdated?.();
    };
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconLayoutKanban: () => <span data-testid="folder-icon">Folder</span>,
  IconMessage: () => <span data-testid="message-icon">Message</span>,
  IconPaperclip: () => <span data-testid="paperclip-icon">Paperclip</span>,
  IconFileText: () => <span data-testid="file-icon">File</span>,
  ...
  IconListTree: () => <span data-testid="list-icon">List</span>,
  IconExternalLink: () => <span data-testid="external-icon">External</span>,
...
describe('TaskViewModal Subtask Operations', () => {
  const onClose = vi.fn();
  ...
  it('shows add subtask button', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByText('إضافة مهمة فرعية')).toBeInTheDocument();
    });
  });

  it('shows subtask form when add button clicked', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByText('إضافة مهمة فرعية')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByText('إضافة مهمة فرعية'));
    await waitFor(() => {
      expect(screen.getByPlaceholderText('عنوان المهمة الفرعية...')).toBeInTheDocument();
    });
  });
  ...
  it('hides form when cancel clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByPlaceholderText('عنوان المهمة الفرعية...')).not.toBeInTheDocument();
    });
  });

  it('creates subtask when form submitted', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByText('إضافة مهمة فرعية')).toBeInTheDocument();
    });
    ...
    const input = await screen.findByPlaceholderText('عنوان المهمة الفرعية...');
    fireEvent.change(input, { target: { value: 'مهمة فرعية جديدة' } });
  ...
  it('shows existing subtasks', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByTestId('subtask-10')).toBeInTheDocument();
      expect(screen.getByTestId('subtask-11')).toBeInTheDocument();
    });
  });
  ...
  it('updates subtask status', async () => {
    ...
    await waitFor(() => {
      expect(tasksApi.updateStatus).toHaveBeenCalledWith(10, 'completed');
    });
  });

  it('updates subtask data', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByTestId('subtask-10')).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(tasksApi.update).toHaveBeenCalledWith(10, expect.any(Object));
    });
  });
...
describe('TaskViewModal Attachments Tab', () => {
  const onClose = vi.fn();
  ...
  it('shows attachments count in tab', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/المرفقات \(2\)/)).toBeInTheDocument();
    });
  });
...
describe('TaskViewModal Activity Tab', () => {
  const onClose = vi.fn();
  ...
  it('shows activity log in activity tab', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
  ...
  it('passes correct props to activity log', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText('Task: 1, Max: 10')).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Comments Tab', () => {
  const onClose = vi.fn();
  ...
  it('shows comments count in tab', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
...
describe('TaskViewModal Task Info Display', () => {
  const onClose = vi.fn();
  ...
  it('shows task description', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText('وصف تفصيلي للمهمة')).toBeInTheDocument();
    });
  });

  it('shows project link', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/PRJ-001 - مشروع اختبار/)).toBeInTheDocument();
    });
  });
  ...
  it('shows assignee name', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/المكلف:.*أحمد/)).toBeInTheDocument();
    });
  });
  ...
  it('shows priority', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/الأولوية:.*عالية/)).toBeInTheDocument();
    });
  });

  it('shows hours info', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/الساعات:.*5.*\/.*10.*ساعة/)).toBeInTheDocument();
    });
  });
  ...
  it('shows creator name', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/المنشئ:.*محمد/)).toBeInTheDocument();
    });
  });

  it('shows milestone name', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText(/المرحلة:.*المرحلة الأولى/)).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Header Actions', () => {
  const onClose = vi.fn();
  ...
  it('shows external link', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      const externalLink = screen.getByTestId('external-icon').closest('a');
      expect(externalLink).toHaveAttribute('href', '/tasks/1');
    });
  });
...
describe('TaskViewModal Sidebar', () => {
  const onClose = vi.fn();
  const onTaskUpdated = vi.fn();
  ...
  it('shows details panel in sidebar', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
  ...
  it('handles status change from sidebar', async () => {
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} onTaskUpdated={onTaskUpdated} />);
    await waitFor(() => {
      expect(screen.getByText('Mark Complete')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByText('Mark Complete'));
    // The status change is handled by the hook which calls onTaskUpdated after API call
    // This test just verifies the button is clickable and renders correctly
    expect(screen.getByText('Mark Complete')).toBeInTheDocument();
  });
});

describe('TaskViewModal Error State', () => {
  const onClose = vi.fn();
  ...
  it('shows error when API fails', async () => {
    (tasksApi.getOne as any).mockRejectedValueOnce(new Error('API Error'));
    render(<TaskViewModal taskId={1} isOpen={true} onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText('فشل في تحميل بيانات المهمة')).toBeInTheDocument();
    });
  });
...
describe('TaskViewModal Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows skeleton while loading', () => {
    (tasksApi.getOne as any).mockImplementation(() => new Promise(() => {}));
    render(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
  });
});

describe('TaskViewModal Closed State', () => {
  it('does not render when closed', () => {
    render(<TaskViewModal taskId={1} isOpen={false} onClose={vi.fn()} />);
    expect(screen.queryByTestId('modal')).not.toBeInTheDocument();
  });
});

describe('TaskViewModal Null TaskId', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('does not fetch when taskId is null', () => {
    render(<TaskViewModal taskId={null} isOpen={true} onClose={vi.fn()} />);
    expect(tasksApi.getOne).not.toHaveBeenCalled();
  });
});
