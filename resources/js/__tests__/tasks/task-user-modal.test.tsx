...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, leftIcon, onClick, variant, loading, type, className }: any) => (
    <button onClick={onClick} type={type} disabled={loading} data-variant={variant} className={className}>
      {leftIcon}
...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, error, leftIcon, type }: any) => (
    <div>
      {leftIcon}
      <input
        type={type}
...
describe('UserModal User Selection', () => {
  ...
  it('displays user emails', () => {
    ...
    expect(screen.getByText('mohammed@test.com')).toBeInTheDocument();
  });
  ...
  it('handles search input change', () => {
    ...
    expect(defaultProps.onSearchChange).toHaveBeenCalledWith('أحمد');
  });

  it('filters users by search query', () => {
    render(<UserModal {...defaultProps} searchQuery="أحمد" />);
  ...
  it('highlights selected user', () => {
    render(<UserModal {...defaultProps} selectedUserId="1" />);

    // Check icon should appear for selected user
    expect(screen.getByTestId('check-icon')).toBeInTheDocument();
  });

  it('shows empty state when no matching users', () => {
    render(<UserModal {...defaultProps} searchQuery="xyz123" />);
    expect(screen.getByText('لا يوجد مستخدمون مطابقون')).toBeInTheDocument();
  });
});

describe('UserModal Footer Actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows cancel button in user list view', () => {
    render(<UserModal {...defaultProps} />);
  ...
  it('shows remove button when user is selected', () => {
    render(<UserModal {...defaultProps} selectedUserId="1" />);
    expect(screen.getByText('إزالة المكلف')).toBeInTheDocument();
  });

  it('hides remove button when no user selected', () => {
    render(<UserModal {...defaultProps} selectedUserId="" />);
...
describe('UserModal User Avatar', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows first letter of user name as avatar', () => {
    ...
    const avatars = screen.getAllByText('أ'); // First letter of أحمد
    expect(avatars.length).toBeGreaterThanOrEqual(1);
  });
});

describe('UserModal Close', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
