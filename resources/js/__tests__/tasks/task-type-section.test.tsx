...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconStack2: () => <span data-testid="layers-icon">Layers</span>,
  IconUser: () => <span data-testid="user-icon">User</span>,
  IconBuilding: () => <span data-testid="building-icon">Building2</span>,
  IconRefresh: () => <span data-testid="refresh-icon">RefreshCw</span>,
...
describe('TaskTypeSection Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders section header', () => {
    render(<TaskTypeSection {...createDefaultProps()} />);
    expect(screen.getByText('نوع المهمة')).toBeInTheDocument();
  });

  it('renders layers icon', () => {
    render(<TaskTypeSection {...createDefaultProps()} />);
    expect(screen.getAllByTestId('layers-icon').length).toBeGreaterThan(0);
  });
...
describe('TaskTypeSection Type Selection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls onChange when clicking project type', () => {
    const props = createDefaultProps();
    render(<TaskTypeSection {...props} />);
    fireEvent.click(screen.getByText('مهمة مشروع'));
    expect(props.onChange).toHaveBeenCalledWith('type', 'project');
  });

  it('calls onChange when clicking personal type', () => {
    const props = createDefaultProps();
    render(<TaskTypeSection {...props} />);
    fireEvent.click(screen.getByText('مهمة شخصية'));
    expect(props.onChange).toHaveBeenCalledWith('type', 'personal');
  });

  it('calls onChange when clicking department type', () => {
    const props = createDefaultProps();
    render(<TaskTypeSection {...props} />);
    fireEvent.click(screen.getByText('مهمة إدارية'));
    expect(props.onChange).toHaveBeenCalledWith('type', 'department');
  });

  it('calls onChange when clicking recurring type', () => {
    const props = createDefaultProps();
    render(<TaskTypeSection {...props} />);
    fireEvent.click(screen.getByText('مهمة متكررة'));
    expect(props.onChange).toHaveBeenCalledWith('type', 'recurring');
  });
});
...
describe('TaskTypeSection Personal Task', () => {
  ...
  it('does not show additional fields for personal tasks', () => {
    const props = createDefaultProps({ formData: { type: 'personal' } });
    render(<TaskTypeSection {...props} />);
    // Personal tasks don't have additional fields in the current implementation
...
describe('TaskTypeSection Department Task', () => {
  ...
  it('shows department select for department tasks', () => {
    const props = createDefaultProps({ formData: { type: 'department' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.getByText('القسم')).toBeInTheDocument();
  });

  it('shows department options in select', () => {
    const props = createDefaultProps({ formData: { type: 'department' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.getByText('قسم التقنية')).toBeInTheDocument();
    expect(screen.getByText('قسم الموارد البشرية')).toBeInTheDocument();
  });
  ...
  it('calls onChange when department changes', () => {
    ...
    expect(props.onChange).toHaveBeenCalledWith('department_id', '2');
  });

  it('shows department_id error when present', () => {
    const props = createDefaultProps({
  ...
  it('shows default department option', () => {
    const props = createDefaultProps({ formData: { type: 'department' } });
    render(<TaskTypeSection {...props} />);
...
describe('TaskTypeSection Recurring Task', () => {
  ...
  it('shows recurrence select for recurring tasks', () => {
    const props = createDefaultProps({ formData: { type: 'recurring' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.getByText('نمط التكرار')).toBeInTheDocument();
  });

  it('shows recurrence options', () => {
    const props = createDefaultProps({ formData: { type: 'recurring' } });
    render(<TaskTypeSection {...props} />);
  ...
  it('shows all recurrence pattern options', () => {
    const props = createDefaultProps({ formData: { type: 'recurring' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.getByText('اختر نمط التكرار')).toBeInTheDocument();
    expect(screen.getByText('كل أسبوعين')).toBeInTheDocument();
    expect(screen.getByText('ربع سنوي')).toBeInTheDocument();
    expect(screen.getByText('سنوي')).toBeInTheDocument();
  });

  it('calls onChange when recurrence changes', () => {
    const props = createDefaultProps({ formData: { type: 'recurring' } });
    render(<TaskTypeSection {...props} />);
    const select = screen.getByTestId('select');
    fireEvent.change(select, { target: { value: 'weekly' } });
    expect(props.onChange).toHaveBeenCalledWith('recurrence_rule', 'weekly');
  });

  it('shows recurrence_rule error when present', () => {
    const props = createDefaultProps({
      formData: { type: 'recurring' },
      errors: { recurrence_rule: ['نمط التكرار مطلوب'] }
    });
    render(<TaskTypeSection {...props} />);
    expect(screen.getByText('نمط التكرار مطلوب')).toBeInTheDocument();
  });
});

describe('TaskTypeSection Project Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('does not show department select for project tasks', () => {
    const props = createDefaultProps({ formData: { type: 'project' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.queryByText('القسم')).not.toBeInTheDocument();
  });

  it('does not show recurrence select for project tasks', () => {
    const props = createDefaultProps({ formData: { type: 'project' } });
    render(<TaskTypeSection {...props} />);
    expect(screen.queryByText('نمط التكرار')).not.toBeInTheDocument();
  });
...
describe('TaskTypeSection Active State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('highlights project type when selected', () => {
    const props = createDefaultProps({ formData: { type: 'project' } });
    render(<TaskTypeSection {...props} />);
    const projectButton = screen.getByText('مهمة مشروع').closest('button');
    expect(projectButton?.className).toContain('accent');
  });

  it('highlights personal type when selected', () => {
    const props = createDefaultProps({ formData: { type: 'personal' } });
    render(<TaskTypeSection {...props} />);
    const personalButton = screen.getByText('مهمة شخصية').closest('button');
    expect(personalButton?.className).toContain('accent');
  });

  it('highlights department type when selected', () => {
    const props = createDefaultProps({ formData: { type: 'department' } });
    render(<TaskTypeSection {...props} />);
    const deptButton = screen.getByText('مهمة إدارية').closest('button');
    expect(deptButton?.className).toContain('accent');
  });

  it('highlights recurring type when selected', () => {
    const props = createDefaultProps({ formData: { type: 'recurring' } });
    render(<TaskTypeSection {...props} />);
    const recurringButton = screen.getByText('مهمة متكررة').closest('button');
    expect(recurringButton?.className).toContain('accent');
  });
});
