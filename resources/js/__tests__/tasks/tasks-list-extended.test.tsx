...
vi.mock('@pages/tasks/list', () => ({
  KanbanView: ({ tasks, onOpenTaskModal }: any) => (
    <div data-testid="kanban-view">
      {tasks.map((task: any) => (
        <div key={task.id} onClick={() => onOpenTaskModal(task.id)}>
          {task.title}
        </div>
      ))}
    </div>
  ),
  CardsView: ({ tasks, onOpenTaskModal }: any) => (
  ...
  TableView: ({ tasks, onOpenTaskModal }: any) => (
  ...
  TasksHeader: ({ tasks, pagination }: any) => (
    <div data-testid="tasks-header">
      ...
      <span>مهام: {tasks.length}</span>
    </div>
  ),
  TasksFilters: ({ viewMode, onViewModeChange, onSearch }: any) => (
    <div data-testid="tasks-filters">
      <button onClick={() => onViewModeChange('kanban')}>كانبان</button>
      <button onClick={() => onViewModeChange('cards')}>بطاقات</button>
      <button onClick={() => onViewModeChange('table')}>جدول</button>
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconSquareCheck: () => <span data-testid="check-icon">✓</span>,
...
describe('TasksList Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TasksList View Modes', () => {
  ...
  it('switches to cards view', async () => {
    ...
    fireEvent.click(screen.getByText('بطاقات'));
...
describe('TasksList Task Modal', () => {
  ...
  it('opens task modal when task clicked', async () => {
    render(<TasksList />);
    await waitFor(() => {
...
describe('TasksList Header Info', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows tasks count', async () => {
    render(<TasksList />);
    // StatStrip breaks the loaded tasks down by status (in_progress / completed).
    await waitFor(() => {
...
describe('TasksList Search', () => {
  ...
  it('renders filters component', async () => {
    render(<TasksList />);
    await waitFor(() => {
      // TasksFilters component should be rendered with view toggle buttons
      expect(screen.getByTestId('tasks-filters')).toBeInTheDocument();
    });
  });
});
...
describe('TasksList Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockResolvedValueOnce({
      data: [],
      current_page: 1,
      last_page: 1,
      total: 0,
...
describe('TasksList Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockImplementationOnce(
...
describe('TasksList Tasks Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
