...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, error }: any) => (
    <div>
      <input
        value={value}
        onChange={onChange}
...
describe('TaskDetailsSection Basic', () => {
  const defaultProps = {
    formData: {
      project_id: '1',
      milestone_id: '',
  ...
  it('renders title label with required marker', () => {
    render(<TaskDetailsSection {...defaultProps} />);
...
describe('TaskDetailsSection onChange', () => {
  const defaultProps = {
    formData: {
      project_id: '1',
      milestone_id: '',
  ...
  it('calls onChange when title changes', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const titleInput = screen.getByPlaceholderText('أدخل عنوان المهمة');
    fireEvent.change(titleInput, { target: { value: 'مهمة جديدة' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('title', 'مهمة جديدة');
  });

  it('calls onChange when description changes', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const descInput = screen.getByPlaceholderText('أدخل وصف المهمة');
    fireEvent.change(descInput, { target: { value: 'وصف المهمة' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('description', 'وصف المهمة');
  });
});

describe('TaskDetailsSection Errors', () => {
  const defaultProps = {
    formData: {
      project_id: '1',
      milestone_id: '',
  ...
  it('shows title error when present', () => {
    const errors = { title: ['عنوان المهمة مطلوب'] };
    render(<TaskDetailsSection {...defaultProps} errors={errors} />);
...
describe('TaskDetailsSection Values', () => {
  const defaultProps = {
    formData: {
      project_id: '1',
      milestone_id: '',
      ...
      assigned_to: '',
      title: 'مهمة حالية',
      description: 'وصف المهمة الحالية',
      status: 'todo',
      priority: 'medium',
      start_date: '',
      due_date: '',
  ...
  it('displays current title value', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const titleInput = screen.getByPlaceholderText('أدخل عنوان المهمة');
    expect(titleInput).toHaveValue('مهمة حالية');
  });

  it('displays current description value', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const descInput = screen.getByPlaceholderText('أدخل وصف المهمة');
    expect(descInput).toHaveValue('وصف المهمة الحالية');
  });
});
