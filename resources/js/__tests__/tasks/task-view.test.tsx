...
// Mock API - must define data inline in the mock factory
vi.mock('@entities/task', () => ({
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({
      ...
      subtasks: [
        { id: 2, title: 'مهمة فرعية 1', status: 'completed' },
        { id: 3, title: 'مهمة فرعية 2', status: 'todo' },
      ],
      comments: [
        { id: 1, content: 'تعليق اختباري', user: { name: 'أحمد' }, created_at: '2025-01-01' },
      ],
      time_indicator: {
        has_due_date: true,
        status: 'normal',
...
vi.mock('@widgets/task', () => ({
  TaskStatusChanger: ({ currentStatus }: { currentStatus: string }) => (
    <div data-testid="task-status-changer">Status: {currentStatus}</div>
  ),
  TaskTimeIndicator: () => <div data-testid="task-time-indicator">Time Indicator</div>,
  TaskActivityLog: () => <div data-testid="task-activity-log">Activity Log</div>,
  TaskDetailsPanel: () => <div data-testid="task-details-panel">Details Panel</div>,
}));

// Mock view components
vi.mock('@pages/tasks/view', () => ({
  CommentsSection: ({ comments }: { comments: any[] }) => (
    <div data-testid="comments-section">Comments: {comments.length}</div>
  ),
  TaskViewSkeleton: () => <div data-testid="task-view-skeleton">Loading...</div>,
  statusLabels: {
    todo: 'للتنفيذ',
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, leftIcon, variant }: any) => (
...
describe('TaskView Component', () => {
  ...
  it('renders task title', async () => {
    ...
    await waitFor(() => {
      ...
      expect(heading?.textContent).toContain('مهمة اختبارية');
    });
  });
  ...
  it('renders project link when task has project', async () => {
    render(<TaskView />);
  ...
  it('renders milestone name when task has milestone', async () => {
    render(<TaskView />);
  ...
  it('renders task status changer', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('task-status-changer')).toBeInTheDocument();
    });
  });
...
describe('TaskView Tabs', () => {
  ...
  it('renders comments tab with count', async () => {
    ...
    await waitFor(() => {
      ...
      const commentsTexts = screen.getAllByText('التعليقات (1)');
      expect(commentsTexts.length).toBeGreaterThan(0);
    });
  });
...
describe('TaskView Details Tab Content', () => {
  ...
  it('renders task description text', async () => {
    render(<TaskView />);
  ...
  it('renders time indicator when task has due date', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('task-time-indicator')).toBeInTheDocument();
    });
  });
});

describe('TaskView Subtasks Tab Content', () => {
  it('renders subtasks section title', async () => {
    render(<TaskView />);
  ...
  it('renders subtask list items', async () => {
    render(<TaskView />);
...
describe('TaskView Comments Tab Content', () => {
...
describe('TaskView Activity Tab Content', () => {
  it('renders activity log component', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('task-activity-log')).toBeInTheDocument();
    });
  });
...
describe('TaskView Sidebar', () => {
  it('renders task details panel', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('task-details-panel')).toBeInTheDocument();
    });
  });
});

describe('TaskView Add Subtask Functionality', () => {
  it('shows add subtask form when button clicked', async () => {
    render(<TaskView />);
    ...
    await userEvent.click(screen.getByText('إضافة'));
  ...
  it('shows cancel button in add subtask form', async () => {
    render(<TaskView />);
  ...
  it('allows typing subtask title', async () => {
    render(<TaskView />);
    ...
    await userEvent.type(input, 'مهمة فرعية جديدة');

    expect(input).toHaveValue('مهمة فرعية جديدة');
  });
...
describe('TaskView Links', () => {
  ...
  it('project badge links to project page', async () => {
    render(<TaskView />);

    await waitFor(() => {
      const projectLink = screen.getByText('PRJ-001').closest('a');
      expect(projectLink).toHaveAttribute('href', '/projects/1');
    });
  });

  it('subtask items link to subtask pages', async () => {
    render(<TaskView />);

    await waitFor(() => {
      const subtaskLink1 = screen.getByText('مهمة فرعية 1').closest('a');
      expect(subtaskLink1).toHaveAttribute('href', '/tasks/2');

      const subtaskLink2 = screen.getByText('مهمة فرعية 2').closest('a');
      expect(subtaskLink2).toHaveAttribute('href', '/tasks/3');
    });
  });
});

describe('TaskView Status Labels', () => {
  it('shows status label for subtasks', async () => {
    render(<TaskView />);

    await waitFor(() => {
      // The subtasks should have status labels
