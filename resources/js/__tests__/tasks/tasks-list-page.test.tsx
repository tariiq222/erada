...
vi.mock('@entities/task', () => ({
  tasksApi: {
    ...
    getOne: vi.fn().mockResolvedValue({
      id: 2,
      subtasks: [
        { id: 10, title: 'فرعية 1', status: 'completed', assignee: null },
        { id: 11, title: 'فرعية 2', status: 'todo', assignee: null },
      ],
    }),
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon, onClick }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void }) => (
...
vi.mock('@pages/tasks/list', () => ({
  KanbanView: ({ tasks, onOpenTaskModal }: { tasks: any[]; onOpenTaskModal: (id: number) => void }) => (
    <div data-testid="kanban-view">
      {tasks.map((t: any) => (
        <div key={t.id} onClick={() => onOpenTaskModal(t.id)}>{t.title}</div>
      ))}
    </div>
  ),
  CardsView: ({ tasks, onOpenTaskModal }: { tasks: any[]; onOpenTaskModal: (id: number) => void }) => (
  ...
  TableView: ({ tasks, onOpenTaskModal }: { tasks: any[]; onOpenTaskModal: (id: number) => void }) => (
  ...
  TasksHeader: ({ pagination }: { pagination: { total: number } }) => (
    <div data-testid="tasks-header">
      <h1>المهام</h1>
      <span>إجمالي: {pagination.total}</span>
    </div>
  ),
  TasksFilters: ({ viewMode, onViewModeChange, onSearch }: { viewMode: string; onViewModeChange: (mode: 'table' | 'cards' | 'kanban') => void; onSearch: (e: React.FormEvent) => void }) => (
    <div data-testid="tasks-filters">
      <button onClick={() => onViewModeChange('table')} data-testid="table-btn">جدول</button>
      <button onClick={() => onViewModeChange('cards')} data-testid="cards-btn">بطاقات</button>
      <button onClick={() => onViewModeChange('kanban')} data-testid="kanban-btn">Kanban</button>
...
describe('TasksList Component', () => {
  ...
  it('renders tasks header', async () => {
    render(<TasksList />);
    // Header is now a shared PageHeader (h1) plus a StatStrip total.
    await waitFor(() => {
  ...
  it('renders tasks in table view', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('مهمة اختبارية 2')).toBeInTheDocument();
    });
  });
});

describe('TasksList View Mode Switching', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TasksList Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('opens task modal when clicking on task', async () => {
    render(<TasksList />);
    await waitFor(() => {
      expect(screen.getByText('مهمة اختبارية 1')).toBeInTheDocument();
    });

    fireEvent.click(screen.getByText('مهمة اختبارية 1'));
...
describe('TasksList Search', () => {
  ...
  it('renders filters component', async () => {
    ...
    // TasksFilters component is rendered and functional
    expect(screen.getByTestId('table-btn')).toBeInTheDocument();
    expect(screen.getByTestId('cards-btn')).toBeInTheDocument();
    expect(screen.getByTestId('kanban-btn')).toBeInTheDocument();
  });
});
...
describe('TasksList Empty State', () => {
  ...
  it('shows empty state when no tasks', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getAll as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: [],
      current_page: 1,
      last_page: 1,
      per_page: 10,
...
describe('TasksList API Error Handling', () => {
  ...
  it('handles API error gracefully', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getAll as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));
...
describe('TasksList isOverdue Function', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders without crashing with overdue tasks', async () => {
    const overdueTask = {
      id: 99,
      title: 'مهمة متأخرة',
      status: 'in_progress',
