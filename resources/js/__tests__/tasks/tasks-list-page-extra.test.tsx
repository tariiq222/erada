...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon }: any) => <button>{leftIcon}{children}</button>,
...
vi.mock('@pages/tasks/list', () => ({
  KanbanView: ({ tasks, onOpenTaskModal }: any) => (
    <div data-testid="kanban-view">
      {tasks.map((t: any) => (
        <div key={t.id} data-testid={`kanban-task-${t.id}`} onClick={() => onOpenTaskModal(t.id)}>
          {t.title}
        </div>
      ))}
    </div>
  ),
  CardsView: ({ tasks, onOpenTaskModal, pagination, onPageChange }: any) => (
    <div data-testid="cards-view">
      {tasks.map((t: any) => (
        <div key={t.id} data-testid={`card-task-${t.id}`} onClick={() => onOpenTaskModal(t.id)}>
          {t.title}
        </div>
      ))}
      <button onClick={() => onPageChange(2)}>الصفحة التالية</button>
      <span data-testid="pagination-info">صفحة {pagination.currentPage} من {pagination.lastPage}</span>
    </div>
  ),
  TableView: ({ tasks, onOpenTaskModal, pagination, onPageChange }: any) => (
    <div data-testid="table-view">
      <table>
        <tbody>
          {tasks.map((t: any) => (
            <tr key={t.id} data-testid={`table-task-${t.id}`} onClick={() => onOpenTaskModal(t.id)}>
              <td>{t.title}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <button onClick={() => onPageChange(2)}>التالي</button>
    </div>
  ),
  TasksHeader: ({ tasks, pagination }: any) => (
    <div data-testid="tasks-header">
      <span data-testid="total-tasks">إجمالي: {pagination.total}</span>
      <span data-testid="task-count">{tasks.length} مهمة</span>
    </div>
  ),
  TasksFilters: ({ filters, viewMode, onFiltersChange, onViewModeChange, onSearch }: any) => (
    <div data-testid="tasks-filters">
      ...
      <button data-testid="table-btn" onClick={() => onViewModeChange('table')}>جدول</button>
      <button data-testid="cards-btn" onClick={() => onViewModeChange('cards')}>بطاقات</button>
      <button data-testid="kanban-btn" onClick={() => onViewModeChange('kanban')}>كانبان</button>
      <button data-testid="search-btn" onClick={onSearch}>بحث</button>
      <label>
        <input
          type="checkbox"
          data-testid="my-tasks-checkbox"
          checked={filters.my_tasks}
          onChange={(e) => onFiltersChange({ ...filters, my_tasks: e.target.checked })}
        />
        مهامي فقط
      </label>
    </div>
...
const mockTasks = [
  { id: 1, title: 'مهمة 1', status: 'todo', priority: 'high', due_date: '2025-12-31', subtasks_count: 0 },
  { id: 2, title: 'مهمة 2', status: 'in_progress', priority: 'medium', due_date: '2025-06-15', subtasks_count: 2 },
  { id: 3, title: 'مهمة 3', status: 'completed', priority: 'low', due_date: null, subtasks_count: 0 },
];
...
describe('TasksList Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockResolvedValue(mockPaginatedResponse);
  });

  it('renders tasks header', async () => {
    renderWithRouter(<TasksList />);
    // Header is now a shared PageHeader (h1) plus a StatStrip.
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'المهام' })).toBeInTheDocument();
    });
  });

  it('renders tasks filters', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
  ...
  it('shows total tasks count', async () => {
    renderWithRouter(<TasksList />);
    // StatStrip renders the total-tasks label with its value (pagination.total).
    await waitFor(() => {
...
describe('TasksList Loading State', () => {
  ...
  it('shows loading skeletons initially', () => {
    (tasksApi.getAll as any).mockImplementation(() => new Promise(() => {}));
    renderWithRouter(<TasksList />);
    expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
  });
});

describe('TasksList View Modes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockResolvedValue(mockPaginatedResponse);
  });
  ...
  it('switches to cards view', async () => {
    ...
    fireEvent.click(screen.getByTestId('cards-btn'));
  ...
  it('switches to kanban view', async () => {
    ...
    fireEvent.click(screen.getByTestId('kanban-btn'));
...
describe('TasksList Task Modal', () => {
  ...
  it('opens modal on task click', async () => {
    ...
    fireEvent.click(screen.getByTestId('table-task-1'));
    expect(screen.getByTestId('task-view-modal')).toBeInTheDocument();
  });

  it('closes modal on close click', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
  ...
  it('shows correct task ID in modal', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByTestId('table-task-2'));
...
describe('TasksList Filters', () => {
  ...
  it('updates search filter', async () => {
    ...
    fireEvent.change(screen.getByTestId('search-input'), { target: { value: 'بحث' } });
    expect(screen.getByTestId('search-input')).toHaveValue('بحث');
  });
  ...
  it('updates status filter', async () => {
    ...
    expect(screen.getByTestId('status-filter')).toHaveValue('todo');
  });
  ...
  it('updates my_tasks filter', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('my-tasks-checkbox')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByTestId('my-tasks-checkbox'));
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByTestId('my-tasks-checkbox')).toBeChecked();
  });
});

describe('TasksList Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TasksList Pagination', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockResolvedValue(mockPaginatedResponse);
  });

  it('shows pagination in cards view', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByTestId('cards-btn'));
    expect(screen.getByTestId('pagination-info')).toHaveTextContent('صفحة 1 من 3');
  });

  it('calls API on page change', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByTestId('cards-btn'));
    fireEvent.click(screen.getByText('الصفحة التالية'));
    await waitFor(() => {
      expect(tasksApi.getAll).toHaveBeenCalledWith({ page: '2' });
    });
  });
});

describe('TasksList Task Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getAll as any).mockResolvedValue(mockPaginatedResponse);
  });

  it('displays task titles in table', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
  ...
  it('displays task titles in kanban', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
  ...
  it('displays task titles in cards', async () => {
    renderWithRouter(<TasksList />);
    await waitFor(() => {
      expect(screen.getByTestId('table-view')).toBeInTheDocument();
    });
...
describe('TasksList API Error', () => {
  ...
  it('handles API error gracefully', async () => {
    ...
    (tasksApi.getAll as any).mockRejectedValue(new Error('API Error'));
    renderWithRouter(<TasksList />);
    await waitFor(() => {
...
describe('TasksList Task Count', () => {
  ...
  it('shows correct task count', async () => {
    renderWithRouter(<TasksList />);
    // The loaded tasks render as rows in the table view (3 mock tasks).
    await waitFor(() => {
      expect(screen.getByTestId('table-task-1')).toBeInTheDocument();
    });
    expect(screen.getByTestId('table-task-2')).toBeInTheDocument();
    expect(screen.getByTestId('table-task-3')).toBeInTheDocument();
  });
});
