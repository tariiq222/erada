...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconCircle: () => <span data-testid="circle-icon">Circle</span>,
...
vi.mock('@pages/projects/constants', () => ({
  ...
  taskStatusIcons: {
    todo: () => <span data-testid="status-todo-icon">Todo</span>,
    in_progress: () => <span data-testid="status-progress-icon">Progress</span>,
    in_review: () => <span data-testid="status-review-icon">Review</span>,
    completed: () => <span data-testid="status-completed-icon">Completed</span>,
  },
}));

import SubtasksAccordion from '@pages/projects/components/sections/tasks/SubtasksAccordion';

const mockSubtasks = [
  { id: 1, title: 'المهمة الفرعية الأولى', status: 'todo', assignee: null },
  { id: 2, title: 'المهمة الفرعية الثانية', status: 'in_progress', assignee: { name: 'أحمد' } },
  { id: 3, title: 'المهمة الفرعية الثالثة', status: 'completed', assignee: { name: 'محمد' } },
];

describe('SubtasksAccordion Basic', () => {
  const defaultProps = {
    taskId: 1,
    subtasks: mockSubtasks,
    openSubtaskStatusMenu: null,
    onUpdateSubtaskStatus: vi.fn(),
    onOpenSubtaskStatusMenu: vi.fn(),
  };
  ...
  it('renders subtasks count', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    expect(screen.getByText('المهام الفرعية (3)')).toBeInTheDocument();
  });

  it('renders completed count', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    expect(screen.getByText('1 مكتملة')).toBeInTheDocument();
  });

  it('renders all subtask titles', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    expect(screen.getByText('المهمة الفرعية الأولى')).toBeInTheDocument();
    expect(screen.getByText('المهمة الفرعية الثانية')).toBeInTheDocument();
    expect(screen.getByText('المهمة الفرعية الثالثة')).toBeInTheDocument();
  });
...
describe('SubtasksAccordion Status Menu', () => {
  const defaultProps = {
    taskId: 1,
    subtasks: mockSubtasks,
    openSubtaskStatusMenu: null,
  ...
  it('opens status menu when clicking status button', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    const statusButtons = screen.getAllByText('للتنفيذ');
    fireEvent.click(statusButtons[0]);
    expect(defaultProps.onOpenSubtaskStatusMenu).toHaveBeenCalledWith(1);
  });

  it('closes status menu when clicking same button', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    const statusButtons = screen.getAllByText('للتنفيذ');
    fireEvent.click(statusButtons[0]);
    expect(defaultProps.onOpenSubtaskStatusMenu).toHaveBeenCalledWith(null);
  });

  it('shows status options when menu is open', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    expect(screen.getByText('تغيير الحالة')).toBeInTheDocument();
  });

  it('shows all status options in menu', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    // When menu is open, all status options appear
    expect(screen.getAllByText('للتنفيذ').length).toBeGreaterThan(1);
    expect(screen.getAllByText('قيد التنفيذ').length).toBeGreaterThan(1);
    expect(screen.getByText('قيد المراجعة')).toBeInTheDocument();
    expect(screen.getAllByText('مكتمل').length).toBeGreaterThan(1);
  });

  it('calls onUpdateSubtaskStatus when selecting different status', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    // Find the completed option button in the menu (different from current todo status)
    const completedOptions = screen.getAllByText('مكتمل');
    // Click the one in the dropdown menu
    const menuOption = completedOptions.find(el => el.closest('.absolute'));
    if (menuOption) {
      fireEvent.click(menuOption.closest('button')!);
      expect(defaultProps.onUpdateSubtaskStatus).toHaveBeenCalled();
    }
  });

  it('does not call onUpdateSubtaskStatus when selecting same status', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    // Click the todo option (same as current) in the menu
    const todoOptions = screen.getAllByText('للتنفيذ');
    const menuOption = todoOptions.find(el => el.closest('.absolute'));
    if (menuOption) {
      fireEvent.click(menuOption.closest('button')!);
      expect(defaultProps.onUpdateSubtaskStatus).not.toHaveBeenCalled();
    }
  });

  it('closes menu after selecting different status', () => {
    render(<SubtasksAccordion {...defaultProps} openSubtaskStatusMenu={1} />);
    const completedOptions = screen.getAllByText('مكتمل');
    const menuOption = completedOptions.find(el => el.closest('.absolute'));
    if (menuOption) {
      fireEvent.click(menuOption.closest('button')!);
      expect(defaultProps.onOpenSubtaskStatusMenu).toHaveBeenCalled();
    }
  });
});

describe('SubtasksAccordion Empty State', () => {
  const emptyProps = {
    taskId: 1,
    subtasks: [],
    openSubtaskStatusMenu: null,
  ...
  it('shows zero count for empty subtasks', () => {
    render(<SubtasksAccordion {...emptyProps} />);
    expect(screen.getByText('المهام الفرعية (0)')).toBeInTheDocument();
  });

  it('shows zero completed for empty subtasks', () => {
    render(<SubtasksAccordion {...emptyProps} />);
    expect(screen.getByText('0 مكتملة')).toBeInTheDocument();
  });
});

describe('SubtasksAccordion Status Colors', () => {
  const defaultProps = {
    taskId: 1,
    subtasks: mockSubtasks,
    openSubtaskStatusMenu: null,
  ...
  it('applies todo styling', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    expect(screen.getAllByTestId('subtask-status-button')).toHaveLength(mockSubtasks.length);
  });

  it('applies in_progress styling', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    const buttons = screen.getAllByTestId('subtask-status-button');
    expect(buttons.some(b => b.dataset.subtaskStatus === 'in_progress')).toBe(true);
  });

  it('applies completed styling', () => {
    render(<SubtasksAccordion {...defaultProps} />);
    const buttons = screen.getAllByTestId('subtask-status-button');
    expect(buttons.some(b => b.dataset.subtaskStatus === 'completed')).toBe(true);
  });
});

describe('SubtasksAccordion All Completed', () => {
  const allCompletedProps = {
    taskId: 1,
    subtasks: [
      { id: 1, title: 'مهمة 1', status: 'completed', assignee: null },
      { id: 2, title: 'مهمة 2', status: 'completed', assignee: null },
    ],
    openSubtaskStatusMenu: null,
    onUpdateSubtaskStatus: vi.fn(),
    onOpenSubtaskStatusMenu: vi.fn(),
  ...
  it('shows all completed count', () => {
    render(<SubtasksAccordion {...allCompletedProps} />);
    expect(screen.getByText('2 مكتملة')).toBeInTheDocument();
  });
});
