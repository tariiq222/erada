...
vi.mock('@entities/task', () => ({
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({
      ...
      status: 'in_progress',
      priority: 'high',
      due_date: '2024-01-01', // past date to test overdue
...
vi.mock('@widgets/task', () => ({
  TaskStatusChanger: ({ currentStatus, onStatusChange }: { currentStatus: string; onStatusChange: (s: string) => void }) => (
    <div data-testid="task-status-changer">
      Status: {currentStatus}
      <button onClick={() => onStatusChange('completed')}>تغيير</button>
    </div>
  ),
...
vi.mock('@pages/tasks/view', () => ({
  CommentsSection: ({ comments, taskId, onCommentAdded }: any) => (
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, leftIcon, variant, disabled }: any) => (
...
describe('TaskView Overdue Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows overdue indicator for past due task', async () => {
    render(<TaskView />);
...
describe('TaskView Parent Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows parent task link', async () => {
    render(<TaskView />);
  ...
  it('parent link goes to parent task page', async () => {
    render(<TaskView />);

    await waitFor(() => {
      const parentLink = screen.getByText('المهمة الأصلية').closest('a');
      expect(parentLink).toHaveAttribute('href', '/tasks/10');
    });
  });
});

describe('TaskView Status Change', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('handles status change', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('task-status-changer')).toHaveTextContent('completed');
    });
  });
});

describe('TaskView Add Subtask API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls API when submitting subtask', async () => {
    render(<TaskView />);
    ...
    const submitButton = addButtons.find(btn => btn.closest('form'));
    if (submitButton) {
...
describe('TaskView Empty Comments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows zero comments count', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('comments-section')).toHaveTextContent('0');
    });
  });
});

describe('TaskView Refresh After Comment', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('has comments section with add button', async () => {
    render(<TaskView />);
...
describe('TaskView Error State', () => {
  let consoleErrorSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(() => {
    vi.clearAllMocks();
    // Silence expected "Network error" rejection logs — these tests assert on error UI, not on the rejection itself.
  ...
  it('shows error state when API fails', async () => {
    (tasksApi.getOne as any).mockRejectedValueOnce(new Error('Network error'));

    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByText('حدث خطأ في تحميل المهمة')).toBeInTheDocument();
    });
  });

  it('shows return button on error', async () => {
    (tasksApi.getOne as any).mockRejectedValueOnce(new Error('Network error'));

    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByText('العودة للمهام')).toBeInTheDocument();
    });
  });

  it('navigates to tasks list when return button clicked', async () => {
    (tasksApi.getOne as any).mockRejectedValueOnce(new Error('Network error'));
    ...
    fireEvent.click(screen.getByText('العودة للمهام'));
    expect(mockNavigate).toHaveBeenCalledWith('/tasks');
  });
});


describe('TaskView has details tab content', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows details tab content area', async () => {
    render(<TaskView />);
  ...
  it('shows subtasks tab content area', async () => {
    render(<TaskView />);

    await waitFor(() => {
      expect(screen.getByTestId('tab-content-subtasks')).toBeInTheDocument();
    });
  });

  it('shows comments tab content area', async () => {
    render(<TaskView />);
  ...
  it('shows activity tab content area', async () => {
    render(<TaskView />);
