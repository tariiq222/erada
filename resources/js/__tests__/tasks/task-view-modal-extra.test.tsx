...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconFileText: () => <span data-testid="filetext-icon">FileText</span>,
  ...
  IconExternalLink: () => <span data-testid="external-icon">ExternalLink</span>,
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, leftIcon, disabled }: any) => (
  ...
  SkeletonText: ({ lines }: any) => <div data-testid="skeleton-text" data-lines={lines} />,
  Modal: ({ children, isOpen, onClose }: any) =>
    isOpen ? <div data-testid="modal" onClick={(e) => e.target === e.currentTarget && onClose()}>{children}</div> : null,
  Tabs: ({ children, defaultValue, value, onValueChange }: any) => (
    <div data-testid="tabs" data-value={value || defaultValue}>{children}</div>
  ),
...
vi.mock('@widgets/task', () => ({
  TaskTimeIndicator: ({ indicator }: any) => <div data-testid="time-indicator">{indicator?.status}</div>,
  TaskActivityLog: ({ taskId }: any) => <div data-testid="activity-log">Activity Log for {taskId}</div>,
  TaskDetailsPanel: ({ task, onStatusChange }: any) => (
    <div data-testid="details-panel">
      <span>{task.title}</span>
      <button onClick={() => onStatusChange('completed')}>Change Status</button>
    </div>
  ),
}));

// Mock task page components
vi.mock('@pages/tasks/components', () => ({
  CommentsSection: ({ taskId }: any) => <div data-testid="comments-section">Comments for {taskId}</div>,
  SubtaskCard: ({ subtask, onStatusChange, onDelete }: any) => (
    <div data-testid="subtask-card">
  ...
  DetailsTab: ({ task, isOverdue }: any) => (
    <div data-testid="details-tab">
      ...
      <div><span>المكلف</span><span>{task?.assignee?.name || 'غير محدد'}</span></div>
      <div><span>الأولوية</span><span>{task?.priority === 'high' ? 'عالية' : task?.priority === 'medium' ? 'متوسطة' : task?.priority === 'low' ? 'منخفضة' : task?.priority === 'critical' ? 'حرجة' : task?.priority}</span></div>
      <div><span>الساعات</span><span>{task?.actual_hours || 0} / {task?.estimated_hours || 0} ساعة</span></div>
      {task?.milestone && <div><span>المرحلة</span><span>{task.milestone.name}</span></div>}
    </div>
  ),
  SubtasksTab: ({ subtasks, onShowForm }: any) => (
    <div data-testid="subtasks-tab">
      ...
      {(!subtasks || subtasks.length === 0) ? (
        ...
        subtasks?.map((s: any) => (
          <div key={s.id} data-testid="subtask-card">
            <span>{s.title}</span>
          </div>
        ))
      )}
  ...
  AttachmentsTab: ({ comments }: any) => <div data-testid="attachments-tab">Attachments</div>,
  useTaskViewModal: ({ taskId, isOpen, onTaskUpdated }: any) => {
    ...
    return {
      ...
      handleSubtaskStatusChange: vi.fn(),
      handleSubtaskUpdate: vi.fn(),
      handleSubtaskDelete: vi.fn(),
    };
  },
}));
...
const mockTask = {
  ...
  subtasks: [
    { id: 10, title: 'مهمة فرعية 1', status: 'todo', priority: 'medium', assignee: null },
    { id: 11, title: 'مهمة فرعية 2', status: 'completed', priority: 'low', assignee: { name: 'سارة' } },
  ],
  comments: [
    {
      id: 1,
      ...
      attachments: [{ id: 1, name: 'file.pdf', url: '/files/1', file_type: 'application/pdf', formatted_size: '1 MB' }]
    },
  ],
  time_indicator: { has_due_date: true, status: 'on_track' },
  parent: null,
};
...
describe('TaskViewModal Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });

  it('renders modal when open', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('does not render when closed', () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={false} onClose={vi.fn()} />);
  ...
  it('calls API with task ID', async () => {
    renderWithRouter(<TaskViewModal taskId={5} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(tasksApi.getOne).toHaveBeenCalledWith(5);
    });
  });
});

describe('TaskViewModal Content', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows task title', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getAllByText('مهمة اختبار').length).toBeGreaterThan(0);
    });
  });
  ...
  it('shows project link', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText('PRJ-001 - مشروع تجريبي')).toBeInTheDocument();
    });
  });
...
describe('TaskViewModal Tabs', () => {
  ...
  it('shows subtasks tab with count', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText(/المهام الفرعية \(2\)/)).toBeInTheDocument();
    });
  });

  it('shows comments tab with count', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('shows attachments tab', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Subtasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows subtask cards', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getAllByTestId('subtask-card').length).toBe(2);
    });
  });

  it('shows subtask titles', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Error State', () => {
  ...
  it('shows error message on API failure', async () => {
    (tasksApi.getOne as any).mockRejectedValue(new Error('API Error'));
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows external link icon', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Details Panel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows task info in panel', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText('المكلف')).toBeInTheDocument();
      expect(screen.getByText('الأولوية')).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Overdue Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const overdueTask = {
      ...mockTask,
      due_date: '2020-01-01', // تاريخ في الماضي
      status: 'todo',
    };
    (tasksApi.getOne as any).mockResolvedValue({ data: overdueTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });
...
describe('TaskViewModal No Description', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const taskNoDescription = { ...mockTask, description: null };
    (tasksApi.getOne as any).mockResolvedValue({ data: taskNoDescription });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });

  it('shows no description message', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText('لا يوجد وصف لهذه المهمة')).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Hours Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows hours information', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText('5 / 10 ساعة')).toBeInTheDocument();
    });
  });
...
describe('TaskViewModal Milestone', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  ...
  it('shows milestone info', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText('المرحلة')).toBeInTheDocument();
      expect(screen.getByText('المرحلة الأولى')).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Priority Labels', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows low priority label', async () => {
    const lowPriorityTask = { ...mockTask, priority: 'low' };
    (tasksApi.getOne as any).mockResolvedValue({ data: lowPriorityTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('shows medium priority label', async () => {
    const mediumPriorityTask = { ...mockTask, priority: 'medium' };
    (tasksApi.getOne as any).mockResolvedValue({ data: mediumPriorityTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('shows critical priority label', async () => {
    const criticalPriorityTask = { ...mockTask, priority: 'critical' };
    (tasksApi.getOne as any).mockResolvedValue({ data: criticalPriorityTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Child Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const childTask = { ...mockTask, parent: { id: 5, title: 'المهمة الأب' } };
    (tasksApi.getOne as any).mockResolvedValue({ data: childTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });

  it('hides subtasks tab for child tasks', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.queryByTestId('tab-subtasks')).not.toBeInTheDocument();
    });
  });
});

describe('TaskViewModal No Assignee', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const taskNoAssignee = { ...mockTask, assignee: null };
    (tasksApi.getOne as any).mockResolvedValue({ data: taskNoAssignee });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });

  it('shows unassigned message', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
...
describe('TaskViewModal Empty Subtasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const taskNoSubtasks = { ...mockTask, subtasks: [] };
    (tasksApi.getOne as any).mockResolvedValue({ data: taskNoSubtasks });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
  });

  it('shows no subtasks message', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('shows subtasks count as zero', async () => {
    renderWithRouter(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);
    await waitFor(() => {
      expect(screen.getByText(/المهام الفرعية \(0\)/)).toBeInTheDocument();
    });
  });
});

describe('TaskViewModal Comments Section', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
...
describe('TaskViewModal Activity Log', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({ data: mockTask });
    (usersApi.getList as any).mockResolvedValue({ data: mockUsers });
