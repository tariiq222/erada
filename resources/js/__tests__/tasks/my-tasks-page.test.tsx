...
vi.mock('@entities/task', () => ({
  unifiedTasksApi: {
    getMyTasks: vi.fn().mockResolvedValue({
      data: [
        ...
        },
        {
          ...
          type: 'project',
          due_date: '2025-01-20',
          project: { id: 1, name: 'مشروع 1', code: 'PRJ-001' },
          assignee: null,
        },
        {
          id: 3,
    ...
    getStats: vi.fn().mockResolvedValue({
      total: 10,
      by_status: { in_progress: 3, completed: 5, todo: 2 },
      by_type: { personal: 3, project: 4, department: 3 },
      overdue: 2,
      due_today: 1,
      due_this_week: 4,
    }),
...
vi.mock('@shared/ui', () => ({
  Card: ({ children, className, onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) => (
    <div className={className} data-testid="card" onClick={onClick}>{children}</div>
  ),
  Button: ({ children, leftIcon, onClick, variant, type, size, disabled }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void; variant?: string; type?: string; size?: string; disabled?: boolean }) => (
    <button onClick={onClick} type={type as 'button' | 'submit'} disabled={disabled}>{leftIcon}{children}</button>
  ),
  Skeleton: ({ className }: { className?: string }) => (
    <div className={`animate-pulse ${className}`} data-testid="skeleton" />
  ),
  StatusBadge: ({ type, status, size }: { type: string; status: string; size?: string }) => (
    <span data-testid="status-badge" data-type={type} data-status={status}>{status}</span>
  ),
  Select: ({ options, value, onChange, className }: { options: { value: string; label: string }[]; value?: string; onChange?: (e: { target: { value: string } }) => void; className?: string }) => (
    <select data-testid="select" value={value} onChange={(e) => onChange?.({ target: { value: e.target.value } })} className={className}>
      {options?.map((opt) => (
        <option key={opt.value} value={opt.value}>{opt.label}</option>
      ))}
    </select>
  ),
  Checkbox: ({ checked, onChange, label }: { checked?: boolean; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; label?: string }) => (
    <label>
  ...
  Input: ({ value, onChange, placeholder, className }: { value?: string; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder?: string; className?: string }) => (
    <input data-testid="input" value={value} onChange={onChange} placeholder={placeholder} className={className} />
  ),
  ...
  FilterField: ({ label, children }: any) => (
...
import MyTasksList from '@pages/tasks/MyTasksList';

describe('MyTasksList Page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders page description', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('إدارة ومتابعة مهامك الشخصية')).toBeInTheDocument();
    });
  });
  ...
  it('fetches tasks on mount', async () => {
    ...
    render(<MyTasksList />);
    await waitFor(() => {
      expect(unifiedTasksApi.getMyTasks).toHaveBeenCalled();
    });
  });
  ...
  it('fetches stats on mount', async () => {
    ...
    render(<MyTasksList />);
    await waitFor(() => {
      expect(unifiedTasksApi.getStats).toHaveBeenCalled();
    });
  });
  ...
  it('renders stats cards after loading', async () => {
    ...
    // Note: "قيد التنفيذ" may appear in multiple places (stats + task status)
  });

  it('renders task list', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('مهمة شخصية 1')).toBeInTheDocument();
      expect(screen.getByText('مهمة مشروع 1')).toBeInTheDocument();
    });
  });
});

describe('MyTasksList Filters', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders search input', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByPlaceholderText('بحث في المهام...')).toBeInTheDocument();
    });
  });
  ...
  it('renders type filter', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('كل الأنواع')).toBeInTheDocument();
    });
  });
  ...
  it('renders priority filter', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('كل الأولويات')).toBeInTheDocument();
    });
  });

  it('renders overdue checkbox', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('المتأخرة فقط')).toBeInTheDocument();
    });
  });
  ...
  it('submits search form', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('بحث في المهام...');
    fireEvent.change(searchInput, { target: { value: 'مهمة' } });
...
describe('MyTasksList View Modes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('defaults to list view', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(screen.getByText('طريقة العرض:')).toBeInTheDocument();
    });
  });
  ...
  it('can switch view modes', async () => {
    ...
    // The component has view toggle buttons - just verify they exist
    const buttons = screen.getAllByRole('button');
    expect(buttons.length).toBeGreaterThan(0);
  });
});

describe('MyTasksList Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders task modal component', async () => {
    render(<MyTasksList />);
    ...
    // The modal is rendered but not visible initially (isOpen=false)
    // Just verify tasks are rendered and clickable
    const taskElement = screen.getByText('مهمة شخصية 1');
    expect(taskElement).toBeInTheDocument();
  });

  it('tasks are clickable', async () => {
    render(<MyTasksList />);
    ...
    // Verify cursor-pointer class exists (tasks are clickable)
    const taskCards = screen.getAllByTestId('card');
    expect(taskCards.length).toBeGreaterThan(0);
  });
});

describe('MyTasksList Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows empty state when no tasks', async () => {
    const { unifiedTasksApi } = await import('@entities/task');
    (unifiedTasksApi.getMyTasks as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: [],
      current_page: 1,
      last_page: 1,
      per_page: 15,
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('لا توجد مهام مسندة إليك حالياً')).toBeInTheDocument();
    });
  });

  it('shows create task link in empty state', async () => {
    const { unifiedTasksApi } = await import('@entities/task');
    (unifiedTasksApi.getMyTasks as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: [],
      current_page: 1,
...
describe('MyTasksList API Error Handling', () => {
  ...
  it('handles API error gracefully', async () => {
    const { unifiedTasksApi } = await import('@entities/task');
    (unifiedTasksApi.getMyTasks as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));
...
describe('MyTasksList Task Types', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows project tasks with project name', async () => {
    render(<MyTasksList />);
  ...
  it('renders tasks with different types', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      expect(screen.getByText('مهمة شخصية 1')).toBeInTheDocument();
      expect(screen.getByText('مهمة مشروع 1')).toBeInTheDocument();
      expect(screen.getAllByText('مهمة إدارية')[0]).toBeInTheDocument();
    });
  });
});
