...
vi.mock('@pages/projects/constants', () => ({
  ...
  taskStatusIcons: {
    todo: () => <span data-testid="todo-icon">Todo</span>,
    in_progress: () => <span data-testid="progress-icon">Progress</span>,
    completed: () => <span data-testid="completed-icon">Completed</span>,
    cancelled: () => <span data-testid="cancelled-icon">Cancelled</span>,
  },
  kanbanColumnStyles: {
    ...
    cancelled: { bg: 'bg-red-50', border: 'border-red-200', headerBg: 'bg-red-100', headerText: 'text-red-600' },
  },
  STATUS_ORDER: ['todo', 'in_progress', 'completed', 'cancelled'],
}));

// Mock TaskCard component
vi.mock('@pages/projects/components/sections/tasks/TaskCard', () => ({
  default: ({
    task,
    onTaskClick,
    onDragStart,
...
import KanbanBoard from '@pages/projects/components/sections/tasks/KanbanBoard';

const mockTasks = [
  { id: 1, title: 'مهمة 1', status: 'todo', priority: 'high', subtasks_count: 2 },
  { id: 2, title: 'مهمة 2', status: 'in_progress', priority: 'medium', subtasks_count: 0 },
  { id: 3, title: 'مهمة 3', status: 'completed', priority: 'low', subtasks_count: 1 },
  { id: 4, title: 'مهمة 4', status: 'todo', priority: 'high', subtasks_count: 0 },
];
...
describe('KanbanBoard', () => {
  ...
  it('shows task count for each column', () => {
    render(<KanbanBoard {...defaultProps} />);
    // 2 tasks in todo, 1 in in_progress, 1 in completed, 0 in cancelled
    // Numbers may appear multiple times
    expect(screen.getAllByText('2').length).toBeGreaterThanOrEqual(1); // todo
    expect(screen.getAllByText('1').length).toBeGreaterThanOrEqual(1); // in_progress or completed
    expect(screen.getAllByText('0').length).toBeGreaterThanOrEqual(1); // cancelled
  });
});
...
describe('KanbanBoard Task Interaction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows subtasks button for tasks with subtasks', () => {
    render(<KanbanBoard {...defaultProps} />);
    expect(screen.getAllByText('مهام فرعية').length).toBeGreaterThanOrEqual(1);
  });

  it('calls onToggleSubtasks when subtasks button clicked', () => {
    render(<KanbanBoard {...defaultProps} />);

    const subtasksButtons = screen.getAllByText('مهام فرعية');
    fireEvent.click(subtasksButtons[0]);
...
describe('KanbanBoard Empty State', () => {
  ...
  it('renders columns even with no tasks', () => {
  ...
  it('shows zero count for empty columns', () => {
    ...
    expect(zeroCounts.length).toBe(4); // All columns empty
  });
});

describe('KanbanBoard Drag Over States', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('applies highlight style when dragging over column', () => {
    render(
      <KanbanBoard
        {...defaultProps}
        draggedTask={mockTasks[0] as any}
...
describe('KanbanBoard with Subtasks Data', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('passes subtasks data to task cards', () => {
    const subtasksData = {
      1: [
        { id: 101, title: 'فرعية 1', status: 'completed' },
        { id: 102, title: 'فرعية 2', status: 'in_progress' },
      ],
    };
...
describe('KanbanBoard Status Icons', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders status icons for columns', () => {
    render(<KanbanBoard {...defaultProps} />);
    // Icons may appear multiple times (in columns and task cards)
    expect(screen.getAllByTestId('todo-icon').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByTestId('progress-icon').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByTestId('completed-icon').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByTestId('cancelled-icon').length).toBeGreaterThanOrEqual(1);
  });
});
