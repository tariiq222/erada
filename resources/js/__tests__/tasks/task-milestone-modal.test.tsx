...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconDeviceFloppy: () => <span data-testid="save-icon">Save</span>,
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, leftIcon, onClick, variant, loading, type }: any) => (
    <button onClick={onClick} type={type} disabled={loading} data-variant={variant}>
      {leftIcon}
...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, error, type, min }: any) => (
    <div>
      <input
        type={type || 'text'}
        value={value}
        ...
        min={min}
        data-testid={placeholder ? `input-${placeholder}` : 'input'}
      />
...
vi.mock('@shared/ui/Modal', () => ({
  ...
  ModalHeader: ({ children, onClose }: any) => (
    <div data-testid="modal-header">
      {children}
      <button onClick={onClose}>إغلاق الهيدر</button>
    </div>
  ),
...
import MilestoneModal from '@pages/tasks/form/MilestoneModal';
...
describe('MilestoneModal', () => {
  ...
  it('shows milestone icon in header', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByTestId('milestone-icon')).toBeInTheDocument();
  });
});

describe('MilestoneModal Name Field', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders name label', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText('اسم المرحلة')).toBeInTheDocument();
  });

  it('renders name input with placeholder', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('مثال: المرحلة الأولى')).toBeInTheDocument();
  });

  it('handles name input change', () => {
    render(<MilestoneModal {...defaultProps} />);

    const input = screen.getByPlaceholderText('مثال: المرحلة الأولى');
    fireEvent.change(input, { target: { value: 'مرحلة جديدة' } });

    expect(defaultProps.onChange).toHaveBeenCalledWith('name', 'مرحلة جديدة');
  });
  ...
  it('shows name error when provided', () => {
    ...
    expect(screen.getByText('اسم المرحلة مطلوب')).toBeInTheDocument();
  });

  it('displays form data value', () => {
    render(
      <MilestoneModal
        {...defaultProps}
        formData={{ ...defaultProps.formData, name: 'مرحلة التخطيط' }}
      />
    );
    expect(screen.getByDisplayValue('مرحلة التخطيط')).toBeInTheDocument();
  });
});

describe('MilestoneModal Description Field', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders description label', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText('وصف المرحلة')).toBeInTheDocument();
  });

  it('renders description textarea with placeholder', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('وصف موجز للمرحلة...')).toBeInTheDocument();
  });

  it('handles description input change', () => {
    render(<MilestoneModal {...defaultProps} />);

    const textarea = screen.getByPlaceholderText('وصف موجز للمرحلة...');
    fireEvent.change(textarea, { target: { value: 'هذه مرحلة مهمة' } });

    expect(defaultProps.onChange).toHaveBeenCalledWith('description', 'هذه مرحلة مهمة');
  });
  ...
  it('displays description value', () => {
    render(
      <MilestoneModal
        {...defaultProps}
        formData={{ ...defaultProps.formData, description: 'وصف المرحلة هنا' }}
      />
    );
    expect(screen.getByDisplayValue('وصف المرحلة هنا')).toBeInTheDocument();
  });
});

describe('MilestoneModal Duration Field', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders duration label', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText('مدة المرحلة')).toBeInTheDocument();
  });

  it('renders duration value input with placeholder', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByPlaceholderText('أدخل الرقم')).toBeInTheDocument();
  });

  it('handles duration value change', () => {
    render(<MilestoneModal {...defaultProps} />);

    const input = screen.getByPlaceholderText('أدخل الرقم');
    fireEvent.change(input, { target: { value: '5' } });

    expect(defaultProps.onChange).toHaveBeenCalledWith('duration_value', '5');
  });

  it('renders duration unit select', () => {
    render(<MilestoneModal {...defaultProps} />);
    const trigger = screen.getByRole('button', { name: /يوم/ });
    fireEvent.click(trigger);
    expect(screen.getByRole('option', { name: /يوم/ })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: /أسبوع/ })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: /شهر/ })).toBeInTheDocument();
  });

  it('handles duration unit change', () => {
    render(<MilestoneModal {...defaultProps} />);

    const trigger = screen.getByRole('button', { name: /يوم/ });
    fireEvent.click(trigger);
    const weekOption = screen.getByRole('option', { name: /أسبوع/ });
    fireEvent.click(weekOption);

    expect(defaultProps.onChange).toHaveBeenCalledWith('duration_unit', 'week');
  });

  it('shows duration error when provided', () => {
    render(
      <MilestoneModal
        {...defaultProps}
        errors={{ duration_value: ['مدة المرحلة مطلوبة'] }}
      />
    );
    expect(screen.getByText('مدة المرحلة مطلوبة')).toBeInTheDocument();
  });

  it('shows duration help text', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText(/سيتم حساب تاريخ النهاية تلقائياً/)).toBeInTheDocument();
  });
});

describe('MilestoneModal Footer Actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders save button', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText('إنشاء مرحلة')).toBeInTheDocument();
  });
  ...
  it('handles save click', () => {
    render(<MilestoneModal {...defaultProps} />);

    fireEvent.click(screen.getByText('إنشاء مرحلة'));

    expect(defaultProps.onSave).toHaveBeenCalled();
  });

  it('shows loading state when saving', () => {
    render(<MilestoneModal {...defaultProps} isSaving={true} />);
  ...
  it('disables save button when saving', () => {
    ...
    const saveButton = screen.getByText('جاري...').closest('button');
    expect(saveButton).toBeDisabled();
  });
});

describe('MilestoneModal Close Actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders close button in header', () => {
    render(<MilestoneModal {...defaultProps} />);
    expect(screen.getByText('إغلاق الهيدر')).toBeInTheDocument();
  });

  it('handles header close click', () => {
    render(<MilestoneModal {...defaultProps} />);

    fireEvent.click(screen.getByText('إغلاق الهيدر'));

    expect(defaultProps.onClose).toHaveBeenCalled();
  });
});

describe('MilestoneModal Required Fields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('MilestoneModal Form Data', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays all form data values', () => {
    render(
      <MilestoneModal
        {...defaultProps}
        formData={{
    ...
    expect(screen.getByDisplayValue('مرحلة التنفيذ')).toBeInTheDocument();
    expect(screen.getByDisplayValue('مرحلة تنفيذ المشروع')).toBeInTheDocument();
