...
vi.mock('@entities/task', () => ({
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({
      ...
      priority: 'high',
      due_date: '2025-01-25',
      project: { id: 1, code: 'PRJ-001', name: 'مشروع اختباري' },
      assignee: { id: 1, name: 'أحمد' },
      creator: { id: 2, name: 'محمد' },
      parent: null,
      subtasks: [
        { id: 2, title: 'مهمة فرعية 1', status: 'completed', priority: 'medium', due_date: null, assignee: null },
        { id: 3, title: 'مهمة فرعية 2', status: 'todo', priority: 'low', due_date: null, assignee: { id: 1, name: 'أحمد' } },
      ],
      comments: [
        { id: 1, content: 'تعليق اختباري', user: { id: 1, name: 'أحمد' }, created_at: '2025-01-15T10:00:00Z', attachments: [] },
      ],
      time_indicator: { status: 'normal', has_due_date: true, days_remaining: 10 },
    }),
    create: vi.fn().mockResolvedValue({ id: 4, title: 'مهمة فرعية جديدة', status: 'todo' }),
...
// Helper function to get mock task data
const getMockTask = () => ({
  id: 1,
  title: 'مهمة اختبارية',
  description: 'وصف المهمة الاختبارية',
...
vi.mock('@widgets/task', () => ({
  TaskTimeIndicator: () => <div data-testid="time-indicator">Time Indicator</div>,
  TaskActivityLog: () => <div data-testid="activity-log">Activity Log</div>,
  TaskDetailsPanel: () => <div data-testid="details-panel">Details Panel</div>,
}));

// Mock task page components
vi.mock('@pages/tasks/components', () => ({
  CommentsSection: ({ comments }: { comments: any[] }) => (
    <div data-testid="comments-section">
      {comments?.map((c: any) => <div key={c.id}>{c.content}</div>)}
    </div>
  ),
  SubtaskCard: ({ subtask }: { subtask: any }) => (
    <div data-testid={`subtask-${subtask.id}`}>{subtask.title}</div>
  ),
  DetailsTab: ({ task, isOverdue }: any) => {
    ...
    return (
      <div data-testid="details-tab">
        <div><span>الوصف</span></div>
        <p>{task?.description || 'لا يوجد وصف لهذه المهمة'}</p>
        <div><span>معلومات المهمة</span></div>
        <div><span>المكلف</span><span>{task?.assignee?.name || 'غير محدد'}</span></div>
        <div><span>الأولوية</span><span>{getPriorityLabel(task?.priority)}</span></div>
        <div><span>تاريخ الاستحقاق</span><span>25/01/2025</span></div>
        <div><span>المنشئ</span><span>{task?.creator?.name || '-'}</span></div>
      </div>
    );
  },
...
describe('TaskViewModal Component', () => {
  ...
  it('renders task title', async () => {
    render(<TaskViewModal {...defaultProps} />);
  ...
  it('renders project badge with code and name', async () => {
    render(<TaskViewModal {...defaultProps} />);
  ...
  it('calls onClose when close button clicked', async () => {
    ...
    // Modal close button has aria-label "إغلاق" via i18n
    await userEvent.click(screen.getByRole('button', { name: /إغلاق/i }));
...
describe('TaskViewModal Tabs', () => {
  ...
  it('renders subtasks tab with count', async () => {
    render(<TaskViewModal {...defaultProps} />);
  ...
  it('renders comments tab with count', async () => {
    render(<TaskViewModal {...defaultProps} />);
...
describe('TaskViewModal Details Tab', () => {
  const defaultProps = {
    taskId: 1,
    isOpen: true,
    onClose: vi.fn(),
  ...
  it('renders description section', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('وصف المهمة الاختبارية')).toBeInTheDocument();
    });
  });

  it('renders task info section', async () => {
    render(<TaskViewModal {...defaultProps} />);
  ...
  it('renders creator name', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('المنشئ')).toBeInTheDocument();
...
describe('TaskViewModal Loading State', () => {
  ...
  it('shows loading skeleton when loading', async () => {
    ...
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve(getMockTask()), 100))
    );

    render(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);

    // Skeleton component exposes data-testid="skeleton"
...
describe('TaskViewModal Error State', () => {
  ...
  it('shows error state when API fails', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
...
describe('TaskViewModal Closed State', () => {
  it('does not render content when modal is closed', async () => {
    render(<TaskViewModal taskId={1} isOpen={false} onClose={vi.fn()} />);

    // Should not render task title when closed
    expect(screen.queryByText('مهمة اختبارية')).not.toBeInTheDocument();
  });

  it('renders empty when taskId is null', async () => {
    render(<TaskViewModal taskId={null} isOpen={true} onClose={vi.fn()} />);

    // Should not render task details without taskId
...
describe('TaskViewModal No Description', () => {
  ...
  it('shows placeholder when no description', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ...getMockTask(),
      description: null,
    });
...
describe('TaskViewModal Overdue Task', () => {
  ...
  it('shows overdue badge when task is overdue', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ...getMockTask(),
...
describe('TaskViewModal Child Task', () => {
  ...
  it('hides subtasks tab when task is a child task', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ...getMockTask(),
      parent: { id: 5, title: 'المهمة الأصلية' },
    });

    render(<TaskViewModal taskId={1} isOpen={true} onClose={vi.fn()} />);

    await waitFor(() => {
      expect(screen.queryByText(/المهام الفرعية/)).not.toBeInTheDocument();
    });
  });
});
...
describe('TaskViewModal No Assignee', () => {
  ...
  it('shows placeholder when no assignee', async () => {
    const { tasksApi } = await import('@entities/task');
    (tasksApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ...getMockTask(),
      assignee: null,
