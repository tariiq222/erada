...
vi.mock('@shared/lib/utils', async (importOriginal) => ({
  ...
  formatDateShort: vi.fn().mockReturnValue('15 يناير'),
}));
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconGripVertical: () => <span data-testid="grip-icon">Grip</span>,
  IconGitBranch: () => <span data-testid="branch-icon">Branch</span>,
  IconChevronDown: () => <span data-testid="chevron-icon">Chevron</span>,
  IconLoader: ({ className }: { className?: string }) => <span data-testid="loader-icon" className={className}>Loader</span>,
...
// Mock renderTimeIndicator
vi.mock('@pages/projects/components/sections/tasks/renderTimeIndicator', () => ({
  renderTimeIndicator: vi.fn().mockReturnValue(<span data-testid="time-indicator">Time</span>),
}));

// Mock SubtasksAccordion
vi.mock('@pages/projects/components/sections/tasks/SubtasksAccordion', () => ({
  default: ({ subtasks, onUpdateStatus }: any) => (
    <div data-testid="subtasks-accordion">
      {subtasks.map((st: any) => (
        <div key={st.id} data-testid={`subtask-${st.id}`}>
          {st.title}
          <button onClick={(e) => onUpdateStatus(e, st.id, 'completed')}>Complete</button>
        </div>
      ))}
    </div>
  ),
...
import TaskCard from '@pages/projects/components/sections/tasks/TaskCard';
...
const mockTask = {
  ...
  due_date: '2025-01-20',
  subtasks_count: 2,
  milestone: { id: 1, name: 'مرحلة التصميم' },
...
describe('TaskCard Drag Events', () => {
  ...
  it('calls onDragStart on drag', () => {
    ...
    expect(defaultProps.onDragStart).toHaveBeenCalledWith(expect.any(Object), mockTask);
  });
...
describe('TaskCard Click Events', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TaskCard Milestone', () => {
  ...
  it('shows milestone icon', () => {
    render(<TaskCard {...defaultProps} />);
    // The milestone row now renders an IconFlag (mocked as flag-icon) next to
    // the milestone name, instead of a dedicated milestone icon.
    const milestoneName = screen.getByText('مرحلة التصميم');
    const flagIcon = milestoneName.parentElement?.querySelector('[data-testid="flag-icon"]');
    expect(flagIcon).toBeInTheDocument();
  });

  it('hides milestone when not present', () => {
    const taskWithoutMilestone = { ...mockTask, milestone: null };
    render(<TaskCard {...defaultProps} task={taskWithoutMilestone as any} />);
    expect(screen.queryByText('مرحلة التصميم')).not.toBeInTheDocument();
  });
});

describe('TaskCard Time Indicator', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows time indicator when has due date', () => {
    render(<TaskCard {...defaultProps} />);
    expect(screen.getByTestId('time-indicator')).toBeInTheDocument();
  });

  it('hides time indicator when no due date', () => {
    const taskNoDueDate = {
      ...mockTask,
      time_indicator: { ...mockTask.time_indicator, has_due_date: false },
    };
    render(<TaskCard {...defaultProps} task={taskNoDueDate as any} />);
    expect(screen.queryByTestId('time-indicator')).not.toBeInTheDocument();
  });
});

describe('TaskCard Dragging State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('applies opacity when being dragged', () => {
    const { container } = render(
      <TaskCard {...defaultProps} draggedTask={mockTask as any} />
    );
    const card = container.firstChild as HTMLElement;
    expect(card.className).toContain('opacity-50');
  });

  it('does not apply opacity when not being dragged', () => {
    const { container } = render(<TaskCard {...defaultProps} />);
    const card = container.firstChild as HTMLElement;
...
describe('TaskCard Subtasks', () => {
  ...
  it('shows subtasks accordion when expanded', () => {
    const subtasksData = {
      1: [
        { id: 101, title: 'فرعية 1', status: 'in_progress' },
        { id: 102, title: 'فرعية 2', status: 'completed' },
      ],
    };
    ...
    expect(screen.getByTestId('subtasks-accordion')).toBeInTheDocument();
  });

  it('hides subtasks accordion when collapsed', () => {
    render(<TaskCard {...defaultProps} expandedSubtasks={null} />);
    expect(screen.queryByTestId('subtasks-accordion')).not.toBeInTheDocument();
  });
});

describe('TaskCard Without Milestone', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders without milestone', () => {
    const taskNoMilestone = { ...mockTask, milestone: null };
    render(<TaskCard {...defaultProps} task={taskNoMilestone as any} />);
    expect(screen.getByText('مهمة اختبارية')).toBeInTheDocument();
    expect(screen.queryByTestId('milestone-icon')).not.toBeInTheDocument();
  });
});

describe('TaskCard Loading Subtasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows loader when loading subtasks', () => {
    render(<TaskCard {...defaultProps} loadingSubtasks={1} />);
    expect(screen.getByTestId('loader-icon')).toBeInTheDocument();
  });

  it('hides loader when not loading', () => {
    render(<TaskCard {...defaultProps} loadingSubtasks={null} />);
