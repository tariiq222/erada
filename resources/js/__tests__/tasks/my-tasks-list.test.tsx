  ...
  unifiedTasksApi: {
    getMyTasks: vi.fn().mockResolvedValue({
      data: [
        {
          id: 1,
          title: 'مهمة اختبارية 1',
          status: 'in_progress',
          priority: 'high',
          type: 'project',
          due_date: '2025-01-20',
          project: { id: 1, name: 'مشروع اختباري' },
        },
        {
          id: 2,
          title: 'مهمة شخصية',
    ...
    getStats: vi.fn().mockResolvedValue({
      ...
      by_type: { project: 5, personal: 3, department: 2 },
      overdue: 2,
      due_today: 1,
      due_this_week: 4,
    }),
...
vi.mock('@pages/tasks/TaskViewModal', () => ({
  default: ({ taskId, isOpen, onClose }: { taskId: number | null; isOpen: boolean; onClose: () => void }) =>
    isOpen ? (
...
describe('MyTasksList Component', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('new task button links to create page', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      const link = screen.getByText('مهمة جديدة').closest('a');
      expect(link).toHaveAttribute('href', '/tasks/create');
    });
  });
});

describe('MyTasksList Stats Cards', () => {
  ...
  it('renders in_progress stats label', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // قيد التنفيذ appears in stats and filter options
      const elements = screen.getAllByText('قيد التنفيذ');
      expect(elements.length).toBeGreaterThan(0);
    });
  });

  it('renders overdue count in stats', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // The stats card shows overdue count with number 2
      const statCards = document.querySelectorAll('.grid.grid-cols-2 > div');
      expect(statCards.length).toBe(4);
    });
  });

  it('renders completed stats label', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // مكتملة appears in stats and filter options
      const elements = screen.getAllByText(/مكتمل/);
      expect(elements.length).toBeGreaterThan(0);
    });
  });
});
...
describe('MyTasksList Filters', () => {
  ...
  it('renders type filter with options', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // Filters use the custom Select (button-based listbox), not native <select>.
      const triggers = document.querySelectorAll('button[aria-haspopup="listbox"]');
      expect(triggers.length).toBeGreaterThan(0);
      // The type filter's default option label is shown in its trigger.
  ...
  it('allows typing in search field', async () => {
    ...
    await userEvent.type(searchInput, 'مهمة');

    expect(searchInput).toHaveValue('مهمة');
  });
});

describe('MyTasksList View Mode', () => {
  ...
  it('defaults to list view', async () => {
    ...
    await waitFor(() => {
      // List view shows tasks in a divided list
      const taskItem = screen.getByText('مهمة اختبارية 1');
      expect(taskItem).toBeInTheDocument();
    });
  });

  it('can switch to cards view', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      expect(screen.getByText('مهمة اختبارية 1')).toBeInTheDocument();
    });

    // Find and click the cards view button (second button after list)
    const viewButtons = screen.getByText('طريقة العرض:').parentElement?.querySelectorAll('button');
...
describe('MyTasksList Task Items', () => {
  it('renders task titles', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      expect(screen.getByText('مهمة اختبارية 1')).toBeInTheDocument();
      expect(screen.getAllByText('مهمة شخصية')[0]).toBeInTheDocument();
      expect(screen.getAllByText('مهمة إدارية')[0]).toBeInTheDocument();
    });
  });

  it('renders project name when task has project', async () => {
    render(<MyTasksList />);
  ...
  it('renders task due dates', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // Due dates are formatted in Arabic
...
describe('MyTasksList Modal', () => {
  it('opens modal when clicking on task', async () => {
    render(<MyTasksList />);
    ...
    // Click on the task
    await userEvent.click(screen.getByText('مهمة اختبارية 1'));
...
describe('MyTasksList Task Types', () => {
  it('displays different icons for task types', async () => {
    render(<MyTasksList />);

    await waitFor(() => {
      // Tasks are loaded - check if type icons are rendered
      const taskItems = document.querySelectorAll('.p-2.rounded-lg');
      expect(taskItems.length).toBeGreaterThan(0);
    });
  });
});

describe('MyTasksList Search Submission', () => {
  it('updates URL params when search is submitted', async () => {
    render(<MyTasksList />);
    ...
    const searchInput = screen.getByPlaceholderText('بحث في المهام...');
    await userEvent.type(searchInput, 'اختبار');

    const applyButton = screen.getByText('تطبيق');
