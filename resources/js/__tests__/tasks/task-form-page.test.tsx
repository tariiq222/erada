...
vi.mock('@pages/tasks/form', () => ({
  useTaskForm: () => ({
    ...
    projects: [
      { id: 1, name: 'مشروع اختبار', code: 'PRJ-001' },
    ],
    milestones: [
  ...
  MilestoneModal: ({ isOpen }: { isOpen: boolean }) =>
    isOpen ? <div data-testid="milestone-modal">Milestone Modal</div> : null,
  UserModal: ({ isOpen }: { isOpen: boolean }) =>
    isOpen ? <div data-testid="user-modal">User Modal</div> : null,
  ProjectMilestoneSection: ({ formData, onChange }: any) => (
    <div data-testid="project-milestone-section">
      <select data-testid="project-select" value={formData.project_id} onChange={(e) => onChange('project_id', e.target.value)}>
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, type, loading, leftIcon, variant }: any) => (
    <button onClick={onClick} type={type} data-loading={loading} data-variant={variant}>
      {leftIcon}
      {children}
    </button>
  ),
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconClipboardCheck: () => <span data-testid="clipboard-check">ClipboardCheck</span>,
...
describe('TaskForm Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders create form title', () => {
    render(<TaskForm />);
    // Found in both breadcrumb and header
    const titles = screen.getAllByText('إنشاء مهمة جديدة');
    expect(titles.length).toBeGreaterThan(0);
  });

  it('renders breadcrumb with tasks link', () => {
    render(<TaskForm />);
...
describe('TaskForm Header', () => {
  ...
  it('shows description text', () => {
    render(<TaskForm />);
    expect(screen.getByText('أدخل بيانات المهمة الجديدة')).toBeInTheDocument();
  });

  it('renders header icon', () => {
    render(<TaskForm />);
    // The header icon is now IconClipboardCheck (rendered via PageHeader).
    const icons = screen.getAllByTestId('clipboard-check');
    expect(icons.length).toBeGreaterThan(0);
  });
});

describe('TaskForm Task Type Section', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows task type section', () => {
    render(<TaskForm />);
    expect(screen.getByTestId('task-type-section')).toBeInTheDocument();
  });

  it('shows type select', () => {
    render(<TaskForm />);
    expect(screen.getByTestId('type-select')).toBeInTheDocument();
  });
});

describe('TaskForm Project Section', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows project milestone section for project type', () => {
    render(<TaskForm />);
    expect(screen.getByTestId('project-milestone-section')).toBeInTheDocument();
  });

  it('shows project select', () => {
    render(<TaskForm />);
...
describe('TaskForm Actions', () => {
  ...
  it('cancel button links to tasks page', () => {
    render(<TaskForm />);
    const cancelButton = screen.getByText('إلغاء').closest('a');
    expect(cancelButton).toHaveAttribute('href', '/tasks');
  });
...
describe('TaskForm Status Section', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows current status', () => {
    render(<TaskForm />);
    expect(screen.getByText(/الحالة: todo/)).toBeInTheDocument();
  });

  it('shows edit mode as no', () => {
    render(<TaskForm />);
    expect(screen.getByText(/وضع التحرير: لا/)).toBeInTheDocument();
  });
});
