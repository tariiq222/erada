...
vi.mock('@shared/ui', () => ({
  ...
  FilterRow: ({ children, onClear, clearLabel }: { children: React.ReactNode; onClear?: () => void; clearLabel?: string }) => (
    <div data-testid="filter-row">
      {children}
      {onClear && <button onClick={onClear}>{clearLabel}</button>}
    </div>
  ),
  FilterField: ({ label, children }: { label: string; children: React.ReactNode }) => (
    <label>
      <span>{label}</span>
      {React.isValidElement(children)
        ? React.cloneElement(children as React.ReactElement<any>, { _fieldLabel: label })
        : children}
    </label>
  ),
  Select: ({ value, onChange, options, _fieldLabel }: {
    value: string;
    ...
    <div>
      <select
        data-testid={`select-${_fieldLabel ?? ''}`}
        value={value}
        onChange={onChange}
      >
        {options.map((opt) => (
...
import TasksFilters from '@pages/projects/components/sections/tasks/TasksFilters';

const mockTasks = [
  { id: 1, title: 'مهمة 1', status: 'todo', priority: 'high', assignee: { id: 1, name: 'أحمد' } },
  { id: 2, title: 'مهمة 2', status: 'in_progress', priority: 'normal', assignee: { id: 2, name: 'محمد' } },
  { id: 3, title: 'مهمة 3', status: 'completed', priority: 'low', assignee: null },
  { id: 4, title: 'مهمة 4', status: 'in_review', priority: 'urgent', assignee: { id: 1, name: 'أحمد' } },
];
...
describe('TasksFilters Basic', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
    priorityFilter: 'all',
    assigneeFilter: 'all',
  ...
  it('renders filter row component', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByTestId('filter-row')).toBeInTheDocument();
  });
});

describe('TasksFilters Status Options', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
  ...
  it('shows all statuses option with count', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('كل الحالات (4)')).toBeInTheDocument();
  });

  it('shows todo option with count', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('للتنفيذ (1)')).toBeInTheDocument();
  });

  it('shows in_progress option with count', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('قيد التنفيذ (1)')).toBeInTheDocument();
  });

  it('shows in_review option with count', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('قيد المراجعة (1)')).toBeInTheDocument();
  });

  it('shows completed option with count', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('مكتمل (1)')).toBeInTheDocument();
  });
});

describe('TasksFilters Priority Options', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
...
describe('TasksFilters Assignee Options', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
  ...
  it('shows all members option', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('كل الأعضاء')).toBeInTheDocument();
  });
...
describe('TasksFilters Change Handlers', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
  ...
  it('calls onStatusFilterChange when status changed', () => {
    render(<TasksFilters {...defaultProps} />);
    const select = screen.getByTestId('select-الحالة');
    fireEvent.change(select, { target: { value: 'in_progress' } });
    expect(defaultProps.onStatusFilterChange).toHaveBeenCalledWith('in_progress');
  });

  it('calls onPriorityFilterChange when priority changed', () => {
    render(<TasksFilters {...defaultProps} />);
    const select = screen.getByTestId('select-الأولوية');
    fireEvent.change(select, { target: { value: 'high' } });
    expect(defaultProps.onPriorityFilterChange).toHaveBeenCalledWith('high');
  });

  it('calls onAssigneeFilterChange when assignee changed', () => {
    render(<TasksFilters {...defaultProps} />);
    const select = screen.getByTestId('select-المسؤول');
    fireEvent.change(select, { target: { value: '1' } });
    expect(defaultProps.onAssigneeFilterChange).toHaveBeenCalledWith('1');
  });
});

describe('TasksFilters Active Filters', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks.slice(0, 2) as any,
  ...
  it('shows reset controls when filters active', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('إعادة تعيين الفلاتر')).toBeInTheDocument();
  });
  ...
  it('calls onResetFilters when reset clicked', () => {
    render(<TasksFilters {...defaultProps} />);
    fireEvent.click(screen.getByText('إعادة تعيين الفلاتر'));
    expect(defaultProps.onResetFilters).toHaveBeenCalled();
  });

  it('shows x icon for reset', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.getByText('إعادة تعيين الفلاتر')).toBeInTheDocument();
  });
});

describe('TasksFilters No Active Filters', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
  ...
  it('does not show reset button when no filters', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.queryByText('إعادة تعيين الفلاتر')).not.toBeInTheDocument();
  });

  it('does not show filter count when no filters', () => {
    render(<TasksFilters {...defaultProps} />);
    expect(screen.queryByText(/عرض .* من .* مهمة/)).not.toBeInTheDocument();
  });
});

describe('TasksFilters Empty Assignees', () => {
  const defaultProps = {
    tasks: mockTasks as any,
    filteredTasks: mockTasks as any,
    statusFilter: 'all',
  ...
  it('shows only all members option when no assignees', () => {
    render(<TasksFilters {...defaultProps} />);
    const select = screen.getByTestId('select-المسؤول');
