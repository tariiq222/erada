...
vi.mock('@shared/lib/utils', () => ({
  formatDate: (date: string | Date) => date ? '25/01/2025' : '-',
...
vi.mock('@shared/ui', () => ({
  ...
  Table: ({ children, hoverable }: { children: React.ReactNode; hoverable?: boolean }) => (
    <table data-hoverable={hoverable}>{children}</table>
  ),
...
import type { Task, PaginationState } from '@pages/tasks/list/types';
...
describe('TableView Component', () => {
  ...
  it('renders project code with link', () => {
    render(<TableView {...defaultProps} />);
    const projectLink = screen.getByText('PRJ-001');
  ...
  it('calls onOpenTaskModal when clicking task title', () => {
    render(<TableView {...defaultProps} />);
    fireEvent.click(screen.getByText('مهمة اختبارية'));
    expect(defaultProps.onOpenTaskModal).toHaveBeenCalledWith(1);
  });

  it('calls onOpenTaskModal when clicking view button', () => {
    render(<TableView {...defaultProps} />);
  ...
  it('does not render pagination when lastPage is 1', () => {
    render(<TableView {...defaultProps} pagination={{ ...mockPagination, lastPage: 1 }} />);
  ...
  it('renders milestone name when present', () => {
    render(<TableView {...defaultProps} />);
    expect(screen.getByText('المرحلة الأولى')).toBeInTheDocument();
  });

  it('does not render milestone when absent', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ milestone: null })]} />);
    expect(screen.queryByText('المرحلة الأولى')).not.toBeInTheDocument();
  });

  it('renders dash for missing project', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ project: null })]} />);
    expect(screen.getAllByText('-').length).toBeGreaterThan(0);
  });

  it('renders dash for missing assignee', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ assignee: null })]} />);
    expect(screen.getAllByText('-').length).toBeGreaterThan(0);
  });

  it('renders dash for missing due date', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ due_date: null })]} />);
    expect(screen.getAllByText('-').length).toBeGreaterThan(0);
  });

  it('applies overdue styling when task is overdue', () => {
    render(
      <TableView {...defaultProps} isOverdue={vi.fn().mockReturnValue(true)} />
    );
    // The due-date cell uses the danger token when isOverdue returns true
    const taskTitle = screen.getByText('مهمة اختبارية');
    const row = taskTitle.closest('tr') as HTMLElement;
    expect(row.innerHTML).toContain('--status-danger');
  });

  it('renders multiple tasks', () => {
    const tasks = [
      createMockTask({ id: 1, title: 'المهمة الأولى' }),
      createMockTask({ id: 2, title: 'المهمة الثانية' }),
      createMockTask({ id: 3, title: 'المهمة الثالثة' }),
    ];
    render(<TableView {...defaultProps} tasks={tasks} />);
    expect(screen.getByText('المهمة الأولى')).toBeInTheDocument();
    expect(screen.getByText('المهمة الثانية')).toBeInTheDocument();
    expect(screen.getByText('المهمة الثالثة')).toBeInTheDocument();
  });

  it('edit link points to correct URL', () => {
    render(<TableView {...defaultProps} />);
    const editLink = screen.getByTitle('تعديل').closest('a');
    expect(editLink).toHaveAttribute('href', '/tasks/1/edit');
  });
});

describe('TableView Status Icons', () => {
  const defaultProps = {
    tasks: [createMockTask()],
    pagination: mockPagination,
    onOpenTaskModal: vi.fn(),
  ...
  it('renders icon for todo status', () => {
    render(
      <TableView {...defaultProps} tasks={[createMockTask({ status: 'todo' })]} />
    );
    expect(screen.getByText('مهمة اختبارية').closest('tr')?.querySelector('svg')).toBeInTheDocument();
  });

  it('renders icon for in_progress status', () => {
    render(
      <TableView {...defaultProps} tasks={[createMockTask({ status: 'in_progress' })]} />
    );
    expect(screen.getByText('مهمة اختبارية').closest('tr')?.querySelector('svg')).toBeInTheDocument();
  });
  ...
  it('renders icon for completed status', () => {
    render(
      <TableView {...defaultProps} tasks={[createMockTask({ status: 'completed' })]} />
    );
    expect(screen.getByText('مهمة اختبارية').closest('tr')?.querySelector('svg')).toBeInTheDocument();
  });
});
...
describe('CardsView Component', () => {
  ...
  it('renders cards grid', () => {
    ...
    // CardsView renders a layout grid container (<div className="grid ...">) wrapping each task card.
  ...
  it('renders project link with code and name', () => {
    render(<CardsView {...defaultProps} />);
    expect(screen.getByText('PRJ-001 - مشروع اختباري')).toBeInTheDocument();
  });
  ...
  it('calls onOpenTaskModal when clicking view button', () => {
    render(<CardsView {...defaultProps} />);
    // The card has multiple buttons; the first button is the "view" trigger in CardView
  ...
  it('does not render pagination when lastPage is 1', () => {
    render(<CardsView {...defaultProps} pagination={{ ...mockPagination, lastPage: 1 }} />);
    expect(screen.queryByTestId('pagination')).not.toBeInTheDocument();
  });
  ...
  it('does not render project section when project is null', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ project: null })]} />);
    expect(screen.queryByText('PRJ-001')).not.toBeInTheDocument();
  });

  it('does not render due date when null', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ due_date: null })]} />);
    expect(screen.queryByText('25/01/2025')).not.toBeInTheDocument();
  });

  it('does not render assignee avatar when null', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ assignee: null })]} />);
    expect(screen.queryByText('أ')).not.toBeInTheDocument();
  });

  it('applies overdue styling when task is overdue', () => {
    render(
      <CardsView {...defaultProps} isOverdue={vi.fn().mockReturnValue(true)} />
    );
    const card = screen.getByText('مهمة اختبارية').closest('[data-testid="card"]') as HTMLElement;
    expect(card.innerHTML).toContain('--status-danger');
  });

  it('renders multiple task cards', () => {
    const tasks = [
      createMockTask({ id: 1, title: 'البطاقة الأولى' }),
      createMockTask({ id: 2, title: 'البطاقة الثانية' }),
    ];
    render(<CardsView {...defaultProps} tasks={tasks} />);
    expect(screen.getByText('البطاقة الأولى')).toBeInTheDocument();
    expect(screen.getByText('البطاقة الثانية')).toBeInTheDocument();
  });
...
describe('CardsView Status Icons', () => {
  const defaultProps = {
    tasks: [createMockTask()],
    pagination: mockPagination,
    onOpenTaskModal: vi.fn(),
  ...
  it('renders icon for different statuses', () => {
    const statuses = ['todo', 'in_progress', 'completed', 'on_hold'];

    statuses.forEach(status => {
      const { unmount } = render(
        <CardsView {...defaultProps} tasks={[createMockTask({ status })]} />
      );
      // The status icon is the SVG inside the badge next to the task title (h3)
      const heading = screen.getByRole('heading', { name: 'مهمة اختبارية' });
...
describe('CardsView Priority Colors', () => {
  const defaultProps = {
    tasks: [createMockTask()],
    pagination: mockPagination,
    onOpenTaskModal: vi.fn(),
  ...
  it('renders low priority', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ priority: 'low' })]} />);
    expect(screen.getByText('منخفضة')).toBeInTheDocument();
  });

  it('renders medium priority', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ priority: 'medium' })]} />);
    expect(screen.getByText('متوسطة')).toBeInTheDocument();
  });

  it('renders high priority', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ priority: 'high' })]} />);
    expect(screen.getByText('عالية')).toBeInTheDocument();
  });

  it('renders urgent priority', () => {
    render(<CardsView {...defaultProps} tasks={[createMockTask({ priority: 'urgent' })]} />);
    expect(screen.getByText('عاجلة')).toBeInTheDocument();
  });
});

describe('TableView Priority Colors', () => {
  const defaultProps = {
    tasks: [createMockTask()],
    pagination: mockPagination,
    onOpenTaskModal: vi.fn(),
  ...
  it('renders low priority', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ priority: 'low' })]} />);
    expect(screen.getByText('منخفضة')).toBeInTheDocument();
  });

  it('renders medium priority', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ priority: 'medium' })]} />);
    expect(screen.getByText('متوسطة')).toBeInTheDocument();
  });

  it('renders high priority', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ priority: 'high' })]} />);
    expect(screen.getByText('عالية')).toBeInTheDocument();
  });

  it('renders urgent priority', () => {
    render(<TableView {...defaultProps} tasks={[createMockTask({ priority: 'urgent' })]} />);
    expect(screen.getByText('عاجلة')).toBeInTheDocument();
  });
});
