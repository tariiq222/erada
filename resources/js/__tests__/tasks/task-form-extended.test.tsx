...
// Mock the form hook with loading state
const mockUseTaskForm = vi.fn();
vi.mock('@pages/tasks/form', () => ({
  useTaskForm: () => mockUseTaskForm(),
  MilestoneModal: ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) =>
    isOpen ? <div data-testid="milestone-modal"><button onClick={onClose}>إغلاق</button></div> : null,
  UserModal: ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) =>
    isOpen ? <div data-testid="user-modal"><button onClick={onClose}>إغلاق</button></div> : null,
  ProjectMilestoneSection: ({ formData }: any) => (
    <div data-testid="project-milestone-section">
      <span>المشروع: {formData.project_id || 'لا يوجد'}</span>
    </div>
  ),
  TaskDetailsSection: ({ formData, onChange }: any) => (
    <div data-testid="task-details-section">
      <input
        type="text"
  ...
  StatusScheduleSection: ({ formData, isEditMode }: any) => (
    <div data-testid="status-schedule-section">
      <span>الحالة: {formData.status}</span>
      <span>وضع التحرير: {isEditMode ? 'نعم' : 'لا'}</span>
    </div>
  ),
  TaskTypeSection: ({ formData, onChange }: any) => (
    <div data-testid="task-type-section">
      <select data-testid="type-select" value={formData.type} onChange={(e) => onChange('type', e.target.value)}>
        <option value="project">مهمة مشروع</option>
        <option value="personal">مهمة شخصية</option>
        <option value="operational">مهمة تشغيلية</option>
      </select>
    </div>
  ),
}));
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, type, loading, leftIcon, variant, size }: any) => (
    <button onClick={onClick} type={type} data-loading={loading} data-variant={variant} data-size={size} disabled={loading}>
      {leftIcon}
      {children}
    </button>
  ),
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconArrowRight: () => <span data-testid="arrow-right">←</span>,
  IconDeviceFloppy: () => <span data-testid="save-icon">Save</span>,
  IconSquareCheck: () => <span data-testid="check-square">CheckSquare</span>,
...
const defaultFormState = {
  ...
  projects: [{ id: 1, name: 'مشروع اختبار', code: 'PRJ-001' }],
  milestones: [{ id: 1, name: 'المرحلة الأولى' }],
  parentTasks: [],
  users: [{ id: 1, name: 'أحمد محمد', email: 'ahmed@example.com' }],
  departments: [{ id: 1, name: 'تقنية المعلومات' }],
  selectedUser: { id: 1, name: 'أحمد محمد' },
  selectedOwner: null,
  ...
  dateConstraints: { minDate: '', maxDate: '' },
  handleChange: vi.fn(),
  handleSubmit: vi.fn((e) => e.preventDefault()),
  showMilestoneModal: false,
  setShowMilestoneModal: vi.fn(),
  milestoneFormData: {},
  milestoneErrors: {},
  isSavingMilestone: false,
  openMilestoneModal: vi.fn(),
  handleMilestoneChange: vi.fn(),
  handleSaveMilestone: vi.fn(),
  showUserModal: false,
  setShowUserModal: vi.fn(),
  userSearchQuery: '',
  setUserSearchQuery: vi.fn(),
  showNewUserForm: false,
  setShowNewUserForm: vi.fn(),
  newUserFormData: {},
  userErrors: {},
  isSavingUser: false,
  openUserModal: vi.fn(),
  handleSelectUser: vi.fn(),
  handleNewUserChange: vi.fn(),
  handleSaveNewUser: vi.fn(),
};
...
describe('TaskForm Loading State', () => {
  ...
  it('does not show form when loading', () => {
    ...
    render(<TaskForm />);
    expect(screen.queryByTestId('task-details-section')).not.toBeInTheDocument();
  });
});
...
describe('TaskForm Edit Mode', () => {
  ...
  it('shows edit mode title', () => {
    ...
    render(<TaskForm />);
    const editTitles = screen.getAllByText('تعديل المهمة');
  ...
  it('shows update description', () => {
    ...
    render(<TaskForm />);
    expect(screen.getByText('تعديل بيانات المهمة')).toBeInTheDocument();
  });
...
describe('TaskForm Create Mode', () => {
  ...
  it('shows create mode title', () => {
    mockUseTaskForm.mockReturnValue(defaultFormState);

    render(<TaskForm />);
    const createTitles = screen.getAllByText('إنشاء مهمة جديدة');
    expect(createTitles.length).toBeGreaterThan(0);
  });
...
describe('TaskForm Project Context', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('hides task type section in project context', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
    ...
    render(<TaskForm />);
    expect(screen.queryByTestId('task-type-section')).not.toBeInTheDocument();
  });

  it('shows project milestone section in project context', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      isProjectContext: true,
    });
    ...
    render(<TaskForm />);
    expect(screen.getByTestId('project-milestone-section')).toBeInTheDocument();
  });
});

describe('TaskForm Non-Project Types', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows task type section for non-project context', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      isProjectContext: false,
    });
    ...
    render(<TaskForm />);
    expect(screen.getByTestId('task-type-section')).toBeInTheDocument();
  });

  it('shows assignee section for personal tasks', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      formData: { ...defaultFormState.formData, type: 'personal' },
      isProjectContext: false,
    });
  ...
  it('hides project section for personal tasks', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      formData: { ...defaultFormState.formData, type: 'personal' },
      isProjectContext: false,
    ...
    render(<TaskForm />);
    expect(screen.queryByTestId('project-milestone-section')).not.toBeInTheDocument();
  });
});
...
describe('TaskForm Form Submission', () => {
  ...
  it('disables submit button when saving', () => {
    ...
    render(<TaskForm />);
    const submitButton = screen.getByText('إنشاء مهمة').closest('button');
    expect(submitButton).toBeDisabled();
  });
});

describe('TaskForm Modals', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders milestone modal when open', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
  ...
  it('renders user modal when open', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      showUserModal: true,
    });
...
describe('TaskForm Navigation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows breadcrumb with tasks link', () => {
    mockUseTaskForm.mockReturnValue(defaultFormState);

    render(<TaskForm />);
    const tasksLink = screen.getByText('المهام');
    expect(tasksLink.closest('a')).toHaveAttribute('href', '/tasks');
  });

  it('shows cancel button linking to tasks', () => {
    mockUseTaskForm.mockReturnValue(defaultFormState);

    render(<TaskForm />);
    const cancelLink = screen.getByText('إلغاء').closest('a');
    expect(cancelLink).toHaveAttribute('href', '/tasks');
  });
});

describe('TaskForm Sections', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows all required sections', () => {
    ...
    expect(screen.getByTestId('status-schedule-section')).toBeInTheDocument();
  });

  it('shows task type section for project type', () => {
    mockUseTaskForm.mockReturnValue({
      ...defaultFormState,
      formData: { ...defaultFormState.formData, type: 'project' },
    });
