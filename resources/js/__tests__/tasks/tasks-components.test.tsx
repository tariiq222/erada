...
const mockTask = {
  ...
  project: { id: 1, code: 'PRJ-001', name: 'مشروع اختبار' },
  milestone: { id: 1, name: 'المرحلة الأولى' },
  assignee: { id: 1, name: 'أحمد محمد', email: 'ahmed@test.com' },
...
const mockTasks = [
  ...
  },
  {
    ...
    project: { id: 2, code: 'PRJ-002', name: 'مشروع آخر' },
    milestone: { id: 2, name: 'المرحلة الثانية' },
    assignee: { id: 2, name: 'سارة أحمد', email: 'sara@test.com' },
    creator: { id: 1, name: 'أحمد محمد' },
    parent: { id: 1, title: 'مهمة اختبار' },
    subtasks_count: 0,
    completed_subtasks_count: 0,
  },
];
...
describe('Task Status Constants', () => {
  ...
  describe('statusVariants', () => {
    it('todo is default', () => {
      expect(statusVariants.todo).toBe('default');
    });

    it('in_progress is accent', () => {
      expect(statusVariants.in_progress).toBe('accent');
    });

    it('in_review is warning', () => {
      expect(statusVariants.in_review).toBe('warning');
    });
  ...
  describe('priorityLabels', () => {
    it('has label for low', () => {
      expect(priorityLabels.low).toBe('priority.low');
    });

    it('has label for medium', () => {
      expect(priorityLabels.medium).toBe('priority.medium');
    });

    it('has label for high', () => {
      expect(priorityLabels.high).toBe('priority.high');
    });

    it('has label for urgent', () => {
      expect(priorityLabels.urgent).toBe('priority.urgent');
    });
  });

  describe('statusIcons', () => {
    it('has icon for todo', () => {
      expect(statusIcons.todo).toBeDefined();
    });

    it('has icon for in_progress', () => {
      expect(statusIcons.in_progress).toBeDefined();
    });

    it('has icon for completed', () => {
      expect(statusIcons.completed).toBeDefined();
    });
  });

  describe('statusColors', () => {
    it('todo has muted surface color', () => {
      expect(statusColors.todo).toContain('var(--surface-muted)');
    });

    it('in_progress has accent color', () => {
      expect(statusColors.in_progress).toContain('var(--accent-default)');
    });

    it('completed has success color', () => {
      expect(statusColors.completed).toContain('var(--status-success-text)');
    });
  });

  describe('priorityColors', () => {
    it('low is tertiary text', () => {
      expect(priorityColors.low).toContain('var(--text-tertiary)');
    });

    it('medium is accent', () => {
      expect(priorityColors.medium).toContain('var(--accent-default)');
    });

    it('high is warning', () => {
      expect(priorityColors.high).toContain('var(--status-warning)');
    });

    it('urgent is danger', () => {
      expect(priorityColors.urgent).toContain('var(--status-danger)');
    });
  });

  describe('kanbanColumnStyles', () => {
    it('todo has styles', () => {
      expect(kanbanColumnStyles.todo).toBeDefined();
      expect(kanbanColumnStyles.todo.bg).toBeDefined();
      expect(kanbanColumnStyles.todo.border).toBeDefined();
    });

    it('in_progress has accent styles', () => {
      expect(kanbanColumnStyles.in_progress.bg).toContain('var(--accent-subtle)');
    });

    it('completed has success styles', () => {
      expect(kanbanColumnStyles.completed.bg).toContain('var(--status-success-subtle)');
    });
  });
});
...
describe('TimeIndicator Component', () => {
  it('renders with normal status', () => {
    ...
    expect(screen.getByText(/15.*يوم/)).toBeInTheDocument();
  });
  ...
  it('renders with warning status', () => {
    ...
    expect(screen.getByText(/3.*يوم/)).toBeInTheDocument();
  });

  it('renders with urgent status showing tomorrow', () => {
    render(
      <TimeIndicator
        indicator={{
          days_remaining: 1,
  ...
  it('renders with overdue status', () => {
    render(
      <TimeIndicator
        indicator={{
          days_remaining: -5,
    ...
    expect(screen.getByText(/متأخر.*5/)).toBeInTheDocument();
  });
  ...
  it('renders completed status', () => {
    ...
    expect(screen.getAllByText(/مكتمل/)[0]).toBeInTheDocument();
  });
  ...
  it('handles no due date', () => {
    ...
    // Should not render anything when no due date
    expect(container.firstChild).toBeNull();
  });

  it('renders today status', () => {
    render(
      <TimeIndicator
        indicator={{
          days_remaining: 0,
...
describe('CardsView Component', () => {
  ...
  const defaultProps = {
    ...
    onPageChange: vi.fn(),
    isOverdue: (task: any) => task.time_indicator?.status === 'overdue',
  };
  ...
  it('renders all tasks', () => {
    ...
    expect(screen.getByText('مهمة ثانية')).toBeInTheDocument();
  ...
  it('renders project codes', () => {
    ...
    const projectCodes = screen.getAllByText(/PRJ-001|PRJ-002/);
  ...
  it('renders assignee initials', () => {
    ...
    // Cards show assignee initial (first letter)
    const initials = screen.getAllByText('أ'); // أحمد
    expect(initials.length).toBeGreaterThan(0);
  });

  it('calls onOpenTaskModal when view button clicked', async () => {
    const onOpenTaskModal = vi.fn();
    render(<CardsView {...defaultProps} onOpenTaskModal={onOpenTaskModal} />);

    // Click on the view button (Eye icon button)
    const viewButtons = document.querySelectorAll('button');
    if (viewButtons.length > 0) {
      await userEvent.click(viewButtons[0]);
      expect(onOpenTaskModal).toHaveBeenCalled();
    }
  });

  it('renders empty state when no tasks', () => {
    render(<CardsView {...defaultProps} tasks={[]} />);
    // No tasks to display
    expect(screen.queryByText('مهمة اختبار')).not.toBeInTheDocument();
  });

  it('renders pagination when multiple pages', () => {
    const multiPagePagination = { ...mockPagination, lastPage: 3, total: 30 };
    render(<CardsView {...defaultProps} pagination={multiPagePagination} />);
...
describe('TableView Component', () => {
  ...
  it('calls onOpenTaskModal when row clicked', async () => {
    const onOpenTaskModal = vi.fn();
    render(<TableView {...defaultProps} onOpenTaskModal={onOpenTaskModal} />);

    const taskTitle = screen.getByText('مهمة اختبار');
  ...
  it('renders empty state when no tasks', () => {
    render(<TableView {...defaultProps} tasks={[]} />);
    // No tasks to display
    expect(screen.queryByText('مهمة اختبار')).not.toBeInTheDocument();
  });
  ...
  it('shows unassigned indicator when no assignee', () => {
    ...
    // Task 2 has no assignee - should show "-" or unassigned text
