...
vi.mock('@entities/task', () => ({
  unifiedTasksApi: {
      ...
      data: [
        {
          ...
          priority: 'high',
          due_date: '2025-01-20',
          time_indicator: { status: 'normal', has_due_date: true },
        },
        {
          id: 2,
          title: 'مهمة مشروع',
          ...
          time_indicator: { status: 'overdue', has_due_date: true },
        },
        {
          id: 3,
          title: 'مهمة إدارية',
    ...
    getStats: vi.fn().mockResolvedValue({
      total: 15,
      by_status: { todo: 5, in_progress: 7, completed: 3 },
      by_type: { personal: 5, project: 8, department: 2 },
      overdue: 2,
      due_today: 1,
      due_this_week: 4,
    }),
...
vi.mock('@pages/tasks/TaskViewModal', () => ({
  default: ({ taskId, isOpen, onClose, onTaskUpdated }: any) =>
    isOpen ? (
      <div data-testid="task-modal">
        <span>Task: {taskId}</span>
        <button onClick={onClose}>Close</button>
        <button onClick={onTaskUpdated}>Update</button>
      </div>
    ) : null,
}));
...
vi.mock('@shared/ui', () => ({
  ...
  Button: ({ children, onClick, leftIcon, variant, size }: any) => (
  ...
  Checkbox: ({ checked, onChange, label }: any) => (
    <label><input type="checkbox" checked={checked ?? false} onChange={onChange} />{label}</label>
  ),
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconSquareCheck: () => <span data-testid="check-icon">Check</span>,
...
describe('MyTasksList Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows loading state initially', async () => {
    (unifiedTasksApi.getMyTasks as any).mockImplementation(() => new Promise(() => {}));
    render(<MyTasksList />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
  });

  it('calls API with my_tasks=true', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
      expect(unifiedTasksApi.getMyTasks).toHaveBeenCalled();
    });
...
describe('MyTasksList Stats Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('MyTasksList Task Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders task modal component', async () => {
    render(<MyTasksList />);
    await act(async () => { await Promise.resolve(); });
    // Modal should be available but closed initially
    expect(screen.queryByTestId('task-modal')).not.toBeInTheDocument();
  });
});

describe('MyTasksList View Mode Toggle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders list icon', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
...
describe('MyTasksList Create Task Link', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows plus icon', async () => {
    render(<MyTasksList />);
    await waitFor(() => {
...
describe('MyTasksList Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (unifiedTasksApi.getMyTasks as any).mockResolvedValue({
      data: [],
      current_page: 1,
      last_page: 1,
      total: 0,
...
describe('MyTasksList API Error', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.spyOn(console, 'error').mockImplementation(() => {});
    (unifiedTasksApi.getMyTasks as any).mockRejectedValue(new Error('API Error'));
  });
