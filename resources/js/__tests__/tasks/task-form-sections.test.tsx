...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, error, type, min, step, disabled }: any) => (
    <div>
      <input
        type={type || 'text'}
        value={value}
...
vi.mock('@shared/ui/DatePicker', () => ({
  DatePicker: ({ value, onChange, placeholder, error, minDate, maxDate, disabled }: any) => (
    <div>
      <input
        type="date"
        value={value}
...
// Import components
import TaskDetailsSection from '@pages/tasks/form/TaskDetailsSection';
import StatusScheduleSection from '@pages/tasks/form/StatusScheduleSection';

// ==================== TaskDetailsSection Tests ====================
...
describe('TaskDetailsSection Component', () => {
  ...
  it('renders title input with value', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByDisplayValue('مهمة اختبارية')).toBeInTheDocument();
  });

  it('renders title label with required indicator', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByText('عنوان المهمة')).toBeInTheDocument();
    expect(screen.getByText('*')).toBeInTheDocument();
  });

  it('renders description input with value', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByDisplayValue('وصف المهمة')).toBeInTheDocument();
  });
  ...
  it('renders title placeholder', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('أدخل عنوان المهمة')).toBeInTheDocument();
  });

  it('renders description placeholder', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('أدخل وصف المهمة')).toBeInTheDocument();
  });

  it('calls onChange when title changes', async () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const titleInput = screen.getByDisplayValue('مهمة اختبارية');
    await userEvent.clear(titleInput);
    await userEvent.type(titleInput, 'عنوان جديد');
    expect(defaultProps.onChange).toHaveBeenCalled();
  });

  it('calls onChange when description changes', async () => {
    render(<TaskDetailsSection {...defaultProps} />);
    const descInput = screen.getByDisplayValue('وصف المهمة');
    await userEvent.clear(descInput);
    await userEvent.type(descInput, 'وصف جديد');
    expect(defaultProps.onChange).toHaveBeenCalled();
  });
});

describe('TaskDetailsSection With Errors', () => {
  const mockFormData = {
    project_id: '1',
    milestone_id: '',
    parent_id: '',
  ...
  it('shows title error when validation fails', () => {
    render(<TaskDetailsSection {...defaultProps} />);
    expect(screen.getByText('عنوان المهمة مطلوب')).toBeInTheDocument();
  });
});

// ==================== StatusScheduleSection Tests ====================

describe('StatusScheduleSection Component', () => {
  const mockFormData = {
    project_id: '1',
    milestone_id: '',
    parent_id: '',
  ...
  it('renders section title', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText('الحالة والجدولة')).toBeInTheDocument();
  });
  ...
  it('does not render status select in create mode', () => {
    render(<StatusScheduleSection {...defaultProps} isEditMode={false} />);
  ...
  it('renders estimated hours input', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText('الساعات المقدرة')).toBeInTheDocument();
  ...
  it('calls onChange when status changes', async () => {
    ...
    expect(defaultProps.onChange).toHaveBeenCalledWith('status', 'completed');
  });
  ...
  it('calls onChange when priority changes', async () => {
    ...
    expect(defaultProps.onChange).toHaveBeenCalledWith('priority', 'urgent');
  });
...
describe('StatusScheduleSection With Date Constraints', () => {
  const mockFormData = {
    project_id: '1',
    milestone_id: '1',
    parent_id: '',
  ...
  it('shows date constraints info box', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText(/الفترة المسموحة/)).toBeInTheDocument();
  });

  it('shows constraint type and name', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText(/المرحلة/)).toBeInTheDocument();
    expect(screen.getByText(/"مرحلة التحليل"/)).toBeInTheDocument();
  });

  it('shows min and max dates', () => {
    render(<StatusScheduleSection {...defaultProps} />);
...
describe('StatusScheduleSection With Project Constraint', () => {
  const dateConstraints = {
    minDate: '2025-01-01',
    maxDate: '2025-12-31',
    constraintType: 'project' as const,
    constraintName: 'مشروع التطوير',
  };
  ...
  it('shows project constraint info', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText(/المشروع/)).toBeInTheDocument();
    expect(screen.getByText(/"مشروع التطوير"/)).toBeInTheDocument();
  });
});

describe('StatusScheduleSection With Errors', () => {
  const mockFormData = {
    project_id: '1',
    milestone_id: '',
    parent_id: '',
  ...
  const defaultProps = {
    formData: mockFormData,
    errors: {
      start_date: ['تاريخ البداية غير صحيح'],
      due_date: ['تاريخ التسليم يجب أن يكون بعد تاريخ البداية'],
    },
    isEditMode: false,
    dateConstraints: null,
    onChange: vi.fn(),
  ...
  it('shows start date error', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText('تاريخ البداية غير صحيح')).toBeInTheDocument();
  });

  it('shows due date error', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    expect(screen.getByText('تاريخ التسليم يجب أن يكون بعد تاريخ البداية')).toBeInTheDocument();
  });
});

describe('StatusScheduleSection Status Options', () => {
  const mockFormData = {
    project_id: '1',
    milestone_id: '',
    parent_id: '',
  ...
  it('renders all status options', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    const statusSelect = document.querySelectorAll('select')[0];
    expect(statusSelect).toContainHTML('للتنفيذ');
    expect(statusSelect).toContainHTML('قيد التنفيذ');
    expect(statusSelect).toContainHTML('قيد المراجعة');
    expect(statusSelect).toContainHTML('مكتمل');
  });

  it('renders all priority options', () => {
    render(<StatusScheduleSection {...defaultProps} />);
    const prioritySelect = document.querySelectorAll('select')[1];
    expect(prioritySelect).toContainHTML('منخفضة');
    expect(prioritySelect).toContainHTML('متوسطة');
    expect(prioritySelect).toContainHTML('عالية');
    expect(prioritySelect).toContainHTML('عاجلة');
  });
});

// ==================== Import and test TaskTypeSection ====================

import TaskTypeSection from '@pages/tasks/form/TaskTypeSection';

describe('TaskTypeSection Component', () => {
  const mockFormData = {
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('نوع المهمة')).toBeInTheDocument();
  });

  it('renders all task type buttons', () => {
    render(<TaskTypeSection {...defaultProps} />);
    expect(screen.getByText('مهمة مشروع')).toBeInTheDocument();
    expect(screen.getByText('مهمة شخصية')).toBeInTheDocument();
    expect(screen.getByText('مهمة إدارية')).toBeInTheDocument();
    expect(screen.getByText('مهمة متكررة')).toBeInTheDocument();
  });

  it('renders task type descriptions', () => {
    render(<TaskTypeSection {...defaultProps} />);
    expect(screen.getByText('مهمة مرتبطة بمشروع محدد')).toBeInTheDocument();
    expect(screen.getByText('مهمة شخصية غير مرتبطة بمشروع')).toBeInTheDocument();
    expect(screen.getByText('مهمة إدارية للقسم')).toBeInTheDocument();
    expect(screen.getByText('مهمة تتكرر بشكل دوري')).toBeInTheDocument();
  });

  it('calls onChange when selecting task type', async () => {
    render(<TaskTypeSection {...defaultProps} />);
    await userEvent.click(screen.getByText('مهمة شخصية'));
    expect(defaultProps.onChange).toHaveBeenCalledWith('type', 'personal');
  });

  it('highlights selected task type', () => {
    render(<TaskTypeSection {...defaultProps} />);
    const projectButton = screen.getByText('مهمة مشروع').closest('button');
    expect(projectButton).toHaveClass('border-[var(--accent-default)]');
  });
});

describe('TaskTypeSection Personal Task', () => {
  const mockFormData = {
  ...
  it('does not show additional fields for personal tasks', () => {
    render(<TaskTypeSection {...defaultProps} />);
    // Personal tasks don't have additional fields in current implementation
    expect(screen.queryByText('القسم')).not.toBeInTheDocument();
    expect(screen.queryByText('نمط التكرار')).not.toBeInTheDocument();
  });
});

describe('TaskTypeSection Department Task', () => {
  const mockFormData = {
  ...
  it('shows department select for department tasks', () => {
    render(<TaskTypeSection {...defaultProps} />);
  ...
  it('shows department options', () => {
    ...
    expect(screen.getByText('قسم المالية')).toBeInTheDocument();
  });

  it('calls onChange when selecting department', async () => {
    render(<TaskTypeSection {...defaultProps} />);
    const deptSelect = document.querySelector('select');
    if (deptSelect) {
      await userEvent.selectOptions(deptSelect, '2');
      expect(defaultProps.onChange).toHaveBeenCalledWith('department_id', '2');
    }
  });
});

describe('TaskTypeSection Recurring Task', () => {
  const mockFormData = {
  ...
  it('shows recurrence select for recurring tasks', () => {
    render(<TaskTypeSection {...defaultProps} />);
  ...
  it('renders recurrence options', () => {
    ...
    expect(recurrenceSelect).toContainHTML('يومي');
    expect(recurrenceSelect).toContainHTML('أسبوعي');
    expect(recurrenceSelect).toContainHTML('شهري');
  });

  it('calls onChange when selecting recurrence', async () => {
    render(<TaskTypeSection {...defaultProps} />);
    const recurrenceSelect = document.querySelector('select');
    if (recurrenceSelect) {
      await userEvent.selectOptions(recurrenceSelect, 'monthly');
      expect(defaultProps.onChange).toHaveBeenCalledWith('recurrence_rule', 'monthly');
    }
  });
});

describe('TaskTypeSection With Department Error', () => {
  const mockFormData = {
    type: 'department' as const,
    project_id: '',
    milestone_id: '',
  ...
  const defaultProps = {
    formData: mockFormData,
    departments: [{ id: 1, name: 'قسم التقنية' }],
    errors: {
      department_id: ['القسم مطلوب'],
    },
    onChange: vi.fn(),
  };

  it('shows department error message', () => {
    render(<TaskTypeSection {...defaultProps} />);
    expect(screen.getByText('القسم مطلوب')).toBeInTheDocument();
  });
});
