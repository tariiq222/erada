...
vi.mock('@pages/tasks/components/constants', () => ({
  statusIcons: {
    todo: () => <span data-testid="icon-todo">○</span>,
    in_progress: () => <span data-testid="icon-in-progress">◐</span>,
    in_review: () => <span data-testid="icon-in-review">◉</span>,
    completed: () => <span data-testid="icon-completed">●</span>,
  },
  statusColors: {
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, disabled, leftIcon, variant, size }: any) => (
...
import SubtaskCard from '@pages/tasks/components/SubtaskCard';

const mockSubtask = {
  id: 1,
  title: 'مهمة فرعية اختبارية',
  status: 'in_progress',
  priority: 'high',
...
describe('SubtaskCard Component', () => {
  const defaultProps = {
    subtask: mockSubtask,
    users: mockUsers,
  ...
  it('renders subtask title', () => {
    render(<SubtaskCard {...defaultProps} />);
    expect(screen.getByText('مهمة فرعية اختبارية')).toBeInTheDocument();
  });
  ...
  it('renders due date', () => {
    render(<SubtaskCard {...defaultProps} />);
    // Due date is formatted in Arabic
    const dateElement = document.querySelector('.text-xs.bg-\\[var\\(--surface-muted\\)\\]');
    expect(dateElement).toBeInTheDocument();
  });
});

describe('SubtaskCard Status Dropdown', () => {
  const defaultProps = {
    subtask: mockSubtask,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
  ...
  it('opens status menu when status button clicked', async () => {
    render(<SubtaskCard {...defaultProps} />);

    const statusButton = screen.getByText('قيد التنفيذ').closest('button');
    await userEvent.click(statusButton!);

    expect(screen.getByText('تغيير الحالة')).toBeInTheDocument();
  });

  it('shows all available statuses in menu', async () => {
    render(<SubtaskCard {...defaultProps} />);
    ...
    // قيد التنفيذ appears twice (current status and in menu)
    expect(screen.getAllByText('قيد التنفيذ').length).toBeGreaterThanOrEqual(1);
  ...
  it('closes status menu after selection', async () => {
    render(<SubtaskCard {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(screen.queryByText('تغيير الحالة')).not.toBeInTheDocument();
    });
  });
});

describe('SubtaskCard Actions Menu', () => {
  const defaultProps = {
    subtask: mockSubtask,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
  ...
  it('opens actions menu when actions button clicked', async () => {
    render(<SubtaskCard {...defaultProps} />);

    // Find the MoreVertical button
    const actionsButton = document.querySelector('button.text-\\[var\\(--text-tertiary\\)\\]');
  ...
  it('switches to edit mode when edit clicked', async () => {
    ...
    expect(screen.getByDisplayValue('مهمة فرعية اختبارية')).toBeInTheDocument();
...
describe('SubtaskCard Edit Mode', () => {
  const defaultProps = {
    subtask: mockSubtask,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
  ...
  it('shows edit form with current values', async () => {
    ...
    // Assignee and priority now use the custom Select (button-based listbox),
    // not native <select>. Expect two listbox triggers.
    const triggers = document.querySelectorAll('button[aria-haspopup="listbox"]');
    expect(triggers.length).toBe(2);
  });

  it('shows user options in assignee dropdown', async () => {
    render(<SubtaskCard {...defaultProps} />);
    ...
    // Open the assignee Select (first listbox trigger) to reveal its options.
    const triggers = document.querySelectorAll('button[aria-haspopup="listbox"]');
    expect(triggers.length).toBe(2); // assignee and priority
    await userEvent.click(triggers[0]);

    const options = screen.getAllByRole('option');
    const labels = options.map((o) => o.textContent);
    expect(labels).toContain('أحمد');
    expect(labels).toContain('محمد');
    expect(labels).toContain('خالد');
  });

  it('shows priority options in priority dropdown', async () => {
    render(<SubtaskCard {...defaultProps} />);
    ...
    // Open the priority Select (second listbox trigger) to reveal its options.
    const triggers = document.querySelectorAll('button[aria-haspopup="listbox"]');
    await userEvent.click(triggers[1]);

    const labels = screen.getAllByRole('option').map((o) => o.textContent);
    expect(labels).toContain('منخفضة');
    expect(labels).toContain('متوسطة');
    expect(labels).toContain('عالية');
    expect(labels).toContain('عاجلة');
  });
  ...
  it('calls onUpdate when saving changes', async () => {
    ...
    const titleInput = screen.getByDisplayValue('مهمة فرعية اختبارية');
  ...
  it('cancels edit and restores original values', async () => {
    ...
    expect(screen.queryByDisplayValue('عنوان جديد')).not.toBeInTheDocument();
    expect(screen.getByText('مهمة فرعية اختبارية')).toBeInTheDocument();
  });
...
describe('SubtaskCard Without Assignee', () => {
  const subtaskWithoutAssignee = {
    ...mockSubtask,
    assignee: null,
  };

  const defaultProps = {
    subtask: subtaskWithoutAssignee,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
    onUpdate: vi.fn().mockResolvedValue(undefined),
    onDelete: vi.fn().mockResolvedValue(undefined),
  ...
  it('does not render assignee badge when no assignee', () => {
    render(<SubtaskCard {...defaultProps} />);

    // Should not show any user badge (only the title should mention the name)
    const userBadges = document.querySelectorAll('.bg-gray-100.text-gray-600');
    // Due date may still be there, but no assignee
    expect(screen.queryByText('أحمد')).not.toBeInTheDocument();
  });
});

describe('SubtaskCard Without Due Date', () => {
  const subtaskWithoutDueDate = {
    ...mockSubtask,
    due_date: null,
  };

  const defaultProps = {
    subtask: subtaskWithoutDueDate,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
    onUpdate: vi.fn().mockResolvedValue(undefined),
    onDelete: vi.fn().mockResolvedValue(undefined),
  ...
  it('does not render due date badge when no due date', () => {
    render(<SubtaskCard {...defaultProps} />);

    // Should still show assignee but not calendar
    expect(screen.getByText('أحمد')).toBeInTheDocument();
    // Calendar icon badges should be limited
  });
});

describe('SubtaskCard Completed Status', () => {
  ...
  it('shows title with strikethrough when completed', () => {
    render(<SubtaskCard {...defaultProps} />);

    const title = screen.getByText('مهمة فرعية اختبارية');
...
describe('SubtaskCard Without Priority', () => {
  const subtaskWithoutPriority = {
    ...mockSubtask,
    priority: null,
  };

  const defaultProps = {
    subtask: subtaskWithoutPriority,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
    onUpdate: vi.fn().mockResolvedValue(undefined),
    onDelete: vi.fn().mockResolvedValue(undefined),
  ...
  it('does not render priority badge when no priority', () => {
    ...
    expect(screen.queryByText('عالية')).not.toBeInTheDocument();
    expect(screen.queryByText('متوسطة')).not.toBeInTheDocument();
    expect(screen.queryByText('منخفضة')).not.toBeInTheDocument();
    expect(screen.queryByText('عاجلة')).not.toBeInTheDocument();
  });
});

describe('SubtaskCard Click Outside', () => {
  const defaultProps = {
    subtask: mockSubtask,
    users: mockUsers,
    onStatusChange: vi.fn().mockResolvedValue(undefined),
  ...
  it('closes status menu when clicking outside', async () => {
    render(<SubtaskCard {...defaultProps} />);
