...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconAt: () => <span data-testid="at-icon">@</span>,
  ...
  IconFile: () => <span data-testid="generic-file-icon">File</span>,
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  MentionInput: ({
    ...
    <div data-testid="mention-input">
      ...
      />
      <button onClick={onSubmit} disabled={disabled}>إرسال</button>
      <input
        type="file"
...
import CommentsSection from '@pages/tasks/view/CommentsSection';
...
const mockComments = [
  ...
  },
  {
    ...
    mentioned_users: [{ id: 2, name: 'محمد' }],
  },
];
...
describe('CommentsSection', () => {
  ...
  it('renders mention input', async () => {
  ...
  it('displays comments', async () => {
    ...
    expect(screen.getByText(/تعليق ثاني/)).toBeInTheDocument();
  });

  it('displays comment authors', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
...
describe('CommentsSection Empty State', () => {
  ...
  it('shows encouragement message in empty state', async () => {
    render(<CommentsSection {...defaultProps} comments={[]} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByText('كن أول من يعلق على هذه المهمة')).toBeInTheDocument();
  });
});

describe('CommentsSection Add Comment', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('handles comment input', async () => {
    ...
    expect(textarea).toHaveValue('تعليق جديد');
  });
  ...
  it('calls onCommentAdded after successful submit', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
...
describe('CommentsSection Delete Comment', () => {
  ...
  it('deletes comment on confirm', async () => {
    const { commentsApi } = await import('@entities/comment');
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
...
describe('CommentsSection Attachments', () => {
  ...
  it('shows attachment count', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByText('المرفقات (1)')).toBeInTheDocument();
  });

  it('shows download button for attachments', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getAllByTestId('download-icon').length).toBeGreaterThanOrEqual(1);
  });
});

describe('CommentsSection Mentioned Users', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays mentioned users', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByText('تمت الإشارة إلى:')).toBeInTheDocument();
  ...
  it('extracts mentions from content', async () => {
    ...
    const textarea = screen.getByTestId('comment-textarea');
    fireEvent.change(textarea, { target: { value: 'مرحبا @أحمد' } });
    ...
    await waitFor(() => {
      expect(commentsApi.create).toHaveBeenCalledWith(
        expect.objectContaining({
          mentioned_users: expect.any(Array),
        })
      );
    });
  });
...
describe('CommentsSection API Errors', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('handles comment submit error', async () => {
  ...
  it('handles comment delete error', async () => {
    window.confirm = vi.fn().mockReturnValue(true);
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const { commentsApi } = await import('@entities/comment');
...
describe('CommentsSection User Avatars', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows user initials in avatar', async () => {
    render(<CommentsSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });
    // First letter of user names as avatars
    const avatars = screen.getAllByText('أ'); // أحمد
