...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconFlag: ({ className }: { className?: string }) => <span className={className} data-testid="flag-icon">Flag</span>,
  IconCircle: ({ className }: { className?: string }) => <span className={className} data-testid="circle-icon">Circle</span>,
...
vi.mock('@pages/projects/constants', () => ({
  ...
  taskStatusIcons: {
    ...
    in_review: () => <span data-testid="review-icon">Review</span>,
...
import TasksListView from '@pages/projects/components/sections/tasks/TasksListView';
...
describe('TasksListView Basic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders card component', () => {
    render(<TasksListView tasks={mockTasks as any} />);
...
describe('TasksListView Task Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('displays task titles', () => {
    render(<TasksListView tasks={mockTasks as any} />);
    expect(screen.getByText('مهمة أولى')).toBeInTheDocument();
    expect(screen.getByText('مهمة ثانية')).toBeInTheDocument();
    expect(screen.getByText('مهمة بدون تاريخ')).toBeInTheDocument();
  });

  it('renders task title as link', () => {
    render(<TasksListView tasks={mockTasks as any} />);
    const link = screen.getByText('مهمة أولى');
  ...
  it('shows flag icons for priority', () => {
    render(<TasksListView tasks={mockTasks as any} />);
    expect(screen.getAllByTestId('flag-icon').length).toBe(3);
  });
});

describe('TasksListView Due Dates', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows dash for null due date', () => {
    render(<TasksListView tasks={mockTasks as any} />);
...
describe('TasksListView Assignees', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays assignee names', () => {
    render(<TasksListView tasks={mockTasks as any} />);
  ...
  it('shows assignee initials', () => {
    ...
    expect(screen.getByText('م')).toBeInTheDocument(); // محمد
  });

  it('shows dash for unassigned tasks', () => {
    render(<TasksListView tasks={mockTasks as any} />);
    // Task 3 has no assignee, so there should be at least 2 dashes (date and assignee)
    const cells = screen.getAllByText('-');
    expect(cells.length).toBeGreaterThanOrEqual(2);
  });
});

describe('TasksListView Status Icons', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows status icons', () => {
    render(<TasksListView tasks={mockTasks as any} />);
    expect(screen.getByTestId('progress-icon')).toBeInTheDocument();
    expect(screen.getByTestId('completed-icon')).toBeInTheDocument();
    expect(screen.getByTestId('todo-icon')).toBeInTheDocument();
  });
});

describe('TasksListView Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders empty table with no tasks', () => {
    render(<TasksListView tasks={[]} />);
    expect(screen.getByTestId('card')).toBeInTheDocument();
...
describe('TasksListView Overdue Tasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders overdue tasks with date styling', () => {
    ...
    render(<TasksListView tasks={overdueTasks as any} />);
    const dateCell = screen.getByText('2020-01-01');
    expect(dateCell.className).toContain('text-[var(--status-danger)]');
  });

  it('does not apply overdue style to completed tasks', () => {
    const completedTasks = [
      {
        ...
        status: 'completed',
        priority: 'high',
        due_date: '2020-01-01', // Past date but completed
        assignee: null,
      },
    ];
    render(<TasksListView tasks={completedTasks as any} />);
    const dateCell = screen.getByText('2020-01-01');
    expect(dateCell.className).not.toContain('text-red-600');
  });
});
