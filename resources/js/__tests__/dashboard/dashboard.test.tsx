...
const mockOverdueTasks = [
  {
    id: 1,
    title: 'إكمال التقرير الشهري',
    due_date: '2025-01-01',
    project: { name: 'مشروع التحول', code: 'PRJ-001' },
    assignee: { name: 'أحمد' },
  },
  {
    id: 2,
...
vi.mock('@shared/api/settings', () => ({
  dashboardApi: {
    ...
    getAdvancedStats: vi.fn(() => Promise.resolve({ overdue_projects: 0, critical_overdue: 0 })),
    getProjectsByStatus: vi.fn(() => Promise.resolve([])),
    getRecentProjects: vi.fn(() => Promise.resolve(mockRecentProjects)),
    getOverdueTasks: vi.fn(() => Promise.resolve(mockOverdueTasks)),
...
describe('Dashboard Page', () => {
  ...
  it('renders welcome header with user name', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('مرحباً، أحمد محمد')).toBeInTheDocument();
    });
  });
  ...
  it('renders dashboard description', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('إليك ملخص نشاطات المشاريع والمهام')).toBeInTheDocument();
    });
  });
  ...
  it('renders stats cards', async () => {
    ...
    await waitFor(() => {
      ...
      // مهام متأخرة appears twice - in stat card and section title
      const overdueTexts = screen.getAllByText('مهام متأخرة');
  ...
  it('renders stat values correctly', async () => {
    ...
    await waitFor(() => {
      // Active projects: 12 (locale may render Arabic or Latin digits)
      expect(screen.getByText((text) => text === '١٢' || text === '12')).toBeInTheDocument();
      // Overdue tasks: 10
      expect(screen.getByText((text) => text === '١٠' || text === '10')).toBeInTheDocument();
      // Active users: 50
      expect(screen.getByText((text) => text === '٥٠' || text === '50')).toBeInTheDocument();
    });
  });
  ...
  it('renders recent project names', async () => {
    render(<Dashboard />);

    await waitFor(() => {
      expect(screen.getByText('مشروع التحول الرقمي')).toBeInTheDocument();
      expect(screen.getByText('تطوير النظام')).toBeInTheDocument();
      expect(screen.getByText('مشروع معلق')).toBeInTheDocument();
    });
  });
  ...
  it('renders project codes', async () => {
    ...
    await waitFor(() => {
      // PRJ-001 may appear multiple times (in project card and task card)
      const prj001 = screen.getAllByText('PRJ-001');
      expect(prj001.length).toBeGreaterThan(0);
  ...
  it('renders overdue tasks section', async () => {
    ...
    await waitFor(() => {
      // المهام المتأخرة appears in both stat card and section
      const overdueTexts = screen.getAllByText('المهام المتأخرة');
      expect(overdueTexts.length).toBeGreaterThan(0);
    });
  });
  ...
  it('renders overdue task titles', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('إكمال التقرير الشهري')).toBeInTheDocument();
      expect(screen.getByText('مراجعة الوثائق')).toBeInTheDocument();
    });
  });

  it('renders task assignees', async () => {
    render(<Dashboard />);
  ...
  it('renders view all links', async () => {
    ...
    await waitFor(() => {
      const viewAllLinks = screen.getAllByText('عرض الكل');
      expect(viewAllLinks.length).toBeGreaterThanOrEqual(2);
    });
  });

  it('renders projects link correctly', async () => {
    render(<Dashboard />);
  ...
  it('renders project detail links', async () => {
    render(<Dashboard />);
  ...
  it('renders task detail links', async () => {
    render(<Dashboard />);

    await waitFor(() => {
      const taskLink = document.querySelector('a[href="/tasks/1"]');
      expect(taskLink).toBeInTheDocument();
    });
  });

  it('renders overdue tasks link', async () => {
    render(<Dashboard />);

    await waitFor(() => {
      const overdueLink = document.querySelector('a[href="/tasks?overdue=true"]');
  ...
  it('renders total values', async () => {
    ...
    await waitFor(() => {
      // Total projects: 20 (locale may render Arabic or Latin digits)
      expect(screen.getByText((text) => text === '٢٠' || text === '20')).toBeInTheDocument();
      // Total tasks: 100
      expect(screen.getByText((text) => text === '١٠٠' || text === '100')).toBeInTheDocument();
    });
  });
});
...
describe('Dashboard Progress Display', () => {
  it('shows progress bars for projects', async () => {
    ...
    await waitFor(() => {
      // The Progress component already exposes role="progressbar" — use the role-based query.
...
describe('Dashboard Project Cards', () => {
  it('renders project cards with hover styles', async () => {
    render(<Dashboard />);

    await waitFor(() => {
      // Project links have hover classes
      const projectLinks = document.querySelectorAll('a[href^="/projects/"]');
      expect(projectLinks.length).toBeGreaterThan(0);
    });
  });
...
describe('Dashboard Task Cards', () => {
  it('renders task cards with danger styling for overdue', async () => {
    render(<Dashboard />);
