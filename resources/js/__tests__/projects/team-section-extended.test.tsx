...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconUsers: () => <span data-testid="users-icon">👥</span>,
  ...
  IconInfoCircle: () => <span data-testid="info-icon">ℹ️</span>,
...
const mockTeamMembers = [
  { user_id: 3, name: '' },
  { user_id: undefined, name: 'عضو جديد' },
];

describe('TeamSection Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TeamSection Team Members', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows user name for member with user_id', () => {
    render(
      <TeamSection
        teamMembers={mockTeamMembers}
        users={mockUsers}
  ...
  it('shows placeholder for member without user_id', () => {
    render(
      <TeamSection
        teamMembers={mockTeamMembers}
        users={mockUsers}
    ...
    // Member without user_id shows the Select placeholder instead of a name
    expect(screen.getByText('[Select Team Member]')).toBeInTheDocument();
  });

  it('shows placeholder for empty member', () => {
    render(
      <TeamSection
        teamMembers={[{ user_id: undefined, name: '' }]}
        users={mockUsers}
        onAddTeamMember={vi.fn()}
        onRemoveTeamMember={vi.fn()}
        onOpenTeamMemberModal={vi.fn()}
  ...
  it('shows task executor label for member with user', () => {
    render(
      <TeamSection
        teamMembers={mockTeamMembers}
        users={mockUsers}
...
describe('TeamSection Add Member', () => {
  ...
  it('calls onAddTeamMember when add button clicked', () => {
    ...
    fireEvent.click(screen.getByText('إضافة عضو للفريق'));
    expect(onAddTeamMember).toHaveBeenCalledTimes(1);
  });
});
...
describe('TeamSection Remove Member', () => {
  ...
  it('does not show delete button for single member', () => {
    render(
      <TeamSection
  ...
  it('calls onRemoveTeamMember when delete clicked', () => {
    const onRemoveTeamMember = vi.fn();
    render(
      <TeamSection
        teamMembers={mockTeamMembers}
        users={mockUsers}
    ...
    expect(onRemoveTeamMember).toHaveBeenCalledWith(0);
  });
});

describe('TeamSection Select Member', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls onSelectTeamMember with selected user id', () => {
    const onSelectTeamMember = vi.fn();
    render(
      <TeamSection
        teamMembers={[{ user_id: undefined, name: '' }]}
        users={mockUsers}
    ...
    // Open the Select dropdown then pick a user option
    fireEvent.click(screen.getByText('[Select Team Member]'));
    fireEvent.click(screen.getByText('أحمد محمد'));
    expect(onSelectTeamMember).toHaveBeenCalledWith(0, 1);
  });

  it('renders user names as options when opened', () => {
    const onSelectTeamMember = vi.fn();
    render(
      <TeamSection
        teamMembers={[{ user_id: undefined, name: '' }]}
...
describe('TeamSection Empty State', () => {
  ...
  it('renders with empty team members', () => {
    render(
      <TeamSection
        teamMembers={[]}
        users={mockUsers}
