...
const mockGetProjectsByStatus = vi.fn();
const mockGetRecentProjects = vi.fn();
const mockGetAdvancedStats = vi.fn();
...
vi.mock('@shared/api/settings', () => ({
  dashboardApi: {
    ...
    getProjectsByStatus: () => mockGetProjectsByStatus(),
    getRecentProjects: () => mockGetRecentProjects(),
    getAdvancedStats: () => mockGetAdvancedStats(),
  },
}));
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconSquareCheck: () => <span data-testid="check-icon">CheckSquare</span>,
  ...
  IconChartPie: () => <span data-testid="pie-chart-icon">PieChart</span>,
  IconArrowLeft: () => <span data-testid="arrow-icon">ArrowLeft</span>,
  IconCalendar: () => <span data-testid="calendar-icon">Calendar</span>,
  IconBuilding: () => <span data-testid="building-icon">Building2</span>,
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Progress: ({ value, size, showValue }: any) => (
    <div data-testid="progress" data-value={value} data-size={size}>
      {showValue !== false && `${value}%`}
    </div>
  ),
  Skeleton: ({ width, height, className, variant }: any) => (
    <div data-testid="skeleton" data-width={width} data-height={height} data-variant={variant} className={className} />
  ),
  Select: ({ value, onChange, options, className }: any) => (
    <select value={value} onChange={onChange} data-testid="select" className={className}>
      {options?.map((opt: any) => (
        <option key={opt.value} value={opt.value}>{opt.label}</option>
      ))}
    </select>
  ),
  DatePicker: ({ value, onChange, className }: any) => (
    <input type="date" value={value} onChange={(e: any) => onChange?.(e.target.value)} className={className} data-testid="date-picker" />
  ),
  StatusBadge: ({ status }: any) => (
    <span data-testid="badge" data-status={status}>{status}</span>
  ),
...
import ProjectStatistics from '@pages/projects/ProjectStatistics';
...
describe('ProjectStatistics Loading', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
    mockGetRecentProjects.mockResolvedValue(mockRecentProjects);
    mockGetAdvancedStats.mockResolvedValue(mockAdvancedStats);
  });

  it('shows skeleton while loading', () => {
    // Keep loading by not resolving
    mockGetStats.mockReturnValue(new Promise(() => {}));
    renderWithRouter(<ProjectStatistics />);
    expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
  });
});

describe('ProjectStatistics Header', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows header title', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('إحصائيات المشاريع')).toBeInTheDocument();
    });
  });
  ...
  it('shows header description', async () => {
    ...
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('نظرة شاملة على أداء المشاريع والمهام')).toBeInTheDocument();
    });
  });
...
describe('ProjectStatistics Date Filter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows all date filter options', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('كل الفترات')).toBeInTheDocument();
      expect(screen.getByText('هذا الشهر')).toBeInTheDocument();
      expect(screen.getByText('ربع سنة')).toBeInTheDocument();
      expect(screen.getByText('هذه السنة')).toBeInTheDocument();
      expect(screen.getByText('مخصص')).toBeInTheDocument();
    });
  });

  it('changes filter when selected', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByTestId('select')).toBeInTheDocument();
  ...
  it('shows custom date pickers when custom selected', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByTestId('select')).toBeInTheDocument();
...
describe('ProjectStatistics Stats Cards', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows active projects label', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
  ...
  it('shows trending icon', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getAllByTestId('trending-icon').length).toBeGreaterThan(0);
    });
  });
...
describe('ProjectStatistics Completion Rates', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows project completion rate label', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('معدل إنجاز المشاريع')).toBeInTheDocument();
    });
  });

  it('shows task completion rate label', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('معدل إنجاز المهام')).toBeInTheDocument();
    });
  });

  it('shows progress components', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
  ...
  it('calculates project completion rate correctly', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      // 5/20 = 25%
  ...
  it('shows in progress tasks label', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getAllByText('قيد التنفيذ').length).toBeGreaterThan(0);
...
describe('ProjectStatistics Status Distribution', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows distribution title', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('توزيع المشاريع حسب الحالة')).toBeInTheDocument();
    });
  });

  it('shows pie chart icon', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
  ...
  it('shows all status labels', async () => {
    // The distribution now renders each status via <StatusBadge status=... />,
    // so assert the badge for every distribution status is present (the badge
    // carries the status key; its localized label lives inside StatusBadge).
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      const statuses = ['draft', 'planning', 'in_progress', 'on_hold', 'completed', 'cancelled'];
      statuses.forEach((status) => {
        expect(document.querySelector(`[data-testid="badge"][data-status="${status}"]`)).toBeInTheDocument();
      });
    });
  });
});

describe('ProjectStatistics Recent Projects', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows recent projects title', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('أحدث المشاريع')).toBeInTheDocument();
    });
  });
  ...
  it('shows project names', async () => {
    ...
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getByText('مشروع تطوير النظام')).toBeInTheDocument();
      expect(screen.getByText('مشروع التحديثات')).toBeInTheDocument();
    });
  });
  ...
  it('shows project progress', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
...
describe('ProjectStatistics Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue(mockStatsData);
    mockGetProjectsByStatus.mockResolvedValue(mockProjectsByStatus);
  ...
  it('shows empty state for no projects', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
...
describe('ProjectStatistics Error Handling', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockRejectedValue(new Error('API Error'));
    mockGetProjectsByStatus.mockRejectedValue(new Error('API Error'));
    mockGetRecentProjects.mockRejectedValue(new Error('API Error'));
    mockGetAdvancedStats.mockRejectedValue(new Error('API Error'));
  });
...
describe('ProjectStatistics Zero Stats', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetStats.mockResolvedValue({
      projects: { total: 0, active: 0, completed: 0, on_hold: 0, cancelled: 0, draft: 0, planning: 0 },
      tasks: { total: 0, completed: 0, in_progress: 0, pending: 0, overdue: 0 },
    });
    mockGetProjectsByStatus.mockResolvedValue({});
    mockGetRecentProjects.mockResolvedValue([]);
    mockGetAdvancedStats.mockResolvedValue({
      avg_completion_time: null,
      budget_summary: { total_budget: 0, total_actual: 0, variance: 0, variance_percentage: 0, over_budget_count: 0 },
      departments_performance: [],
      overdue_projects: { total: 0, critical: 0 },
      monthly_trends: [],
    });
  });

  it('shows zero for all stats', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
  ...
  it('shows 0% completion rate', async () => {
    renderWithRouter(<ProjectStatistics />);
    await act(async () => { await Promise.resolve(); });
    await waitFor(() => {
      expect(screen.getAllByText('0%').length).toBeGreaterThan(0);
