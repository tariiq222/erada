...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'projects.actual_cost': 'التكلفة الفعلية', 'projects.objectives': 'الأهداف', 'projects.scope': 'نطاق المشروع', 'projects.in_scope': 'ما يشمله النطاق',
    ...
    'projects.enter_description': '[Enter Description]', 'projects.enter_item': 'أدخل البند', 'projects.enter_mitigation_plan': '[Enter Mitigation Plan]', 'projects.enter_objective': '[Enter Objective]',
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, leftIcon, onClick, variant, size, type, className }: any) => (
    <button onClick={onClick} type={type} data-variant={variant} data-size={size} className={className}>
      {leftIcon}{children}
    </button>
  ),
}));
...
import ScopeSection from '@pages/projects/form/ScopeSection';

const defaultProps = {
  inScope: ['نطاق 1', 'نطاق 2'],
  outOfScope: ['خارج 1'],
  onInScopeChange: vi.fn(),
  onOutOfScopeChange: vi.fn(),
  onAddInScope: vi.fn(),
  onAddOutOfScope: vi.fn(),
  onRemoveInScope: vi.fn(),
  onRemoveOutOfScope: vi.fn(),
};
...
describe('ScopeSection', () => {
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('نطاق المشروع')).toBeInTheDocument();
  });
  ...
  it('renders in scope section', () => {
    render(<ScopeSection {...defaultProps} />);
    expect(screen.getByText('ما يشمله النطاق')).toBeInTheDocument();
  });

  it('renders out of scope section', () => {
    render(<ScopeSection {...defaultProps} />);
    expect(screen.getByText('خارج النطاق')).toBeInTheDocument();
  });
});

describe('ScopeSection In Scope Items', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('displays in scope items', () => {
    render(<ScopeSection {...defaultProps} />);
    expect(screen.getByDisplayValue('نطاق 1')).toBeInTheDocument();
    expect(screen.getByDisplayValue('نطاق 2')).toBeInTheDocument();
  });

  it('handles in scope item change', () => {
    render(<ScopeSection {...defaultProps} />);

    const inputs = screen.getAllByPlaceholderText('أدخل البند');
    fireEvent.change(inputs[0], { target: { value: 'نطاق معدل' } });

    expect(defaultProps.onInScopeChange).toHaveBeenCalledWith(0, 'نطاق معدل');
  });

  it('shows remove button for in scope items', () => {
    render(<ScopeSection {...defaultProps} />);
    // 2 remove buttons for 2 in scope items + 1 for out of scope (only shown when more than 1)
    expect(screen.getAllByTestId('trash-icon').length).toBeGreaterThanOrEqual(1);
  });

  it('calls onRemoveInScope when remove clicked', () => {
    render(<ScopeSection {...defaultProps} />);

    const trashButtons = screen.getAllByTestId('trash-icon');
    fireEvent.click(trashButtons[0].closest('button')!);

    expect(defaultProps.onRemoveInScope).toHaveBeenCalledWith(0);
  });
});

describe('ScopeSection Out of Scope Items', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays out of scope items', () => {
    render(<ScopeSection {...defaultProps} />);
    expect(screen.getByDisplayValue('خارج 1')).toBeInTheDocument();
  });

  it('handles out of scope item change', () => {
    render(<ScopeSection {...defaultProps} />);

    const inputs = screen.getAllByPlaceholderText('أدخل البند');
    // Last input is for out of scope
    fireEvent.change(inputs[inputs.length - 1], { target: { value: 'خارج معدل' } });

    expect(defaultProps.onOutOfScopeChange).toHaveBeenCalledWith(0, 'خارج معدل');
  });

  it('hides remove button when only one out of scope item', () => {
    render(<ScopeSection {...defaultProps} />);
    // With 1 out of scope item, should not show remove button for it
    const trashButtons = screen.getAllByTestId('trash-icon');
    // Only in scope items should have remove buttons (2 items = 2 buttons)
    expect(trashButtons.length).toBe(2);
  });
});

describe('ScopeSection Add Items', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows add buttons', () => {
    render(<ScopeSection {...defaultProps} />);
    const addButtons = screen.getAllByText('إضافة');
    expect(addButtons.length).toBe(2); // One for each section
  });

  it('calls onAddInScope when add clicked', () => {
    render(<ScopeSection {...defaultProps} />);

    const addButtons = screen.getAllByText('إضافة');
    fireEvent.click(addButtons[0]);

    expect(defaultProps.onAddInScope).toHaveBeenCalled();
  });

  it('calls onAddOutOfScope when add clicked', () => {
    render(<ScopeSection {...defaultProps} />);

    const addButtons = screen.getAllByText('إضافة');
    fireEvent.click(addButtons[1]);

    expect(defaultProps.onAddOutOfScope).toHaveBeenCalled();
  });

  it('shows plus icon on add buttons', () => {
    render(<ScopeSection {...defaultProps} />);
    expect(screen.getAllByTestId('plus-icon').length).toBe(2);
  });
});

describe('ScopeSection Single Item', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('hides remove button when single in scope item', () => {
    render(
      <ScopeSection
        {...defaultProps}
        inScope={['نطاق واحد']}
        outOfScope={['خارج 1', 'خارج 2']}
      />
    );

    // 0 remove for single in scope + 2 for out of scope
    const trashButtons = screen.getAllByTestId('trash-icon');
    expect(trashButtons.length).toBe(2);
  });
});

describe('ScopeSection Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders with empty arrays', () => {
    ...
    expect(screen.getByText('نطاق المشروع')).toBeInTheDocument();
    expect(screen.getAllByText('إضافة').length).toBe(2);
  });
});

describe('ScopeSection Multiple Items', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders multiple in scope items', () => {
    render(
      <ScopeSection
        {...defaultProps}
        inScope={['نطاق 1', 'نطاق 2', 'نطاق 3']}
      />
    );

    expect(screen.getByDisplayValue('نطاق 1')).toBeInTheDocument();
    expect(screen.getByDisplayValue('نطاق 2')).toBeInTheDocument();
    expect(screen.getByDisplayValue('نطاق 3')).toBeInTheDocument();
  });

  it('renders multiple out of scope items', () => {
    render(
      <ScopeSection
        {...defaultProps}
        outOfScope={['خارج 1', 'خارج 2', 'خارج 3']}
      />
    );

    expect(screen.getByDisplayValue('خارج 1')).toBeInTheDocument();
    expect(screen.getByDisplayValue('خارج 2')).toBeInTheDocument();
    expect(screen.getByDisplayValue('خارج 3')).toBeInTheDocument();
  });
});
