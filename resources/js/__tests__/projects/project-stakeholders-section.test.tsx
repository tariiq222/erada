...
vi.mock('@entities/project', () => ({
  projectsApi: {
    addStakeholder: vi.fn().mockResolvedValue({}),
    removeStakeholder: vi.fn().mockResolvedValue({}),
  },
}));
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon, onClick, variant, size }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void; variant?: string; size?: string }) => (
    <button onClick={onClick} data-variant={variant} data-size={size}>{leftIcon}{children}</button>
  ),
  ...
  DeleteConfirmationModal: ({ isOpen, item, onConfirm, onClose }: { isOpen: boolean; item: unknown; onClose: () => void; onConfirm: () => void }) => (
    isOpen && item ? (
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconUserCheck: () => <span data-testid="user-check-icon">UserCheck</span>,
  IconUserPlus: () => <span data-testid="user-plus-icon">UserPlus</span>,
  IconList: () => <span data-testid="list-icon">List</span>,
  IconLayoutGrid: () => <span data-testid="grid-icon">LayoutGrid</span>,
  ...
  IconEyeOff: () => <span data-testid="eye-off-icon">EyeOff</span>,
  ...
  IconPhone: () => <span data-testid="phone-icon">Phone</span>,
...
// Mock AddStakeholderModal and ViewEditStakeholderModal
vi.mock('@pages/projects/components/modals', () => ({
  AddStakeholderModal: ({
    ...
    isOpen ? (
      <div data-testid="add-stakeholder-modal">
        <input
  ...
  ViewEditStakeholderModal: ({
    ...
    isOpen ? (
      <div data-testid="view-edit-stakeholder-modal">
...
import StakeholdersSection from '@pages/projects/components/sections/StakeholdersSection';

const mockStakeholders = [
  { id: 1, name: 'أحمد محمد', role: 'sponsor', organization: 'الشركة أ' },
  { id: 2, name: 'سارة علي', role: 'owner', organization: 'الشركة ب' },
];

const defaultProps = {
  stakeholders: mockStakeholders,
  projectId: 1,
  onStakeholderAdded: vi.fn(),
  onStakeholderRemoved: vi.fn(),
};
...
describe('StakeholdersSection', () => {
  ...
  it('shows stakeholder count', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('2 شخص')).toBeInTheDocument();
  });
  ...
  it('displays stakeholders in table', () => {
    render(<StakeholdersSection {...defaultProps} />);
  ...
  it('displays stakeholder organizations', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('الشركة أ')).toBeInTheDocument();
    expect(screen.getByText('الشركة ب')).toBeInTheDocument();
  });
});

describe('StakeholdersSection View Toggle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('has table view toggle button', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByTestId('list-icon')).toBeInTheDocument();
  });

  it('has cards view toggle button', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByTestId('grid-icon')).toBeInTheDocument();
  });
});

describe('StakeholdersSection Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows empty state when no stakeholders', () => {
    render(<StakeholdersSection {...defaultProps} stakeholders={[]} />);
    expect(screen.getByText('لا يوجد أصحاب مصلحة')).toBeInTheDocument();
  });

  it('shows add stakeholders button in empty state', () => {
    render(<StakeholdersSection {...defaultProps} stakeholders={[]} />);
    expect(screen.getAllByText('إضافة صاحب مصلحة').length).toBeGreaterThan(0);
  });
});

describe('StakeholdersSection Add Stakeholder', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('opens add stakeholder modal', () => {
    render(<StakeholdersSection {...defaultProps} />);

    fireEvent.click(screen.getByText('إضافة صاحب مصلحة'));

    expect(screen.getByTestId('add-stakeholder-modal')).toBeInTheDocument();
  });

  it('closes add stakeholder modal', () => {
    render(<StakeholdersSection {...defaultProps} />);
    ...
    expect(screen.queryByTestId('add-stakeholder-modal')).not.toBeInTheDocument();
  });

  it('adds stakeholder successfully', async () => {
    const { projectsApi } = await import('@entities/project');
    render(<StakeholdersSection {...defaultProps} />);

    fireEvent.click(screen.getByText('إضافة صاحب مصلحة'));

    fireEvent.change(screen.getByTestId('name-input'), { target: { value: 'محمد جديد' } });
    fireEvent.change(screen.getByTestId('role-input'), { target: { value: 'sponsor' } });

    fireEvent.click(screen.getByText('إضافة'));

    await waitFor(() => {
      expect(projectsApi.addStakeholder).toHaveBeenCalledWith(1, expect.objectContaining({
        name: 'محمد جديد',
        role: 'sponsor',
      }));
    });
  ...
  it('shows success toast on add', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', 'تم إضافة صاحب المصلحة بنجاح');
    });
  });
  ...
  it('shows error when name is empty', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'يجب إدخال اسم ودور صاحب المصلحة');
    });
  });
});

describe('StakeholdersSection Remove Stakeholder', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    window.confirm = vi.fn().mockReturnValue(true);
  });

  it('removes stakeholder on confirm', async () => {
    const { projectsApi } = await import('@entities/project');
    render(<StakeholdersSection {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(projectsApi.removeStakeholder).toHaveBeenCalledWith(1, 1);
    });
  });
  ...
  it('shows success toast on remove', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', 'تم حذف صاحب المصلحة بنجاح');
    });
  });

  it('calls onStakeholderRemoved callback', async () => {
    render(<StakeholdersSection {...defaultProps} />);
    ...
    await waitFor(() => {
      expect(defaultProps.onStakeholderRemoved).toHaveBeenCalled();
    });
  });
  ...
  it('does not remove when cancelled', async () => {
    ...
    expect(projectsApi.removeStakeholder).not.toHaveBeenCalled();
  });
});

describe('StakeholdersSection Error Handling', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    window.confirm = vi.fn().mockReturnValue(true);
  });

  it('handles add stakeholder error', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.addStakeholder as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في إضافة صاحب المصلحة');
    });
  });

  it('handles remove stakeholder error', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.removeStakeholder as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في حذف صاحب المصلحة');
    });
  });
});

describe('StakeholdersSection Role Labels', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('displays role labels', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('الراعي')).toBeInTheDocument();
