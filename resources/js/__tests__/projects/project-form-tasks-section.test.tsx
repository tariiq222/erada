...
const mockMilestones = [
  { name: 'مرحلة 1', start_date: '2025-01-01', due_date: '2025-02-01' },
  { name: 'مرحلة 2', start_date: '2025-02-01', due_date: '2025-03-01' },
];
...
describe('InlineTasksEditor Basic', () => {
  const defaultProps = {
    tasks: mockTasks,
  ...
  it('renders section title', () => {
    render(<InlineTasksEditor {...defaultProps} />);
  ...
  it('shows date range info', () => {
    render(<InlineTasksEditor {...defaultProps} />);
    expect(screen.getByText('[Task Date Range]', { exact: false })).toBeInTheDocument();
  });
...
describe('InlineTasksEditor Without Dates', () => {
  ...
  it('shows warning when no dates', () => {
    render(<InlineTasksEditor {...propsWithoutDates} />);
    expect(screen.getByText('[Set Dates First Tasks]', { exact: false })).toBeInTheDocument();
  });

  it('disables add task button when no dates', () => {
    render(<InlineTasksEditor {...propsWithoutDates} />);
    expect(screen.getByText('إضافة مهمة')).toBeDisabled();
  });
});

describe('InlineTasksEditor Task Fields', () => {
  const defaultProps = {
    tasks: mockTasks,
    milestones: mockMilestones,
    users: mockUsers,
  ...
  it('shows milestone label', () => {
    render(<InlineTasksEditor {...defaultProps} />);
    expect(screen.getByText('[Milestone Optional]')).toBeInTheDocument();
  });
...
describe('InlineTasksEditor Task Interactions', () => {
  const defaultProps = {
    tasks: mockTasks,
    milestones: mockMilestones,
    users: mockUsers,
  ...
  it('calls onTaskChange when name changes', () => {
    render(<InlineTasksEditor {...defaultProps} />);
    const input = screen.getByPlaceholderText('[Task Name Placeholder]');
    fireEvent.change(input, { target: { value: 'مهمة جديدة' } });
    expect(defaultProps.onTaskChange).toHaveBeenCalledWith(0, 'name', 'مهمة جديدة');
  });
  ...
  it('calls onSelectAssignee when an assignee is chosen', () => {
    render(<InlineTasksEditor {...defaultProps} />);
    // The assignee Select renders the user names as options
    const assigneeSelect = screen.getByText('أحمد محمد').closest('select')!;
    fireEvent.change(assigneeSelect, { target: { value: '1' } });
    expect(defaultProps.onSelectAssignee).toHaveBeenCalledWith(0, 1);
  });
});

describe('InlineTasksEditor Multiple Tasks', () => {
  const multipleTasks = [
    { name: 'مهمة 1', description: '', priority: 'normal', milestone_index: undefined, assigned_to: undefined, assigned_to_name: '', start_date: '', due_date: '' },
    { name: 'مهمة 2', description: '', priority: 'high', milestone_index: undefined, assigned_to: undefined, assigned_to_name: '', start_date: '', due_date: '' },
  ];
  ...
  it('renders multiple tasks', () => {
    render(<InlineTasksEditor {...propsWithMultipleTasks} />);
  ...
  it('calls onRemoveTask when delete clicked', () => {
    ...
    expect(propsWithMultipleTasks.onRemoveTask).toHaveBeenCalledWith(0);
  });
});

describe('InlineTasksEditor Single Task', () => {
  const singleTask = [
    { name: 'مهمة واحدة', description: '', priority: 'normal', milestone_index: undefined, assigned_to: undefined, assigned_to_name: '', start_date: '', due_date: '' },
  ];
  ...
  it('does not show delete button for single task', () => {
    render(<InlineTasksEditor {...propsWithSingleTask} />);
...
describe('InlineTasksEditor Assigned User Display', () => {
  ...
  it('shows assigned user name', () => {
    render(<InlineTasksEditor {...propsWithAssignee} />);
