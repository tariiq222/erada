...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'common.warning': 'تنبيه', 'common.info': 'معلومة', 'common.export': 'تصدير', 'common.import': 'استيراد',
    ...
    'projects.cannot_move_task_to': 'لا يمكن نقل المهمة إلى', 'projects.because_incomplete_subtasks': 'بسبب وجود {{count}} مهمة فرعية غير مكتملة', 'projects.remaining_subtasks': 'المهام الفرعية المتبقية', 'projects.and_more_tasks': 'و {{count}} مهام أخرى',
...
import ErrorModal from '@pages/projects/components/modals/ErrorModal';
import IncompleteSubtasksModal from '@pages/projects/components/modals/IncompleteSubtasksModal';
...
describe('ErrorModal', () => {
  ...
  it('does not render when closed', () => {
    ...
    expect(screen.queryByText('غير مسموح')).not.toBeInTheDocument();
  });

  it('displays error message', () => {
    render(<ErrorModal {...defaultProps} />);
    expect(screen.getByText('رسالة خطأ اختبارية')).toBeInTheDocument();
  });

  it('shows x-circle icon', () => {
    render(<ErrorModal {...defaultProps} />);
  ...
  it('calls onClose when button clicked', () => {
    render(<ErrorModal {...defaultProps} />);
    fireEvent.click(screen.getByText('حسناً'));
...
// IncompleteSubtasksModal Tests
describe('IncompleteSubtasksModal', () => {
  const mockSubtasks = [
    { id: 1, title: 'مهمة فرعية 1', status: 'todo' },
    { id: 2, title: 'مهمة فرعية 2', status: 'in_progress' },
    { id: 3, title: 'مهمة فرعية 3', status: 'in_review' },
  ];

  const defaultProps = {
    isOpen: true,
    onClose: vi.fn(),
    taskTitle: 'المهمة الرئيسية',
    incompleteSubtasks: mockSubtasks,
    targetStatus: 'مكتمل',
  };
  ...
  it('renders when open', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText('تنبيه')).toBeInTheDocument();
  });

  it('does not render when closed', () => {
    render(<IncompleteSubtasksModal {...defaultProps} isOpen={false} />);
  ...
  it('displays incomplete subtasks count', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText(/بسبب وجود 3/, { exact: false })).toBeInTheDocument();
  });

  it('displays target status', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText('مكتمل')).toBeInTheDocument();
  });

  it('lists incomplete subtasks', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText('مهمة فرعية 1')).toBeInTheDocument();
    expect(screen.getByText('مهمة فرعية 2')).toBeInTheDocument();
    expect(screen.getByText('مهمة فرعية 3')).toBeInTheDocument();
  });
  ...
  it('calls onClose when button clicked', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    fireEvent.click(screen.getByText('فهمت'));
    expect(defaultProps.onClose).toHaveBeenCalled();
  });
  ...
  it('shows description about incomplete subtasks', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText('توجد مهام فرعية غير مكتملة')).toBeInTheDocument();
  });

  it('shows label for subtasks list', () => {
    render(<IncompleteSubtasksModal {...defaultProps} />);
    expect(screen.getByText('المهام الفرعية المتبقية:')).toBeInTheDocument();
  });
});

describe('IncompleteSubtasksModal with many subtasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows truncation message for more than 5 subtasks', () => {
    const manySubtasks = Array.from({ length: 8 }, (_, i) => ({
      id: i + 1,
      title: `مهمة فرعية ${i + 1}`,
      status: 'todo',
    }));
    ...
    render(
      <IncompleteSubtasksModal
        ...
        onClose={vi.fn()}
        taskTitle="المهمة"
        incompleteSubtasks={manySubtasks}
        targetStatus="مكتمل"
      />
    );

    expect(screen.getByText(/و 3 مهام أخرى/)).toBeInTheDocument();
  });

  it('shows only first 5 subtasks', () => {
    const manySubtasks = Array.from({ length: 8 }, (_, i) => ({
      id: i + 1,
      title: `مهمة فرعية ${i + 1}`,
      status: 'todo',
    ...
    expect(screen.getByText('مهمة فرعية 1')).toBeInTheDocument();
    expect(screen.getByText('مهمة فرعية 5')).toBeInTheDocument();

    // 6th and beyond should not be visible
    expect(screen.queryByText('مهمة فرعية 6')).not.toBeInTheDocument();
  });
});

describe('IncompleteSubtasksModal status colors', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders subtasks with different statuses', () => {
    const subtasksWithStatuses = [
      { id: 1, title: 'قيد الانتظار', status: 'todo' },
      { id: 2, title: 'قيد التنفيذ', status: 'in_progress' },
      { id: 3, title: 'قيد المراجعة', status: 'in_review' },
    ];
    ...
    render(
      <IncompleteSubtasksModal
        ...
        onClose={vi.fn()}
        taskTitle="المهمة"
        incompleteSubtasks={subtasksWithStatuses}
        targetStatus="مكتمل"
      />
    );
