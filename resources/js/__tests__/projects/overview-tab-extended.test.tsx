...
vi.mock('@shared/lib/utils', async (importOriginal) => ({
  ...
  formatDate: (date: any) => '2025-01-15',
}));
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconTrendingUp: () => <span data-testid="trending-icon">Trending</span>,
  ...
  IconCircleCheck: () => <span data-testid="check-icon">Check</span>,
...
const mockProject = {
  id: 1,
  name: 'مشروع اختبار',
  description: 'وصف المشروع التفصيلي',
  objectives: ['هدف 1', 'هدف 2', 'هدف 3'],
  in_scope: ['ضمن النطاق 1', 'ضمن النطاق 2'],
  out_of_scope: ['خارج النطاق 1'],
  kpis: [
    { id: 1, indicator: 'مؤشر 1', current_value: 75, target: 100 },
    { id: 2, indicator: 'مؤشر 2', current_value: 50, target: 80 },
    { id: 3, indicator: 'مؤشر 3', current_value: 30, target: 60 },
    { id: 4, indicator: 'مؤشر 4', current_value: 10, target: 100 },
  ],
  risks: [
    { id: 1, risk: 'خطر 1', probability: 'high' },
    { id: 2, risk: 'خطر 2', probability: 'medium' },
    { id: 3, risk: 'خطر 3', probability: 'low' },
    { id: 4, risk: 'خطر 4', probability: 'high' },
  ],
  milestones: [
    { id: 1, name: 'مرحلة 1', status: 'completed', progress: 100, due_date: '2025-01-01' },
    { id: 2, name: 'مرحلة 2', status: 'in_progress', progress: 50, due_date: '2099-01-01' },
    { id: 3, name: 'مرحلة 3', status: 'pending', progress: 0, due_date: '2020-01-01' },
  ],
  members: [
    { id: 1, name: 'أحمد' },
    { id: 2, name: 'محمد' },
  ...
  stakeholders: [
    { id: 1, name: 'عبدالله', role: 'راعي' },
    { id: 2, name: 'سلطان', role: 'مدير' },
    { id: 3, name: 'ناصر', role: 'مستشار' },
    { id: 4, name: 'عمر', role: 'مراجع' },
    { id: 5, name: 'زياد', role: 'مستفيد' },
    { id: 6, name: 'ماجد', role: 'مستفيد' },
    { id: 7, name: 'حسن', role: 'مستفيد' },
  ],
};

describe('OverviewTab Header', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders overview header', () => {
    ...
    expect(screen.getByText('وصف المشروع التفصيلي')).toBeInTheDocument();
  });
...
describe('OverviewTab Description', () => {
  ...
  it('hides description when not present', () => {
    ...
    expect(screen.queryByText('وصف المشروع التفصيلي')).not.toBeInTheDocument();
  });
});
...
describe('OverviewTab Objectives', () => {
  ...
  it('shows objectives header with count', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('الأهداف (3)')).toBeInTheDocument();
  });

  it('shows all objectives', () => {
    render(<OverviewTab project={mockProject as any} />);
  ...
  it('hides objectives when empty', () => {
...
describe('OverviewTab Scope', () => {
  ...
  it('shows in-scope header with count', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('ضمن النطاق (2)')).toBeInTheDocument();
  });

  it('shows out-of-scope header with count', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('خارج النطاق (1)')).toBeInTheDocument();
  });

  it('shows in-scope items', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('ضمن النطاق 1')).toBeInTheDocument();
  });

  it('shows out-of-scope items', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('خارج النطاق 1')).toBeInTheDocument();
  });
});

describe('OverviewTab Milestones', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows milestones header with count', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('المراحل (3)')).toBeInTheDocument();
  });

  it('shows milestone names', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('مرحلة 1')).toBeInTheDocument();
    expect(screen.getByText('مرحلة 2')).toBeInTheDocument();
    expect(screen.getByText('مرحلة 3')).toBeInTheDocument();
  });
  ...
  it('shows overdue indicator for past due milestones', () => {
    render(<OverviewTab project={mockProject as any} />);
...
describe('OverviewTab Empty States', () => {
  ...
  it('handles empty project gracefully', () => {
    ...
    render(<OverviewTab project={emptyProject as any} />);
    expect(screen.getByText('وصف المشروع التفصيلي')).toBeInTheDocument();
  });
});
