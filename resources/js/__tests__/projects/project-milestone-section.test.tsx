...
vi.mock('@shared/ui/Select', () => ({
  Select: ({ value, onChange, options, disabled }: {
    ...
    <select value={value} onChange={onChange} disabled={disabled} data-testid="select">
      {options.map((opt) => (
...
import ProjectMilestoneSection from '@pages/tasks/form/ProjectMilestoneSection';

const mockProjects = [
  { id: 1, code: 'PRJ-001', name: 'مشروع التطوير' },
  { id: 2, code: 'PRJ-002', name: 'مشروع التحسين' },
];

const mockMilestones = [
  { id: 1, name: 'مرحلة التحليل' },
  { id: 2, name: 'مرحلة التنفيذ' },
];

const mockParentTasks = [
  { id: 1, title: 'المهمة الرئيسية' },
  { id: 2, title: 'مهمة أخرى' },
];
...
describe('ProjectMilestoneSection Basic', () => {
  const defaultProps = {
    ...
    errors: {},
    onChange: vi.fn(),
    onOpenMilestoneModal: vi.fn(),
    onOpenUserModal: vi.fn(),
  };
  ...
  it('renders section title', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('المشروع والمرحلة')).toBeInTheDocument();
  });
  ...
  it('renders project label with required indicator', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
  ...
  it('renders parent task label', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('المهمة الأصلية')).toBeInTheDocument();
  });

  it('renders assignee label', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('المكلف')).toBeInTheDocument();
  });
});

describe('ProjectMilestoneSection Project Select', () => {
  const defaultProps = {
    formData: mockFormData,
    projects: mockProjects,
    milestones: mockMilestones,
  ...
  it('shows default project option', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('اختر المشروع')).toBeInTheDocument();
  });

  it('shows project options with code and name', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('PRJ-001 - مشروع التطوير')).toBeInTheDocument();
    expect(screen.getByText('PRJ-002 - مشروع التحسين')).toBeInTheDocument();
  });
  ...
  it('calls onChange when project changes', () => {
    ...
    expect(defaultProps.onChange).toHaveBeenCalledWith('project_id', '1');
  });

  it('shows validation error for project', () => {
    render(<ProjectMilestoneSection {...defaultProps} errors={{ project_id: ['المشروع مطلوب'] }} />);
    expect(screen.getByText('المشروع مطلوب')).toBeInTheDocument();
  });
});

describe('ProjectMilestoneSection Milestone Select', () => {
  const defaultProps = {
    formData: mockFormData,
    projects: mockProjects,
    milestones: mockMilestones,
  ...
  it('shows default milestone option', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('اختر المرحلة')).toBeInTheDocument();
  });

  it('shows milestone options', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('مرحلة التحليل')).toBeInTheDocument();
    expect(screen.getByText('مرحلة التنفيذ')).toBeInTheDocument();
  });

  it('calls onChange when milestone changes', () => {
    render(<ProjectMilestoneSection {...defaultProps} formData={{ ...mockFormData, project_id: '1' }} />);
    const selects = screen.getAllByTestId('select');
    fireEvent.change(selects[1], { target: { value: '1' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('milestone_id', '1');
  });

  it('disables milestone select when no project selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    const selects = screen.getAllByTestId('select');
    expect(selects[1]).toBeDisabled();
  });

  it('enables milestone select when project is selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} formData={{ ...mockFormData, project_id: '1' }} />);
    const selects = screen.getAllByTestId('select');
...
describe('ProjectMilestoneSection Add Milestone Button', () => {
  const defaultProps = {
    formData: mockFormData,
    projects: mockProjects,
    milestones: mockMilestones,
  ...
  it('disables add milestone button when no project selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    const buttons = screen.getAllByTestId('button');
    expect(buttons[0]).toBeDisabled();
  });

  it('enables add milestone button when project is selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} formData={{ ...mockFormData, project_id: '1' }} />);
    const buttons = screen.getAllByTestId('button');
    expect(buttons[0]).not.toBeDisabled();
  });

  it('calls onOpenMilestoneModal when clicked', () => {
    render(<ProjectMilestoneSection {...defaultProps} formData={{ ...mockFormData, project_id: '1' }} />);
    const buttons = screen.getAllByTestId('button');
    fireEvent.click(buttons[0]);
    expect(defaultProps.onOpenMilestoneModal).toHaveBeenCalled();
  });
});

describe('ProjectMilestoneSection Parent Task Select', () => {
  const defaultProps = {
    formData: mockFormData,
    projects: mockProjects,
    milestones: mockMilestones,
  ...
  it('shows default parent task option', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('لا يوجد')).toBeInTheDocument();
  });

  it('shows parent task options', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('المهمة الرئيسية')).toBeInTheDocument();
    expect(screen.getByText('مهمة أخرى')).toBeInTheDocument();
  });

  it('calls onChange when parent task changes', () => {
    render(<ProjectMilestoneSection {...defaultProps} formData={{ ...mockFormData, project_id: '1' }} />);
    const selects = screen.getAllByTestId('select');
    fireEvent.change(selects[2], { target: { value: '1' } });
    expect(defaultProps.onChange).toHaveBeenCalledWith('parent_id', '1');
  });

  it('disables parent task select when no project selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    const selects = screen.getAllByTestId('select');
...
describe('ProjectMilestoneSection User Selection', () => {
  const defaultProps = {
    formData: mockFormData,
    projects: mockProjects,
    milestones: mockMilestones,
  ...
  it('shows placeholder when no user selected', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    expect(screen.getByText('اختر المكلف')).toBeInTheDocument();
  });

  it('shows selected user name', () => {
    render(<ProjectMilestoneSection {...defaultProps} selectedUser={{ id: 1, name: 'أحمد محمد' }} />);
    expect(screen.getByText('أحمد محمد')).toBeInTheDocument();
  });

  it('calls onOpenUserModal when user button clicked', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
    const userButton = screen.getByText('اختر المكلف').closest('button');
    fireEvent.click(userButton!);
    expect(defaultProps.onOpenUserModal).toHaveBeenCalled();
  });
  ...
  it('renders plus icon for user selection', () => {
    render(<ProjectMilestoneSection {...defaultProps} />);
