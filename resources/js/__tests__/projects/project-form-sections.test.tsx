...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'projects.out_of_scope': 'خارج النطاق', 'projects.milestones': 'المراحل الرئيسية', 'projects.tasks': 'المهام', 'projects.team': 'فريق العمل',
    ...
    'projects.no_department': 'بدون قسم', 'projects.leader': 'قائد', 'projects.not_assigned': 'غير محدد', 'projects.supervisor': 'مشرف المشروع',
    ...
    'profile.subtitle': 'إدارة معلوماتك الشخصية وكلمة المرور', 'profile.role_super_admin': 'مدير النظام', 'profile.role_admin': 'مدير إدارة', 'profile.role_project_manager': 'مدير مشروع', 'profile.role_team_member': 'عضو فريق', 'profile.role_viewer': 'مشاهد', 'projects.add_deliverable': 'إضافة مخرج', 'projects.assignee_optional': '[Assignee Optional]', 'projects.budget_sar': '[Budget Sar]',
    'projects.core_members': '[Core Members]', 'projects.core_stakeholders': '[Core Stakeholders]', 'projects.deliverable_name': '[Deliverable Name]', 'projects.deliverables': 'المخرجات',
    'projects.department': '[Department]', 'projects.description': '[Description]', 'projects.description_optional': 'الوصف (اختياري)', 'projects.end_date': 'تاريخ التسليم',
    ...
    'projects.enter_project_name': '[Enter Project Name]', 'projects.enter_risk_description': 'أدخل وصف الخطر...', 'projects.financial_resources': '[Financial Resources]', 'projects.financial_resources_placeholder': '[Financial Resources Placeholder]',
    ...
    'projects.kpi_target_value': 'القيمة المستهدفة', 'projects.link_to_program_hint': '[Link To Program Hint]', 'projects.manager': 'مدير المشروع', 'projects.milestone_date_range': 'نطاق التواريخ المسموح للمراحل',
    'projects.milestone_description_placeholder': '[Milestone Description Placeholder]', 'projects.milestone_name_placeholder': '[Milestone Name Placeholder]', 'projects.milestone_optional': '[Milestone Optional]', 'projects.mitigation_plan': 'خطة التخفيف',
    ...
    'projects.select_sponsor': '[Select Sponsor]', 'projects.select_stakeholder': '-- اختر صاحب المصلحة --', 'projects.select_start_date': '[Select Start Date]', 'projects.select_supervisor': '[Select Supervisor]',
    'projects.select_team_member': '-- اختر عضو الفريق --', 'projects.set_dates_first_milestones': 'يرجى تحديد تاريخ بداية ونهاية المشروع', 'projects.set_dates_first_tasks': '[Set Dates First Tasks]', 'projects.settings_and_timeline': '[Settings And Timeline]',
    'projects.sponsor': 'راعي المشروع', 'projects.stakeholders_description': 'أصحاب المصلحة هم الأفراد أو الجهات', 'projects.standalone_project': '[Standalone Project]', 'projects.start_date': 'تاريخ البداية',
    ...
    'projects.task_name_placeholder': '[Task Name Placeholder]', 'projects.team_description': 'فريق العمل هم الأشخاص المسؤولون عن تنفيذ المشروع', 'projects.technical_resources': '[Technical Resources]', 'projects.technical_resources_placeholder': '[Technical Resources Placeholder]',
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, disabled, leftIcon, type, variant, size, className }: any) => (
...
vi.mock('@shared/ui/Input', () => ({
  Input: ({ value, onChange, placeholder, className, disabled }: any) => (
    <input
      value={value}
      onChange={onChange}
...
vi.mock('@shared/ui/Select', () => ({
  Select: ({ value, onChange, options, className, disabled }: any) => (
...
vi.mock('@shared/ui/DatePicker', () => ({
  DatePicker: ({ value, onChange, placeholder, disabled, minDate, maxDate }: any) => (
    <input
      type="date"
      value={value}
...
// Import components
import RisksSection from '@pages/projects/form/RisksSection';
import MilestonesSection from '@pages/projects/form/MilestonesSection';

// ==================== RisksSection Tests ====================

describe('RisksSection Component', () => {
  const mockRisks = [
    {
      description: 'خطر تأخير التسليم',
      impact: 'high',
      probability: 'medium',
      mitigation: 'متابعة يومية',
    },
  ];
  ...
  it('renders section title', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('المخاطر')).toBeInTheDocument();
  });

  it('renders risk description input', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByDisplayValue('خطر تأخير التسليم')).toBeInTheDocument();
  });

  it('renders risk number label', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('خطر 1')).toBeInTheDocument();
  });

  it('renders impact select', () => {
    render(<RisksSection {...defaultProps} />);
  ...
  it('renders probability select', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('الاحتمالية')).toBeInTheDocument();
  });

  it('renders mitigation input', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('خطة التخفيف')).toBeInTheDocument();
    expect(screen.getByDisplayValue('متابعة يومية')).toBeInTheDocument();
  });

  it('renders add risk button', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('إضافة خطر')).toBeInTheDocument();
  });

  it('calls onAddRisk when clicking add button', async () => {
    render(<RisksSection {...defaultProps} />);
    await userEvent.click(screen.getByText('إضافة خطر'));
    expect(defaultProps.onAddRisk).toHaveBeenCalled();
  });

  it('calls onRiskChange when editing description', async () => {
    render(<RisksSection {...defaultProps} />);
    const input = screen.getByDisplayValue('خطر تأخير التسليم');
    await userEvent.clear(input);
    await userEvent.type(input, 'خطر جديد');
    expect(defaultProps.onRiskChange).toHaveBeenCalled();
  });

  it('does not show delete button for single risk', () => {
    render(<RisksSection {...defaultProps} />);
    // With only one risk, delete button should not appear
    const trashButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    expect(trashButtons.length).toBe(0);
  });
});

describe('RisksSection Multiple Risks', () => {
  const mockRisks = [
    {
      description: 'خطر 1',
      impact: 'high',
  ...
  it('renders multiple risk cards', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getByText('خطر 1')).toBeInTheDocument();
    expect(screen.getByText('خطر 2')).toBeInTheDocument();
  });

  it('shows delete buttons when multiple risks', () => {
    render(<RisksSection {...defaultProps} />);
    // With multiple risks, delete buttons should appear
    const deleteButtons = document.querySelectorAll('button');
    // Should have delete buttons + add button
    expect(deleteButtons.length).toBeGreaterThan(1);
  });

  it('calls onRemoveRisk when clicking delete', async () => {
    render(<RisksSection {...defaultProps} />);
    // Find the first trash button
    const deleteButton = document.querySelector('button.text-\\[var\\(--status-danger\\)\\]');
    if (deleteButton) {
      await userEvent.click(deleteButton);
      expect(defaultProps.onRemoveRisk).toHaveBeenCalledWith(0);
    }
  });
});

// ==================== MilestonesSection Tests ====================

describe('MilestonesSection Component', () => {
  const mockMilestones = [
    {
      ...
      deliverables: [{ name: 'وثيقة المتطلبات' }],
    },
  ];
  ...
  const defaultProps = {
    ...
    onDeliverableChange: vi.fn(),
    onAddDeliverable: vi.fn(),
    onRemoveDeliverable: vi.fn(),
  };
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('المراحل الرئيسية')).toBeInTheDocument();
  });

  it('renders milestone name input', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByDisplayValue('مرحلة التحليل')).toBeInTheDocument();
  });

  it('renders date range info when dates provided', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText(/نطاق التواريخ المسموح للمراحل/)).toBeInTheDocument();
    expect(screen.getByText('2025-01-01')).toBeInTheDocument();
    expect(screen.getByText('2025-12-31')).toBeInTheDocument();
  });

  it('renders milestone number badge', () => {
    render(<MilestonesSection {...defaultProps} />);
    // Should show number 1 in badge
    const badge = document.querySelector('.rounded-full.bg-\\[var\\(--accent-default\\)\\]');
  ...
  it('renders due date picker', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('تاريخ التسليم')).toBeInTheDocument();
  });

  it('renders description input', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('الوصف (اختياري)')).toBeInTheDocument();
    expect(screen.getByDisplayValue('تحليل المتطلبات')).toBeInTheDocument();
  });

  it('renders deliverables section', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('المخرجات')).toBeInTheDocument();
    expect(screen.getByDisplayValue('وثيقة المتطلبات')).toBeInTheDocument();
  });

  it('renders add milestone button', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('إضافة مرحلة')).toBeInTheDocument();
  });

  it('renders add deliverable button', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('إضافة مخرج')).toBeInTheDocument();
  });

  it('calls onAddMilestone when clicking add milestone', async () => {
    render(<MilestonesSection {...defaultProps} />);
    await userEvent.click(screen.getByText('إضافة مرحلة'));
    expect(defaultProps.onAddMilestone).toHaveBeenCalled();
  });

  it('calls onAddDeliverable when clicking add deliverable', async () => {
    render(<MilestonesSection {...defaultProps} />);
    await userEvent.click(screen.getByText('إضافة مخرج'));
    expect(defaultProps.onAddDeliverable).toHaveBeenCalledWith(0);
  });

  it('calls onMilestoneChange when editing name', async () => {
    render(<MilestonesSection {...defaultProps} />);
    const nameInput = screen.getByDisplayValue('مرحلة التحليل');
    await userEvent.clear(nameInput);
    await userEvent.type(nameInput, 'مرحلة جديدة');
    expect(defaultProps.onMilestoneChange).toHaveBeenCalled();
  });

  it('calls onDeliverableChange when editing deliverable', async () => {
    render(<MilestonesSection {...defaultProps} />);
    const deliverableInput = screen.getByDisplayValue('وثيقة المتطلبات');
    await userEvent.clear(deliverableInput);
    await userEvent.type(deliverableInput, 'وثيقة جديدة');
    expect(defaultProps.onDeliverableChange).toHaveBeenCalled();
  });
});

describe('MilestonesSection Without Dates', () => {
  const mockMilestones = [
    {
      name: '',
      start_date: '',
  ...
  it('shows warning when no project dates', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText(/يرجى تحديد تاريخ بداية ونهاية المشروع/)).toBeInTheDocument();
  });

  it('disables add milestone button when no dates', () => {
    render(<MilestonesSection {...defaultProps} />);
    const addButton = screen.getByText('إضافة مرحلة');
    expect(addButton).toBeDisabled();
  });

  it('disables date pickers when no project dates', () => {
    render(<MilestonesSection {...defaultProps} />);
    const dateInputs = document.querySelectorAll('input[type="date"]');
...
describe('MilestonesSection Multiple Milestones', () => {
  const mockMilestones = [
    {
      name: 'مرحلة 1',
      start_date: '2025-01-01',
  ...
  it('renders multiple milestone cards', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByDisplayValue('مرحلة 1')).toBeInTheDocument();
    expect(screen.getByDisplayValue('مرحلة 2')).toBeInTheDocument();
  });

  it('renders number badges for each milestone', () => {
    render(<MilestonesSection {...defaultProps} />);
    const badges = document.querySelectorAll('.rounded-full.bg-\\[var\\(--accent-default\\)\\]');
    expect(badges.length).toBe(2);
  });

  it('shows delete buttons for multiple milestones', () => {
    render(<MilestonesSection {...defaultProps} />);
    // With multiple milestones, delete buttons should appear
    const deleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    // Should have 2 milestone delete buttons + 0 deliverable delete (single each)
    expect(deleteButtons.length).toBeGreaterThanOrEqual(2);
  });

  it('calls onRemoveMilestone with correct index', async () => {
    render(<MilestonesSection {...defaultProps} />);
    // Find milestone delete buttons (not deliverable ones)
    const milestoneCards = document.querySelectorAll('.p-4.border');
    const firstMilestoneDeleteBtn = milestoneCards[0]?.querySelector('button.text-\\[var\\(--status-danger\\)\\]');

    if (firstMilestoneDeleteBtn) {
      await userEvent.click(firstMilestoneDeleteBtn);
      expect(defaultProps.onRemoveMilestone).toHaveBeenCalledWith(0);
    }
  });
});

describe('MilestonesSection Multiple Deliverables', () => {
  const mockMilestones = [
    {
      name: 'مرحلة 1',
      start_date: '2025-01-01',
  ...
  it('renders all deliverables', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByDisplayValue('مخرج 1')).toBeInTheDocument();
    expect(screen.getByDisplayValue('مخرج 2')).toBeInTheDocument();
    expect(screen.getByDisplayValue('مخرج 3')).toBeInTheDocument();
  });

  it('shows deliverable numbers', () => {
    render(<MilestonesSection {...defaultProps} />);
  ...
  it('shows delete buttons for multiple deliverables', () => {
    render(<MilestonesSection {...defaultProps} />);
    // Should have delete buttons for deliverables (3) but not milestone (only 1)
    const allDeleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    expect(allDeleteButtons.length).toBe(3); // 3 deliverable deletes, no milestone delete
  });

  it('calls onRemoveDeliverable with correct indices', async () => {
    render(<MilestonesSection {...defaultProps} />);
    // Find the second deliverable delete button
    const deleteButtons = document.querySelectorAll('.flex.items-center.gap-2.p-2 button');
    if (deleteButtons.length >= 2) {
      await userEvent.click(deleteButtons[1]);
      expect(defaultProps.onRemoveDeliverable).toHaveBeenCalledWith(0, 1);
    }
  });
});

// Import more components
import TeamSection from '@pages/projects/form/TeamSection';
import StakeholdersSection from '@pages/projects/form/StakeholdersSection';
...
describe('TeamSection Component', () => {
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('فريق العمل')).toBeInTheDocument();
  });

  it('renders section description', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.getByText(/فريق العمل هم الأشخاص المسؤولون عن تنفيذ المشروع/)).toBeInTheDocument();
  });

  it('renders team member card', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.getByText('عضو فريق 1')).toBeInTheDocument();
  });
  ...
  it('renders add team member button', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.getByText('إضافة عضو للفريق')).toBeInTheDocument();
  });

  it('calls onAddTeamMember when clicking add button', async () => {
    render(<TeamSection {...defaultProps} />);
    await userEvent.click(screen.getByText('إضافة عضو للفريق'));
    expect(defaultProps.onAddTeamMember).toHaveBeenCalled();
  });

  it('calls onOpenTeamMemberModal when clicking member button', async () => {
    render(<TeamSection {...defaultProps} />);
    const memberButton = screen.getByText('أحمد محمد').closest('button');
    if (memberButton) {
      await userEvent.click(memberButton);
      expect(defaultProps.onOpenTeamMemberModal).toHaveBeenCalledWith(0);
    }
  });
});

describe('TeamSection Without Manager/Supervisor', () => {
  const mockTeamMembers = [
    {
      user_id: undefined,
      name: '',
  ...
  it('does not show basic members section when no manager/supervisor', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.queryByText('أعضاء أساسيون')).not.toBeInTheDocument();
  });

  it('renders the team member select with user options', () => {
    render(<TeamSection {...defaultProps} />);
    // The team member is chosen via a Select listing the available users
    expect(screen.getByText('أحمد')).toBeInTheDocument();
  });
});

describe('TeamSection Multiple Members', () => {
  const mockTeamMembers = [
    { user_id: 1, name: 'أحمد', role: 'مطور' },
    { user_id: 2, name: 'محمد', role: 'محلل' },
  ];
  ...
  it('renders multiple team member cards', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.getByText('عضو فريق 1')).toBeInTheDocument();
    expect(screen.getByText('عضو فريق 2')).toBeInTheDocument();
  });

  it('shows delete buttons for multiple members', () => {
    render(<TeamSection {...defaultProps} />);
    const deleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    expect(deleteButtons.length).toBe(2);
  });

  it('calls onRemoveTeamMember with correct index', async () => {
    render(<TeamSection {...defaultProps} />);
    const deleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    if (deleteButtons.length > 0) {
      await userEvent.click(deleteButtons[0]);
      expect(defaultProps.onRemoveTeamMember).toHaveBeenCalledWith(0);
    }
  });
});

// ==================== StakeholdersSection Tests ====================

describe('StakeholdersSection Component', () => {
  const mockStakeholders = [
    {
      user_id: 1,
      name: 'أحمد',
      role: 'end_user',
      contact: 'ahmed@test.com',
      influence: 'high',
    },
  ];
  ...
  const defaultProps = {
    ...
    managerId: '3',
    supervisorId: '4',
    onStakeholderChange: vi.fn(),
    onAddStakeholder: vi.fn(),
    onRemoveStakeholder: vi.fn(),
    onOpenStakeholderModal: vi.fn(),
  };
  ...
  it('renders section title', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('أصحاب المصلحة')).toBeInTheDocument();
  });

  it('renders section description', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText(/أصحاب المصلحة هم الأفراد أو الجهات/)).toBeInTheDocument();
  });

  it('renders stakeholder card', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('صاحب مصلحة 1')).toBeInTheDocument();
  });
  ...
  it('renders user, role and influence selects', () => {
    render(<StakeholdersSection {...defaultProps} />);
    const selects = document.querySelectorAll('select');
    expect(selects.length).toBe(3); // user, role and influence
  });

  it('renders add stakeholder button', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('إضافة صاحب مصلحة')).toBeInTheDocument();
  });

  it('calls onAddStakeholder when clicking add button', async () => {
    render(<StakeholdersSection {...defaultProps} />);
    await userEvent.click(screen.getByText('إضافة صاحب مصلحة'));
    expect(defaultProps.onAddStakeholder).toHaveBeenCalled();
  });

  it('calls onOpenStakeholderModal when clicking stakeholder button', async () => {
    render(<StakeholdersSection {...defaultProps} />);
    const stakeholderButton = screen.getByText('أحمد').closest('button');
    if (stakeholderButton) {
      await userEvent.click(stakeholderButton);
      expect(defaultProps.onOpenStakeholderModal).toHaveBeenCalledWith(0);
    }
  });

  it('calls onStakeholderChange when changing role', async () => {
    render(<StakeholdersSection {...defaultProps} />);
    const selects = document.querySelectorAll('select');
    // selects[0] is the user select; the role select is the second one
    await userEvent.selectOptions(selects[1], 'consultant');
    expect(defaultProps.onStakeholderChange).toHaveBeenCalledWith(0, 'role', 'consultant');
  });
});

describe('StakeholdersSection Without Basic Stakeholders', () => {
  const mockStakeholders = [
    {
      user_id: undefined,
      name: '',
  ...
  it('does not show basic stakeholders section', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.queryByText('أصحاب مصلحة أساسيون')).not.toBeInTheDocument();
  });

  it('renders the user, role and influence selects when stakeholder not selected', () => {
    render(<StakeholdersSection {...defaultProps} />);
    const selects = document.querySelectorAll('select');
    expect(selects.length).toBe(3); // user, role and influence
  });
...
describe('StakeholdersSection Multiple Stakeholders', () => {
  const mockStakeholders = [
    { user_id: 1, name: 'أحمد', role: 'end_user', contact: '', influence: 'high' },
    { user_id: 2, name: 'محمد', role: 'consultant', contact: '', influence: 'medium' },
  ];
  ...
  it('renders multiple stakeholder cards', () => {
    render(<StakeholdersSection {...defaultProps} />);
    expect(screen.getByText('صاحب مصلحة 1')).toBeInTheDocument();
    expect(screen.getByText('صاحب مصلحة 2')).toBeInTheDocument();
  });

  it('shows delete buttons for multiple stakeholders', () => {
    render(<StakeholdersSection {...defaultProps} />);
    const deleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    expect(deleteButtons.length).toBe(2);
  });

  it('calls onRemoveStakeholder with correct index', async () => {
    render(<StakeholdersSection {...defaultProps} />);
    const deleteButtons = document.querySelectorAll('button.text-\\[var\\(--status-danger\\)\\]');
    if (deleteButtons.length > 1) {
      await userEvent.click(deleteButtons[1]);
      expect(defaultProps.onRemoveStakeholder).toHaveBeenCalledWith(1);
    }
  });
});
