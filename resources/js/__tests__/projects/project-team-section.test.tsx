...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Select: ({ value, onChange, options, placeholder, 'aria-label': ariaLabel }: any) => (
    <select value={value ?? ''} onChange={onChange} aria-label={ariaLabel} data-testid="select">
  ...
  DeleteConfirmationModal: ({ isOpen, item, onConfirm, onClose }: { isOpen: boolean; item: unknown; title: string; itemName: string; warningMessage: string; isDeleting: boolean; onClose: () => void; onConfirm: () => void }) => (
    isOpen && item ? (
...
import TeamSection from '@pages/projects/components/sections/TeamSection';
...
describe('TeamSection', () => {
  ...
  it('renders section title', () => {
    ...
    expect(screen.getByText('فريق المشروع')).toBeInTheDocument();
  });

  it('shows member count', () => {
    render(<TeamSection {...defaultProps} />);
    expect(screen.getByText('2 عضو')).toBeInTheDocument();
  });
  ...
  it('displays members in table view by default', () => {
    render(<TeamSection {...defaultProps} />);
...
describe('TeamSection View Toggle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('TeamSection Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows empty state when no members', () => {
    render(<TeamSection {...defaultProps} members={[]} />);
    expect(screen.getByText('لا يوجد أعضاء في الفريق')).toBeInTheDocument();
  });

  it('shows add members button in empty state', () => {
    render(<TeamSection {...defaultProps} members={[]} />);
    expect(screen.getByText('إضافة أعضاء')).toBeInTheDocument();
  });
});

describe('TeamSection Add Member', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('opens add member modal', async () => {
    ...
    fireEvent.click(screen.getByText('إضافة عضو'));
  ...
  it('fetches available users when opening modal', async () => {
    ...
    await waitFor(() => {
      expect(usersApi.getList).toHaveBeenCalled();
    });
  });

  it('closes add member modal', async () => {
    render(<TeamSection {...defaultProps} />);
  ...
  it('adds member successfully', async () => {
    ...
    // Select user
    fireEvent.change(screen.getByTestId('user-select'), { target: { value: '3' } });
  ...
  it('shows error when no user selected', async () => {
    ...
    // Click add without selecting user
    fireEvent.click(screen.getByText('إضافة'));

    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'يرجى اختيار عضو');
    });
  });
});

describe('TeamSection Remove Member', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    window.confirm = vi.fn().mockReturnValue(true);
  });

  it('removes member on confirm', async () => {
    const { projectsApi } = await import('@entities/project');
    render(<TeamSection {...defaultProps} />);
  ...
  it('shows success toast on remove', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', 'تم إزالة العضو بنجاح');
    });
  });

  it('calls onMemberRemoved callback', async () => {
    render(<TeamSection {...defaultProps} />);
...
describe('TeamSection Error Handling', () => {
  ...
  it('handles add member error', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في إضافة العضو');
    });
  });

  it('handles remove member error', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.removeMember as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Failed'));
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في إزالة العضو');
    });
  });
});
