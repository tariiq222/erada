...
vi.mock('@entities/task', () => ({
  tasksApi: {
    ...
    getOne: vi.fn().mockResolvedValue({ data: { subtasks: [] } }),
...
vi.mock('@pages/projects/components/modals', () => ({
  StatusTransitionModal: ({ isOpen, onClose, onConfirm }: any) =>
    isOpen ? (
      <div data-testid="status-transition-modal">
        <button onClick={onClose}>إغلاق</button>
        <button onClick={() => onConfirm('تعليق')}>تأكيد</button>
      </div>
    ) : null,
  ...
  IncompleteSubtasksModal: ({ isOpen, onClose }: any) =>
    isOpen ? (
      <div data-testid="incomplete-subtasks-modal">
        <button onClick={onClose}>إغلاق</button>
      </div>
    ) : null,
}));
...
vi.mock('@pages/projects/components/sections/tasks', () => ({
  KanbanBoard: ({ tasks, onTaskClick, onDragStart, onDragEnd, onDrop, onToggleSubtasks }: any) => (
    <div data-testid="kanban-board">
      ...
      <div
        data-testid="drop-zone-completed"
  ...
  TasksListView: ({ tasks }: any) => (
    <div data-testid="tasks-list-view">
      {tasks.map((task: any) => (
        <div key={task.id} data-testid={`list-task-${task.id}`}>
          {task.title}
        </div>
      ))}
    </div>
  ),
  TasksFilters: ({ onStatusFilterChange, onPriorityFilterChange, onAssigneeFilterChange, onResetFilters }: any) => (
    <div data-testid="tasks-filters">
      ...
      <select data-testid="priority-filter" onChange={(e) => onPriorityFilterChange(e.target.value)}>
        <option value="all">الكل</option>
        <option value="high">عالية</option>
        <option value="medium">متوسطة</option>
        <option value="low">منخفضة</option>
      </select>
      <select data-testid="assignee-filter" onChange={(e) => onAssigneeFilterChange(e.target.value)}>
        <option value="all">الكل</option>
      </select>
      <button data-testid="reset-filters" onClick={onResetFilters}>إعادة تعيين</button>
    </div>
  ),
  TasksHeader: ({ viewMode, onViewModeChange, onToggleFilters, onCreateTask, taskCount }: any) => (
    <div data-testid="tasks-header">
      <span data-testid="task-count">{taskCount}</span>
      <button data-testid="toggle-kanban" onClick={() => onViewModeChange('kanban')}>كانبان</button>
      <button data-testid="toggle-list" onClick={() => onViewModeChange('list')}>قائمة</button>
      <button data-testid="toggle-filters" onClick={onToggleFilters}>فلاتر</button>
      <button data-testid="create-task" onClick={onCreateTask}>إضافة مهمة</button>
    </div>
  ),
  TasksEmptyState: ({ onCreateTask }: any) => (
    <div data-testid="tasks-empty-state">
      <p>لا توجد مهام</p>
      <button onClick={onCreateTask}>إنشاء مهمة</button>
    </div>
  ),
}));

import ProjectTasksSection from '@pages/projects/components/sections/ProjectTasksSection';
...
const mockProject = {
  ...
  end_date: '2025-12-31',
  milestones: [
    { id: 1, name: 'مرحلة 1', status: 'in_progress', start_date: '2025-01-01', due_date: '2025-06-30' },
  ],
};
...
describe('ProjectTasksSection Basic Rendering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders tasks header', () => {
    render(<ProjectTasksSection {...defaultProps} />);
  ...
  it('renders all tasks in kanban', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.getByTestId('kanban-task-1')).toBeInTheDocument();
    expect(screen.getByTestId('kanban-task-2')).toBeInTheDocument();
    expect(screen.getByTestId('kanban-task-3')).toBeInTheDocument();
  });
});

describe('ProjectTasksSection Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows empty state when no tasks', () => {
    render(<ProjectTasksSection {...defaultProps} tasks={[]} />);
...
describe('ProjectTasksSection View Mode Toggle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows tasks in list view', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('toggle-list'));
    expect(screen.getByTestId('list-task-1')).toBeInTheDocument();
    expect(screen.getByTestId('list-task-2')).toBeInTheDocument();
  });
});

describe('ProjectTasksSection Filters', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('hides filters by default', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.queryByTestId('tasks-filters')).not.toBeInTheDocument();
  });

  it('shows filters when toggle clicked', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('toggle-filters'));
    expect(screen.getByTestId('tasks-filters')).toBeInTheDocument();
  });

  it('hides filters when toggle clicked again', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('toggle-filters'));
    fireEvent.click(screen.getByTestId('toggle-filters'));
    expect(screen.queryByTestId('tasks-filters')).not.toBeInTheDocument();
  ...
  it('filters by status', () => {
    ...
    fireEvent.click(screen.getByTestId('toggle-list'));
    expect(screen.getByTestId('list-task-1')).toBeInTheDocument();
    expect(screen.queryByTestId('list-task-2')).not.toBeInTheDocument();
  });
  ...
  it('resets filters', () => {
    ...
    expect(screen.getByTestId('list-task-1')).toBeInTheDocument();
    expect(screen.getByTestId('list-task-2')).toBeInTheDocument();
    expect(screen.getByTestId('list-task-3')).toBeInTheDocument();
  });
});

describe('ProjectTasksSection Create Task Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('ProjectTasksSection Task View Modal', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('opens task view modal on task click', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('kanban-task-1'));
    expect(screen.getByTestId('task-view-modal')).toBeInTheDocument();
  });

  it('closes task view modal', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('kanban-task-1'));
    fireEvent.click(screen.getByText('إغلاق'));
    expect(screen.queryByTestId('task-view-modal')).not.toBeInTheDocument();
  });
});

describe('ProjectTasksSection Subtasks', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (tasksApi.getOne as any).mockResolvedValue({
      data: {
        subtasks: [
          { id: 10, title: 'مهمة فرعية 1', status: 'todo' },
          { id: 11, title: 'مهمة فرعية 2', status: 'completed' },
        ],
      },
    });
  });

  it('shows subtasks button for tasks with subtasks', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.getByTestId('toggle-subtasks-2')).toBeInTheDocument();
  });

  it('does not show subtasks button for tasks without subtasks', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.queryByTestId('toggle-subtasks-1')).not.toBeInTheDocument();
  });

  it('fetches subtasks on toggle click', async () => {
    render(<ProjectTasksSection {...defaultProps} />);
    fireEvent.click(screen.getByTestId('toggle-subtasks-2'));
    await waitFor(() => {
      expect(tasksApi.getOne).toHaveBeenCalledWith(2);
    });
  });
});

describe('ProjectTasksSection Drag and Drop', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('handles drag start', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    const task = screen.getByTestId('kanban-task-1');
...
describe('ProjectTasksSection Props Update', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('updates tasks when props change', () => {
    const { rerender } = render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.getByTestId('task-count')).toHaveTextContent('3');

    const newTasks = [
      { id: 4, title: 'مهمة 4', status: 'todo', priority: 'high', assignee: null, subtasks_count: 0 },
    ];
    rerender(<ProjectTasksSection {...defaultProps} tasks={newTasks} />);
    expect(screen.getByTestId('task-count')).toHaveTextContent('1');
  });
});

describe('ProjectTasksSection Multiple Filters', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('combines status and priority filters', () => {
    const tasks = [
      { id: 1, title: 'مهمة 1', status: 'todo', priority: 'high', assignee: null, subtasks_count: 0 },
      { id: 2, title: 'مهمة 2', status: 'todo', priority: 'low', assignee: null, subtasks_count: 0 },
      { id: 3, title: 'مهمة 3', status: 'in_progress', priority: 'high', assignee: null, subtasks_count: 0 },
    ];
    render(<ProjectTasksSection {...defaultProps} tasks={tasks} />);
    fireEvent.click(screen.getByTestId('toggle-filters'));
    fireEvent.change(screen.getByTestId('status-filter'), { target: { value: 'todo' } });
    fireEvent.change(screen.getByTestId('priority-filter'), { target: { value: 'high' } });
    fireEvent.click(screen.getByTestId('toggle-list'));
    expect(screen.getByTestId('list-task-1')).toBeInTheDocument();
    expect(screen.queryByTestId('list-task-2')).not.toBeInTheDocument();
    expect(screen.queryByTestId('list-task-3')).not.toBeInTheDocument();
  });
});

describe('ProjectTasksSection Assignee Filter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
