...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'projects.enter_description': '[Enter Description]', 'projects.enter_item': '[Enter Item]', 'projects.enter_mitigation_plan': 'أدخل خطة التخفيف من الخطر...', 'projects.enter_objective': '[Enter Objective]',
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, leftIcon, variant, size, type, className }: any) => (
    <button
      onClick={onClick}
...
const mockRisks = [
  { description: 'خطر تأخير التسليم', impact: 'high', probability: 'medium', mitigation: 'متابعة مستمرة' },
  { description: 'خطر نقص الموارد', impact: 'medium', probability: 'low', mitigation: 'توفير بدائل' },
];
...
describe('RisksSection Risk Items', () => {
  const defaultProps = {
    risks: mockRisks,
    onRiskChange: vi.fn(),
    onAddRisk: vi.fn(),
  ...
  it('shows risk descriptions', () => {
    ...
    expect(inputs[0]).toHaveValue('خطر تأخير التسليم');
    expect(inputs[2]).toHaveValue('خطر نقص الموارد');
  });

  it('shows description placeholder', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByPlaceholderText('أدخل وصف الخطر...').length).toBe(2);
  });

  it('shows mitigation placeholder', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByPlaceholderText('أدخل خطة التخفيف من الخطر...').length).toBe(2);
  });

  it('shows field labels', () => {
    render(<RisksSection {...defaultProps} />);
    expect(screen.getAllByText('وصف الخطر').length).toBe(2);
    expect(screen.getAllByText('التأثير').length).toBe(2);
    expect(screen.getAllByText('الاحتمالية').length).toBe(2);
    expect(screen.getAllByText('خطة التخفيف').length).toBe(2);
  });

  it('shows impact selects', () => {
    render(<RisksSection {...defaultProps} />);
    const selects = screen.getAllByTestId('select');
    expect(selects.length).toBe(4); // 2 impact + 2 probability
  });
});

describe('RisksSection Interactions', () => {
  const defaultProps = {
    risks: mockRisks,
    onRiskChange: vi.fn(),
    onAddRisk: vi.fn(),
  ...
  it('calls onRiskChange when description changes', () => {
    render(<RisksSection {...defaultProps} />);
    const inputs = screen.getAllByTestId('input');
    fireEvent.change(inputs[0], { target: { value: 'وصف جديد' } });
    expect(defaultProps.onRiskChange).toHaveBeenCalledWith(0, 'description', 'وصف جديد');
  });

  it('calls onRiskChange when impact changes', () => {
    render(<RisksSection {...defaultProps} />);
    const selects = screen.getAllByTestId('select');
    fireEvent.change(selects[0], { target: { value: 'low' } });
    expect(defaultProps.onRiskChange).toHaveBeenCalledWith(0, 'impact', 'low');
  });

  it('calls onRiskChange when probability changes', () => {
    render(<RisksSection {...defaultProps} />);
    const selects = screen.getAllByTestId('select');
    fireEvent.change(selects[1], { target: { value: 'high' } });
    expect(defaultProps.onRiskChange).toHaveBeenCalledWith(0, 'probability', 'high');
  });

  it('calls onRiskChange when mitigation changes', () => {
    render(<RisksSection {...defaultProps} />);
    const inputs = screen.getAllByTestId('input');
    fireEvent.change(inputs[1], { target: { value: 'خطة جديدة' } });
    expect(defaultProps.onRiskChange).toHaveBeenCalledWith(0, 'mitigation', 'خطة جديدة');
  });
});

describe('RisksSection Add/Remove', () => {
  const defaultProps = {
    risks: mockRisks,
    onRiskChange: vi.fn(),
    onAddRisk: vi.fn(),
...
describe('RisksSection Single Risk', () => {
  const singleRiskProps = {
    risks: [{ description: 'خطر واحد', impact: 'low', probability: 'low', mitigation: '' }],
    onRiskChange: vi.fn(),
    onAddRisk: vi.fn(),
    onRemoveRisk: vi.fn(),
  };
  ...
  it('hides remove button for single risk', () => {
    render(<RisksSection {...singleRiskProps} />);
    expect(screen.queryByTestId('trash-icon')).not.toBeInTheDocument();
  });

  it('still shows add button', () => {
    render(<RisksSection {...singleRiskProps} />);
    expect(screen.getByText('إضافة خطر')).toBeInTheDocument();
  });
});
...
describe('RisksSection Empty State', () => {
  ...
  it('does not show risk items', () => {
    render(<RisksSection {...emptyProps} />);
    expect(screen.queryByText('خطر 1')).not.toBeInTheDocument();
  });
});
