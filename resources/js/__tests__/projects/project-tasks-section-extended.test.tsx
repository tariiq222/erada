...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'tasks.enter_description': '[Task Description Placeholder]', 'tasks.type_desc_project': 'مهمة مرتبطة بمشروع محدد', 'tasks.type_desc_personal': 'مهمة شخصية غير مرتبطة بمشروع', 'tasks.type_desc_department': 'مهمة إدارية للقسم',
...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, leftIcon, type, variant, size, disabled }: any) => (
    <button onClick={onClick} type={type} data-variant={variant} data-size={size} disabled={disabled}>
      {leftIcon}
      {children}
    </button>
  ),
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconClipboardList: () => <span data-testid="clipboard-icon">📋</span>,
...
vi.mock('@pages/projects/form/constants', () => ({
  taskPriorityOptions: [
...
import InlineTasksEditor from '@pages/projects/form/InlineTasksEditor';
...
const mockMilestones = [
  { name: 'المرحلة الأولى', start_date: '2025-01-01', due_date: '2025-03-31' },
  { name: 'المرحلة الثانية', start_date: '2025-04-01', due_date: '2025-06-30' },
];
...
const mockTasks = [
  ...
  },
  {
    ...
    assigned_to_name: 'عضو جديد',
    start_date: '',
    due_date: '',
  },
];

describe('InlineTasksEditor Basic Render', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('InlineTasksEditor Date Range Messages', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows date range info when dates provided', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    expect(screen.getByText(/\[Task Date Range\]/)).toBeInTheDocument();
  });

  it('shows warning when no date range', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    expect(screen.getByText(/\[Set Dates First Tasks\]/)).toBeInTheDocument();
  });

  it('disables add button when no date range', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    const addButton = screen.getByText('إضافة مهمة').closest('button');
    expect(addButton).toBeDisabled();
  });

  it('enables add button when date range provided', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
...
describe('InlineTasksEditor Tasks Display', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders task cards', () => {
    ...
    const nameLabels = screen.getAllByText('[Task Name]');
    expect(nameLabels.length).toBe(2);
  });

  it('shows task name inputs', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    const inputs = screen.getAllByPlaceholderText('[Task Name Placeholder]');
    expect(inputs.length).toBe(2);
  });

  it('shows description inputs', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    const inputs = screen.getAllByPlaceholderText('[Task Description Placeholder]');
    expect(inputs.length).toBe(2);
  });

  it('renders assignee options with user names', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    // The user name appears as an option in each task's assignee Select
    expect(screen.getAllByText('أحمد محمد').length).toBeGreaterThan(0);
  });

  it('calls onSelectAssignee when an assignee is chosen', () => {
    const onSelectAssignee = vi.fn();
    const tasksNoAssignee = [
      { name: '', description: '', priority: 'medium', milestone_index: undefined, assigned_to: undefined, assigned_to_name: '', start_date: '', due_date: '' },
    ];
    render(
      <InlineTasksEditor
        tasks={tasksNoAssignee}
        milestones={mockMilestones}
        users={mockUsers}
        projectStartDate="2025-01-01"
        projectEndDate="2025-12-31"
    ...
    const assigneeSelect = screen.getByText('أحمد محمد').closest('select')!;
    fireEvent.change(assigneeSelect, { target: { value: '1' } });
    expect(onSelectAssignee).toHaveBeenCalledWith(0, 1);
  });
});

describe('InlineTasksEditor Add Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows add task button', () => {
    ...
    expect(screen.getByText('إضافة مهمة')).toBeInTheDocument();
  });
  ...
  it('calls onAddTask when add button clicked', () => {
    ...
    fireEvent.click(screen.getByText('إضافة مهمة'));
    expect(onAddTask).toHaveBeenCalledTimes(1);
  });
});

describe('InlineTasksEditor Remove Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows delete buttons for multiple tasks', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
  ...
  it('does not show delete button for single task', () => {
    render(
      <InlineTasksEditor
        tasks={[mockTasks[0]]}
        milestones={mockMilestones}
  ...
  it('calls onRemoveTask when delete clicked', () => {
    ...
    expect(onRemoveTask).toHaveBeenCalledWith(0);
  });
});

describe('InlineTasksEditor Task Changes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls onTaskChange when name changes', () => {
    ...
    const nameInputs = screen.getAllByPlaceholderText('[Task Name Placeholder]');
    fireEvent.change(nameInputs[0], { target: { value: 'مهمة جديدة' } });
    expect(onTaskChange).toHaveBeenCalledWith(0, 'name', 'مهمة جديدة');
  });

  it('calls onTaskChange when description changes', () => {
    const onTaskChange = vi.fn();
    render(
      <InlineTasksEditor
        tasks={mockTasks}
    ...
    const descInputs = screen.getAllByPlaceholderText('[Task Description Placeholder]');
    fireEvent.change(descInputs[0], { target: { value: 'وصف جديد' } });
    expect(onTaskChange).toHaveBeenCalledWith(0, 'description', 'وصف جديد');
  });

  it('calls onTaskChange when priority changes', () => {
    const onTaskChange = vi.fn();
    render(
      <InlineTasksEditor
        tasks={mockTasks}
    ...
    // Second select in first card is priority
    fireEvent.change(selects[1], { target: { value: 'low' } });
    expect(onTaskChange).toHaveBeenCalledWith(0, 'priority', 'low');
  });

  it('calls onTaskChange when milestone changes', () => {
    const onTaskChange = vi.fn();
    render(
      <InlineTasksEditor
        tasks={mockTasks}
    ...
    const selects = screen.getAllByTestId('select');
    // First select is milestone
    fireEvent.change(selects[0], { target: { value: '1' } });
    expect(onTaskChange).toHaveBeenCalledWith(0, 'milestone_index', 1);
  });

  it('calls onTaskChange with undefined when milestone cleared', () => {
    const onTaskChange = vi.fn();
    render(
      <InlineTasksEditor
        tasks={mockTasks}
    ...
    expect(onTaskChange).toHaveBeenCalledWith(0, 'milestone_index', undefined);
  });
});

describe('InlineTasksEditor Assignee Selection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls onSelectAssignee when assignee changes', () => {
    const onSelectAssignee = vi.fn();
    render(
      <InlineTasksEditor
        tasks={mockTasks}
    ...
    const assigneeSelect = screen.getAllByText('أحمد محمد')[0].closest('select')!;
    fireEvent.change(assigneeSelect, { target: { value: '2' } });
    expect(onSelectAssignee).toHaveBeenCalledWith(0, 2);
  });
});

describe('InlineTasksEditor Milestone Options', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows milestones in dropdown', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    // Multiple selects show milestones (one per task)
    const milestone1Options = screen.getAllByText('المرحلة الأولى');
    expect(milestone1Options.length).toBeGreaterThan(0);
    const milestone2Options = screen.getAllByText('المرحلة الثانية');
    expect(milestone2Options.length).toBeGreaterThan(0);
  });

  it('shows no milestone option', () => {
    render(
      <InlineTasksEditor
        tasks={mockTasks}
        milestones={mockMilestones}
    ...
    const options = screen.getAllByText('[No Milestone]');
    expect(options.length).toBeGreaterThan(0);
  });

  it('filters out empty milestone names', () => {
    const milestonesWithEmpty = [
      { name: 'المرحلة الأولى', start_date: '', due_date: '' },
    ...
    render(
      <InlineTasksEditor
        tasks={[mockTasks[0]]}
        milestones={milestonesWithEmpty}
        users={mockUsers}
        projectStartDate="2025-01-01"
        projectEndDate="2025-12-31"
        onTaskChange={vi.fn()}
    ...
    // Only valid milestone should appear
    expect(screen.getByText('المرحلة الأولى')).toBeInTheDocument();
  });
});

describe('InlineTasksEditor Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
