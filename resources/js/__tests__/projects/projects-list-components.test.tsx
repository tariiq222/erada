...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  StatCard: ({ label, value, icon: Icon, color }: { label: string; value: number; icon: React.FC<{ className?: string }>; color: string }) => (
  ...
  Button: ({ children, leftIcon, onClick, type, variant, size, className }: { children: React.ReactNode; leftIcon?: React.ReactNode; onClick?: () => void; type?: string; variant?: string; size?: string; className?: string }) => (
    <button onClick={onClick} type={type as 'button' | 'submit' | 'reset'} data-variant={variant} className={className}>
      {leftIcon}
      {children}
    </button>
  ),
...
describe('FiltersCard Component', () => {
  const defaultProps = {
    ...
    onClose: vi.fn(),
    programs: [
      { id: 1, name: 'مبادرة اختبارية', code: 'PGM-001' },
      { id: 2, name: 'مبادرة ثانية', code: 'PGM-002' },
    ],
  };
  ...
  it('renders filter card structure', () => {
    render(<FiltersCard {...defaultProps} />);
    expect(screen.getByTestId('filter-row')).toBeInTheDocument();
  ...
  it('calls onFiltersChange when typing in search', () => {
    ...
    fireEvent.change(searchInput, { target: { value: 'اختبار' } });
  ...
  it('calls onFiltersChange when selecting status', () => {
    render(<FiltersCard {...defaultProps} />);
  ...
  it('calls onFiltersChange when selecting priority', () => {
    render(<FiltersCard {...defaultProps} />);
    const selects = screen.getAllByRole('combobox');
  ...
  it('displays current search value', () => {
    render(<FiltersCard {...defaultProps} filters={{ search: 'بحث حالي', status: '', priority: '', program_id: '' }} />);
    const input = screen.getByPlaceholderText('بحث بالاسم أو الرمز...') as HTMLInputElement;
    expect(input.value).toBe('بحث حالي');
  });
});

describe('FiltersCard Status Options', () => {
  const defaultProps = {
    filters: { search: '', status: '', priority: '', program_id: '' },
    onFiltersChange: vi.fn(),
    onSearch: vi.fn(),
