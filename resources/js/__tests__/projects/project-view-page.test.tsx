...
vi.mock('@entities/project', () => ({
  projectsApi: {
    getOne: vi.fn().mockResolvedValue({
      ...
      progress: 50,
      milestones: [
        { id: 1, name: 'مرحلة 1', due_date: '2025-03-01', deliverables: [] },
      ],
      tasks: [
        { id: 1, title: 'مهمة 1', status: 'completed' },
        { id: 2, title: 'مهمة 2', status: 'in_progress' },
      ],
      members: [
        { id: 1, name: 'عضو 1' },
        { id: 2, name: 'عضو 2' },
      ...
      stakeholders: [
        { id: 1, name: 'صاحب مصلحة 1' },
      ],
      risks: [
        { id: 1, risk: 'خطر 1', probability: 'high', impact: 'high', status: 'open' },
      ],
      kpis: [
        { id: 1, indicator: 'مؤشر 1', target: '100', current_value: '50' },
      ],
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconBuilding: () => <span data-testid="building-icon">Building</span>,
  IconTarget: () => <span data-testid="target-icon">Target</span>,
  IconListCheck: () => <span data-testid="list-icon">ListChecks</span>,
  ...
  IconCurrencyDollar: () => <span data-testid="dollar-icon">DollarSign</span>,
  IconFileText: () => <span data-testid="file-icon">FileText</span>,
  IconHistory: () => <span data-testid="history-icon">History</span>,
    IconUserCheck: () => <span data-testid="usercheck-icon">UserCheck</span>,
    IconBriefcase: () => <span data-testid="briefcase-icon">Briefcase</span>,
  IconRocket: () => <span data-testid="rocket-icon">Rocket</span>,
  IconLayoutKanban: () => <span data-testid="folder-icon">FolderKanban</span>,
  IconChevronLeft: () => <span data-testid="chevron-icon">ChevronLeft</span>,
  IconAlertCircle: () => <span data-testid="alert-circle-icon">AlertCircle</span>,
  IconChevronRight: () => <span data-testid="chevron-right-icon">ChevronRight</span>,
  ...
  IconExternalLink: () => <span data-testid="external-link-icon">ExternalLink</span>,
  IconEye: () => <span data-testid="eye-icon">Eye</span>,
  IconLink: () => <span data-testid="link-icon">Link</span>,
  IconArrowUpRight: () => <span data-testid="arrow-up-right-icon">ArrowUpRight</span>,
  IconArrowDownRight: () => <span data-testid="arrow-down-right-icon">ArrowDownRight</span>,
  IconFlag: () => <span data-testid="milestone-icon">Milestone</span>,
  IconClipboardList: () => <span data-testid="clipboard-icon">ClipboardList</span>,
  IconClock: () => <span data-testid="clock-icon">Clock</span>,
  IconCircleCheck: () => <span data-testid="check-circle-icon">CheckCircle</span>,
  IconCircleX: () => <span data-testid="x-circle-icon">XCircle</span>,
  IconNotebook: () => <span data-testid="scroll-text-icon">ScrollText</span>,
  IconInfoCircle: () => <span data-testid="info-icon">Info</span>,
  IconChartBar: () => <span data-testid="bar-chart-icon">BarChart3</span>,
  IconDotsVertical: () => <span data-testid="more-icon">MoreVertical</span>,
  IconPencil: () => <span data-testid="pencil-icon">Pencil</span>,
  IconUserPlus: () => <span data-testid="user-plus-icon">UserPlus</span>,
  IconMessage: () => <span data-testid="message-icon">MessageSquare</span>,
  IconPaperclip: () => <span data-testid="paperclip-icon">Paperclip</span>,
  IconDownload: () => <span data-testid="download-icon">Download</span>,
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, leftIcon, variant, size }: {
    children: React.ReactNode;
    leftIcon?: React.ReactNode;
    variant?: string;
    size?: string;
  }) => (
    <button data-variant={variant} data-size={size}>{leftIcon}{children}</button>
  ),
...
vi.mock('@widgets/project/ui/ProjectReportCard', () => ({
  default: ({ project }: { project: any }) => (
    <div data-testid="project-report-card">بطاقة المشروع: {project.name}</div>
  ),
}));

vi.mock('@widgets/project/ui/ProjectActivityLog', () => ({
  default: ({ projectId }: { projectId: number }) => (
    <div data-testid="project-activity-log">سجل النشاط: {projectId}</div>
  ),
}));

vi.mock('@features/project-expenses/ui/ProjectExpenses', () => ({
  default: ({ projectId, budget }: { projectId: number; budget: number }) => (
    <div data-testid="project-expenses">المصروفات: {budget}</div>
  ),
}));

vi.mock('@pages/projects/components/ProjectStatsCard', () => ({
  default: ({ project }: { project: any }) => (
    <div data-testid="project-stats-card">إحصائيات: {project.progress}%</div>
  ),
}));

vi.mock('@pages/projects/components/ProjectViewSkeleton', () => ({
  default: () => <div data-testid="skeleton">جاري التحميل...</div>,
}));

vi.mock('@pages/projects/components/tabs', () => ({
  OverviewTab: ({ project }: { project: any }) => (
    <div data-testid="overview-tab">نظرة عامة: {project.description}</div>
  ),
}));

vi.mock('@pages/strategy/components/GoldenChainTag', () => ({
  default: () => <div data-testid="golden-chain-tag" />,
}));

vi.mock('@pages/projects/components/sections', () => ({
  TeamSection: ({ members }: { members: any[] }) => (
    <div data-testid="team-section">الفريق: {members.length} عضو</div>
  ),
  StakeholdersSection: ({ stakeholders }: { stakeholders: any[] }) => (
    <div data-testid="stakeholders-section">أصحاب المصلحة: {stakeholders.length}</div>
  ),
  KPIsSection: ({ kpis }: { kpis: any[] }) => (
    <div data-testid="kpis-section">مؤشرات الأداء: {kpis.length}</div>
  ),
  RisksSection: ({ risks }: { risks: any[] }) => (
    <div data-testid="risks-section">المخاطر: {risks.length}</div>
  ),
  TasksSection: ({ tasks }: { tasks: any[] }) => (
    <div data-testid="tasks-section">المهام: {tasks.length}</div>
  ),
  ProjectTasksSection: ({ tasks }: { tasks: any[] }) => (
    <div data-testid="tasks-section">المهام: {tasks.length}</div>
  ),
}));
...
vi.mock('@shared/contexts/AuthContext', () => ({
  useAuth: () => ({
    user: { id: 1, name: 'test', permissions: ['strategy.view', 'strategy.create', 'strategy.edit'] },
    hasPermission: () => true,
    canAccess: () => true,
    isAdmin: () => false,
    isSuperAdmin: () => false,
...
describe('ProjectView Loading State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('ProjectView Header', () => {
  ...
  it('renders project name after loading', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getAllByText('مشروع اختبار').length).toBeGreaterThanOrEqual(1);
    });
  });
...
describe('ProjectView Info Bar', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows department name', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  it('shows building icon for department', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  // Regression: the supervisor pill was intentionally removed from ProjectView.
  // Render the real ProjectView (sections/api are mocked, but the layout code
  // runs as in production) and assert the i18n key for supervisor is never
  // surfaced. If a future change re-introduces a supervisor pill/label that
  // renders `t('projects.supervisor')` (Arabic "مشرف"), this test fails.
  it('does not render the supervisor label in the project info bar', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      // Wait for the project fixture to load so the info bar is in the DOM.
      expect(screen.getByText('قسم التقنية')).toBeInTheDocument();
    });
    expect(screen.queryByText('مشرف')).not.toBeInTheDocument();
  });
});

describe('ProjectView Stats Card', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders project stats card inside the Overview tab content', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('content-overview')).toBeInTheDocument();
    });
    const overviewPanel = screen.getByTestId('content-overview');
    expect(
      within(overviewPanel).getByTestId('project-stats-card'),
    ).toBeInTheDocument();
  });

  it('does NOT render the project stats card above the tabs', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('content-overview')).toBeInTheDocument();
    });
    const tabs = screen.getByTestId('tabs');
    // The card must live INSIDE the Overview tab panel, not as a sibling of <Tabs>.
    expect(tabs.contains(screen.getByTestId('project-stats-card'))).toBe(true);
    // And there is exactly one occurrence.
    expect(screen.getAllByTestId('project-stats-card')).toHaveLength(1);
  });

  it('shows progress percentage inside the Overview tab content', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('content-overview')).toBeInTheDocument();
    });
    const overviewPanel = screen.getByTestId('content-overview');
    expect(within(overviewPanel).getByText('إحصائيات: 50%')).toBeInTheDocument();
  });
});
...
describe('ProjectView Tabs', () => {
  ...
  it('renders tasks tab with count', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  it('renders team tab with count', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  it('renders stakeholders tab with count', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByText('أصحاب المصلحة (1)')).toBeInTheDocument();
    });
  });

  it('renders risks tab with count', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByText('المخاطر (1)')).toBeInTheDocument();
    });
  });

  it('renders kpis tab with count', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByText('مؤشرات الأداء (1)')).toBeInTheDocument();
    });
  });

  it('renders expenses tab', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  it('renders report card tab', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByText('بطاقة المشروع')).toBeInTheDocument();
    });
  });
...
describe('ProjectView Tab Content', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders stakeholders section', async () => {
    render(<ProjectView />);
    await waitFor(() => {
  ...
  it('renders expenses component', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('project-expenses')).toBeInTheDocument();
    });
  });

  it('renders report card component', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('project-report-card')).toBeInTheDocument();
    });
  });
  ...
  it('renders activity log component', async () => {
    render(<ProjectView />);
    await waitFor(() => {
      expect(screen.getByTestId('project-activity-log')).toBeInTheDocument();
    });
  });
});

describe('ProjectView Not Found', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows not found message when project is null', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce(null);
  ...
  it('shows back link when project not found', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.getOne as ReturnType<typeof vi.fn>).mockResolvedValueOnce(null);
    ...
    await waitFor(() => {
      expect(screen.getByText('العودة للمشاريع')).toBeInTheDocument();
    });
  });
});

describe('ProjectView API Error', () => {
  ...
  it('handles API error gracefully', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.getOne as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));

    render(<ProjectView />);

    await waitFor(() => {
      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to fetch project:', expect.any(Error));
    });
  });
});
