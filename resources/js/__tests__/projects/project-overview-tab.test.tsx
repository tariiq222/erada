...
vi.mock('@pages/projects/constants', () => ({
  kanbanColumnStyles: {
    todo: { bg: 'bg-gray-50', border: 'border-gray-200', headerBg: 'bg-gray-100', headerText: 'text-gray-600' },
    in_progress: { bg: 'bg-blue-50', border: 'border-blue-200', headerBg: 'bg-blue-100', headerText: 'text-blue-600' },
    completed: { bg: 'bg-green-50', border: 'border-green-200', headerBg: 'bg-green-100', headerText: 'text-green-600' },
  },
}));

import OverviewTab from '@pages/projects/components/tabs/OverviewTab';
...
const mockProject = {
  ...
  in_scope: ['نطاق 1', 'نطاق 2'],
  out_of_scope: ['خارج 1', 'خارج 2'],
  kpis: [
    { id: 1, indicator: 'مؤشر 1', current_value: 50, target: 100 },
    { id: 2, indicator: 'مؤشر 2', current_value: 80, target: 100 },
  ],
  risks: [
    { id: 1, description: 'خطر 1', impact: 'high', probability: 'medium', status: 'identified' },
    { id: 2, description: 'خطر 2', impact: 'low', probability: 'low', status: 'mitigated' },
  ],
  milestones: [
    { id: 1, name: 'مرحلة 1', status: 'completed', due_date: '2025-03-31' },
    { id: 2, name: 'مرحلة 2', status: 'in_progress', due_date: '2025-06-30' },
  ],
  tasks: [],
  members: [],
  stakeholders: [],
...
describe('OverviewTab', () => {
  ...
  it('renders section title', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('وصف المشروع الاختباري')).toBeInTheDocument();
  });

  it('renders section subtitle', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('الأهداف (3)')).toBeInTheDocument();
  });

  it('shows target icon', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getAllByTestId('target-icon').length).toBeGreaterThanOrEqual(1);
  });
});
...
describe('OverviewTab Description', () => {
  ...
  it('hides description section if no description', () => {
    const projectWithoutDesc = { ...mockProject, description: null };
    render(<OverviewTab project={projectWithoutDesc as any} />);
    expect(screen.queryByText('وصف المشروع الاختباري')).not.toBeInTheDocument();
  });
});

describe('OverviewTab Objectives', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('displays objectives section', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('الأهداف (3)')).toBeInTheDocument();
  });
  ...
  it('displays all objectives', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('هدف 1')).toBeInTheDocument();
    expect(screen.getByText('هدف 2')).toBeInTheDocument();
    expect(screen.getByText('هدف 3')).toBeInTheDocument();
  });

  it('shows numbered list for objectives', () => {
    render(<OverviewTab project={mockProject as any} />);
    // Numbers appear multiple times (objectives + other sections)
    expect(screen.getAllByText('1').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('2').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('3').length).toBeGreaterThanOrEqual(1);
  });

  it('hides objectives section if empty', () => {
    const projectNoObjectives = { ...mockProject, objectives: [] };
    render(<OverviewTab project={projectNoObjectives as any} />);
    expect(screen.queryByText(/الأهداف/)).not.toBeInTheDocument();
  });
});

describe('OverviewTab Scope', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('displays in scope section', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('ضمن النطاق (2)')).toBeInTheDocument();
  });

  it('displays in scope items', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('نطاق 1')).toBeInTheDocument();
    expect(screen.getByText('نطاق 2')).toBeInTheDocument();
  });

  it('displays out of scope section', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('خارج النطاق (2)')).toBeInTheDocument();
  });

  it('displays out of scope items', () => {
    render(<OverviewTab project={mockProject as any} />);
    expect(screen.getByText('خارج 1')).toBeInTheDocument();
    expect(screen.getByText('خارج 2')).toBeInTheDocument();
  });

  it('hides in scope section if empty', () => {
    const projectNoInScope = { ...mockProject, in_scope: [] };
    render(<OverviewTab project={projectNoInScope as any} />);
    expect(screen.queryByText(/ضمن النطاق/)).not.toBeInTheDocument();
  });

  it('hides out of scope section if empty', () => {
    const projectNoOutScope = { ...mockProject, out_of_scope: [] };
    render(<OverviewTab project={projectNoOutScope as any} />);
    expect(screen.queryByText(/خارج النطاق/)).not.toBeInTheDocument();
  });
});
...
describe('OverviewTab Empty States', () => {
  ...
  it('renders with minimal project data', () => {
    ...
    render(<OverviewTab project={minimalProject as any} />);
    // The component mounts and renders its layout root (a generic <div>) even with all-empty data.
