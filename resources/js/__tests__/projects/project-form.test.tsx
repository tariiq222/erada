...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'profile.subtitle': 'إدارة معلوماتك الشخصية وكلمة المرور', 'profile.role_super_admin': 'مدير النظام', 'profile.role_admin': 'مدير إدارة', 'profile.role_project_manager': 'مدير مشروع', 'profile.role_team_member': 'عضو فريق', 'profile.role_viewer': 'مشاهد',
    'projects.project_autosave_hint': 'يتم حفظ مسودة المشروع تلقائياً حتى تتمكن من إكمالها لاحقاً.', 'projects.improvement_autosave_ready': 'الحفظ التلقائي جاهز', 'projects.project_autosave_ready': 'الحفظ التلقائي جاهز', 'projects.section_nav_label': 'التنقل بين الأقسام',
    'projects.pmbok_charter_fields': 'ميثاق المشروع', 'projects.business_case': 'مبرر المشروع', 'projects.manager_authority': 'صلاحيات مدير المشروع',
    'projects.success_criteria': 'معايير النجاح', 'projects.requirements': 'المتطلبات', 'projects.approval_criteria': 'معايير القبول', 'projects.exit_criteria': 'معايير الإغلاق',
    'projects.add_deliverable': '[Add Deliverable]', 'projects.assignee_optional': '[Assignee Optional]', 'projects.budget_sar': '[Budget Sar]',
...
describe('ProjectForm Component', () => {
  ...
  it('renders in create mode as a single-page new-project form', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'مشروع جديد' })).toBeInTheDocument();
      expect(screen.getAllByText('يتم حفظ مسودة المشروع تلقائياً حتى تتمكن من إكمالها لاحقاً.').length).toBeGreaterThan(0);
    });
  });
  ...
  it('renders breadcrumb navigation', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('المشاريع').length).toBeGreaterThan(0);
    });
  });

  it('renders all single-page project sections in create mode', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByTestId('basic-info-step')).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '2. ميثاق المشروع' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '3. الأهداف والنطاق' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '4. فريق العمل' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '5. أصحاب المصلحة' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '6. المراحل' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '7. المهام' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '8. المخاطر' })).toBeInTheDocument();
      expect(screen.getByRole('heading', { name: '9. [Resources And Support]' })).toBeInTheDocument();
  ...
  it('shows autosave status instead of wizard progress in create mode', async () => {
    render(<ProjectForm />);
  ...
  it('renders project description textarea', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByPlaceholderText('أدخل وصفاً تفصيلياً للمشروع')).toBeInTheDocument();
    });
  });
  ...
  it('renders create submit action immediately in create mode', async () => {
    render(<ProjectForm />);
  ...
  it('renders single-page (no wizard navigation) in edit mode', async () => {
    mockUseParams.mockReturnValue({ id: '1' });

    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByRole('button', { name: /حفظ|إنشاء مشروع/ })).toBeInTheDocument();
    });

    expect(screen.queryByText('التالي')).not.toBeInTheDocument();
    expect(screen.queryByText('السابق')).not.toBeInTheDocument();
    expect(screen.queryByText('الخطوة 1 من 6')).not.toBeInTheDocument();
  });

  it('does not render previous wizard action in create mode', async () => {
    render(<ProjectForm />);
  ...
  it('renders objectives and scope sections on the create page', async () => {
    render(<ProjectForm />);
  ...
  it('renders team and stakeholders sections on the create page', async () => {
    render(<ProjectForm />);
  ...
  it('renders milestones section on the create page', async () => {
    render(<ProjectForm />);
  ...
  it('renders tasks section on the create page', async () => {
    render(<ProjectForm />);
  ...
  it('renders risks and resources sections on the create page', async () => {
    render(<ProjectForm />);
  ...
  it('does not require navigating to a last step before submitting in create mode', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.queryByText('الخطوة 6 من 6')).not.toBeInTheDocument();
      expect(screen.getByRole('button', { name: /إنشاء مشروع/ })).toBeInTheDocument();
    });
  });

  it('keeps create mode on a single page after editing basic fields', async () => {
    render(<ProjectForm />);
    ...
    fireEvent.change(screen.getByPlaceholderText('أدخل اسم المشروع'), { target: { value: 'مشروع اختباري' } });

    expect(screen.getByPlaceholderText('أدخل اسم المشروع')).toHaveValue('مشروع اختباري');
    expect(screen.getByTestId('tasks-section')).toBeInTheDocument();
    expect(screen.queryByText('التالي')).not.toBeInTheDocument();
  });
  ...
  it('allows typing in project name field', async () => {
    ...
    const nameInput = screen.getByPlaceholderText('أدخل اسم المشروع');
    fireEvent.change(nameInput, { target: { value: 'مشروع اختباري' } });

    expect(nameInput).toHaveValue('مشروع اختباري');
  });
  ...
  it('allows typing in description field', async () => {
    ...
    const descInput = screen.getByPlaceholderText('أدخل وصفاً تفصيلياً للمشروع');
    fireEvent.change(descInput, { target: { value: 'وصف المشروع' } });
...
describe('ProjectForm Status Options', () => {
  ...
  it('renders priority dropdown', async () => {
    ...
    await waitFor(() => {
      // Priority label with required indicator
...
describe('ProjectForm User Selection', () => {
  it('renders manager selection button', async () => {
    render(<ProjectForm />);
  ...
  it('renders sponsor selection button', async () => {
    render(<ProjectForm />);
  ...
  it('shows placeholder for unselected manager', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByText('-- اختر مدير المشروع --')).toBeInTheDocument();
    });
  });

  it('shows placeholder for unselected sponsor', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByText('-- اختر راعي / مالك المشروع --')).toBeInTheDocument();
    });
  });
});
...
describe('ProjectForm Budget Field', () => {
  it('renders budget input', async () => {
    render(<ProjectForm />);
  ...
  it('allows entering budget value', async () => {
    render(<ProjectForm />);
...
describe('ProjectForm Card Sections', () => {
  it('renders basic info card', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      // Looking for the card title
      const titles = screen.getAllByText('المعلومات الأساسية');
      expect(titles.length).toBeGreaterThan(0);
    });
  });

  it('renders settings and timeline card', async () => {
    render(<ProjectForm />);
