...
vi.mock('@entities/project', () => ({
  projectsApi: {
    getAll: vi.fn().mockResolvedValue({
      data: [
        // Phase 9.3 freeze cleanup: per-record `abilities.delete` must be
        // present in the list fixture so the row-level Delete gate stays
        // honest. Without it the table hides the action for everyone.
        { id: 1, code: 'PRJ-001', name: 'مشروع التطوير', status: 'in_progress', priority: 'high', progress: 60, department: { name: 'IT' }, abilities: { edit: true, delete: true } },
        { id: 2, code: 'PRJ-002', name: 'مشروع التحديث', status: 'completed', priority: 'medium', progress: 100, department: null, abilities: { edit: true, delete: true } },
        { id: 3, code: 'PRJ-003', name: 'مشروع جديد', status: 'planning', priority: 'low', progress: 10, department: { name: 'HR' }, abilities: { edit: true, delete: true } },
      ],
      current_page: 1,
      last_page: 2,
      per_page: 15,
...
vi.mock('@pages/projects/list', () => ({
  StatsCards: ({ projects, total }: { projects: any[]; total: number }) => (
    <div data-testid="stats-cards">
      <span>إجمالي: {total}</span>
      <span>قيد التنفيذ: {projects.filter((p: any) => p.status === 'in_progress').length}</span>
    </div>
  ),
  FiltersCard: ({
    filters,
    ...
    <div data-testid="filters-card">
      ...
      <button onClick={onClose}>إغلاق الفلتر</button>
    </div>
  ),
  ProjectsTable: ({
    projects,
    ...
    loading: boolean;
    pagination: any;
    // Phase 9.3 freeze cleanup: prop renamed from `isSuperAdmin` to
    // `canDeleteProject` — module-level capability. The row-level
    // `project.abilities?.delete` is the per-record refinement.
        ...
        projects.map((project: any) => (
          <div key={project.id} data-testid="project-row">
            ...
            {canDeleteProject && project.abilities?.delete && (
              <button onClick={() => onDelete(project)}>حذف</button>
            )}
          </div>
        ))
      )}
...
describe('ProjectsList Page', () => {
  ...
  it('renders page description', async () => {
    render(<ProjectsList />);
    await waitFor(() => {
      expect(screen.getByText('إدارة ومتابعة جميع المشاريع')).toBeInTheDocument();
    });
  });
  ...
  it('does not render stats cards (removed in redesign)', async () => {
    render(<ProjectsList />);
    await waitFor(() => {
...
describe('ProjectsList Filters', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows filters card on filter button click', async () => {
    render(<ProjectsList />);
  ...
  it('hides filters card on close', async () => {
    render(<ProjectsList />);
    ...
    fireEvent.click(screen.getByText('إغلاق الفلتر'));
  ...
  it('updates search params on search', async () => {
    render(<ProjectsList />);
    ...
    const form = screen.getByText('بحث').closest('form');
    if (form) {
...
describe('ProjectsList Delete', () => {
  ...
  it('shows success toast on delete', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('success', expect.stringContaining('تم حذف المشروع'));
    });
  });
});

describe('ProjectsList canManageMembers=false (Phase 9.3 freeze cleanup)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('hides delete buttons when canDeleteProject is false', async () => {
    // Phase 9.3 freeze cleanup (2026-07-06): the table is now gated on
    // the canonical `projects.delete` capability + per-record abilities,
  ...
  it('hides delete buttons even when super-admin role string is set but the canonical projects.delete capability is absent', async () => {
    // Regression pin: the role-string check used to be the gate. The
...
describe('ProjectsList Pagination', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe('ProjectsList API Errors', () => {
  ...
  it('handles fetch error gracefully', async () => {
    const { projectsApi } = await import('@entities/project');
    (projectsApi.getAll as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API Error'));
  ...
  it('shows error toast on delete failure', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('error', 'لا يمكن حذف هذا المشروع');
    });
  });
});
