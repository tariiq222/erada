...
const mockPerformanceApi = vi.hoisted(() => ({
  listContextKPIs: vi.fn(),
  createKPI: vi.fn(),
...
vi.mock('@entities/project', () => ({
  projectsApi: {
    addKPI: vi.fn().mockResolvedValue({}),
    removeKPI: vi.fn().mockResolvedValue({}),
  },
}));
vi.mock('@entities/performance', () => ({
  performanceApi: mockPerformanceApi,
}));
vi.mock('@entities/task', () => ({
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({ subtasks: [] }),
...
vi.mock('@shared/ui', async (importOriginal) => ({
  ...
  Button: ({ children, onClick, leftIcon, variant, size, disabled }: any) => (
    <button onClick={onClick} disabled={disabled} data-variant={variant} data-size={size}>{leftIcon}{children}</button>
  ),
  ...
  FilterField: ({ label, children }: any) => (<label><span>{label}</span>{children}</label>),
  DeleteConfirmationModal: ({ isOpen, item, onConfirm, onClose }: any) => (
    isOpen && item ? (
      <div data-testid="delete-confirm-modal">
        <button onClick={onConfirm}>تأكيد الحذف</button>
...
// Mock modals
vi.mock('@pages/projects/components/modals', () => ({
  AddKPIModal: ({ isOpen, onClose, onAdd, formData, onFormChange, isLoading }: any) => (
    isOpen ? (
      <div data-testid="kpi-modal">
        <input placeholder="المؤشر" value={formData?.indicator || ''} onChange={(e) => onFormChange?.({ ...formData, indicator: e.target.value })} />
        <input placeholder="المستهدف" value={formData?.target || ''} onChange={(e) => onFormChange?.({ ...formData, target: e.target.value })} />
        <input placeholder="الحالي" value={formData?.current_value || ''} onChange={(e) => onFormChange?.({ ...formData, current_value: e.target.value })} />
        <input placeholder="الوحدة" value={formData?.unit || ''} onChange={(e) => onFormChange?.({ ...formData, unit: e.target.value })} />
        <button onClick={onAdd} disabled={isLoading}>إضافة</button>
        <button onClick={onClose}>إلغاء</button>
      </div>
    ) : null
  ),
  StatusTransitionModal: ({ isOpen, onClose, onConfirm }: any) => (
    isOpen ? (
  ...
  ErrorModal: ({ isOpen, onClose, message }: any) => (
    isOpen ? (
      <div data-testid="error-modal">{message}<button onClick={onClose}>إغلاق</button></div>
    ) : null
  ),
  IncompleteSubtasksModal: ({ isOpen, onClose }: any) => (
    isOpen ? <div data-testid="subtasks-modal"><button onClick={onClose}>إغلاق</button></div> : null
  ),
}));
...
vi.mock('@tabler/icons-react', async () => {
  ...
  IconList: () => <span data-testid="list-icon">List</span>,
  IconLayoutGrid: () => <span data-testid="grid-icon">Grid</span>,
  IconTarget: () => <span data-testid="target-icon">Target</span>,
  IconCircleX: () => <span data-testid="x-icon">X</span>,
  IconClipboardList: () => <span data-testid="clipboard-icon">Clipboard</span>,
  IconFilter: () => <span data-testid="filter-icon">Filter</span>,
  IconChevronDown: () => <span data-testid="chevron-icon">ChevronDown</span>,
  IconUsers: () => <span data-testid="users-icon">Users</span>,

  };
});

// Mock child components for TasksSection
vi.mock('@pages/projects/components/sections/tasks', () => ({
  KanbanBoard: ({ tasks, onTaskClick }: { tasks: any[]; onTaskClick: (id: number) => void }) => (
    <div data-testid="kanban-board">
      {tasks.map((t: any) => <div key={t.id} onClick={() => onTaskClick(t.id)}>{t.title}</div>)}
    </div>
  ),
  TasksListView: ({ tasks, onTaskClick }: { tasks: any[]; onTaskClick: (id: number) => void }) => (
    <div data-testid="tasks-list">
      {tasks.map((t: any) => <div key={t.id} onClick={() => onTaskClick(t.id)}>{t.title}</div>)}
    </div>
  ),
  TasksFilters: ({ onFilterChange, onReset }: any) => (
    <div data-testid="tasks-filters">
      <button onClick={onReset}>إعادة تعيين</button>
    </div>
  ),
  TasksHeader: ({ total, onAddTask }: { total: number; onAddTask: () => void }) => (
    <div data-testid="tasks-header">
      <span>المهام: {total}</span>
      <button onClick={onAddTask}>إضافة مهمة</button>
    </div>
  ),
  TasksEmptyState: ({ onAddTask }: { onAddTask: () => void }) => (
...
vi.mock('@pages/tasks/TaskViewModal', () => ({
  default: ({ isOpen, taskId, onClose }: { isOpen: boolean; taskId: number | null; onClose: () => void }) => (
    isOpen ? <div data-testid="task-view-modal">Task: {taskId}<button onClick={onClose}>إغلاق</button></div> : null
  ),
}));
  ...
  default: ({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) => (
    isOpen ? (
      <div data-testid="create-task-modal">
        <button onClick={onClose}>إلغاء</button>
        <button onClick={onSuccess}>إنشاء</button>
      </div>
    ) : null
  ),
}));

import KPIsSection from '@pages/projects/components/sections/KPIsSection';
import { projectsApi } from '@entities/project';
import { performanceApi } from '@entities/performance';

beforeEach(() => {
  mockPerformanceApi.listContextKPIs.mockReset();
  mockPerformanceApi.createKPI.mockReset();
  mockPerformanceApi.createMeasurement.mockReset();
  mockPerformanceApi.createLink.mockReset();
  mockPerformanceApi.deleteLink.mockReset();

  mockPerformanceApi.listContextKPIs.mockImplementation(() => new Promise(() => {}));
  mockPerformanceApi.createKPI.mockResolvedValue({
    message: 'created',
    kpi: { id: 10, name: 'مؤشر جديد', target: '100', current_value: '25', unit: '%' },
  });
  mockPerformanceApi.createLink.mockResolvedValue({
    message: 'linked',
    link: { id: 20, linkable_id: 1, relationship_type: 'primary' },
  });
  mockPerformanceApi.deleteLink.mockResolvedValue({ message: 'deleted' });
});

describe('KPIsSection Component', () => {
  const mockKPIs = [
    { id: 1, indicator: 'نسبة الإنجاز', target: '100', current_value: '75', unit: '%' },
    { id: 2, indicator: 'الميزانية', target: '500000', current_value: '300000', unit: 'ريال' },
  ];
  ...
  it('renders section title', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
  ...
  it('renders add button', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
    expect(screen.getByText('إضافة مؤشر أداء', { exact: false })).toBeInTheDocument();
  });

  it('renders view toggle buttons', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
    expect(screen.getByTitle('عرض جدول')).toBeInTheDocument();
    expect(screen.getByTitle('عرض بطاقات')).toBeInTheDocument();
  });

  it('renders KPI names in table', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
  ...
  it('shows empty state when no KPIs', () => {
    render(<KPIsSection kpis={[]} projectId={1} />);
    expect(screen.getByText('لا توجد مؤشرات أداء')).toBeInTheDocument();
  });

  it('shows add button in empty state', () => {
    render(<KPIsSection kpis={[]} projectId={1} />);
    expect(screen.getAllByText('إضافة مؤشر أداء', { exact: false })[0]).toBeInTheDocument();
  });

  it('toggles view mode', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);

    const cardsButton = screen.getByTitle('عرض بطاقات');
    fireEvent.click(cardsButton);

    // Should switch to cards view
  ...
  it('opens add modal on button click', () => {
    ...
    fireEvent.click(screen.getByText('إضافة مؤشر أداء', { exact: false }));

    expect(screen.getByTestId('kpi-modal')).toBeInTheDocument();
  });

  it('shows table headers', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
    expect(screen.getByText('المؤشر')).toBeInTheDocument();
...
describe('KPIsSection Empty State', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders empty state message', () => {
    render(<KPIsSection kpis={[]} projectId={1} />);
    expect(screen.getByText('لا توجد مؤشرات أداء')).toBeInTheDocument();
    expect(screen.getByText('أضف مؤشرات الأداء للمشروع')).toBeInTheDocument();
  });
  ...
  it('opens modal from empty state button', () => {
    render(<KPIsSection kpis={[]} projectId={1} />);

    fireEvent.click(screen.getAllByText('إضافة مؤشر أداء', { exact: false })[0]);

    expect(screen.getByTestId('kpi-modal')).toBeInTheDocument();
  });
});

describe('KPIsSection View Toggle', () => {
  const mockKPIs = [
    { id: 1, indicator: 'مؤشر 1', target: '100', current_value: '50', unit: '%' },
  ];
  ...
  it('switches to cards view', () => {
    ...
    fireEvent.click(screen.getByTitle('عرض بطاقات'));

    // Table should be hidden in cards view
    expect(screen.queryByRole('table')).not.toBeInTheDocument();
  });
  ...
  it('switches back to table view', () => {
    ...
    fireEvent.click(screen.getByTitle('عرض جدول'));

    expect(screen.getByRole('table')).toBeInTheDocument();
  });
});

describe('KPIsSection API Operations', () => {
  const mockKPIs = [
    { id: 1, indicator: 'مؤشر 1', target: '100', current_value: '50', unit: '%' },
  ];
  const mockOnKPIAdded = vi.fn();
  const mockOnKPIRemoved = vi.fn();
  ...
  it('opens modal for adding KPI', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} onKPIAdded={mockOnKPIAdded} />);
  ...
  it('renders with callback props', () => {
    render(<KPIsSection kpis={mockKPIs} projectId={1} onKPIAdded={mockOnKPIAdded} onKPIRemoved={mockOnKPIRemoved} />);

    expect(screen.getByText('مؤشرات الأداء الرئيسية')).toBeInTheDocument();
  });

  it('fetches and renders linked Performance KPIs when available', async () => {
    const mockOnKPICountChange = vi.fn();
    vi.mocked(performanceApi.listContextKPIs).mockResolvedValueOnce({
      data: [
        {
          id: 55,
          name: 'رضا المستفيدين',
          ...
          current_value: '72',
          unit: '%',
          links: [{ id: 99, linkable_id: 1, relationship_type: 'primary' }],
        },
      ],
    });

    render(<KPIsSection kpis={mockKPIs} projectId={1} onKPICountChange={mockOnKPICountChange} />);

    expect(await screen.findByText('رضا المستفيدين')).toBeInTheDocument();
    expect(screen.queryByText('مؤشر 1')).not.toBeInTheDocument();
    expect(performanceApi.listContextKPIs).toHaveBeenCalledWith('project', 1);
    expect(mockOnKPICountChange).toHaveBeenCalledWith(1);
  });

  it('keeps legacy KPIs as fallback when Performance KPI fetch fails', async () => {
    const mockOnKPICountChange = vi.fn();
    vi.mocked(performanceApi.listContextKPIs).mockRejectedValueOnce(new Error('Performance API unavailable'));

    render(<KPIsSection kpis={mockKPIs} projectId={1} onKPICountChange={mockOnKPICountChange} />);

    expect(screen.getByText('مؤشر 1')).toBeInTheDocument();
    await waitFor(() => expect(mockOnKPICountChange).toHaveBeenCalledWith(1));
  });

  it('creates a Performance KPI and project link from the add flow', async () => {
    const mockOnKPIAdded = vi.fn();
    const mockOnKPICountChange = vi.fn();
    vi.mocked(performanceApi.listContextKPIs)
    ...
    vi.mocked(performanceApi.createKPI).mockResolvedValueOnce({
      message: 'created',
      kpi: { id: 55, name: 'مؤشر جديد', target: '100', current_value: '25', unit: '%' },
    });
    vi.mocked(performanceApi.createLink).mockResolvedValueOnce({
      message: 'linked',
      link: { id: 99, linkable_id: 1, relationship_type: 'primary' },
    });
    ...
    render(
      <KPIsSection
        ...
        projectId={1}
        organizationId={12}
        onKPIAdded={mockOnKPIAdded}
        onKPICountChange={mockOnKPICountChange}
      />,
    );

    fireEvent.click(screen.getAllByText('إضافة مؤشر أداء', { exact: false })[0]);
    fireEvent.change(screen.getByPlaceholderText('المؤشر'), { target: { value: 'مؤشر جديد' } });
    fireEvent.change(screen.getByPlaceholderText('المستهدف'), { target: { value: '100' } });
    fireEvent.change(screen.getByPlaceholderText('الحالي'), { target: { value: '25' } });
    fireEvent.change(screen.getByPlaceholderText('الوحدة'), { target: { value: '%' } });
    fireEvent.click(screen.getByText('إضافة'));

    await waitFor(() => {
      expect(performanceApi.createKPI).toHaveBeenCalledWith({
        name: 'مؤشر جديد',
        target: '100',
        current_value: '25',
        unit: '%',
    ...
    expect(performanceApi.createLink).toHaveBeenCalledWith(55, {
      linkable_type: 'project',
      linkable_id: 1,
      relationship_type: 'primary',
    });
    expect(projectsApi.addKPI).not.toHaveBeenCalled();
    expect(mockOnKPIAdded).toHaveBeenCalled();
    expect(mockOnKPICountChange).toHaveBeenCalledWith(1);
  });

  it('deletes linked Performance KPI links without calling the legacy delete endpoint', async () => {
    vi.mocked(performanceApi.listContextKPIs).mockResolvedValueOnce({
      data: [
        {
          id: 55,
    ...
    await screen.findByText('رضا المستفيدين');
    fireEvent.click(screen.getByTitle('حذف'));
    fireEvent.click(await screen.findByText('تأكيد الحذف'));

    await waitFor(() => expect(performanceApi.deleteLink).toHaveBeenCalledWith(55, 99));
    expect(projectsApi.removeKPI).not.toHaveBeenCalled();
  });

  it('does not use legacy delete for Performance KPIs without a matching project link', async () => {
    vi.mocked(performanceApi.listContextKPIs).mockResolvedValueOnce({
      data: [
        {
          id: 56,
          name: 'مؤشر بلا رابط مشروع',
          target: '90',
          current_value: '72',
          unit: '%',
          links: [
            { id: 99, linkable_id: 2, linkable_type: 'project', relationship_type: 'primary' },
            { id: 100, linkable_id: 1, linkable_type: 'task', relationship_type: 'primary' },
          ],
        },
      ],
    });
    ...
    await screen.findByText('مؤشر بلا رابط مشروع');
    fireEvent.click(screen.getByTitle('حذف'));
    fireEvent.click(await screen.findByText('تأكيد الحذف'));

    await waitFor(() => expect(mockShowToast).toHaveBeenCalledWith('error', 'فشل في حذف المؤشر'));
    expect(performanceApi.deleteLink).not.toHaveBeenCalled();
    expect(projectsApi.removeKPI).not.toHaveBeenCalled();
  });

  it('cannot delete fallback KPIs that lack a Performance link', async () => {
    // KPIs migrated to the Performance module: the legacy project-KPI delete
    // endpoint is gone. A fallback KPI without a performance_link_id can no
    // longer be deleted; confirming surfaces an error and hits no endpoint.
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
...
// Simple TasksSection test
describe('TasksSection Helper Functions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders section correctly', () => {
    // Just verify KPIsSection can be rendered
    const mockKPIs = [{ id: 1, indicator: 'Test', target: '100', current_value: '50', unit: '%' }];
    render(<KPIsSection kpis={mockKPIs} projectId={1} />);
    expect(screen.getByText('مؤشرات الأداء الرئيسية')).toBeInTheDocument();
  });
});
