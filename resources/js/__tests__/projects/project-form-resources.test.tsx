...
vi.mock('@shared/ui/Textarea', () => ({
  Textarea: ({ value, onChange, placeholder, label, rows, className }: {
    value: string;
...
import ResourcesSection from '@pages/projects/form/ResourcesSection';

describe('ResourcesSection Basic', () => {
  const defaultProps = {
    humanResources: 'فريق التطوير',
    technicalResources: 'أدوات البرمجة',
    financialResources: 'الميزانية المخصصة',
    onHumanResourcesChange: vi.fn(),
    onTechnicalResourcesChange: vi.fn(),
    onFinancialResourcesChange: vi.fn(),
  };
  ...
  it('renders card component', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByTestId('card')).toBeInTheDocument();
  });

  it('renders section title', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByText('[Resources And Support]')).toBeInTheDocument();
  });
  ...
  it('renders human resources label', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByText('[Human Resources]')).toBeInTheDocument();
  });

  it('renders technical resources label', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByText('[Technical Resources]')).toBeInTheDocument();
  });

  it('renders financial resources label', () => {
    render(<ResourcesSection {...defaultProps} />);
...
describe('ResourcesSection Values', () => {
  const defaultProps = {
    humanResources: 'فريق التطوير',
    technicalResources: 'أدوات البرمجة',
    financialResources: 'الميزانية المخصصة',
  ...
  it('displays human resources value', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    expect(textareas[0]).toHaveValue('فريق التطوير');
  });

  it('displays technical resources value', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    expect(textareas[1]).toHaveValue('أدوات البرمجة');
  });

  it('displays financial resources value', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    expect(textareas[2]).toHaveValue('الميزانية المخصصة');
  });
});

describe('ResourcesSection Interactions', () => {
  const defaultProps = {
    humanResources: '',
    technicalResources: '',
    financialResources: '',
  ...
  it('calls onHumanResourcesChange when human resources changes', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    fireEvent.change(textareas[0], { target: { value: 'فريق جديد' } });
    expect(defaultProps.onHumanResourcesChange).toHaveBeenCalledWith('فريق جديد');
  });

  it('calls onTechnicalResourcesChange when technical resources changes', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    fireEvent.change(textareas[1], { target: { value: 'أدوات جديدة' } });
    expect(defaultProps.onTechnicalResourcesChange).toHaveBeenCalledWith('أدوات جديدة');
  });

  it('calls onFinancialResourcesChange when financial resources changes', () => {
    render(<ResourcesSection {...defaultProps} />);
    const textareas = screen.getAllByTestId('textarea');
    fireEvent.change(textareas[2], { target: { value: 'ميزانية جديدة' } });
    expect(defaultProps.onFinancialResourcesChange).toHaveBeenCalledWith('ميزانية جديدة');
  });
});

describe('ResourcesSection Placeholders', () => {
  const defaultProps = {
    humanResources: '',
    technicalResources: '',
    financialResources: '',
  ...
  it('shows human resources placeholder', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('[Human Resources Placeholder]')).toBeInTheDocument();
  });

  it('shows technical resources placeholder', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('[Technical Resources Placeholder]')).toBeInTheDocument();
  });

  it('shows financial resources placeholder', () => {
    render(<ResourcesSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('[Financial Resources Placeholder]')).toBeInTheDocument();
  });
});

describe('ResourcesSection Empty Values', () => {
  const emptyProps = {
    humanResources: '',
    technicalResources: '',
    financialResources: '',
  ...
  it('renders with empty values', () => {
    render(<ResourcesSection {...emptyProps} />);
    const textareas = screen.getAllByTestId('textarea');
