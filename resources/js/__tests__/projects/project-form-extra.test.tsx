...
const mockRouteState = vi.hoisted(() => ({ projectId: undefined as string | undefined }));
const mockHandleSubmit = vi.hoisted(() => vi.fn((event?: { preventDefault?: () => void }) => event?.preventDefault?.()));

afterEach(() => {
  mockRouteState.projectId = undefined;
});
...
vi.mock('react-i18next', () => {
  const translations: Record<string, string> = {
    ...
    'projects.program_optional': '[Program Optional]', 'projects.resources_and_support': 'الموارد والدعم', 'projects.select_assignee': '[Select Assignee]', 'projects.select_end_date': '[Select End Date]',
    ...
    'projects.approval_criteria_placeholder': 'اكتب معايير قبول المشروع...',
    'projects.business_case': 'مبررات المشروع',
    'projects.business_case_placeholder': 'اشرح مبررات المشروع...',
    'projects.exit_criteria': 'معايير الإغلاق',
    'projects.exit_criteria_placeholder': 'اكتب شروط إغلاق المشروع...',
    'projects.improvement_autosave_error': 'تعذر حفظ المسودة',
    'projects.improvement_autosave_hint': 'يتم حفظ مسودة التحسين تلقائياً أثناء إدخال البيانات',
    'projects.improvement_autosave_ready': 'الحفظ التلقائي جاهز',
    'projects.improvement_autosave_restored': 'تمت استعادة المسودة',
    'projects.improvement_autosave_saved': 'تم حفظ المسودة',
    'projects.improvement_autosave_saved_at': 'تم حفظ المسودة في {{time}}',
    'projects.improvement_autosave_saving': 'جاري حفظ المسودة...',
    'projects.manager_authority': 'صلاحيات مدير المشروع',
    'projects.manager_authority_placeholder': 'حدد صلاحيات مدير المشروع...',
    'projects.pmbok_charter_fields': 'حقول ميثاق المشروع',
    'projects.project_autosave_ready': 'الحفظ التلقائي جاهز',
    'projects.section_nav_label': 'التنقل بين الأقسام',
    'projects.project_autosave_hint': 'يتم حفظ مسودة المشروع تلقائياً أثناء إدخال البيانات',
    'projects.requirements': 'المتطلبات',
    'projects.requirements_placeholder': 'اكتب المتطلبات الرئيسية...',
    'projects.success_criteria': 'معايير النجاح',
    'projects.success_criteria_placeholder': 'اكتب معايير نجاح المشروع...',
  };
...
vi.mock('react-router-dom', () => ({
  ...
  useParams: () => (mockRouteState.projectId ? { id: mockRouteState.projectId } : {}),
...
// Mock InviteUserModal
vi.mock('@features/user-invitation', () => ({
  InviteUserModal: ({ open, onClose }: { open: boolean; onClose: () => void }) =>
    open ? <div data-testid="invite-modal"><button onClick={onClose}>Close</button></div> : null,
}));

// Mock form sections
vi.mock('@pages/projects/form', () => ({
  MilestonesSection: () => <div data-testid="milestones-section">Milestones</div>,
  TasksSection: () => <div data-testid="tasks-section">Tasks</div>,
  InlineTasksEditor: () => <div data-testid="tasks-section">Tasks</div>,
  RisksSection: () => <div data-testid="risks-section">Risks</div>,
  TeamSection: () => <div data-testid="team-section">Team</div>,
  StakeholdersSection: () => <div data-testid="stakeholders-section">Stakeholders</div>,
  ObjectivesSection: () => <div data-testid="objectives-section">Objectives</div>,
  ScopeSection: () => <div data-testid="scope-section">Scope</div>,
  ResourcesSection: () => <div data-testid="resources-section">Resources</div>,
  BasicInfoStep: ({ formData, handleChange }: any) => {
    const [name, setName] = React.useState(formData.name || '');
    const [description, setDescription] = React.useState(formData.description || '');
    const [budget, setBudget] = React.useState(formData.budget || '');
    return (
      <div data-testid="basic-info-step">
        <div>المعلومات الأساسية</div>
        <input
        ...
        />
        <div>الإعدادات والجدول الزمني</div>
        <div>الحالة</div>
        ...
        <div>-- اختر مدير المشروع --</div>
        <div>راعي / مالك المشروع</div>
        <div>-- اختر راعي / مالك المشروع --</div>
  ...
  useProjectForm: ({ id }: { id?: string }) => {
    ...
    const [formData, setFormData] = React.useState({
      ...
      end_date: '',
      budget: '',
      milestones: [{ name: '', description: '', target_date: '', deliverables: [] }],
      tasks: [{ name: '', description: '', priority: 'medium', start_date: '', due_date: '' }],
      risks: [{ description: '', probability: 'medium', impact: 'medium', mitigation: '' }],
      team_members: [{ user_id: undefined, name: '', role: '' }],
      stakeholders: [{ name: '', role: '', influence: 'medium', interest: 'medium' }],
      business_case: '',
      manager_authority: '',
      success_criteria: '',
      requirements: '',
    ...
    return {
      ...
      handleChange: vi.fn((field: string, value: any) => setFormData((prev: any) => ({ ...prev, [field]: value }))),
      handleArrayItemChange: vi.fn(),
      addArrayItem: vi.fn(),
      removeArrayItem: vi.fn(),
      handleMilestoneChange: vi.fn(),
      addMilestone: vi.fn(),
      removeMilestone: vi.fn(),
      handleDeliverableChange: vi.fn(),
      addDeliverable: vi.fn(),
      removeDeliverable: vi.fn(),
      handleRiskChange: vi.fn(),
      addRisk: vi.fn(),
      removeRisk: vi.fn(),
      ...
      handleStakeholderChange: vi.fn(),
      addStakeholder: vi.fn(),
      removeStakeholder: vi.fn(),
      updateStakeholder: vi.fn(),
      goToNextStep: () => setCurrentStep((prev: number) => Math.min(prev + 1, 5)),
      goToPreviousStep: () => setCurrentStep((prev: number) => Math.max(prev - 1, 0)),
  ...
  emptyMilestone: { name: '', description: '', target_date: '', deliverables: [] },
  emptyTask: { name: '', description: '', priority: 'medium', start_date: '', due_date: '' },
  emptyRisk: { description: '', probability: 'medium', impact: 'medium', mitigation: '' },
  emptyTeamMember: { user_id: undefined, name: '', role: '' },
  emptyStakeholder: { name: '', role: '', influence: 'medium', interest: 'medium' },
  emptyDeliverable: { name: '', description: '' },
}));
...
describe('ProjectForm Submit Create Mode', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockNavigate.mockClear();
    (projectsApi.create as any).mockResolvedValue({ id: 1 });
  });

  it('submits form directly in single-page create mode', async () => {
    render(<ProjectForm />);

    const nameInput = await screen.findByPlaceholderText('أدخل اسم المشروع');
    const submitButton = screen.getByRole('button', { name: /إنشاء مشروع/ });

    await userEvent.type(nameInput, 'مشروع جديد');
    await userEvent.click(submitButton);

    expect(nameInput).toHaveValue('مشروع جديد');
    expect(mockHandleSubmit).toHaveBeenCalledTimes(1);
  });

  it('renders all create sections on one page without wizard navigation', async () => {
    render(<ProjectForm />);

    await screen.findByPlaceholderText('أدخل اسم المشروع');

    expect(screen.queryByRole('button', { name: 'التالي' })).not.toBeInTheDocument();
    expect(screen.queryByText('الخطوة 1 من 6')).not.toBeInTheDocument();
    ...
    expect(screen.getByTestId('stakeholders-section')).toBeInTheDocument();
    expect(screen.getByTestId('milestones-section')).toBeInTheDocument();
    expect(screen.getByTestId('tasks-section')).toBeInTheDocument();
    expect(screen.getByTestId('risks-section')).toBeInTheDocument();
    expect(screen.getByTestId('resources-section')).toBeInTheDocument();
  });

  it('shows the create autosave strip and project hint', async () => {
    render(<ProjectForm />);

    await screen.findByPlaceholderText('أدخل اسم المشروع');

    expect(screen.getByRole('heading', { name: 'مشروع جديد' })).toBeInTheDocument();
    expect(screen.getByText('الحفظ التلقائي جاهز')).toBeInTheDocument();
    expect(screen.getAllByText('يتم حفظ مسودة المشروع تلقائياً أثناء إدخال البيانات').length).toBeGreaterThan(0);
  });

  it('keeps basic fields and create action visible together', async () => {
    render(<ProjectForm />);

    await screen.findByPlaceholderText('أدخل اسم المشروع');

    expect(screen.getByText('الإعدادات والجدول الزمني')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '2. حقول ميثاق المشروع' })).toBeInTheDocument();
    expect(screen.getByText('مبررات المشروع')).toBeInTheDocument();
    expect(screen.getByText('صلاحيات مدير المشروع')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /إنشاء مشروع/ })).toBeInTheDocument();
  });

  it('submits empty create form through the current create action', async () => {
    render(<ProjectForm />);
...
describe('ProjectForm Data Loading', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('loads departments data from hook', async () => {
    render(<ProjectForm />);

    // Data loading is handled by the mocked useProjectForm hook
    // which returns allDepartments from its mock
    await waitFor(() => {
      expect(screen.getByPlaceholderText('أدخل اسم المشروع')).toBeInTheDocument();
    });
  });

  it('loads users data from hook', async () => {
    render(<ProjectForm />);

    // Data loading is handled by the mocked useProjectForm hook
    // which returns users from its mock
    await waitFor(() => {
      expect(screen.getByPlaceholderText('أدخل اسم المشروع')).toBeInTheDocument();
    });
  });

  it('renders form even with empty data', async () => {
    render(<ProjectForm />);

    // Should render form with empty data from the mock
    await waitFor(() => {
      expect(screen.getByPlaceholderText('أدخل اسم المشروع')).toBeInTheDocument();
    });
  });

  it('shows form fields after data loads', async () => {
    render(<ProjectForm />);
  ...
  it('provides data through hook to form', async () => {
    render(<ProjectForm />);

    // The hook provides allDepartments and users.
    await waitFor(() => {
      expect(screen.getByText('القسم')).toBeInTheDocument();
    });
  });

  it('handles loading state from hook', async () => {
    render(<ProjectForm />);

    // The mocked hook returns isLoading: false
    await waitFor(() => {
      expect(screen.getByPlaceholderText('أدخل اسم المشروع')).toBeInTheDocument();
    });
  });
...
describe('ProjectForm Form Fields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('updates project name field', async () => {
    render(<ProjectForm />);

    const nameInput = await screen.findByPlaceholderText('أدخل اسم المشروع');
    await userEvent.type(nameInput, 'اسم المشروع');

    expect(nameInput).toHaveValue('اسم المشروع');
  });

  it('updates project description field', async () => {
    render(<ProjectForm />);

    const descInput = await screen.findByPlaceholderText('أدخل وصفاً تفصيلياً للمشروع');
    await userEvent.type(descInput, 'وصف المشروع');

    expect(descInput).toHaveValue('وصف المشروع');
  });

  it('updates budget field', async () => {
    render(<ProjectForm />);

    const budgetInput = await screen.findByPlaceholderText('0.00');
...
describe('ProjectForm Cancel Navigation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('has cancel link to projects', async () => {
    render(<ProjectForm />);
    ...
    const cancelLink = screen.getByText('إلغاء');
    expect(cancelLink.closest('a')).toHaveAttribute('href', '/projects');
  });
});

describe('ProjectForm User Selection Fields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (usersApi.getList as any).mockResolvedValue([
      { id: 1, name: 'مستخدم 1' },
      { id: 2, name: 'مستخدم 2' },
    ]);
  });

  it('shows manager selection field', async () => {
    render(<ProjectForm />);
  ...
  it('shows sponsor selection field', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByText('راعي / مالك المشروع')).toBeInTheDocument();
    });
  });
});

describe('ProjectForm Department Field', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (departmentsApi.getHierarchy as any).mockResolvedValue({
      all: [
        { id: 1, name: 'قسم 1', level_name: 'مستوى 1' },
        { id: 2, name: 'قسم 2', level_name: 'مستوى 2' },
      ],
    });
  });

  it('shows department selection field', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      expect(screen.getByText(/قسم/)).toBeInTheDocument();
    });
  });
});

describe('ProjectForm Date Fields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows start date field', async () => {
    render(<ProjectForm />);
...
describe('ProjectForm Status and Priority', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('shows status field', async () => {
    render(<ProjectForm />);
  ...
  it('shows priority field', async () => {
    render(<ProjectForm />);

    await waitFor(() => {
      const priorityLabels = screen.getAllByText(/الأولوية/);
      expect(priorityLabels.length).toBeGreaterThan(0);
    });
  });
});

describe('ProjectForm single-page tabs', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRouteState.projectId = undefined;
  });

  it('shows the section tabs in the sticky top bar (no step indicator)', async () => {
    render(<ProjectForm />);
    ...
    expect(screen.queryByText('الخطوة 1 من 6')).not.toBeInTheDocument();
    expect(screen.getAllByText('الأهداف والنطاق').length).toBeGreaterThan(0);
    expect(screen.getAllByText('فريق العمل').length).toBeGreaterThan(0);
    expect(screen.getAllByText('أصحاب المصلحة').length).toBeGreaterThan(0);
    expect(screen.getAllByText('المراحل').length).toBeGreaterThan(0);
    expect(screen.getAllByText('المهام').length).toBeGreaterThan(0);
  });
});
