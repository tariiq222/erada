...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, disabled, leftIcon, className }: {
...
import ObjectivesSection from '@pages/projects/form/ObjectivesSection';

describe('ObjectivesSection Basic', () => {
  const defaultProps = {
  ...
  it('renders card component', () => {
    render(<ObjectivesSection {...defaultProps} />);
  ...
  it('renders all objectives', () => {
    ...
    expect(inputs[0]).toHaveValue('هدف 1');
    expect(inputs[1]).toHaveValue('هدف 2');
  });

  it('renders objective numbers', () => {
    render(<ObjectivesSection {...defaultProps} />);
  ...
  it('renders add objective button', () => {
    render(<ObjectivesSection {...defaultProps} />);
    expect(screen.getByText('إضافة هدف')).toBeInTheDocument();
  });
...
describe('ObjectivesSection Interactions', () => {
  const defaultProps = {
    objectives: ['هدف 1', 'هدف 2'],
    onObjectiveChange: vi.fn(),
    onAddObjective: vi.fn(),
  ...
  it('calls onObjectiveChange when input changes', () => {
    render(<ObjectivesSection {...defaultProps} />);
    const inputs = screen.getAllByTestId('input');
    fireEvent.change(inputs[0], { target: { value: 'هدف جديد' } });
    expect(defaultProps.onObjectiveChange).toHaveBeenCalledWith(0, 'هدف جديد');
  });

  it('calls onAddObjective when add button clicked', () => {
    render(<ObjectivesSection {...defaultProps} />);
    fireEvent.click(screen.getByText('إضافة هدف'));
    expect(defaultProps.onAddObjective).toHaveBeenCalled();
  });

  it('calls onRemoveObjective when delete button clicked', () => {
    render(<ObjectivesSection {...defaultProps} />);
    const deleteButtons = screen.getAllByTestId('trash-icon');
    fireEvent.click(deleteButtons[0].closest('button')!);
    expect(defaultProps.onRemoveObjective).toHaveBeenCalledWith(0);
  });

  it('shows delete buttons when multiple objectives exist', () => {
    render(<ObjectivesSection {...defaultProps} />);
...
describe('ObjectivesSection Single Objective', () => {
  const singleProps = {
    objectives: ['هدف واحد'],
    onObjectiveChange: vi.fn(),
    onAddObjective: vi.fn(),
  ...
  it('does not show delete button for single objective', () => {
    render(<ObjectivesSection {...singleProps} />);
    expect(screen.queryByTestId('trash-icon')).not.toBeInTheDocument();
  });

  it('shows objective number 1', () => {
    render(<ObjectivesSection {...singleProps} />);
    expect(screen.getByText('1')).toBeInTheDocument();
  });
});

describe('ObjectivesSection Empty State', () => {
  const emptyProps = {
    objectives: [],
    onObjectiveChange: vi.fn(),
    onAddObjective: vi.fn(),
  ...
  it('shows add button when empty', () => {
    render(<ObjectivesSection {...emptyProps} />);
    expect(screen.getByText('إضافة هدف')).toBeInTheDocument();
  });

  it('has no inputs when empty', () => {
    render(<ObjectivesSection {...emptyProps} />);
    expect(screen.queryByTestId('input')).not.toBeInTheDocument();
  });
});

describe('ObjectivesSection Input Placeholder', () => {
  const defaultProps = {
    objectives: [''],
    onObjectiveChange: vi.fn(),
    onAddObjective: vi.fn(),
  ...
  it('shows placeholder text', () => {
    render(<ObjectivesSection {...defaultProps} />);
    expect(screen.getByPlaceholderText('[Enter Objective]')).toBeInTheDocument();
  });
});
