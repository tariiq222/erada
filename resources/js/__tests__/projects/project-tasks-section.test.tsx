  ...
  tasksApi: {
    getOne: vi.fn().mockResolvedValue({
      data: {
        ...
        title: 'مهمة 1',
        subtasks: [
          { id: 101, title: 'مهمة فرعية 1', status: 'in_progress' },
          { id: 102, title: 'مهمة فرعية 2', status: 'completed' },
        ],
      },
    }),
...
vi.mock('@features/task-create', () => ({
  default: ({ isOpen, onClose, onTaskCreated }: { isOpen: boolean; onClose: () => void; onTaskCreated: () => void }) => (
    isOpen ? (
      <div data-testid="create-task-modal">
        <button onClick={onClose}>إغلاق</button>
        <button onClick={onTaskCreated}>إنشاء</button>
      </div>
    ) : null
  ),
}));
...
vi.mock('@pages/projects/components/modals', () => ({
  StatusTransitionModal: ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => (
    isOpen ? <div data-testid="status-transition-modal"><button onClick={onClose}>إغلاق</button></div> : null
  ),
  ErrorModal: ({ isOpen, onClose, message }: { isOpen: boolean; onClose: () => void; message: string }) => (
    isOpen ? <div data-testid="error-modal">{message}<button onClick={onClose}>إغلاق</button></div> : null
  ),
  IncompleteSubtasksModal: ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => (
    isOpen ? <div data-testid="incomplete-subtasks-modal"><button onClick={onClose}>إغلاق</button></div> : null
  ),
}));

// Mock tasks sub-components
vi.mock('@pages/projects/components/sections/tasks', () => ({
  KanbanBoard: ({
    ...
    onUpdateSubtaskStatus: any;
    onOpenSubtaskStatusMenu: any;
  }) => (
      ...
      {tasks.map((task: any) => (
        ...
        >
          ...
          {task.subtasks_count > 0 && (
            <button onClick={(e) => onToggleSubtasks(e, task.id)}>
              مهام فرعية ({task.subtasks_count})
            </button>
          )}
        </div>
      ))}
  ...
  TasksFilters: ({
    ...
    <div data-testid="tasks-filters">
      ...
      <button onClick={onResetFilters}>إعادة تعيين</button>
    </div>
  ),
  TasksHeader: ({
    taskCount,
    ...
    <div data-testid="tasks-header">
      <span>المهام: {taskCount}</span>
      <span>الفلاتر النشطة: {activeFiltersCount}</span>
      <button onClick={() => onViewModeChange(viewMode === 'kanban' ? 'list' : 'kanban')}>
        {viewMode === 'kanban' ? 'قائمة' : 'كانبان'}
      </button>
  ...
  TasksEmptyState: ({ onCreateTask }: { onCreateTask: () => void }) => (
    <div data-testid="tasks-empty-state">
      <span>لا توجد مهام</span>
      <button onClick={onCreateTask}>إنشاء مهمة جديدة</button>
    </div>
  ),
}));
...
vi.mock('@pages/projects/constants', () => ({
  ...
  STATUS_ORDER: ['todo', 'in_progress', 'completed', 'cancelled', 'on_hold'],
  getStatusTransitionRules: vi.fn().mockReturnValue({
    allowed: true,
...
const mockProject = {
  ...
  end_date: '2025-12-31',
  milestones: [
    { id: 1, name: 'مرحلة 1', status: 'in_progress' },
  ],
};

const mockMembers = [
  { id: 1, name: 'أحمد', pivot: { role: 'مدير' } },
  { id: 2, name: 'محمد', pivot: { role: 'عضو' } },
];
...
describe('ProjectTasksSection View Modes', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('can switch to list view', () => {
    ...
    const switchButton = screen.getByText('قائمة');
    fireEvent.click(switchButton);

    expect(screen.getByTestId('tasks-list-view')).toBeInTheDocument();
  });

  it('can switch back to kanban view', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    ...
    fireEvent.click(screen.getByText('كانبان'));
    expect(screen.getByTestId('kanban-board')).toBeInTheDocument();
  });
});
...
describe('ProjectTasksSection Filters', () => {
  ...
  it('toggles filters panel', () => {
    ...
    // Initially filters not shown
    expect(screen.queryByTestId('tasks-filters')).not.toBeInTheDocument();

    // Show filters
    fireEvent.click(screen.getByText('إظهار الفلاتر'));
    expect(screen.getByTestId('tasks-filters')).toBeInTheDocument();

    // Hide filters
    fireEvent.click(screen.getByText('إخفاء الفلاتر'));
    expect(screen.queryByTestId('tasks-filters')).not.toBeInTheDocument();
  });

  it('shows active filters count', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.getByText('الفلاتر النشطة: 0')).toBeInTheDocument();
  });
});

describe('ProjectTasksSection Task Interaction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('opens task view modal on click', () => {
    render(<ProjectTasksSection {...defaultProps} />);
...
describe('ProjectTasksSection Subtasks', () => {
  ...
  it('shows subtasks button for tasks with subtasks', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    expect(screen.getByText('مهام فرعية (2)')).toBeInTheDocument();
  });

  it('can toggle subtasks', async () => {
    render(<ProjectTasksSection {...defaultProps} />);
    await act(async () => { await Promise.resolve(); });

    // Click subtasks button - just verify it doesn't crash
    const subtasksButton = screen.getByText('مهام فرعية (2)');
...
describe('ProjectTasksSection Create Task', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('opens create task modal', () => {
    ...
    fireEvent.click(screen.getByText('إنشاء مهمة'));
  ...
  it('calls onTaskCreated callback', () => {
    render(<ProjectTasksSection {...defaultProps} />);
...
describe('ProjectTasksSection Empty State', () => {
  ...
  it('can create task from empty state', () => {
    render(<ProjectTasksSection {...defaultProps} tasks={[]} />);
...
describe('ProjectTasksSection Filter Logic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('filters tasks by status in list view', () => {
    render(<ProjectTasksSection {...defaultProps} />);
    ...
    // Should show only in_progress task
    const listView = screen.getByTestId('tasks-list-view');
    expect(listView).toContainHTML('مهمة 1');
  });
