...
vi.mock('@shared/ui/Button', () => ({
  Button: ({ children, onClick, disabled, leftIcon, type, variant, size, className }: {
    ...
    <button onClick={onClick} disabled={disabled} data-type={type} className={className}>
      {leftIcon}{children}
    </button>
  ),
}));
...
vi.mock('@shared/ui/DatePicker', () => ({
  DatePicker: ({ value, onChange, placeholder, disabled, minDate, maxDate }: {
    value: string;
...
describe('MilestonesSection Basic', () => {
  const defaultProps = {
  ...
  it('shows date range info', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText(/\[Milestone Date Range\]/)).toBeInTheDocument();
  });

  it('shows project dates in info', () => {
    render(<MilestonesSection {...defaultProps} />);
...
describe('MilestonesSection Without Dates', () => {
  ...
  it('shows warning when no dates', () => {
    render(<MilestonesSection {...propsWithoutDates} />);
    expect(screen.getByText(/\[Set Dates First Milestones\]/)).toBeInTheDocument();
  });

  it('disables add milestone button when no dates', () => {
    render(<MilestonesSection {...propsWithoutDates} />);
    expect(screen.getByText('إضافة مرحلة')).toBeDisabled();
  });
});

describe('MilestonesSection Milestone Fields', () => {
  const defaultProps = {
    milestones: mockMilestones,
    projectStartDate: '2025-01-01',
    projectEndDate: '2025-06-30',
  ...
  it('shows deliverables label', () => {
    render(<MilestonesSection {...defaultProps} />);
    expect(screen.getByText('[Deliverables]')).toBeInTheDocument();
  });
});

describe('MilestonesSection Interactions', () => {
  const defaultProps = {
    milestones: mockMilestones,
    projectStartDate: '2025-01-01',
    projectEndDate: '2025-06-30',
  ...
  it('calls onMilestoneChange when name changes', () => {
    render(<MilestonesSection {...defaultProps} />);
    const input = screen.getByPlaceholderText('[Milestone Name Placeholder]');
    fireEvent.change(input, { target: { value: 'مرحلة جديدة' } });
    expect(defaultProps.onMilestoneChange).toHaveBeenCalledWith(0, 'name', 'مرحلة جديدة');
  });

  it('calls onAddMilestone when add button clicked', () => {
    render(<MilestonesSection {...defaultProps} />);
    fireEvent.click(screen.getByText('إضافة مرحلة'));
    expect(defaultProps.onAddMilestone).toHaveBeenCalled();
  });

  it('calls onAddDeliverable when add deliverable clicked', () => {
    render(<MilestonesSection {...defaultProps} />);
    fireEvent.click(screen.getByText('[Add Deliverable]'));
    expect(defaultProps.onAddDeliverable).toHaveBeenCalledWith(0);
  });

  it('calls onDeliverableChange when deliverable changes', () => {
    render(<MilestonesSection {...defaultProps} />);
    const input = screen.getByPlaceholderText('[Deliverable Name]');
    fireEvent.change(input, { target: { value: 'مخرج جديد' } });
    expect(defaultProps.onDeliverableChange).toHaveBeenCalledWith(0, 0, 'name', 'مخرج جديد');
  });
});

describe('MilestonesSection Multiple Milestones', () => {
  const multipleMilestones = [
    { name: 'مرحلة 1', description: '', start_date: '', due_date: '', deliverables: [{ name: '' }] },
    { name: 'مرحلة 2', description: '', start_date: '', due_date: '', deliverables: [{ name: '' }] },
  ];

  const propsWithMultipleMilestones = {
  ...
  it('renders multiple milestones', () => {
    render(<MilestonesSection {...propsWithMultipleMilestones} />);
  ...
  it('calls onRemoveMilestone when delete clicked', () => {
    render(<MilestonesSection {...propsWithMultipleMilestones} />);
    const deleteButtons = screen.getAllByTestId('trash-icon');
    fireEvent.click(deleteButtons[0].parentElement!);
    expect(propsWithMultipleMilestones.onRemoveMilestone).toHaveBeenCalledWith(0);
  });
});

describe('MilestonesSection Single Milestone', () => {
  const singleMilestone = [
    { name: 'مرحلة واحدة', description: '', start_date: '', due_date: '', deliverables: [{ name: '' }] },
  ];

  const propsWithSingleMilestone = {
  ...
  it('does not show delete button for single milestone', () => {
    render(<MilestonesSection {...propsWithSingleMilestone} />);
    expect(screen.queryByTestId('trash-icon')).not.toBeInTheDocument();
  });
});

describe('MilestonesSection Multiple Deliverables', () => {
  const milestoneWithMultipleDeliverables = [
    {
      name: 'مرحلة',
      description: '',
      start_date: '',
      due_date: '',
      deliverables: [{ name: 'مخرج 1' }, { name: 'مخرج 2' }],
    },
  ];

  const propsWithMultipleDeliverables = {
    milestones: milestoneWithMultipleDeliverables,
    projectStartDate: '2025-01-01',
    projectEndDate: '2025-06-30',
    onMilestoneChange: vi.fn(),
    onAddMilestone: vi.fn(),
  ...
  it('renders multiple deliverables', () => {
    render(<MilestonesSection {...propsWithMultipleDeliverables} />);
    expect(screen.getByText('1.')).toBeInTheDocument();
    expect(screen.getByText('2.')).toBeInTheDocument();
  });
  ...
  it('shows delete buttons for multiple deliverables', () => {
    render(<MilestonesSection {...propsWithMultipleDeliverables} />);
    // The trash icons should be for deliverables (no milestone delete since only 1)
    expect(screen.getAllByTestId('trash-icon').length).toBe(2);
  });

  it('calls onRemoveDeliverable when delete clicked', () => {
    render(<MilestonesSection {...propsWithMultipleDeliverables} />);
    const deleteButtons = screen.getAllByTestId('trash-icon');
    fireEvent.click(deleteButtons[0].parentElement!);
    expect(propsWithMultipleDeliverables.onRemoveDeliverable).toHaveBeenCalledWith(0, 0);
  });
});
