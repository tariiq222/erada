...
// Import components
import FiltersCard from '@pages/projects/list/FiltersCard';
import ProjectsTable from '@pages/projects/list/ProjectsTable';
import DeleteProjectModal from '@pages/projects/list/DeleteProjectModal';
import { statusLabels, statusVariants, priorityLabels, priorityVariants } from '@pages/projects/list/constants';
...
const mockProjects = [
  ...
  },
  {
    ...
    priority: 'urgent' as const, // Use urgent instead of critical
    progress: 0,
...
describe('Projects FiltersCard', () => {
  const defaultProps = {
    filters: { search: '', status: '', priority: '', program_id: '' },
  ...
  it('renders filter section', () => {
    ...
    expect(screen.getByPlaceholderText('بحث بالاسم أو الرمز...')).toBeInTheDocument();
  });
  ...
  it('calls onClose when close button clicked', async () => {
    const onClose = vi.fn();
    render(<FiltersCard {...defaultProps} onClose={onClose} />);

    // Find the close button (button without text content, contains X icon)
  ...
  it('calls onFiltersChange when search input changes', async () => {
    ...
    render(<FiltersCard {...defaultProps} onFiltersChange={onFiltersChange} />);

    const searchInput = screen.getByPlaceholderText('بحث بالاسم أو الرمز...');
    await userEvent.type(searchInput, 'مشروع');
  ...
  it('calls onSearch when form is submitted', async () => {
    ...
    render(<FiltersCard {...defaultProps} onSearch={onSearch} />);
  ...
  it('displays initial filter values', () => {
    const filters = { search: 'test search', status: '', priority: '', program_id: '' };
    render(<FiltersCard {...defaultProps} filters={filters} />);

    const searchInput = screen.getByPlaceholderText('بحث بالاسم أو الرمز...') as HTMLInputElement;
...
describe('Projects Table', () => {
  ...
  it('renders empty state when no projects', () => {
    ...
    expect(screen.getByText('ابدأ بإنشاء مشروعك الأول')).toBeInTheDocument();
  ...
  it('renders project names', () => {
    ...
    expect(screen.getByText('مشروع التحول الرقمي')).toBeInTheDocument();
    expect(screen.getByText('مشروع التطوير المؤسسي')).toBeInTheDocument();
    expect(screen.getByText('مشروع البنية التحتية')).toBeInTheDocument();
  });

  it('renders project codes', () => {
    render(<ProjectsTable {...defaultProps} />);
  ...
  it('renders dash for missing department', () => {
    render(<ProjectsTable {...defaultProps} />);

    // Project 3 has no department
  ...
  it('renders action links for each project', () => {
    render(<ProjectsTable {...defaultProps} />);

    // Check that links to view and edit exist for each project
    mockProjects.forEach(project => {
      const viewLink = screen.getByRole('link', { name: new RegExp(project.name) });
  ...
  it('renders pagination when multiple pages', () => {
    ...
    // Pagination block shows the total-count label
    expect(screen.getByText('الإجمالي:')).toBeInTheDocument();
  });

  it('hides pagination when single page', () => {
    render(<ProjectsTable {...defaultProps} pagination={{ currentPage: 1, lastPage: 1, total: 4 }} />);

    // Pagination block (and its total-count label) should not be visible
    expect(screen.queryByText('الإجمالي:')).not.toBeInTheDocument();
  });

  it('renders project name links with correct href', () => {
    render(<ProjectsTable {...defaultProps} />);

    // Project name links should point to project detail page
    const projectLink = screen.getByText('مشروع التحول الرقمي').closest('a');
    expect(projectLink).toHaveAttribute('href', '/projects/1');
  });

  it('renders links to edit page', () => {
    render(<ProjectsTable {...defaultProps} />);

    // Check that edit links (href containing /edit) exist
    const links = screen.getAllByRole('link');
    const editLinks = links.filter(link => link.getAttribute('href')?.includes('/edit'));
...
describe('Delete Project Modal', () => {
  ...
  it('renders modal title', () => {
    ...
    expect(screen.getByText('تأكيد حذف المشروع')).toBeInTheDocument();
  });
  ...
  it('renders warning message', () => {
    render(<DeleteProjectModal {...defaultProps} />);
    expect(screen.getByText(/سيتم حذف جميع البيانات المرتبطة بالمشروع/)).toBeInTheDocument();
  });
  ...
  it('calls onConfirm when confirm button clicked', async () => {
    ...
    render(<DeleteProjectModal {...defaultProps} onConfirm={onConfirm} />);

    const confirmButton = screen.getByText('حذف المشروع');
  ...
  it('calls onClose when cancel button clicked', async () => {
    const onClose = vi.fn();
    render(<DeleteProjectModal {...defaultProps} onClose={onClose} />);
  ...
  it('shows loading state when deleting', () => {
    render(<DeleteProjectModal {...defaultProps} isDeleting={true} />);

    // Button should show loading icon (spinner) and the button should be disabled
    const confirmButton = screen.getByText('حذف المشروع').closest('button');
  ...
  it('does not render when closed', () => {
    ...
    expect(screen.queryByText('تأكيد حذف المشروع')).not.toBeInTheDocument();
  });

  it('handles null project', () => {
    render(<DeleteProjectModal {...defaultProps} project={null} />);

    // Should not crash, modal might not show content
    // The DeleteConfirmationModal handles null items
  });
});

describe('Project Status and Priority Constants', () => {
  it('has all status labels', () => {
    expect(statusLabels.draft).toBe('status.draft');
    expect(statusLabels.planning).toBe('status.planning');
    expect(statusLabels.in_progress).toBe('status.in_progress');
    expect(statusLabels.on_hold).toBe('status.on_hold');
    expect(statusLabels.completed).toBe('status.completed');
    expect(statusLabels.cancelled).toBe('status.cancelled');
  });

  it('has all status variants', () => {
    expect(statusVariants.draft).toBeDefined();
    expect(statusVariants.planning).toBeDefined();
    expect(statusVariants.in_progress).toBeDefined();
    expect(statusVariants.on_hold).toBeDefined();
    expect(statusVariants.completed).toBeDefined();
    expect(statusVariants.cancelled).toBeDefined();
  });
  ...
  it('has all priority variants', () => {
    expect(priorityVariants.low).toBeDefined();
    expect(priorityVariants.medium).toBeDefined();
    expect(priorityVariants.high).toBeDefined();
    expect(priorityVariants.urgent).toBeDefined(); // urgent instead of critical
  });
});
