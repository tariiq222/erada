...
export interface AdminNavItem {
  ...
  fallback: string;
  /**
   * Visibility semantics per group:
   *  - `governance` / `controls` — visible to anyone who reaches the admin SPA
   *    (the surrounding boundary already restricts the audience).
   *  - `system` — restricted to backend `is_super_admin === true`.
   *  - `org` — visible to either super admins or curated org admins
   *    (`is_org_admin === true` OR `is_super_admin === true`).
   *  - `org-super` — visible to either super admins or genuine organization
   *    super admins (`is_organization_super_admin === true` OR `is_super_admin === true`).
   *    Never use role names from `user.roles` here — authorization is
   *    derived from the backend-computed payload flags.
   */
  group: 'governance' | 'controls' | 'system' | 'org' | 'org-super';
  icon: ComponentType<{ className?: string }>;
}

export function isAdminNavItemVisible(
  item: AdminNavItem,
  user: { is_super_admin?: boolean; is_org_admin?: boolean; is_organization_super_admin?: boolean } | null | undefined,
): boolean {
...
export const ADMIN_NAV_ITEMS: AdminNavItem[] = [
  ...
  { href: '/users', labelKey: 'admin.shell.nav.users', fallback: 'Users', group: 'org-super', icon: IconUsers },
...
const SECTION_HEADER_KEYS: Record<AdminNavItem['group'], string> = {
  governance: 'admin.shell.sidebar.section_primary',
  controls: 'admin.shell.sidebar.section_secondary',
  system: 'admin.shell.sidebar.section_secondary',
  org: 'admin.shell.sidebar.section_secondary',
  'org-super': 'admin.shell.sidebar.section_org_super',
};
