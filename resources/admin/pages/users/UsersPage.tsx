...
export function UsersPage() {
  ...
  const load = useCallback(async (requestedOrganizationId = organizationId, requestedSearch = search) => {
    const generation = ++rowRequestGeneration.current;
    setLoading(true);
    setError(null);
    try {
      const response = await adminApi.users.list({ organization_id: requestedOrganizationId, search: requestedSearch, page: 1, per_page: 20 });
      if (generation === rowRequestGeneration.current) setRows(response.data);
    } catch (caught) {
      if (generation === rowRequestGeneration.current) setError(apiErrorMessage(caught, t('users.load_error')));
    } finally {
