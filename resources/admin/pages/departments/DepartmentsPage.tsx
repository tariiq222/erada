...
export function DepartmentsPage() {
  ...
  const { user } = useAuth();
  const organizationId = (user as { organization_id?: number | null } | null)?.organization_id ?? undefined;
  const [rows, setRows] = useState<AdminDepartment[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const load = useCallback(async () => { setLoading(true); setError(null); try { const response = await adminApi.departments.list({ organization_id: organizationId, page: 1, per_page: 20 }); setRows(response.data); } catch (caught) { setError(apiErrorMessage(caught, t('hr.departments_load_error'))); } finally { setLoading(false); } }, [organizationId, t]);
