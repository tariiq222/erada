...
export function GovernanceRulesPage() {
  const { t } = useTranslation(); const [rows, setRows] = useState<GovernanceRule[]>([]); const [departments, setDepartments] = useState<DepartmentSummary[]>([]); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(true);
  useEffect(() => { void Promise.all([adminApi.governance.list(), adminApi.departments.summary()]).then(([rules, units]) => { setRows(rules.data); setDepartments(units.data); }).catch((caught) => setError(apiErrorMessage(caught, t('common.error')))).finally(() => setLoading(false)); }, [t]);
  const save = async (row: GovernanceRule, value: string) => { const governing_unit_id = value ? Number(value) : null; setRows((current) => current.map((item) => item.resource_type === row.resource_type ? { ...item, governing_unit_id } : item)); try { await adminApi.governance.update({ resource_type: row.resource_type, governing_unit_id }); } catch (caught) { setError(apiErrorMessage(caught, t('common.error'))); } };
